...
SERVICE_NAME="recruiter-chat-api"
...
if [ "$BRANCH" != "master" ]; then
    ...
    echo "  git checkout master && git pull --ff-only origin master"
    exit 1
fi
...
gcloud compute ssh "$VM_NAME" --zone="$ZONE" --project="$PROJECT" --command="
  ...
  sudo mkdir -p data && sudo mv /tmp/\${RELEASE_TAG}-free-tier.json data/free-tier-limits.json
...
echo ""
echo "Smoke test..."
SMOKE_RESULT=$(curl -s -X POST "https://projecthub-chat.bradleymatera.dev/api/chat" \
  -H "Content-Type: application/json" \
  -H "Origin: https://bradleymatera.github.io" \
  -d '{"message":"What is Bradley strongest technical background?","history":[]}' 2>/dev/null | \
