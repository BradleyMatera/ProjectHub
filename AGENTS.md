...
- **AI backend:** Recruiter chat API at `https://projecthub-chat.bradleymatera.dev/api/chat` (production) and `https://dev.projecthub-chat.bradleymatera.dev/api/chat` (staging). **Scout** is the portable intelligence/orchestration engine; **ProjectHub Recruiter Alpha** is the app powered by Scout. Primary inference for staging/production is **Cloudflare Workers AI** (`@cf/meta/llama-3.1-8b-instruct-fast`); Ollama remains the dev/test runtime and optional fallback architecture (not qualified for production). Browser/WebGPU inference is experimental. The inference layer is behind an adapter boundary (`lib/local-model-router.js`) for swappability. Deterministic evidence tools, BM25 retrieval, bundled knowledge, memory, stance consistency, and strict validation compensate for the small model. All user-visible conversational replies must come from generative inference — deterministic code may decide, route, and build contracts but may NOT write final chatbot prose. Every reply carries a `proseSource` of `DIRECT_KB`, `MODEL_GENERATION`, or `TECHNICAL_ERROR`; there is **no deterministic chatbot fallback prose**.
...
- **Generative usage:** Deterministic logic handles understanding, routing, evidence gathering, and contract construction. Open-ended recruiter conversation uses generative inference (Cloudflare Workers AI in staging/production; Ollama in dev/test) with up to five recent turns plus per-topic stances. Generated replies must pass safety, entity, number, length, source-overlap, polarity, temporal grounding, and overclaim validation. If invalid, generative repair is attempted, then strict constrained recovery generation. Runtime JS never authors normal chatbot prose; every user-visible reply is emitted with a `proseSource` of `DIRECT_KB` (canonical tenant facts — explicit `SCOUT_DIRECT_KB_ENABLED=true` opt-in / legacy compatibility only, not the default authoring path), `MODEL_GENERATION` (model output — the normal default for all conversational prose, including profile summaries), or `TECHNICAL_ERROR` (infrastructure). Unknown tools fail closed and no public tool performs writes or arbitrary web access. 15s end-to-end response budget. Employment claims are open-world with three-valued TRUE/FALSE/UNKNOWN evidence status.
- **Conversation control intents:** `classifyResponsePolicy` (`lib/response-policy-classifier.js`) detects conversational control turns before retrieval: `GREETING`, `USER_PROFILE_UPDATE`, `USER_PROFILE_QUERY`, `THANKS`, `FAREWELL`, `HELP`, `CONVERSATIONAL`, `SMALL_TALK`, `REQUEST_TO_SAY`, and `CLARIFY_PREVIOUS_ASSISTANT`. Control turns skip anaphora rewriting and BM25 retrieval, route to `no_tool` with a compact fact-free prompt, and use relaxed length validation. Speaker/addressee roles (AGENT vs SUBJECT) prevent agent-directed patterns from firing on candidate questions. `sessionState.applyControlIntent` commits visitor names and control state before generation.
...
- **Think Mode:** REMOVED. The former self-improvement loop is no longer part of the runtime; `learnedData`/`thinkRunning` exist only as inert stubs in `server-gemini.js`.
...
- **Current branch/focus:** `develop` — integration branch with the Scout lite agent (default release mode `SCOUT_AGENT_MODE=lite`), conversational control intents, human-conversation regression fixes, and Cloudflare Workers AI primary inference. Feature branch `feat/agent-systems-network` is historical.
- **Continuation status:** Read `docs/current-feature-handoff.md` before editing or deploying. Production (`master`) is frozen and unchanged during current staging work; all current work is validated on dev staging first.
...
| `server-gemini.js` | Backend server — generative chat, safety, analytics, BM25 retrieval, and memory |
| `lib/lite-agent.js` | Scout lite agent — deterministic pre-routing, control-mode no_tool path, compact packet building, validation, and repair |
| `lib/response-policy-classifier.js` | Response policy + conversational control intent classifier with speaker/addressee roles |
| `lib/session-state.js` | Server-owned session state — `applyControlIntent`, referent tracking, visitor name |
| `lib/conversation-resolver.js` | Anaphora/referent resolution from user turns |
| `lib/grounding-validator.js` | Answer validation — grounding, overclaim, control-mode relaxations |
| `scripts/manual-deploy-dev.js` | Node dev-VM deploy helper — tarballs lib/, swaps files, restarts service, health + smoke checks |
...
| `test/human-conversation.test.js` | Conversational act classification regression (manual transcript + stress turns) |
| `test/manual-gate-regression.test.js` | Manual gate regression tests for control intents |
| `test/agent-mode.test.js` | Agent mode (lite default) tests |
| `test/final-visible-validation.test.js` | Deterministic final-visible validation tests |
