...
- **AI backend:** Recruiter chat API at `https://projecthub-chat.bradleymatera.dev/api/chat` on a GCP e2-micro VM with Caddy HTTPS. Uses deterministic grounded tools, a memory-bounded local Ollama formatter, and a free overflow network (Cloudflare Workers AI, Google Gemini, xAI Grok, OpenAI-compatible). Groq is disabled by default and known retired model IDs are blocked at runtime. GitHub Models inference is retired and hard-blocked. If every provider is unavailable or a reply fails validation, the final fallback is a fast, grounded answer from `data/recruiter-knowledge.json`.
