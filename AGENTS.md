...
- **AI backend:** Recruiter chat API at `https://projecthub-chat.bradleymatera.dev/api/chat` on a GCP e2-micro VM with Caddy HTTPS. Uses deterministic grounded tools, a memory-bounded local Ollama style controller, and a free overflow network (Cloudflare Workers AI, Google Gemini, xAI Grok, OpenAI-compatible). Groq is disabled by default and known retired model IDs are blocked at runtime. GitHub Models inference is retired and hard-blocked. If every provider is unavailable or a reply fails validation, the final fallback is a fast, grounded answer from `data/recruiter-knowledge.json`.
...
- **Generative usage:** Grounded-first deterministic engine answers factual queries. Safety and false-claim checks run BEFORE learned answers. Evidence-heavy requests execute five read-only local tools deterministically, then optionally use a quantized Ollama model only to choose an allowlisted presentation style; Ollama never rewrites the factual answer. An opt-in Groq planner can use the same tools with an explicit current model. Unknown tools fail closed and no public tool performs writes or arbitrary web access. Open-ended overflow is sent through the remaining free provider network; every reply is validated and falls back to grounded output. 15s total latency budget. Out-of-scope questions are forced to grounded replies.
