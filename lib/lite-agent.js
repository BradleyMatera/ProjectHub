...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens }) {
  ...
  const system = [
    `You are Scout, assistant to ${scoutIdentity.getSubjectName()}.`,
    'Answer naturally from FACTS. 1-3 sentences. Be conversational, not robotic.',
    'Synthesize from facts — do not copy them verbatim.',
    'If a claim is not in FACTS, say No and correct it. Intern ≠ production.',
    'No "as an AI" or "I don\'t have personal opinions" — just answer.',
    '',
    `FACTS:`,
    truncate(compressedEvidence, systemBudget - 200)
  ].join('\n');
