...
function compressMatchRole(result, budget) {
  const lines = [];
  if (result.summary) lines.push(`Fit: ${truncate(result.summary, 100)}`);
  if (result.strengths?.length) {
    lines.push(`Strengths: ${result.strengths.slice(0, 3).map(s => truncate(s, 60)).join('; ')}.`);
  }
  if (result.gaps?.length) {
    lines.push(`Gaps: ${result.gaps.slice(0, 3).map(g => truncate(g, 60)).join('; ')}.`);
  }
  if (result.honestNote) lines.push(`- ${truncate(result.honestNote, 100)}`);
...
function compressBrief(result, budget) {
  const lines = [];
  if (result.candidateName) lines.push(`Candidate: ${result.candidateName}.`);
  if (result.headline) lines.push(`Headline: ${truncate(result.headline, 100)}`);
  if (result.topProjects?.length) {
    lines.push(`Projects: ${result.topProjects.slice(0, 4).map(p => p.name).join(', ')}.`);
  }
  if (result.topSkills?.length) {
    const skills = result.topSkills.slice(0, 8).map(s => s.skill).join(', ');
    lines.push(`Skills: ${skills}.`);
  }
  if (result.certifications?.length) {
    lines.push(`Certs: ${result.certifications.slice(0, 3).map(c => c.name).join(', ')}.`);
  }
  if (result.experience?.length) {
    lines.push(`Experience: ${result.experience.slice(0, 3).map(e => `${e.role} at ${e.company}`).join('; ')}.`);
  }
  if (result.education?.degree) {
    lines.push(`Education: ${result.education.degree} from ${result.education.school || ''}.`);
  }
  if (result.honestGaps?.length) {
    lines.push(`Gaps: ${result.honestGaps.slice(0, 2).map(g => truncate(g, 60)).join('; ')}.`);
  }
  if (result.targetRoles?.length) {
    lines.push(`Target roles: ${result.targetRoles.slice(0, 3).join(', ')}.`);
  }
  return joinWithinBudget(lines, budget);
}
