...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens, structuredFacts, plan, planText, responseContract }) {
  ...
  if (hasContractConstraint || (plan && planIntents.has(plan.intent)) || isNegationConfirmation || isFutureCapability || isNegativeAssessment) {
    ...
    if (responseContract?.subIntent === 'QUALIFICATIONS' || responseContract?.subIntent === 'SKILL_EVIDENCE') {
      const skillTopic = responseContract?.requestedTopic || responseContract?.activeEntity || 'the topic';
      constraints.push(`${responseContract?.subIntent}: Answer in 1-2 complete sentences about the candidate. Do NOT output a JSON object, bullet list, or comma-separated inventory of technologies. Name only the strongest verified items from FACTS. Mention ${skillTopic} explicitly. Do not invent seniority, employers, or titles.`);
    }
    // Mentorship questions: keep the answer about the candidate, not the assistant.
    if (/\b(?:mentor|mentorship)\b/i.test(question)) {
      constraints.push('MENTORSHIP: This question is about whether mentorship would help the CANDIDATE learn or improve. Answer in third person about the candidate; do NOT offer to help or describe what you as an assistant can do.');
    }
    ...
    if (isNegativeAssessment) {
      if (responseContract?.factState === 'TRUE') {
        ...
        if (asksCurrentProgress) {
          ...
          const gapFacts = (responseContract?.keyFacts || []).filter(f => /\b(?:data\s+structure|algorithm|leetcode|gap|learning\s+area|learning\s+need|weakness|dsa)\b/i.test(f));
          const gapList = gapFacts.slice(0, 2).join('; ');
          constraints.push(`NEGATIVE: A documented learning/gap area is not automatically a personal weakness. Do not rank or label a gap as the subject's weakness. If no personal weakness is explicitly verified, say it is unknown or not established in a complete sentence; mention documented gaps only as learning/gap areas.${gapList ? ` Documented gaps/learning areas: ${gapList}.` : ''}`);
        }
      }
    }
