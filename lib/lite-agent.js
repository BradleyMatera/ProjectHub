...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  // Conversational control intents don't need factual tools.
  const CONTROL_MODES = new Set(['GREETING', 'USER_PROFILE_UPDATE', 'USER_PROFILE_QUERY', 'THANKS', 'FAREWELL', 'HELP']);
  ...
  if (route.tool === 'no_tool') {
    toolResult = { control: policyContract.mode, mode: policyContract.mode, visitorName: policyContract.visitorName, userName: conversationState?.userName };
