...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens, structuredFacts, plan, planText, adversarialCaveat, responseContract }) {
  ...
  if (responseContract && responseContract.naturalInstructions) {
    contractInstructions = truncate(responseContract.naturalInstructions, 300);
  }
  ...
  const system = [
    ...
    '- Do NOT invent technologies or companies. Only use named entities in FACTS.',
    ...
    structuredFacts ? '' : '',
    `FACTS:`,
    truncate(compressedEvidence, systemBudget - 600 - (planConstraints ? 250 : 0) - (structuredFacts ? 400 : 0) - (contractInstructions ? 320 : 0))
...
function buildCompletenessRepairPacket({ question, currentAnswer, compressedEvidence, reason, intent, maxTokens, planText, responseContract }) {
  ...
  let contractInstructions = '';
  if (responseContract && responseContract.naturalInstructions) {
    contractInstructions = truncate(responseContract.naturalInstructions, 200);
  }
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model }) {
  ...
  if (!genResult.ok) {
    ...
    return makeFallback(events, contextTokens, startedAt, selectedModel, route, rewritten, toolResult, compressed, knowledge);
  }
    ...
    if (!completeness.complete && LITE_ENABLE_REPAIR) {
      ...
      if (completeness.reason === 'TOO_SHORT' || completeness.reason === 'ADVERSARIAL_TOO_TERSE') {
        ...
        if (isTerseDenial || isTerseAffirmation) {
          // Build a grounded expansion from the question and knowledge
          const expandedAnswer = expandTerseAdversarial(answer, rewritten, knowledge, compressed);
          if (expandedAnswer && expandedAnswer.length >= 20) {
            const expValidation = validateAnswer(expandedAnswer, sourceText, rewritten, knowledge, stateHistory, reqGraph);
            if (expValidation.valid) {
              const expCompleteness = evaluateCompleteness(expandedAnswer, rewritten, evidence);
              if (expCompleteness.complete || expandedAnswer.length > answer.length * 1.3) {
                emit('lite_complete', { outcome: 'completeness_repaired', totalMs: Date.now() - startedAt, deterministic: true });
                return {
                  reply: expandedAnswer,
                  provider: 'ollama-lite',
                  model: selectedModel,
                  steps: [{ type: 'lite_generate' }, { type: 'lite_completeness_repair_deterministic' }],
      ...
      // For COMPARISON_MISSING_ENTITY, the repaired answer MUST pass
      // the completeness check (mention both entities). Don't accept
      // a longer answer that still only mentions one entity.
      ...
      if (compRepairResult.ok) {
        ...
        if (compAnswer.length >= 10) {
          ...
          const hasContradiction = /\b(?:no|not|never|didn'?t|did not|wasn'?t|was not|isn'?t|is not|has no|has not)\b/i.test(answer) &&
              !/\b(?:no|not|never|didn'?t|did not|wasn'?t|was not|isn'?t|is not|has no|has not)\b/i.test(compAnswer);
          ...
          if (hasLeak) {
            ...
            if (compValidation.valid) {
              ...
              if (compCompleteness.complete || (!requireCompleteness && compAnswer.length > answer.length * 1.3)) {
      ...
      if (requireCompleteness && completeness.reason === 'COMPARISON_MISSING_ENTITY' && toolResult.projects) {
        ...
        if (compareMatch) {
          ...
          if (missingProj) {
            const tech = missingProj.tech && missingProj.tech.length
              ? ` (tech: ${missingProj.tech.slice(0, 3).join(', ')})`
              : '';
            // Try with description first, then without if validation fails
            const desc = missingProj.description
              ? missingProj.description.split(/(?<=[.!?])\s/)[0]
              : '';
            const tryAnswers = [
              desc ? `${answer} ${missingProj.name} — ${truncate(desc, 80)}${tech}.` : null,
              `${answer} ${missingProj.name}${tech}.`
            ].filter(Boolean);

            let appendedAnswer = null;
            let appendedValidation = null;
            for (const tryAns of tryAnswers) {
              const val = validateAnswer(tryAns, sourceText, rewritten, knowledge, stateHistory, reqGraph);
              if (val.valid) {
                appendedAnswer = tryAns;
                appendedValidation = val;
                break;
              }
            }

            if (appendedAnswer && appendedValidation) {
              const appendedCompleteness = evaluateCompleteness(appendedAnswer, rewritten, evidence);
              if (appendedCompleteness.complete) {
                emit('lite_completeness_deterministic_repair', { reason: completeness.reason });
                emit('lite_complete', { outcome: 'completeness_repaired', totalMs: Date.now() - startedAt });
                return {
                  reply: appendedAnswer,
                  provider: 'ollama-lite',
                  model: selectedModel,
                  steps: [{ type: 'lite_generate' }, { type: 'lite_completeness_deterministic_repair' }],
                  toolResults: [{ tool: route.tool, result: toolResult }],
                  events,
                  contextTokens,
                  latencyMs: Date.now() - startedAt,
        ...
        // If deterministic repair failed, fall back
...
function buildGroundedFallback(toolResult, route, rewritten, compressed, knowledge) {
  ...
  if (/\b(?:worth|interview|recommend|hire|good fit)\b/i.test(rewrittenStr)) {
    if (knowledge && knowledge.summary && knowledge.summary.whoIAm) {
      // Extract just the first sentence of whoIAm to avoid overly verbose fallback
      const firstSentence = knowledge.summary.whoIAm.split(/(?<=[.!?])\s/)[0];
  ...
  // For invented-entity questions (e.g., "What did he do at Netflix?"),
  // check if the question asks about an unknown company and deny it
  const inventedCompanyMatch = rewrittenStr.match(/\b(?:at|with|for)\s+(?:his\s+time\s+at\s+)?([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)?)\??$/);
  if (inventedCompanyMatch) {
    const companyName = inventedCompanyMatch[1];
    const companyLower = companyName.toLowerCase();
    // Check if this company appears anywhere in the compressed evidence
    if (compressed && !compressed.toLowerCase().includes(companyLower)) {
      // Company not found in evidence — deny employment generically
      // Extract actual companies from tool result if available
      const knownCompanies = [];
      if (toolResult.results && Array.isArray(toolResult.results)) {
        for (const r of toolResult.results) {
          if (r.company && !knownCompanies.includes(r.company)) {
            knownCompanies.push(r.company);
          }
        }
      }
      const companyList = knownCompanies.slice(0, 3).join(', ');
      if (companyList) {
        return `There is no evidence he worked at ${companyName}. His work experience includes ${companyList}.`;
      }
      return `There is no evidence he worked at ${companyName}.`;
    }
  }
...
function makeFallback(events, contextTokens, startedAt, model, route, rewritten, toolResult, compressed, knowledge) {
  const reply = buildGroundedFallback(toolResult, route, rewritten, compressed, knowledge);
  return {
    reply,
    provider: 'ollama-lite',
    model,
