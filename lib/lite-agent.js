...
function formatResponseContract(responseContract, maxChars = 500) {
  ...
  const lines = [
    `INTENT: ${responseContract.subIntent || responseContract.intent}`,
    ...
    responseContract.boundary ? `BOUNDARY: ${responseContract.boundary}` : '',
...
function buildCompletenessRepairPacket({ question, currentAnswer, compressedEvidence, reason, intent, maxTokens, planText, responseContract, missingEntities }) {
  ...
  const system = [
    ...
    '',
    'FACTS:',
    truncate(compressedEvidence, budget - 600),
    '',
    'Rules: Keep all relationships exact. Do not invent facts. Do not use overclaim language.',
 ...
 */
function meaningPreserved(original, repaired) {
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model }) {
  ...
  if (rewrite.clarificationRequired) {
    const reply = rewrite.referentType === 'other_project'
      ? 'Which projects are you comparing, and which one do you mean by the other project?'
      : 'Which project or experience do you mean?';
  ...
  if (validation.valid) {
    ...
    if (!completeness.complete && LITE_ENABLE_REPAIR) {
        ...
        if (isTerseDenial || isTerseAffirmation) {
          ...
          if (expandedAnswer && expandedAnswer.length >= 20) {
            ...
            if (expValidation.valid) {
              const expCompleteness = evaluateCompleteness(expandedAnswer, rewritten, evidence, responseContract);
      ...
      if (compRepairResult.ok) {
        ...
        if (compAnswer.length >= 10) {
          ...
          const hasMeaningReversal = !meaningPreserved(answer, compAnswer);
        ...
        if (compareMatch) {
          ...
          if (missingProj) {
            ...
            if (appendedAnswer && appendedValidation) {
              const appendedCompleteness = evaluateCompleteness(appendedAnswer, rewritten, evidence, responseContract);
    ...
    // Expand terse yes/no answers with brief evidence
    answer = expandTerseYesNo(answer, rewritten, compressed, knowledge, responseContract);
