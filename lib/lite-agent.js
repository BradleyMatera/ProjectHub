...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens, structuredFacts, plan, planText, responseContract }) {
  ...
  if (hasContractConstraint || (plan && planIntents.has(plan.intent)) || isNegationConfirmation || isFutureCapability) {
    const constraints = [];
    ...
    if (isFutureCapability) {
      constraints.push(`FUTURE: This is a future/potential question. Do NOT start with "No". State current evidence and learning potential. Do not deny current ability or claim the target is already known.`);
    }
  ...
  const system = [
    ...
    'Do NOT use inflation language (expert, extensive, proficient). Use the career level indicated in FACTS.',
    'Do NOT invent technologies or companies. Only use entities in FACTS.',
    'If a claim is not in FACTS, state that it is unknown or not verified. Only say No and correct it when the claim is directly contradicted by the facts.',
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  if (route.tool === 'no_tool') {
    toolResult = {
      control: policyContract.mode,
      mode: policyContract.mode,
      visitorName: policyContract.visitorName,
      userName: conversationState?.userName,
      requestedText: policyContract.requestedText,
      previousAssistant: policyContract.previousAssistant,
      previousAssistantText: policyContract.previousAssistantText
    };
