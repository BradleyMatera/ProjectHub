...
function buildStructuredFacts(graph, entityNames, maxLines = 12) {
  ...
  for (const entityName of entityNames) {
    ...
    for (const t of subjTriples) {
      // Skip is_type and has_property (too noisy for structured facts)
...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens, structuredFacts, plan, planText }) {
  ...
  let planConstraints = '';
  const planIntents = new Set(['ADVERSARIAL', 'JOB_FIT', 'SKILL', 'COMPARISON', 'YES_NO', 'RECRUITER']);
  if (plan && planIntents.has(plan.intent)) {
    const constraints = [];
    // Only the most critical constraints — keep it under 150 chars
    if (plan.directAnswer) {
      constraints.push(`ANSWER: ${plan.directAnswer}`);
    }
    if (plan.jobFit) {
      constraints.push(`FIT: ${plan.jobFit.fitLevel}`);
    ...
    if (plan.recruiterBrief) {
      ...
      if (rb.topStrengths.length > 0) {
        constraints.push(`STRENGTHS: ${rb.topStrengths.slice(0, 3).join(',')}`);
      }
      if (rb.gaps.length > 0) {
  ...
  const system = [
    ...
    'Answer directly first, then add useful detail. 1-3 sentences.',
    ...
    '- Do NOT say "extensive experience", "expertise in", "specializing in", "adept at", "proficient in". He is entry-level.',
    '- Do NOT invent technologies for projects. Only use the tech listed in FACTS for each project.',
    ...
    '- ONLY use entities from ALLOWED_ENTITIES. Do NOT introduce new named entities.',
    ...
    'Example: Q: "Was he senior?" A: "No. He was an intern, not senior."',
    ...
    planConstraints ? truncate(planConstraints, 200) : '',
    planConstraints ? '' : '',
...
function buildCompletenessRepairPacket({ question, currentAnswer, compressedEvidence, reason, intent, maxTokens, planText }) {
  ...
  const intentGuidance = {
    YES_NO: 'Start with Yes/No, then add one specific fact from FACTS that supports it. Example: "Yes. React is part of his verified skill set and appears in his web development work."',
    SKILL: 'Start with Yes/No, then name the specific project or experience where this skill was used. Example: "Yes. His AWS internship capstone used DynamoDB as part of a serverless metadata workflow."',
    ADVERSARIAL: 'Start with No, then state the correct fact from FACTS. Add what IS true. Example: "No. He was an intern, not senior. His AWS work was a serverless metadata extraction capstone project."',
    ...
    PROJECT: 'Describe what it is, what it does, and 1-2 key technologies from FACTS. Example: "ProjectHub is an AI recruiter assistant that embeds a chat widget on any site. It uses JavaScript, Node.js, and Express."',
    COMPARISON: 'Mention both projects and one meaningful difference using facts from FACTS. Example: "ProjectHub uses JavaScript and Node.js for its chat widget, while Voice Ops uses JavaScript and Node.js for voice operations management."',
    JOB_FIT: 'State the strongest match, one piece of evidence, and one honest gap. Example: "He has direct experience with React from his skills and projects. His AWS internship provides cloud exposure. He lacks verified enterprise experience."',
    RECRUITER: 'Give a concise summary with the best 2-3 facts from FACTS. Example: "He is an entry-level developer with an AWS internship, React skills, and a recruiter AI project. Best fit: junior frontend or cloud support roles."',
    OPINION: 'State your opinion, then support it with 1-2 specific facts from FACTS. Example: "I think ProjectHub is his strongest project because it combines AI with practical web development using JavaScript and Node.js."',
    FOLLOW_UP: 'Answer the follow-up question directly with specific facts from FACTS. Use the context from the conversation.',
    GENERAL: 'Expand the answer with 1-2 specific details from FACTS. Write at least 15 words.'
  };
  ...
  if (graph && validation && validation.reasons) {
    ...
    if (unsupported.length > 0 || overclaim.length > 0) {
      ...
      for (const o of overclaim.slice(0, 2)) {
        lines.push(`OVERCLAIM: ${o} — use simpler language (e.g., "has experience with" not "expertise in")`);
      }
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model }) {
  ...
  const validation = validateAnswer(answer, sourceText, rewritten, knowledge);
    ...
    if (!completeness.complete && LITE_ENABLE_REPAIR) {
      ...
      if (compRepairResult.ok) {
        ...
        if (compAnswer.length >= 10) {
          ...
          const compValidation = validateAnswer(compAnswer, sourceText, rewritten, knowledge);
  ...
  if (LITE_ENABLE_REPAIR && validation.reasons?.length) {
    ...
    if (repairResult.ok) {
      ...
      if (repairAnswer.length >= 10) {
        ...
        const repairValidation = validateAnswer(repairAnswer, sourceText, rewritten, knowledge);
