...
async function makeRecoveryAttempt(events, contextTokens, startedAt, model, route, rewritten, toolResult, compressed, knowledge, question, emit, validationCtx) {
  ...
  // Only rejects on safety-critical issues; accepts minor grounding imperfections
  // from the small model to avoid returning "Scout is temporarily unavailable."
  const CRITICAL_REJECTION_PREFIXES = [
    'fabricated_employment',
    'adversarial_polarity_violation',
    'forbidden_claim',
    'oos_policy_violation',
    'ai_slop',
    'leaked_prompt_syntax',
  ...
  function lenientValidate(candidate) {
    ...
    if (sourceText && knowledge) {
      ...
      if (!valResult.valid) {
        const criticalReasons = (valResult.reasons || []).filter(r =>
          CRITICAL_REJECTION_PREFIXES.some(p => r.startsWith(p))
        );
        if (criticalReasons.length > 0) {
          return { valid: false, reasons: criticalReasons };
        }
        // Non-critical reasons only — accept with a note
