...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model }) {
  ...
  if (!genResult.ok) {
    ...
    return makeFallback(events, contextTokens, startedAt, selectedModel, route, rewritten_, toolResult, compressed);
  }
...
function buildGroundedFallback(toolResult, route, rewritten, compressed) {
  ...
  if (inventedCompanyMatch) {
    ...
    // Check if this company is in the compressed evidence
    const companyLower = companyName.toLowerCase();
    if (compressed && !compressed.toLowerCase().includes(companyLower) && !rewrittenStr.toLowerCase().includes('aws')) {
      // Company not found in evidence — deny employment
      return `There is no evidence Bradley worked at ${companyName}. His tech experience includes AWS internship and CIRIS freelance work.`;
    }
  }
  ...
  if (toolResult.results && Array.isArray(toolResult.results) && toolResult.results.length > 0) {
    const top = toolResult.results[0];
  ...
  // Skill evidence — extract useful details instead of just the type indicator
  if (toolResult.evidence && !['direct', 'partial', 'none', 'unknown'].includes(toolResult.evidence)) {
    return String(toolResult.evidence).substring(0, 200);
  }
  if (toolResult.details && Array.isArray(toolResult.details) && toolResult.details.length > 0) {
    const parts = [];
    for (const d of toolResult.details.slice(0, 3)) {
      if (d.items && Array.isArray(d.items)) {
        parts.push(`${d.type || 'evidence'}: ${d.items.join(', ')}`);
      } else if (d.source && d.tech) {
        parts.push(`${d.source}: ${d.tech.join(', ')}`);
      }
    }
    if (parts.length > 0) {
      return `His experience includes ${parts.join('. ')}.`;
    }
  }
  ...
  if (compressed && typeof compressed === 'string') {
    ...
    if (lines.length > 0) {
      ...
      const contentLine = lines.find(l => !/^(?:FACT|SKILL|DIRECT|DETAILS|STRONG|PARTIAL|BEST):/i.test(l)) || lines[0];
...
function makeFallback(events, contextTokens, startedAt, model, route, rewritten_, toolResult, compressed) {
  const reply = buildGroundedFallback(toolResult, route, rewritten_, compressed);
  return {
    reply,
    provider: 'ollama-lite',
    model,
