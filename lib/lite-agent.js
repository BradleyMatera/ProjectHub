...
// Token estimation — uses a word-based heuristic that is more accurate than
// the chars/4 approximation. Words average ~1.3 tokens for English text;
// punctuation and short words add overhead. This gives a tighter budget.
function estimateTokens(text) {
  if (!text) return 0;
  const str = String(text);
  const words = str.split(/\s+/).filter(Boolean);
  const punctTokens = (str.match(/[.,;:!?()[\]{}"'`/\\@#$%^&*+=|<>~]/g) || []).length;
  return Math.ceil(words.length * 1.3 + punctTokens * 0.5);
}

// Compute context budget from actual token estimates, not fixed char budgets.
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract }) {
  ...
  emit('lite_packet', { tokens: packet.estimatedTokens, chars: packet.systemPrompt.length + packet.userPrompt.length, hasPlan: !!planText, tokenBudget });
