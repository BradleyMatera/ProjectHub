...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens, structuredFacts }) {
  ...
  const system = [
    ...
    structuredFacts ? truncate(structuredFacts, 400) : '',
    structuredFacts ? '' : '',
    `FACTS:`,
    truncate(compressedEvidence, systemBudget - 600 - (structuredFacts ? 450 : 0))
  ].join('\n');
...
function buildCompletenessRepairPacket({ question, currentAnswer, compressedEvidence, reason, intent, maxTokens }) {
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model }) {
  ...
  // 5. Build lite packet with entity-scoped structured facts
  let structuredFacts = '';
  if (knowledge) {
