...
const { isCapabilityAllowed } = require('./domain-package');
...
// --- Action runtime (optional) ---
// When the server configures a tool runtime, capability calls run through the
// executor: package-level allow/deny, permission scopes, deadline budget, and
// an audit record per attempt. Without a runtime the deterministic helpers run
// directly — identical behavior, just unaudited.
let toolExecutor = null;
let toolManifest = null;
function configureToolRuntime(runtime) {
  toolExecutor = runtime?.executor || null;
  toolManifest = runtime?.manifest || null;
}
async function runCapabilityFacts(capabilityId, args, { knowledge, deadlineAt, fallback } = {}) {
  if (toolExecutor && toolManifest) {
    if (!isCapabilityAllowed(toolManifest, capabilityId)) return [];
    const remainingMs = deadlineAt ? Math.max(0, deadlineAt - Date.now()) : undefined;
    const result = await toolExecutor.execute(capabilityId, args, { knowledge, remainingMs });
...
const REPAIR_HINTS = {
  ...
  missing_computed_result: 'The user asked for a calculation or remaining/total quantity. State the COMPUTED result exactly (you may phrase the sentence naturally).',
  missing_facet_value: 'The user asked for specific items. Name at least one of the documented values listed in FACTS for the requested facet; do not substitute an unrelated fact.',
  facet_not_addressed: 'The requested facet is not documented. Say it is not documented/verified; do not answer with an unrelated fact.',
  enumeration_missing_values: 'The question asks for specific values (a list or enumeration). The answer must directly address the requested facet and name supported concrete values from the supplied FACTS — do not answer with a generic statement.',
  false_unknown_with_support: 'The answer claims the information is not documented or unknown, but the supplied FACTS do contain supported values for what was asked. State the supported values instead of claiming no evidence.',
  ...
  persona_confusion: 'You spoke as the subject (first person). You are Scout, the assistant. Use third person (he/his, she/her, they/their), not "I/my", when talking about the subject.',
  negative_assessment_ranked_weakness: 'Do not call a learning or gap area a personal weakness and do not rank a biggest/main/primary weakness. The contract says the personal weakness is UNKNOWN. State that no personal weakness is verified, then separately name a documented learning/gap area only if useful.',
  negative_personal: 'Do not assert a specific personal-weakness phrase (e.g. "struggles with", "bad at", "their weakness is"). State that no verified personal weakness is documented, then name the documented learning/gap area as a neutral learning area.',
  forbidden_claim: 'You used a forbidden phrase. Remove it and rephrase using the allowed wording from the RESPONSE INSTRUCTIONS.',
  ...
  refusal_mismatch: 'You wrote a refusal or decline, but this question is answerable from the FACTS (or is a general question you can answer directly). Answer it plainly — do not decline.',
  stance_mismatch: 'Your leading stance does not match the contract. Follow the required stance: affirm when the fact is documented, deny when it is contradicted, qualify when evidence is incomplete.',
  fabricated_occupation: 'You asserted a job title, role, or identity that is not documented in FACTS. Do not invent roles or attribute occupations — only state documented ones.',
  fabricated_employment: 'You named an employer that is not documented in FACTS. Only name employers explicitly listed in FACTS.',
  future_capability_wrong_frame: 'The question asks about FUTURE potential, not current ability. Do not open with a flat yes/no or "no information". Answer whether they could learn or grow into it, using hedged future language.',
  future_capability_missing_future_language: 'Include explicit future-oriented language (could learn, would be able to, potential to) — the question asks about future capability.',
  future_capability_current_ability_overclaim: 'Do not claim the subject can already do the target role. Frame it as future potential only.'
};
...
function rewriteQuery(question, state, knowledge, history) {
  ...
  // the server-owned discourse frame for generic alternative sets)
  if (history && knowledge) {
    const convState = buildConversationState(history, knowledge, state);
    const result = resolveReferent(q, convState, knowledge);
    if (result.resolved && result.rewrittenQuery !== q) {
      return { rewritten: result.rewrittenQuery, rewritten_: true, resolvedEntity: result.entity, referentType: result.referentType, referentContext: result.referentContext };
    }
...
function preRoute(question, state, knowledge) {
  ...
  if (nonTechExperience.test(q) || experienceWithNonTech.test(q) || nonTechFollowedByExperience.test(q)) {
    return { operation: 'search', tool: 'no_tool', args: {} };
  }
  ...
  if (earlyIntent === 'EXPERIENCE' ||
      new RegExp(`\\b(?:companies?|employers?|where\\s+(?:has|did|does)\\s+(?:${knowledgeAccess.getSubjectAlternation(knowledge)})\\s+(?:work|worked|been\\s+employed)|work\\s+history|employment\\s+history)\\b`).test(q)) {
  ...
  if (earlyIntent === 'QUALIFICATIONS' ||
      new RegExp(`\\b(?:what\\s+qualifications?|(?:${knowledgeAccess.getPossessiveAlternation(knowledge)})\\s+qualifications?|qualifications?\\s+(?:does|has|do)\\b)\\b`).test(q)) {
  ...
  if (intent === 'RECRUITER' && new RegExp(`\\b(?:worth|interview|recommend|ask\\s+(?:${knowledgeAccess.getObjectAlternation(knowledge)}))\\b`, 'i').test(q)) {
    return { operation: 'brief', tool: 'build_recruiter_brief', args: { focus: '' } };
  }
  ...
  if (new RegExp(`\\b(?:tell me about (?:${knowledgeAccess.getObjectAlternation(knowledge)})|about (?:${knowledgeAccess.getObjectAlternation(knowledge)})|(?:${knowledgeAccess.getPossessiveAlternation(knowledge)}) (?:background|skills|experience|education|certifications|goals))\\b`, 'i').test(q)) {
...
function compressControlTool(result, maxChars) {
  ...
  if (result?.computed?.length) lines.push(`COMPUTED: ${formatComputedFacts(result.computed)}`);
...
function compressMatchRole(result, budget) {
  ...
  // Explicit criteria the user supplied — these are the only terms that may
  // be assessed as requirements. Anything else is context, not a requirement.
  if (result.explicitCriteria?.length) {
    lines.push(`CRITERIA: ${result.explicitCriteria.slice(0, 6).join('; ')}.`);
  }
  ...
  // Generic candidate learning gaps are not job requirements
...
function formatResponseContract(responseContract, maxChars = 500) {
  ...
  const lines = [
    ...
    (responseContract.claimCeiling || responseContract.evidenceStrength) ? 'NOTE: ceiling and evidence labels are internal constraints — never quote them in the reply.' : ''
  ];
...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens, structuredFacts, plan, planText, responseContract, knowledge }) {
  const budget = (maxTokens || LITE_MAX_TOKENS) * 4; // chars per token estimate
  const subjectName = knowledgeAccess.getSubjectName(knowledge) ||
    (responseContract?.tenantSubject || responseContract?.subject) ||
    scoutIdentity.getSubjectName() ||
    'the subject';
  const pronouns = knowledgeAccess.getSubjectPronouns(knowledge) || { subject: 'they', object: 'them', possessive: 'their' };
  const subjectPronounList = `${pronouns.subject}/${pronouns.possessive}`;
  ...
  if (operation === 'control') {
    ...
    const scopeGuide = isScopeDecline
      ? `This question is outside your scope. ${boundary ? 'Say: ' + boundary : 'Decline and offer to discuss the candidate\'s professional background.'}\nDo not repeat the user's request, mention system prompts, API keys, passwords, or describe yourself as a large language model or AI. Keep the refusal natural and brief — one or two short sentences, under 30 words total.\n`
      : '';
    let profileGuide = '';
    if (mode === 'USER_PROFILE_QUERY') {
      ...
      if (visitorName) {
        profileGuide = `The user wants to be called "${visitorName}". Greet them warmly (e.g. "Nice to meet you" or "Hello"), say you are Scout, and ask what they want to know.\n`;
      } else if (boundary) {
    ...
    // Contract boundaries (e.g. "acknowledge user-supplied context but note it
    // is not verified") apply to any control mode, not only scope declines.
    const boundaryGuide = boundary && !isScopeDecline ? `Constraint: ${boundary}\n` : '';
    const computedGuide = responseContract?.computedFacts?.length
      ? `Computed result: ${formatComputedFacts(responseContract.computedFacts)}. If the user asked for a calculation or remaining/total quantity, state the computed result exactly.\n`
      : '';
  ...
  if (hasContractConstraint || (plan && planIntents.has(plan.intent)) || isNegationConfirmation || isFutureCapability || isNegativeAssessment) {
    ...
    if (isFutureCapability) {
      const futureTarget = responseContract?.requestedTopic || responseContract?.activeEntity || 'the target';
      constraints.push(`FUTURE: This is a future/potential question. MANDATORY: Do NOT start your answer with the word "No". Start by describing current relevant skills and experiences, then say the subject could learn or grow into ${futureTarget}. Do not deny current ability or claim ${futureTarget} is already known.`);
      constraints.push(`FUTURE-STYLE: Use a fresh sentence. Do NOT reuse or echo the previous turn's exact disclaimer. Do NOT end with "there is no verified evidence of ${futureTarget}; it is not documented in the profile" or any similar repeated wording.`);
      if (futureTarget && futureTarget !== 'the target') {
        constraints.push(`FORBIDDEN_PHRASE: Do NOT write "there is no verified evidence of ${futureTarget}; it is not documented in the profile".`);
      }
    }
    ...
    if (responseContract?.subIntent === 'QUALIFICATIONS') {
      const isTechStack = /\b(?:tech\s+stack|technology\s+stack|stack)\b/i.test(question || '');
      if (isTechStack) {
        constraints.push('TECH_STACK: This asks for the subject\'s technology stack. List ONLY the concrete languages, frameworks, tools, and services that are explicitly named in the FACTS/evidence packet. Do not include general categories like "AWS services" or "cloud services" unless the specific services are also listed. If the stack is incomplete or unknown, say so.');
      } else {
    ...
    // Mentorship: assess learning-readiness from documented gaps, not the assistant's help.
    if (responseContract?.policyMode === 'MENTORSHIP') {
      constraints.push('MENTORSHIP: This asks whether mentorship would help the subject. Base your answer on documented gaps/learning areas from FACTS. State whether a mentor would address them. Do not claim unverified growth, future mastery, or readiness without evidence.');
    }
    ...
    if (isNegativeAssessment) {
      if (responseContract?.factState === 'TRUE') {
        ...
        if (asksCurrentProgress) {
          constraints.push(`NEGATIVE: This asks about current progress on a documented gap/learning area. State current progress only if FACTS explicitly establish it; otherwise say it is unknown or not verified in the public profile, in 1-2 complete sentences. Mention the documented learning/gap area by name and include relevant evidence such as courses, learning, or mentorship. Do not infer active work from transferable skills.`);
        } else if (asksBadAt) {
          constraints.push(`NEGATIVE: The question asks what the subject is "bad at". Do not claim the subject is bad at any specific skill. Say the answer is unknown or not established, then name the documented learning/gap areas from FACTS in 1-2 complete sentences.`);
        } else {
          const gapFacts = (responseContract?.keyFacts || []).filter(f => /\b(?:gap|learning\s+area|learning\s+need|weakness(?:es)?|mentor(?:ship)?|course(?:work)?|study(?:ing)?)\b/i.test(f));
          const gapList = gapFacts.slice(0, 3).join('; ');
          constraints.push(`NEGATIVE: A documented learning/gap area is not automatically a personal weakness. Do not rank or label a gap as a personal weakness. If no personal weakness is explicitly verified, say it is unknown or not established in a complete sentence, then name the documented learning/gap areas from the list below as neutral learning areas. Do not present them as personal weaknesses.${gapList ? ` Documented gaps/learning areas: ${gapList}.` : ''}`);
          constraints.push('NEUTRAL_WORDING: Use fact-based, neutral language. Do NOT write "struggle", "struggles", "struggling", "struggled", "difficulty", "difficult", "bad at", "poor at", "weak at", "terrible at", or "a personal weakness is" when describing the documented gap/learning area.');
        }
      }
    }
  ...
  const baseSystem = isMeta ? [
    `You are Scout, an assistant for the tenant.`,
    ...
    `Do not describe ${subjectName} as if ${pronouns.subject} uses these systems; the runtime facts are about Scout, not the subject.`,
    ...
    `You are Scout, an AI assistant for ${subjectName}.`,
    `You are NOT ${subjectName}. Talk ABOUT ${subjectName} in third person using ${subjectPronounList}.`,
    'Answer directly in 1-2 complete sentences. Use specific facts from FACTS.',
    ...
    'For factual claims not supported by FACTS, use UNKNOWN/NOT VERIFIED language. Only make a definite negative claim when FACTS contain an explicit contradiction or authoritative negative boundary. Do NOT start an answer with "No" or "Not" when the claim is unverified — absence of evidence is not a denial; say it is not documented or not verified.',
    'No "as an AI" or "would you like" — just answer.',
    'NEVER use I/my/me when talking about the subject.'
  ];
  const systemLines = [
    ...baseSystem,
    responseContract?.subIntent === 'FUTURE_CAPABILITY' ? `This is a future/potential question. Do NOT start with "No". Use a fresh sentence. Do NOT reuse or echo the previous turn's exact disclaimer.` : '',
    '',
    ...
    responseContract?.computedFacts?.length ? `COMPUTED RESULTS (verified calculation — state exactly if the user asked): ${formatComputedFacts(responseContract.computedFacts)}` : '',
    `FACTS:`,
...
function buildCompletenessRepairPacket({ question, currentAnswer, compressedEvidence, reason, intent, maxTokens, planText, responseContract, missingEntities }) {
  ...
  if (responseContract) {
    ...
    // Answer obligation: enumeration questions must name supported values;
    // a false "no evidence" while supported values exist must be corrected.
    const obligation = responseContract.answerObligation;
    if (obligation?.enumeration && obligation.supportedValues?.length) {
      slots.push('The question asks for specific values. Name supported concrete values from the supplied evidence — do not answer with a generic statement.');
    }
    if (obligation && obligation.unknownAllowed === false && obligation.supportedValuesAvailable) {
      slots.push('Do not claim the requested information is undocumented or unknown — supported values exist in the evidence; state them.');
    }
    // Boundary
...
function buildLiteRepairPacket({ question, compressedEvidence, rejectionDetails, maxTokens, knowledge, validation, graph, responseContract }) {
  ...
  if (graph && validation && validation.reasons) {
    ...
    if (unsupported.length > 0 || overclaim.length > 0) {
      ...
      if (reasons.includes('fabricated_entity')) {
        lines.push('FABRICATED ENTITIES: Remove named entities not in FACTS. Use ONLY facts from FACTS.');
      }
  ...
  if (responseContract?.subIntent === 'FUTURE_CAPABILITY') {
    const futureTarget = responseContract?.requestedTopic || responseContract?.activeEntity || 'the target';
    extraConstraints.push(`FUTURE: This is a future/potential question. MANDATORY: Do NOT start your answer with the word "No". Start by describing current relevant skills and experiences, then say the subject could learn or grow into ${futureTarget}. Do not deny current ability or claim ${futureTarget} is already known.`);
    extraConstraints.push(`FUTURE-STYLE: Use a fresh sentence. Do NOT reuse or echo the previous turn's exact disclaimer. Do NOT end with "there is no verified evidence of ${futureTarget}; it is not documented in the profile" or any similar repeated wording.`);
    if (futureTarget && futureTarget !== 'the target') {
      extraConstraints.push(`FORBIDDEN_PHRASE: Do NOT write "there is no verified evidence of ${futureTarget}; it is not documented in the profile".`);
    }
  }
  ...
  const system = [
    ...
    'FACTS:',
    truncate(compressedEvidence, budget - 400 - repairContext.length - (contractInstructions ? 220 : 0) - extraConstraints.join('\n').length),
    '',
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  // Count every actual provider call request-locally. The shared router must
  // never be mutated: an early return would otherwise leave a per-request
  // wrapper installed and let later requests inflate this request's count
  // (or vice versa).
  const countedGenerate = async (...args) => {
    providerCallCount++;
  ...
  // Deterministic arithmetic / general-reasoning subtasks. These are computed
  // facts (tool output), not prose — the model still writes the answer.
  // Routed through the action runtime when configured: package-gated,
  // permission-checked, deadline-bounded, and audited.
  const computedData = await runCapabilityFacts('calculator',
    { expression: rewritten },
    { knowledge, deadlineAt, fallback: () => ({ facts: findArithmeticSubtasks(rewritten) }) });
  const computedFacts = computedData.facts || [];
  emit('lite_rewrite', { rewritten, changed: rewritten_, clarificationRequired: !!rewrite.clarificationRequired, computed: computedFacts.length });
  ...
  // Computed arithmetic/reasoning facts ride in the evidence packet for
  // non-control turns (control turns get them via compressControlTool). The
  // model may cite them but still writes all visible prose.
  if (!isControlMode && computedFacts.length) {
    compressed = compressed + '\nCOMPUTED (verified calculation): ' + formatComputedFacts(computedFacts);
  }
  ...
  try {
    responseContract = buildResponseContract(rewritten, compressed, knowledge, stateHistory, policyContract);
  } catch (e) {
  ...
  if (policyContract) {
    ...
    // Merge policy-supplied forbidden claims (e.g. future-capability targets and
    // exact disclaimer phrasing) with the contract's own forbidden set. The policy
    // must not remove any existing safety constraints.
    if (policyContract.forbiddenClaims?.length) {
      const existing = responseContract?.forbiddenClaims || [];
      responseContract.forbiddenClaims = [...new Set([...existing, ...policyContract.forbiddenClaims])];
    }
  ...
  // Attach computed arithmetic/reasoning facts to the contract so both the
  // control packet and the evidence packet can surface them to the model.
  ...
  const genResult = await countedGenerate(selectedModel, [
  ...
  const validation = validateAnswer(answer, sourceText, rewritten, knowledge, stateHistory, reqGraph, responseContract?.policyMode || null, responseContract, evidence);
  ...
  if (validation.valid) {
    ...
    if (!completeness.complete && LITE_ENABLE_REPAIR) {
      ...
      if (completeness.reason === 'TOO_SHORT' || completeness.reason === 'ADVERSARIAL_TOO_TERSE') {
        ...
        if (isTerseDenial || isTerseAffirmation) {
          ...
          if (advContract) {
            ...
            try {
              const advGen = await countedGenerate(selectedModel, [
              ...
              if (advGen.ok && advGen.text) {
                ...
                if (advAnswer.length >= 20) {
                  ...
                  if (advContract.directAnswer === 'NO' && /^(?:yes|correct|right|true)\b/i.test(advAnswer)) {
                    ...
                    const expValidation = validateAnswer(advAnswer, sourceText, rewritten, knowledge, stateHistory, reqGraph, responseContract?.policyMode || null, responseContract, evidence);
      ...
      const requireCompleteness = completeness.reason === 'COMPARISON_MISSING_ENTITY' ||
        completeness.reason === 'ENUMERATION_MISSING_VALUES' ||
        completeness.reason === 'FALSE_UNKNOWN_WITH_SUPPORT' ||
      ...
      try {
        compRepairResult = await countedGenerate(selectedModel, [
      ...
      if (compRepairResult.ok) {
        ...
        if (compAnswer.length >= 10) {
          ...
          if (hasLeak) {
            ...
            const compValidation = validateAnswer(compAnswer, sourceText, rewritten, knowledge, stateHistory, reqGraph, responseContract?.policyMode || null, responseContract, evidence);
      ...
      if (requireCompleteness && completeness.reason === 'COMPARISON_MISSING_ENTITY' && toolResult.projects) {
        ...
        return await makeRecoveryAttempt(events, contextTokens, startedAt, selectedModel, route, rewritten, toolResult, compressed, knowledge, question, emit, { sourceText, stateHistory, reqGraph, evidence, responseContract, requestDeadline, requestAbort, remainingMs, attemptTimeout, deadlineExceeded, generationCalls, providerCallCount: () => providerCallCount, countedGenerate, missingEntities: completeness.missingEntities || [] });
      }
    ...
    if (terseContract) {
      ...
      if (terseTimeout === 0) {
        ...
        try {
          const terseGen = await countedGenerate(selectedModel, [
  ...
  if (LITE_ENABLE_REPAIR && validation.reasons?.length) {
    ...
    let repairResult;
    try {
      repairResult = await countedGenerate(selectedModel, [
    ...
    if (repairResult.ok) {
      ...
      if (repairAnswer.length >= 10) {
        ...
        const repairValidation = validateAnswer(repairAnswer, sourceText, rewritten, knowledge, stateHistory, reqGraph, responseContract?.policyMode || null, responseContract, evidence);
...
async function makeRecoveryAttempt(events, contextTokens, startedAt, model, route, rewritten, toolResult, compressed, knowledge, question, emit, validationCtx) {
  ...
  const countedGenerate = validationCtx?.countedGenerate || ((...args) => router.generate(...args));
  ...
  const subjectName = knowledgeAccess.getSubjectName(knowledge) || scoutIdentity.getSubjectName() || 'the subject';
  const pronouns = knowledgeAccess.getSubjectPronouns(knowledge) || { subject: 'they', object: 'them', possessive: 'their' };
  const subjectPronounList = `${pronouns.subject}/${pronouns.possessive}`;
  ...
  // Obligation-aware recovery: enumeration answers must name supported
  // values; a false "no evidence" while supported values exist must be
  // corrected. The evidence carries the values — recovery is told WHAT kind
  // of answer is owed, never which words to write.
  const obligation = responseContract?.answerObligation;
  if (obligation?.enumeration && obligation.supportedValues?.length) {
    contract.instructions = [contract.instructions,
      'The question asks for specific values. Name supported concrete values from the supplied facts — do not answer with a generic statement.'
    ].filter(Boolean).join(' ');
  }
  if (obligation && obligation.unknownAllowed === false && obligation.supportedValuesAvailable) {
    contract.instructions = [contract.instructions,
      'Do not claim the requested information is undocumented or unknown — supported values exist in the evidence; state them.'
    ].filter(Boolean).join(' ');
  }
  ...
  function lenientValidate(candidate) {
    ...
    if (sourceText && knowledge) {
      const valResult = validateAnswer(candidate, sourceText, rewritten, knowledge, stateHistory, reqGraph, responseContract?.policyMode || null, responseContract, evidence);
      if (!valResult.valid) {
  ...
  if (!deadlineExceeded()) {
    ...
    if (t1 > 0) {
      try {
        ...
        generationAttempts++;
        const genResult = await countedGenerate(model, [
  ...
  if (!reply && !deadlineExceeded()) {
    ...
    if (t2 > 0) {
      try {
        const minimalPrompt = {
          systemPrompt: `You are Scout, an AI assistant. Answer in 1-2 complete sentences. Talk ABOUT ${subjectName} in third person using ${subjectPronounList}. Do not use inflation language.\n\nBased on the evidence: ${contract.keyFacts.map(formatKeyFact).join(' ')}\n\n${contract.instructions}`,
        ...
        generationAttempts++;
        const gen2 = await countedGenerate(model, [
  ...
  if (!reply && !deadlineExceeded()) {
    ...
    if (t3 > 0) {
      try {
        ...
        generationAttempts++;
        const gen3 = await countedGenerate(model, [
