...
function buildGroundedFallback(toolResult, route, rewritten, compressed) {
  ...
  const adversarialMatch = rewrittenStr.match(/\b(\d+)\s+years?\s+(?:of\s+)?(?:experience|exp)\b/i);
  if (adversarialMatch) {
    const years = parseInt(adversarialMatch[1], 10);
    // Check if the claimed years is plausible given the evidence
    // For entry-level candidates, any claim of 5+ years is suspicious
    if (years >= 5 && compressed && /entry.level|intern|trainee|junior/i.test(compressed)) {
