    ...
    if (isNegativeAssessment) {
      if (responseContract?.factState === 'TRUE') {
        ...
        if (asksCurrentProgress) {
          ...
          const gapFacts = (responseContract?.keyFacts || []).filter(f => /\b(?:data\s+structures?|algorithm(?:s)?|leetcode|gap|learning\s+area|learning\s+need|weakness(?:es)?|dsa|mentor(?:ship)?)\b/i.test(f));
          const gapList = gapFacts.slice(0, 3).join('; ');
          constraints.push(`NEGATIVE: A documented learning/gap area is not automatically a personal weakness. Do not rank or label a gap as the subject's weakness. If no personal weakness is explicitly verified, say it is unknown or not established in a complete sentence, then name the documented learning/gap areas from the list below as neutral learning areas. Do not present them as personal weaknesses.${gapList ? ` Documented gaps/learning areas: ${gapList}.` : ''}`);
          constraints.push('NEUTRAL_WORDING: Use fact-based, neutral language. Do NOT write "struggle", "struggles", "struggling", "struggled", "difficulty", "difficult", "bad at", "poor at", "weak at", "terrible at", or "his weakness is" when describing the documented gap/learning area.');
        }
      }
    }
