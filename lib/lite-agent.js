...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  emit('lite_generate_call', { timeoutMs: primaryTimeout, remaining: remainingMs() });
  ...
  if (!genResult.ok) {
    ...
    return await makeRecoveryAttempt(events, contextTokens, startedAt, selectedModel, route, rewritten, toolResult, compressed, knowledge, question, emit, { sourceText, stateHistory, reqGraph, evidence, responseContract, requestDeadline, requestAbort, remainingMs, attemptTimeout, deadlineExceeded });
  }
...
async function makeRecoveryAttempt(events, contextTokens, startedAt, model, route, rewritten, toolResult, compressed, knowledge, question, emit, validationCtx) {
  // validationCtx = { sourceText, stateHistory, reqGraph, evidence, responseContract, requestDeadline, requestAbort, remainingMs, attemptTimeout, deadlineExceeded }
