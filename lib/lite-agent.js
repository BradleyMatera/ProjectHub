...
function formatResponseContract(responseContract, maxChars = 500) {
  ...
  const lines = [
    `INTENT: ${responseContract.subIntent || responseContract.intent || responseContract.policyMode || ''}`,
    ...
    responseContract.polarity ? `POLARITY: ${responseContract.polarity}` : '',
    responseContract.instruction ? `INSTRUCTION: ${truncate(responseContract.instruction, 300)}` : '',
  ...
  // Policy contract keyFacts — direct evidence to include
  if (responseContract.keyFacts?.length) {
    const kf = responseContract.keyFacts.slice(0, 3).map(f => truncate(String(f), 120));
    lines.push(`KEY FACTS: ${kf.join(' | ')}`);
  }
...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract }) {
  ...
  // allowed facts, and instruction. The responseContract above is supplementary.
  if (policyContract) {
    responseContract = {
      ...(responseContract || {}),
      ...policyContract,
      // Merge allowedFacts into the evidence
      policyMode: policyContract.mode || null,
    };
    // Inject policy allowedFacts as additional evidence
    if (policyContract.allowedFacts && policyContract.allowedFacts.length > 0) {
      const policyEvidence = policyContract.allowedFacts.map(f => truncate(f, 200)).join('\n');
      compressed = compressed + '\n' + policyEvidence;
    }
  }
  ...
  if (!genResult.ok) {
    ...
    return await makeRecoveryAttempt(events, contextTokens, startedAt, selectedModel, route, rewritten, toolResult, compressed, knowledge, question, emit, { sourceText, stateHistory, reqGraph, evidence, responseContract });
  }
              ...
              if (advGen.ok && advGen.text) {
                ...
                if (advAnswer.length >= 20) {
                  ...
                  if (advContract.directAnswer === 'NO' && /^(?:yes|correct|right|true)\b/i.test(advAnswer)) {
                    ...
                    const expValidation = validateAnswer(advAnswer, sourceText, rewritten, knowledge, stateHistory, reqGraph);
...
async function makeRecoveryAttempt(events, contextTokens, startedAt, model, route, rewritten, toolResult, compressed, knowledge, question, emit, validationCtx) {
  // validationCtx = { sourceText, stateHistory, reqGraph, evidence, responseContract }
  const { sourceText, stateHistory, reqGraph, evidence, responseContract } = validationCtx || {};
  ...
  function fullValidate(candidate) {
    ...
    if (sourceText && knowledge) {
      const valResult = validateAnswer(candidate, sourceText, rewritten, knowledge, stateHistory, reqGraph);
    ...
    if (evidence && responseContract) {
      ...
      if (!completeness.complete && completeness.reason === 'TOO_SHORT') {
        return { valid: false, reasons: ['recovery_too_short'] };
      }
    }
    return { valid: true };
  }
  ...
  const recoveryTimeout = isAdversarial ? 8000 : RECOVERY_TIMEOUT_MS;
  let reply = null;
  ...
  try {
    emit('lite_recovery_generate', { intent: contract.intent, attempt: 1 });
    generationAttempts++;
    ...
    if (genResult.ok && genResult.text) {
      ...
      if (answer.length >= 10) {
        const v = fullValidate(answer);
        if (v.valid) {
          reply = answer;
          emit('lite_recovery_ok', { intent: contract.intent, attempt: 1, latencyMs: genResult.latencyMs, answerLen: answer.length });
        } else {
  ...
  if (!reply) {
    try {
      ...
      emit('lite_recovery_generate', { intent: contract.intent, attempt: 2 });
      generationAttempts++;
      ...
      if (gen2.ok && gen2.text) {
        ...
        if (answer2.length >= 10) {
          const v = fullValidate(answer2);
          if (v.valid) {
            reply = answer2;
            emit('lite_recovery_ok', { intent: contract.intent, attempt: 2, latencyMs: gen2.latencyMs });
          } else {
  ...
  if (!reply && (Date.now() - startedAt) < 60000) {
    try {
      const ultraPrompt = {
      ...
      emit('lite_recovery_generate', { intent: contract.intent, attempt: 3 });
      generationAttempts++;
      ...
      if (gen3.ok && gen3.text) {
        ...
        if (answer3.length >= 10) {
          const v = fullValidate(answer3);
          if (v.valid) {
            reply = answer3;
            emit('lite_recovery_ok', { intent: contract.intent, attempt: 3, latencyMs: gen3.latencyMs });
          } else {
