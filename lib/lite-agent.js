...
async function runLiteAgent({ question, conversationState, evidence, knowledge, sessionId, model }) {
  ...
  if (!genResult.ok) {
    ...
    return await makeRecoveryAttempt(events, contextTokens, startedAt, selectedModel, route, rewritten, toolResult, compressed, knowledge, question, emit);
  }
...
async function makeRecoveryAttempt(events, contextTokens, startedAt, model, route, rewritten, toolResult, compressed, knowledge, question, emit) {
  ...
  let reply = null;
  try {
    emit('lite_recovery_generate', { intent: contract.intent });
    ...
    if (genResult.ok && genResult.text) {
      ...
      if (answer.length >= 10) {
        // Safety: if the contract says NO (adversarial denial), reject answers that confirm the claim
        if (contract.directAnswer === 'NO' && /^(?:yes|correct|right|true|absolutely|indeed)\b/i.test(answer)) {
          emit('lite_recovery_safety_reject', { intent: contract.intent, answer: answer.slice(0, 80) });
        } else {
          reply = answer;
          emit('lite_recovery_ok', { intent: contract.intent, latencyMs: genResult.latencyMs, answerLen: answer.length });
        }
      }
    }
  } catch (e) {
    emit('lite_recovery_error', { error: e.message });
  }

  // If generative recovery failed, try one final minimal attempt
  if (!reply) {
    try {
      const minimalPrompt = {
      ...
      if (gen2.ok && gen2.text) {
        ...
        if (answer2.length >= 10) {
          // Safety: same adversarial check on second attempt
          if (contract.directAnswer === 'NO' && /^(?:yes|correct|right|true|absolutely|indeed)\b/i.test(answer2)) {
            emit('lite_recovery_fallback_safety_reject', { answer: answer2.slice(0, 80) });
          } else {
            reply = answer2;
            emit('lite_recovery_fallback_ok', { latencyMs: gen2.latencyMs });
          }
        }
      }
    } catch (e) {
      emit('lite_recovery_fallback_error', { error: e.message });
    }
  }

  // If all generative attempts failed, use a minimal evidence-boundary response
  // This is the ONLY remaining deterministic prose — a last resort when inference is unavailable
  if (!reply) {
    if (contract.keyFacts && contract.keyFacts.length > 0) {
      let fact = contract.keyFacts[0];
      // For adversarial denials, ensure the last resort starts with No
      if (contract.intent === 'ADVERSARIAL_DENY' && !/^(?:no|there is no)\b/i.test(fact)) {
        fact = `No. ${fact}.`;
      }
      reply = fact;
    } else {
      reply = "I don't have enough grounded information to answer that reliably.";
    }
    emit('lite_recovery_deterministic_last_resort', { intent: contract.intent });
  }
