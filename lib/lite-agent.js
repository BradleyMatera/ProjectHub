...
function buildGroundedFallback(toolResult, route, rewritten, compressed, knowledge) {
  ...
  if (/\b(?:worth|interview|recommend|hire|good fit)\b/i.test(rewrittenStr)) {
    if (knowledge && knowledge.summary && knowledge.summary.whoIAm) {
      return `Yes, ${knowledge.summary.whoIAm}. He has built several projects and completed an AWS internship.`;
    }
  ...
  if (toolResult.results && Array.isArray(toolResult.results) && toolResult.results.length > 0) {
    ...
    // get_project result — has a single project object
    ...
    // creating, or coding. Experience entries are often irrelevant for these.
    const isBuildQuestion = /\b(?:build|built|creating|created|code|coding|develop|developed|project|software|application)\b/i.test(rewrittenStr);
    ...
    let top;
    if (isBuildQuestion && projectResults.length > 0) {
