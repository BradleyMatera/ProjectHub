...
function buildLitePacket({ question, compressedEvidence, operation, maxTokens, structuredFacts, plan, planText, responseContract }) {
  ...
  if (operation === 'control') {
    ...
    } else if (mode === 'USER_PROFILE_UPDATE') {
      if (visitorName) {
        profileGuide = `The user wants to be called "${visitorName}". Greet them by that name and ask what they want to know.\n`;
      } else if (boundary) {
    ...
    const user = requestedText
      ? `User said: ${question}\nRequested phrase: "${requestedText}"\nReturn JSON: {"answer":"<your answer>"}`
      : `Q: ${question}\nReturn JSON: {"answer":"<your answer>"}`;
    return {
      systemPrompt: system,
      userPrompt: user,
  ...
  if (hasContractConstraint || (plan && planIntents.has(plan.intent)) || isNegationConfirmation) {
    const constraints = [];
