...
function compressMatchRole(result, budget) {
  ...
  // Strong match — what clearly fits
  if (result.matchedSkills?.length) {
    lines.push(`STRONG MATCH: ${result.matchedSkills.slice(0, 6).join(', ')}.`);
  }
  ...
  // Honest gaps — what doesn't fit
  if (result.honestGaps?.length) {
    lines.push(`GAPS: ${result.honestGaps.slice(0, 3).map(g => truncate(g, 60)).join('; ')}.`);
  }

  // Partial match — adjacent experience
  if (result.partialMatch?.length) {
    lines.push(`PARTIAL: ${result.partialMatch.slice(0, 2).map(g => truncate(g, 50)).join('; ')}.`);
  }
