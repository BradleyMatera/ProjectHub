  ...
  if (validation.valid) {
    ...
    if (!completeness.complete && LITE_ENABLE_REPAIR) {
      ...
      if (requireCompleteness && completeness.reason === 'COMPARISON_MISSING_ENTITY' && toolResult.projects) {
        ...
        if (compareMatch) {
          const entity1 = compareMatch[1].trim().split(/[,.\s]/)[0].toLowerCase();
          const entity2 = compareMatch[2].trim().split(/[,.\s]/)[0].toLowerCase();
          const answerLower = answer.toLowerCase();
          const hasEntity1 = answerLower.includes(entity1);
          const hasEntity2 = answerLower.includes(entity2);
          ...
          const missingProj = toolResult.projects.find(p =>
            p.name && p.name.toLowerCase().includes(missingEntity)
          );
          if (missingProj) {
            ...
            const tech = missingProj.tech && missingProj.tech.length
              ...
              : '';
            const appendedAnswer = `${answer} ${missingProj.name}${desc ? ` — ${truncate(desc, 80)}` : ''}${tech}.`;
            // Re-validate the appended answer
            const appendedValidation = validateAnswer(appendedAnswer, sourceText, rewritten, knowledge, stateHistory, reqGraph);
            if (appendedValidation.valid) {
