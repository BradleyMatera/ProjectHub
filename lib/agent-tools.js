...
function matchRole(knowledge, args) {
  ...
  // First, extract slash-separated terms like "CI/CD" from the RAW description
  // before normalization (which strips slashes)
  const rawDesc = String(args.jobDescription || args.role || '').slice(0, 4000);
  const slashTerms = rawDesc.match(/\b[A-Za-z]+\/[A-Za-z]+(?:\/[A-Za-z]+)*\b/g) || [];

  // Only extract requirements if the input looks like a job description
  // (contains "requiring", "role", "position", "job", or explicit skill mentions)
  // Opinion/recommendation questions like "bet on him succeeding in one type of role"
  // should NOT produce skill gaps
  ...
  // Build required terms: only include terms that match known skills/technologies
  // OR appear after "requiring/needs/must have" in a job description context
  const STOPWORDS = new Set([
    ...
    // Question/opinion words that should never be skill gaps
  ...
  const wordTerms = haystack.split(/[\s,;.|/()-]+/)
    .filter(w => w.length >= 3)
    .filter(w => !STOPWORDS.has(w));

  // Only include word terms as requirements if:
  // 1. They match a known skill/technology, OR
  // 2. The input has explicit job context (hasJobContext)
  ...
  for (const term of requiredTerms) {
    ...
    // Skip very short terms (but allow slash-terms like "ci/cd" which are meaningful)
    if (termNorm.length < 4 && !termNorm.includes('/')) continue;
    // Skip stopwords (already filtered but double-check)
