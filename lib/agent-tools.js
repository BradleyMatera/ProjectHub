...
const REQUIREMENT_MARKER = /\b(?:requiring|requires?|need(?:s|ing)?|must\s+(?:have|know)|looking\s+for|seeking|experience\s+(?:with|in)|proficiency\s+(?:with|in)|familiar(?:ity)?\s+with|skilled\s+(?:with|in)|knowledge\s+of|expertise\s+(?:in|with)|(?:job\s+)?requirements?|qualifications?|job\s+description|positions?|postings?)\b\s*:?/gi;

// Tenant-configured pronoun forms carry no criterion meaning either — they are
// subject references, not requirements.
function isSyntaxWord(w, knowledge) {
  if (REQUEST_SYNTAX_WORDS.has(w)) return true;
  const pr = knowledgeAccess.getSubjectPronouns(knowledge) || {};
  return [pr.subject, pr.object, pr.possessive].includes(w);
}

function extractExplicitCriteria(rawDesc, knownSkills, role, knowledge) {
  const criteria = [];
  const add = term => {
  ...
  // strip the title span before scanning for verbatim skill mentions. When no
  // explicit role arg was passed, recover the requested role generically so
  // titles that do not end in "role"/"position"/"job" (e.g. arbitrary
  // specialist titles) still cannot leak title words into the criteria.
  // Marker clauses are scanned on the same title-stripped text so an
  // introducing marker like "looking for" cannot promote title words into
  // requirements.
  let criteriaText = ` ${normalized(rawDesc)} `;
  const roleText = role ||
    (knowledge ? require('./response-contract').extractRequestedRole(rawDesc, knowledge) : null);
  if (roleText) criteriaText = criteriaText.replace(normalized(roleText), ' ');
  criteriaText = criteriaText.replace(/\b(?:a|an|the|our|this|that|for|as)\s+[a-z0-9+#.-]+(?:\s+[a-z0-9+#.-]+){0,4}\s+(?:role|position|job)\b/g, ' role ');
  // A slash-joined bare title ("React/TypeScript developer") is a role title,
  // not a requirements list — strip the whole title span so neither the
  // marker clause, the slash scan, nor the skill token scan can promote it.
  for (const titleSpan of rawDesc.match(/\b[a-z0-9+#.-]+(?:\/[a-z0-9+#.-]+)+\s+(?:developer|engineer|scientist|programmer|designer|architect|analyst|consultant|specialist|administrator|lead|manager|intern|contractor|freelancer|devops)\b/gi) || []) {
    criteriaText = criteriaText.replace(normalized(titleSpan), ' role ');
  }

  REQUIREMENT_MARKER.lastIndex = 0;
  const innerMarker = new RegExp(REQUIREMENT_MARKER.source, 'i');
  let m;
  while ((m = REQUIREMENT_MARKER.exec(criteriaText)) !== null) {
    let clause = criteriaText.slice(m.index + m[0].length).split(/[.!?;\n]/)[0];
    // A clause belongs to one marker — cut it at the next marker so
    // "looking for <title> requiring X" does not merge title words with the
    // "requiring" clause.
    const nextMarker = clause.slice(1).search(innerMarker);
    if (nextMarker >= 0) clause = clause.slice(0, nextMarker + 1);
    for (const chunk of clause.split(/,|\s+and\s+|\s+or\s+/)) {
      const kept = chunk.trim().split(/\s+/)
        .filter(w => w && !isSyntaxWord(w, knowledge) && !/^\d+\+?$/.test(w));
      if (kept.length) add(kept.join(' '));
    }
  }

  // Slash-joined terms are explicit criteria only when they survive title
  // stripping — "React/TypeScript developer" is a role title, not a supplied
  // requirement.
  for (const term of rawDesc.match(/\b[A-Za-z]+\/[A-Za-z]+(?:\/[A-Za-z]+)*\b/g) || []) {
    if (criteriaText.includes(normalized(term))) add(term);
  }

  // Token-aware verbatim matching: a skill is a criterion only on token
  // boundaries, so "Go" cannot match "good" nor "Java" match "JavaScript".
  for (const skill of knownSkills) {
    const s = normalized(skill);
    if (s.length < 2) continue;
    const skillRe = new RegExp(
      '(?<![a-z0-9+#.])' + s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?![a-z0-9+#])');
    if (skillRe.test(criteriaText)) add(skill);
  }
...
function matchRole(knowledge, args) {
  ...
  const explicitCriteria = extractExplicitCriteria(rawDesc, knownSkills, role, knowledge);

  // Token-boundary containment — "Java" must not satisfy a "JavaScript"
  // criterion and vice versa (same rule as the verbatim scan above).
  const skillOverlaps = (skill, term) => {
    const s = normalized(skill);
    const t = normalized(term);
    if (s === t) return true;
    if (s.length < 2 || t.length < 2) return false;
    const bound = str => new RegExp('(?<![a-z0-9+#.])' + str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?![a-z0-9+#])');
    return bound(s).test(t) || bound(t).test(s);
  };
  ...
  const requestWords = normalized(rawDesc).split(/[\s,;.|/()-]+/)
    .filter(w => w.length >= 2 && !isSyntaxWord(w, knowledge));
  const boundRe = str => new RegExp('(?<![a-z0-9+#.])' + str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?![a-z0-9+#])');
  const contextMatches = knownSkills.filter(skill => {
    if (criteriaSkills.includes(skill)) return false;
    const s = normalized(skill);
    return requestWords.some(w => s === w || boundRe(w).test(s) || boundRe(s).test(w));
  });
  ...
  for (const term of requiredTerms) {
    ...
    // Token-aware like the verbatim scan above — "Java" must not satisfy a
    // "JavaScript" criterion (substring containment would hide the gap).
    const isKnown = knownSkills.some(s => skillOverlaps(s, termNorm));
