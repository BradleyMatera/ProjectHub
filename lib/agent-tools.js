...
function matchRole(knowledge, args) {
  ...
  const requiredTerms = [
    ...
    ...haystack.split(/[\s,;.|/()-]+/)
      .filter(w => w.length >= 3)
      .filter(w => !/^(?:the|and|for|with|that|this|will|must|have|able|work|role|team|years?|experience|including|etc|strong|excellent|good|ability|plus|preferred|nice|how|does|fit|requiring|required|require|needs?|null)\b/.test(w))
  ];
