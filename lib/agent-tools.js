...
function matchRole(knowledge, args) {
  ...
  // A role name alone does not imply a fixed skill checklist. Only explicit
  // criteria in the request or skills documented in the tenant knowledge are
  // considered when deciding fit evidence.
  ...
  // Build the set of target skills from explicit criteria in the request and
  // from skills that literally match the requested role name when it is a known
  // skill or technology in the tenant knowledge.
  const explicitKnownSkills = knownSkills.filter(skill =>
    rawDesc.toLowerCase().includes(normalized(skill))
  );
  const roleNameMatches = knownSkills.filter(skill => {
    const s = normalized(skill);
    return roleWords.some(w => s === w || s.includes(w) || w.includes(s));
  });
  const targetSkills = [...new Set([...explicitKnownSkills, ...roleNameMatches])];
