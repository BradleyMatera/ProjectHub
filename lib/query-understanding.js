...
function rewriteQuery(query, history) {
  ...
  const refAlt = _subjectRefAlt();
  const isBareFollowup = new RegExp(`^(\\s*)(what about|how about|and (?:${refAlt})|also what|tell me about (?:${refAlt})|${refAlt}|that|it|more about)\\b`, 'i').test(qLower);
  ...
  const lastUserWords = lastUserLower
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
  ...
  const lastAssistantWords = lastAssistantLower
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
  ...
  const queryWords = qLower
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
    .filter(w => w.length > 3 && !['what', 'about', 'how', 'and', 'also', 'tell', 'more', 'his', 'her', 'that', 'this', 'it', 'he', 'she', 'they', 'them', 'their', ..._subjectRefForms].includes(w));
