...
// Configured subject-reference forms (all three pronoun cases). A tenant using
// ze/zir must have those tokens treated as subject references everywhere the
// built-in he|she|they forms are.
let _subjectRefForms = [];
function configureSubjectPronouns(knowledge) {
  const pr = require('./knowledge-access').getSubjectPronouns(knowledge) || {};
  _subjectRefForms = [pr.subject, pr.object, pr.possessive]
    .map(v => String(v || '').trim().toLowerCase()).filter(Boolean);
}
function _subjectRefAlt() {
  return [...new Set(['he', 'she', 'they', 'him', 'her', 'them', 'his', 'their', ..._subjectRefForms])]
...
// Narrow referent-resolution renderer. History may contribute ONLY resolved
// referents (pronouns, ordinals, elliptical references) via the structured
// conversation resolver — never raw salient words from prior turns. A short
// or follow-up question that supplies its own semantic target is returned
// unchanged so the current turn always has priority over stale context.
function rewriteQuery(query, history, knowledge) {
  const q = String(query || '').trim();
  if (!q || !Array.isArray(history) || history.length === 0) return q;
  if (!knowledge) return q;
  try {
    ...
    if (result.resolved && result.rewrittenQuery && result.rewrittenQuery !== q) {
      return result.rewrittenQuery;
    }
  } catch {
    // Referent resolution is best-effort; an unresolved turn returns as-is.
  }
  return q;
}

// ============ FULL QUERY UNDERSTANDING PIPELINE ============
function understandQuery(query, history, chunks, knowledge, options = {}) {
  // 1. Normalize
  ...
  // 3. Referent resolution — narrow, structured. No history word merging.
  const resolved = rewriteQuery(normalized, history, knowledge);

  // 4. Expand common natural-language phrasing into locally retrievable concepts.
  const expanded = expandQueryAliases(resolved);

  // 5. Classify intent
  const intent = classifyIntent(expanded);

  // Backward-compatible "rewritten" is the resolved + expanded current-turn
  // text — it contains only current-turn tokens plus substituted referents.
  const rewritten = expanded;

  // Structured retrieval legs derived from the semantic plan. When no plan
  // is supplied, legs degrade to the current-turn literal + expanded views.
  const plan = options.plan || null;
  const legs = plan
    ? require('./semantic-plan').buildRetrievalLegs({ plan, normalize: q => normalizeQuery(q, knowledge), expand: expandQueryAliases })
    : [
        { name: 'literal', query: normalized, weight: 1.0 },
        ...(expanded !== normalized ? [{ name: 'expanded', query: expanded, weight: 1.0 }] : []),
      ];
