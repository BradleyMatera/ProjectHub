...
// Optional `provenance` is an array of evidence objects whose `name`/`title`
// and `description`/`text` fields carry source identity that should be
// grounded even if the rendered packet is truncated.
function buildEntityRegistry(knowledge, sourceText, provenance = null) {
  ...
  // Configured subject pronouns are never entities either.
  const cePr = require('./knowledge-access').getSubjectPronouns(knowledge) || {};
  for (const form of [cePr.subject, cePr.object, cePr.possessive]) {
    if (form) registry.add(normalizeEntity(form));
  }
  ...
  if (knowledge) {
    ...
    // 2a. Add assistant name from identity config (e.g. "Scout")
    try {
      const scoutIdentity = require('./scout-identity');
      const assistantName = scoutIdentity.getAssistantName();
      if (assistantName) {
        registry.add(normalizeEntity(assistantName));
      }
    } catch {}

    // 2b. Add subject aliases from knowledge base
    if (Array.isArray(knowledge.subjectAliases)) {
      for (const alias of knowledge.subjectAliases) {
        if (alias) registry.add(normalizeEntity(alias));
      }
    }
    ...
    // 4. Add employer/company names and role titles from experience
    if (Array.isArray(knowledge.experience)) {
      for (const exp of knowledge.experience) {
        ...
        // Documented role titles are known entities — an answer enumerating
        // roles must not fail grounding on a verbatim documented title, and
        // "/" splits compound titles ("Associate / Volunteer") so each side
        // grounds independently.
        const roleName = exp.role || exp.title || '';
        if (roleName) {
          registry.add(normalizeEntity(roleName));
          const parts = roleName.split(/[\s:&()/-]+/).filter(p => p.length > 1);
          for (const part of parts) {
            if (part.length >= 3) registry.add(normalizeEntity(part));
          }
        }
    ...
    // 7e. Add identity title and shortPitch entities
    if (knowledge.identity?.title) {
      registry.add(normalizeEntity(knowledge.identity.title));
      for (const word of knowledge.identity.title.split(/[\s,;:&()\-]+/).filter(p => p.length >= 2)) {
        registry.add(normalizeEntity(word));
      }
    }
    if (knowledge.identity?.shortPitch) {
      const pitchWords = knowledge.identity.shortPitch.match(/\b[A-Z][A-Za-z0-9+#.-]{1,}\b/g) || [];
    ...
    // 7g. Add identity contact URLs — extract a generic platform token from the
    // hostname so public contact URLs are known entities without hardcoding a
    // brand list.
  ...
  // 7h. Add names/titles from structured provenance
  if (Array.isArray(provenance)) {
    for (const item of provenance) {
      for (const field of [item.name, item.title]) {
        const value = String(field || '');
        if (!value) continue;
        registry.add(normalizeEntity(value));
  ...
  if (sourceText) {
    ...
    // Allow up to 10 words so blog post titles and other long proper names
    // that the model may quote verbatim are recognized as grounded.
    const sourcePhrases = sourceText.match(/\b[A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+){0,10}\b/g) || [];
