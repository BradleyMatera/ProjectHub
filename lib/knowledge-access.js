...
function findAuthoritativeNegativeAssessment(knowledge, question) {
  ...
  const isRanked = /\b(?:biggest|main|primary|greatest|worst|honest)\s+(?:honest\s+)?weakness\b/i.test(qLower);
    ...
    for (const pattern of ans.questionPatterns) {
      try {
        if (new RegExp(pattern, 'i').test(qLower)) {
          // ranked is determined by the matched pattern (or the user question),
          // not by the answer text. The answer may say "biggest current gap"
          // without being an answer to a "biggest weakness" question.
          const patternRanked = /\b(?:biggest|main|primary|greatest|worst|honest)\s+(?:honest\s+)?weakness\b/i.test(pattern);
          return {
  ...
  const weaknessRe = /\b(?:biggest|main|primary|greatest|worst|honest)\s+(?:honest\s+)?weakness\b/i;
