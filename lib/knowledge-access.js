 ...
 * authoritative answer to "What is the subject bad at?" unless the prompt itself
 * asks that.
 *
...
function getSubjectPronouns(knowledge) {
  ...
  const pr = id?.pronouns;
  // Object form: fill missing fields so callers can always read
  // .subject/.object/.possessive (e.g. { subject: 'ze' } must not emit
  // "ze/undefined" in prompts). Without a subject form there is nothing to
  // normalize — fall through to the default.
  if (pr && typeof pr === 'object') {
    const subj = String(pr.subject || pr.nominative || '').trim().toLowerCase();
    if (subj) {
      const obj = String(pr.object || pr.accusative || '').trim().toLowerCase() || subj;
      const poss = String(pr.possessive || pr.genitive || '').trim().toLowerCase() || obj;
      return { subject: subj, object: obj, possessive: poss };
    }
  }
  // Legacy slash-delimited form ("she/her", "ze/zir/hir") — normalize so
  // callers can always read .subject/.object/.possessive.
  if (typeof pr === 'string') {
    const parts = pr.split('/').map(s => s.trim().toLowerCase()).filter(Boolean);
    if (parts.length) {
      return { subject: parts[0], object: parts[1] || parts[0], possessive: parts[2] || parts[1] || parts[0] };
    }
  }
  // Default to they/them/their — generic, tenant-neutral fallback until the
  // tenant supplies explicit pronouns in identity config.
...
/**
 * Escape a token for safe interpolation into a RegExp alternation.
 * @param {string} token
 * @returns {string}
 */
...
/**
 * The canonical subject-reference model. Every module that needs to decide
 * "does this text position refer to the tenant subject" MUST use these
 * helpers instead of splicing a private he|she|they alternation.
 *
 * Three alternations cover the grammatical positions seen in questions and
 * generated answers:
 *   - getSubjectAlternation:    subject position — name, aliases, and every
 *                               configured pronoun form plus he|she|they.
 *   - getPossessiveAlternation: possessive position — his|her|their plus the
 *                               configured possessive.
 *   - getObjectAlternation:     object position — him|her|them plus the
 *                               configured object form plus name/aliases.
 * getSubjectReferenceSet returns the flat lowercase token set for membership
 * checks (e.g. "is this clause head a subject reference").
 */

/**
 * Escaped alternation for any pronoun-form or name reference to the tenant
 * subject in subject position. Includes he|she|they plus the configured
 * subject/object/possessive forms (users and models freely misapply forms —
 * "zir weakness is" is still a subject reference) plus subject names/aliases.
 * @param {object} knowledge
 * @param {{includeNames?: boolean}} [opts]
 * @returns {string} alternation body, always non-empty
 */
function getSubjectAlternation(knowledge, opts = {}) {
  const includeNames = opts.includeNames !== false;
  const pr = getSubjectPronouns(knowledge);
  const tokens = new Set(['he', 'she', 'they', pr.subject, pr.object, pr.possessive]);
  if (includeNames) {
    for (const a of getSubjectAliasSet(knowledge)) {
      if (a.length >= 2) tokens.add(a);
    }
  }
  return Array.from(tokens).filter(t => t && t.length >= 2).map(escapeRegExpToken).join('|');
}

/**
 * Escaped alternation for possessive references to the tenant subject:
 * his|her|their plus the configured possessive form. Names are excluded —
 * name possession is written with "'s" and is handled by name patterns.
 * @param {object} knowledge
 * @returns {string}
 */
function getPossessiveAlternation(knowledge) {
  const pr = getSubjectPronouns(knowledge);
  const tokens = new Set(['his', 'her', 'their', pr.possessive]);
  return Array.from(tokens).filter(t => t && t.length >= 2).map(escapeRegExpToken).join('|');
}

/**
 * Escaped alternation for object-position references to the tenant subject:
 * him|her|them plus the configured object form plus subject names/aliases.
 * @param {object} knowledge
 * @param {{includeNames?: boolean}} [opts]
 * @returns {string}
 */
function getObjectAlternation(knowledge, opts = {}) {
  const includeNames = opts.includeNames !== false;
  const pr = getSubjectPronouns(knowledge);
  const tokens = new Set(['him', 'her', 'them', pr.object, pr.possessive]);
  if (includeNames) {
    for (const a of getSubjectAliasSet(knowledge)) {
      if (a.length >= 2) tokens.add(a);
    }
...
/**
 * Flat lowercase set of every token that can refer to the tenant subject:
 * names/aliases plus all pronoun forms (configured and default he/she/they,
 * him/her/them, his/her/their). For membership checks such as "is this clause
 * head a subject reference".
 * @param {object} knowledge
 * @returns {Set<string>}
 */
function getSubjectReferenceSet(knowledge) {
  const pr = getSubjectPronouns(knowledge);
  const set = new Set(['he', 'she', 'they', 'him', 'her', 'them', 'his', 'their',
    pr.subject, pr.object, pr.possessive]);
  for (const a of getSubjectAliasSet(knowledge)) {
    if (a.length >= 2) set.add(a);
  }
  for (const t of Array.from(set)) {
    if (!t || t.length < 2) set.delete(t);
  }
  return set;
}
...
module.exports = {
  ...
  getSubjectAlternation,
  getPossessiveAlternation,
  getObjectAlternation,
  getSubjectReferenceSet,
  getKnownCompanySet,
  isCategoryComplete,
