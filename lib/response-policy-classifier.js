...
function classifyResponsePolicy(question, history, knowledge) {
  ...
  const futureMatch = q.match(futureLearnRe) || q.match(futureLearnSpeedRe);
  if (futureMatch) {
    const futureStop = new Set(['a','an','the','new','like','framework','language','tool','or','and','with','for','in','on','at','of','to','from','by','as','it','that','this','there','ride','fly','cook','swim','drive','dance','sing','play','camel']);
