...
function parseClaim(question, subjectName) {
  ...
  if (/\b(?:could|would|will|can|might)\s+(?:he|she|they)\s+(?:become|learn|do|work|debug|troubleshoot|handle|pick\s+up|get\s+good|be\s+able\s+to)\b/i.test(q)) {
    return null;
  }
...
function classifyResponsePolicy(question, history, knowledge) {
  ...
  //   POSITIVE + UNSUPPORTED/CONTRADICTED → DENY
  //   NEGATIVE + UNSUPPORTED/CONTRADICTED → AFFIRM_NEGATION
  ...
  if (claim) {
    ...
    if (polarity === 'POSITIVE' && (evidenceStatus === 'UNSUPPORTED' || evidenceStatus === 'CONTRADICTED')) {
      answerStance = 'DENY';
    ...
    } else if (polarity === 'NEGATIVE' && (evidenceStatus === 'UNSUPPORTED' || evidenceStatus === 'CONTRADICTED')) {
  ...
  const introMatch = q.match(/^(?:hey|hi|hello|yo|sup|good morning|good afternoon|good evening)?\b.*?\b(?:my name is|call me|i'?m|i am|this is)\s+([a-z][a-z\s.'-]+)/i);
  if (introMatch) {
  ...
  // ===== HELP =====
  if (/\b(?:help|what can you do|what can i ask|how do i use this|how does this work|what topics|what questions|what do you know about)\b/i.test(q) &&
