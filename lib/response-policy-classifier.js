...
const { extractContinuation, extractEntitiesFromText } = require('./conversation-resolver');
...
function classifyResponsePolicy(question, history, knowledge, discourse) {
  ...
  // so a real topic change still reclassifies normally.
  ...
  if (activeFrame && (activeFrame.alternatives || []).length >= 1) {
    ...
    if (continuation && !continuation.referential) {
  ...
  if (!frameHasSet && /\bwhich\s+of\s+(?:them|these|those)\b/i.test(normalized)) {
    return {
      mode: 'CLARIFICATION',
