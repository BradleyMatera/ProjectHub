...
function classifyConversationalAct(question, history, knowledge) {
  ...
  const arithmeticExpression = /^(?:what\s+is\s+|what\s+s\s+|what['’]s\s+|calculate\s+|compute\s+|solve\s+|how\s+much\s+is\s+)?\s*[-\d\s.]+\s*(?:plus|minus|add(?:ed\s+to)?|subtracted\s+from|[+\-]|times|multiplied\s+by|x|divided\s+by|\/|%(?:\s+(?:of|off))?)\s*[-\d\s.]+(?:\s*(?:equals?|is|makes|what\s+is\s+it|what\s+do\s+i\s+get))?\s*\?*$/i;
...
function classifyResponsePolicy(question, history, knowledge) {
  ...
  const nameNicknames = { bradley: ['brad'] };
  const rawSubjectNames = [subjectName, ...subjectName.split(/\s+/)].filter(Boolean).map(s => s.toLowerCase());
  const extraNicknames = [];
  for (const n of rawSubjectNames) {
    if (nameNicknames[n]) extraNicknames.push(...nameNicknames[n]);
  }
  const subjectNames = [...new Set([...rawSubjectNames, ...extraNicknames])].join('|');
  const skillAskRe = new RegExp(`\\b(?:does (?:he|she|they|${subjectNames}) know|does (?:he|she|they|${subjectNames}) use|is (?:he|she|they|${subjectNames}) good at|is (?:he|she|they|${subjectNames}) (?:great|bad|terrible|decent|proficient|experienced|skilled|strong|weak) at|can (?:he|she|they|${subjectNames}) use|can (?:he|she|they|${subjectNames}) work with|is (?:he|she|they|${subjectNames}) familiar with|does (?:he|she|they|${subjectNames}) have)\\s+(?:in\\s+|with\\s+|at\\s+)?([a-z0-9+#.]{2,})`);
  const bestSkillRe = /\b(?:is|are)\s+([a-z0-9+#.]+)\s+(?:his|their|brad(?:ley)?(?:['']?s)?|the)?\s*(?:best|strongest|top|main|primary|favorite)\s+(?:skill|language|framework|technology|tool)\b/i;
  ...
  const roleFitPattern = new RegExp([
    ...
    '(?:would|could|should|will)\\s+(?:he|she|they|brad(?:ley)?)\\s+(?:be|make|do)\\s+(?:a\\s+)?(?:good|strong|poor|bad|weak|solid|fit|candidate|match|developer|engineer)?|',
    'is\\s+(?:he|she|they|brad(?:ley)?)\\s+(?:a|the)?\\s*(?:good|strong|poor|bad|weak|solid|fit|fit\\s+for|candidate|match|developer|engineer)\\b|',
  ...
  const futureLearnRe = /\b(?:can|could|would|will|should)\s+(?:he|she|they|brad(?:ley)?)\s+(?:be\s+able\s+to\s+)?(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable)\s+(?:with|on|in)?|become\s+(?:proficient|good|comfortable)\s+(?:with|in)?|adapt\s+to|transition\s+to|get\s+used\s+to)\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})/i;
  const futureLearnSubjectFirstRe = /\b(?:he|she|they|brad(?:ley)?)\s+(?:can|could|would|will|should)\s+(?:be\s+able\s+to\s+)?(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable)\s+(?:with|on|in)?|become\s+(?:proficient|good|comfortable)\s+(?:with|in)?|adapt\s+to|transition\s+to|get\s+used\s+to)\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})/i;
  const futureLearnSpeedRe = /\bhow\s+(?:quickly|fast|easily|long)\s+(?:could|would|can|will)\s+(?:he|she|they|brad(?:ley)?)\s+(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable))\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})/i;
  // ===== UNKNOWN TECHNOLOGY DEBUG / ISOLATION QUESTIONS =====
  const debugMatch = q.match(/\b(?:can|could|does|is)\s+(?:he|she|they|brad(?:ley)?)\s+debug\s+([a-z0-9+#.]{2,})/i);
  if (debugMatch) {
  ...
  if (futureMatch) {
    const futureStop = new Set(['a','an','the','new','like','framework','language','tool','or','and','with','for','in','on','at','of','to','from','by','as','it','that','this','there','ride','fly','cook','swim','drive','dance','sing','play','camel','right','please','yeah','but']);
    const target = futureMatch[1]
      .split(/\s+/)
    ...
    if (target && target.length >= 2) {
      return { mode: 'FUTURE_CAPABILITY', directAnswer: 'UNKNOWN',
        subjectEntity: subjectName, activeEntity: target,
        evidenceStatus: 'UNVERIFIED',
        requiredEntities: [subjectName, target],
        responseShape: getResponseShape('FUTURE_CAPABILITY'),
        evidenceRequirements: ['subject.learning_history', 'subject.learning_approach', 'subject.adjacent_skills'],
        boundary: 'Assess future learning potential from adjacent documented skills and learning history. Do NOT claim existing knowledge of ' + target + '.',
        forbiddenClaims: ['already knows ' + target, 'verified experience with ' + target] };
    }
  }
