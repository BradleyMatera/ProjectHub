...
function classifyResponsePolicy(question, history, knowledge) {
  ...
  // Detect greeting + visitor name introduction: "Hi, my name is Alex", "Hello, I'm Sam"
  const introMatch = q.match(/^(?:hey|hi|hello|yo|sup|good morning|good afternoon|good evening)\b.*\b(?:my name is|i'?m|i am|this is)\s+([a-z][a-z\s.-]+)/i);
  if (introMatch) {
    const rawName = introMatch[1].replace(/[,.!?]+$/, '').trim();
    const visitorName = rawName.charAt(0).toUpperCase() + rawName.slice(1);
    return { mode: 'GREETING', visitorName, requiredEntities: [agentName, subjectName],
