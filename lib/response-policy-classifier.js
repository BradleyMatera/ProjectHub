...
const { assessComparisonSupport, assessRelationSupport } = require('./response-planner');
...
function detectKnownGapTarget(question, graph) {
  ...
  for (const t of gapTriples) {
    ...
    const objNorm = String(t.objectNorm || '').toLowerCase();
...
function classifyResponsePolicy(question, history, knowledge, discourse) {
  ...
  const gapAskRe = /\b(?:is|are|was|were)\s+([a-z0-9+#.\s-]{2,40})\s+(?:documented as|listed as|a|an)\s+(?:gap|learning gap|learning area|weakness|documented weakness|area of improvement|improvement area)\b|\b(?:is|are)\s+([a-z0-9+#.\s-]{2,40})\s+(?:documented as|listed as)\s+(?:a gap|a weakness|a learning gap)\b/i;
  const gapMatch = q.match(gapAskRe);
  if (gapMatch) {
    const captured = gapMatch[1] || gapMatch[2];
    ...
    if (asked) {
      ...
      return { mode: 'SKILL_EVIDENCE', directAnswer: isGap ? 'YES' : 'UNKNOWN',
  ...
  if (mentionedEntities.length === 1) {
    ...
    return {
      mode: 'VERIFIED_FACT', subjectEntity: entity.name, activeEntity: entity.name,
      entityType: entity.type, requiredEntities: [entity.name],
      ...
      evidenceRequirements: [`entity.${entity.name}`], boundary: null, forbiddenClaims: []
    };
  }
  ...
  // warranty?" — the user is asking for a property/relation of the active entity
  // from the previous turn. Preserve the active entity and surface the facet.
  const facetPattern = /\b(?:what about|how about)\s+(?:its?|the|their)?\s*([a-z0-9+#.\s-]{2,40})/i;
  const facetMatch = q.match(facetPattern);
  if (facetMatch) {
    const requestedFacet = facetMatch[1].trim().toLowerCase();
    ...
    if (active) {
      ...
      return { mode: 'VERIFIED_FACT', subjectEntity: subjectName, activeEntity: active,
        ...
        boundary: 'Answer the requested facet from verified entity properties/relations only.',
        forbiddenClaims: [] };
    }
  }
