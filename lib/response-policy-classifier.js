...
const { assessComparisonSupport } = require('./response-planner');
...
function classifyResponsePolicy(question, history, knowledge, discourse) {
  ...
  if (frameHasSet &&
    ...
    const dim = assessComparisonSupport(question, members, graph);
    ...
    return {
      ...
      boundary: [
        unverified.length
          ...
          : null,
        dim.dimensionKind === 'property' && dim.support === 'UNKNOWN'
      ...
      forbiddenClaims: [
        ...
        ...(dim.dimensionKind === 'property' && dim.support === 'UNKNOWN'
          ? [`a comparative ${dim.dimension} claim the evidence does not show`] : [])
      ]
    };
  }
  ...
  if (/\b(?:compare|comparing|versus|vs\.?|difference\s+between|choosing\s+between|choose\s+between|deciding\s+between)\b/i.test(q)) {
    ...
    entities = (entities || []).filter(Boolean).slice(0, 2);
    ...
    return { mode: 'COMPARISON', requiredEntities: entities,
      ...
      boundary: dim.dimensionKind === 'property' && dim.support === 'UNKNOWN'
        ? `No verified "${dim.dimension}" data exists for these options — say the evidence cannot answer that comparison instead of inventing a winner.`
        : null,
      forbiddenClaims: dim.dimensionKind === 'property' && dim.support === 'UNKNOWN'
        ? [`a comparative ${dim.dimension} claim the evidence does not show`]
        : [] };
  }
