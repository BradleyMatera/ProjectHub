...
function classifyResponsePolicy(question, history, knowledge, discourse) {
  ...
  const futureLearnRe = new RegExp(
    String.raw`\b(?:can|could|would|will|should)\s+(?:he|she|they|@@SUBJECT_NAME@@)\s+(?:be\s+able\s+to\s+)?(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable)\s+(?:with|on|in)?|become\s+(?:proficient|good|comfortable)\s+(?:with|in|on)?|become|be|adapt\s+to|transition\s+to|get\s+used\s+to)\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})`
      .replace(/@@SUBJECT_NAME@@/g, `(?:${subjectNamePattern})`),
    'i'
  );
  const futureLearnSubjectFirstRe = new RegExp(
    String.raw`\b(?:he|she|they|@@SUBJECT_NAME@@)\s+(?:can|could|would|will|should)\s+(?:be\s+able\s+to\s+)?(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable)\s+(?:with|on|in)?|become\s+(?:proficient|good|comfortable)\s+(?:with|in|on)?|become|be|adapt\s+to|transition\s+to|get\s+used\s+to)\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})`
      .replace(/@@SUBJECT_NAME@@/g, `(?:${subjectNamePattern})`),
    'i'
  );
