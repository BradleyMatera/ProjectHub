    ...
    if (continuation && !continuation.referential && !extractExplicitSet(continuation.name)) {
      ...
      if (!incompatible) {
        return {
          ...
          // Open-world: the user introduced this option; that does not make it
          // verified knowledge. Direct evidence state is UNKNOWN, not FALSE.
          evidenceStatus: 'UNVERIFIED',
          boundary: `"${name}" was introduced by the user in conversation — it is not established as verified experience. Assess it only against the supplied evidence; if the evidence does not directly cover it, say the verified evidence is insufficient rather than inventing experience or a confident fit.`,
          forbiddenClaims: [`verified experience with ${name} that the evidence does not show`]
        };
      }
    }
  }
