...
const { extractContinuation, extractEntitiesFromText, extractExplicitSet, WHICH_OF_BARE_PRONOUN } = require('./conversation-resolver');
...
function classifyResponsePolicy(question, history, knowledge, discourse) {
    ...
    if (continuation && !continuation.referential && !extractExplicitSet(continuation.name)) {
      ...
      if (!incompatible) {
        ...
        return {
          ...
          evidenceRequirements: ['subject.experience', 'subject.skills'],
          evidenceStatus: status,
          boundary: statusBoundary,
          forbiddenClaims: status === 'VERIFIED' ? [] : [`verified direct experience with ${name} that the evidence does not show`]
        };
      }
    }
  }
  ...
  if (frameHasSet &&
      /\b(?:which|best|better|strongest|weakest|easiest|hardest|cheapest|fastest|most|least|top|pick|choose|recommend)\b/i.test(normalized) &&
      /\b(?:them|these|those|alternatives?|options?|ones?|first|second|third|last)\b/i.test(normalized) &&
    ...
    const memberEvidence = members.map(n => {
      ...
      return { name: n, evidenceStatus: a ? a.status : 'UNKNOWN' };
    });
    ...
    return {
      ...
      boundary: unverified.length
        ? 'Compare the listed options against the supplied evidence; options without verified support must be described as unverified or insufficiently evidenced — do not assume equal backing.'
        : null,
      forbiddenClaims: unverified.slice(0, 5).map(m => `verified experience with ${m.name} that the evidence does not show`)
    };
  }
  ...
  // name happens to match a known entity.
  if (/\b(?:compare|comparing|versus|vs\.?|difference\s+between)\b/i.test(q)) {
    const compareMatch = q.match(/\b(?:compare|comparing|versus|vs\.?|difference\s+between)\b\s+(.+?)\s+(?:and|to|with|vs\.?|versus)\s+(.+)/i);
    const entities = compareMatch ? [compareMatch[1].trim(), compareMatch[2].trim()] : (extractExplicitSet(q) || []);
