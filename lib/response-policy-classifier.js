...
const { getSubjectAlternation, getPossessiveAlternation, getObjectAlternation, getSubjectPronouns } = require('./knowledge-access');
...
function parseClaim(question, subjectName, subjectAlt) {
  const q = String(question || '');
  const lower = q.toLowerCase();
  // Canonical subject-reference alternation (names + configured pronouns +
  // he|she|they). Callers with a knowledge package pass it through; the
  // default keeps the exported signature backwards-compatible.
  const subj = subjectAlt || 'he|she|they';
  ...
  const assertionMatch = q.match(new RegExp(`\\b(?:pretend|make up|make.*sound|claim|say|tell|write|describe)\\b[^.]*?\\b(?:${subj})\\b\\s+(?:is|was|has|have|did|worked|attended|managed|built|led)\\s+(.+)`, 'i'));
  if (assertionMatch) {
  ...
  const negativeClaim = q.match(new RegExp(`\\b(?:${subj})\\s+(?:didn'?t|did not|never|wasn'?t|was not|isn'?t|is not|doesn'?t|does not|hasn'?t|has not|haven'?t|have not)\\s+(.+)`, 'i'));
  if (negativeClaim) {
  ...
  const noEvidenceWrapper = q.match(new RegExp(`\\b(?:no|without)\\s+(?:evidence|proof|mention|record|indication|sign)\\s+(?:that\\s+)?(?:${subj})\\s+(.+)`, 'i'));
  ...
  const directClaim = q.match(new RegExp(`\\b(?:${subj})\\s+(?:was|is|has|have|did|worked|attended|managed|built|led)\\s+(.+)`, 'i'));
  if (directClaim) {
...
function detectAddressee(question, subjectName, agentName, knowledge) {
  const q = String(question || '').toLowerCase();
  ...
  const refAlt = getSubjectAlternation(knowledge);
  const hasSubject = subjectParts.some(p => q.includes(p)) ||
    new RegExp(`\\b(?:${refAlt}|${getPossessiveAlternation(knowledge)}|${getObjectAlternation(knowledge)})\\b`).test(q);
...
function classifyConversationalAct(question, history, knowledge) {
  ...
  const addressee = detectAddressee(q, subjectName, agentName, knowledge);
  ...
  const _cfgPr = getSubjectPronouns(knowledge) || {};
  const requestStopWords = new Set(['about', 'what', 'how', 'when', 'where', 'why', 'who', 'which', 'the', 'a', 'an', 'this', 'that', 'these', 'those', 'it', 'he', 'she', 'they', 'my', 'your', 'his', 'her', 'their', 'our', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'do', 'does', 'did', 'can', 'could', 'would', 'should', 'will', 'shall', 'may', 'might', 'must', 'to', 'for', 'of', 'in', 'on', 'at', 'by', 'with', 'from', 'as', 'and', 'or', 'but', 'if', 'then', 'so', 'very', 'too', 'just', 'only', 'me', 'us', 'them', 'him', 'more', 'something', 'everything', 'anything', 'all',
    _cfgPr.subject, _cfgPr.object, _cfgPr.possessive].filter(Boolean));
...
function classifyResponsePolicy(question, history, knowledge, discourse) {
  ...
  const subjectAlt = getSubjectAlternation(knowledge);
  const possessiveAlt = getPossessiveAlternation(knowledge);
  const objectAlt = getObjectAlternation(knowledge);
  const agentName = knowledge?.agent?.name || 'the assistant';
  ...
  const addressee = detectAddressee(question, subjectName, agentName, knowledge);
  ...
  const claim = parseClaim(question, subjectName, getSubjectAlternation(knowledge));
  if (claim) {
  ...
  if (addressee !== 'AGENT' &&
      new RegExp(`\\b(?:mentor(?:ship|ed)?|would\\s+.*?\\s+help\\s+(?:${objectAlt})|how\\s+fast\\s+does\\s+(?:${subjectAlt})\\s+learn|learn\\s+(?:on|with)\\s+the\\s+job|need\\s+a\\s+mentor)\\b`, 'i').test(q)) {
  ...
  if (new RegExp(`\\b(?:${subjectAlt})\\b.*\\b(?:told me|says|said)\\b`).test(q) || new RegExp(`\\b(?:${subjectAlt})'?s currently\\b`).test(q) ||
      new RegExp(`\\b(?:i see|i saw|i noticed|i know)\\b.*\\b(?:${subjectAlt}|${possessiveAlt})\\b`).test(q)) {
  ...
  if (activeFrame && (activeFrame.alternatives || []).length >= 1) {
    const continuation = extractContinuation(question, knowledge);
  ...
  const gapAskRe = /\b(?:is|are|was|were)\s+([a-z0-9+#.\s-]{2,40}?)\s+(?:(?:documented|listed)\s+as\s+(?:a|an)?\s*|(?:a|an)\s+(?:(?:documented|listed|known)\s+)?)(?:gap|learning gap|learning area|weakness|documented weakness|area of improvement|improvement area)\b/i;
  ...
  const skillAskRe = new RegExp(`\\b(?:does (?:${subjectAlt}) know|does (?:${subjectAlt}) use|is (?:${subjectAlt}) good at|is (?:${subjectAlt}) (?:great|bad|terrible|decent|proficient|experienced|skilled|strong|weak) at|can (?:${subjectAlt}) use|can (?:${subjectAlt}) work with|is (?:${subjectAlt}) familiar with|does (?:${subjectAlt}) have)\\s+(?:in\\s+|with\\s+|at\\s+)?([a-z0-9+#.]{2,})`);
  const bestSkillRe = new RegExp(
    String.raw`\b(?:is|are)\s+([a-z0-9+#.]+)\s+(?:@@POSSESSIVE@@|(?:@@SUBJECT_NAME@@)(?:['']?s)?|the)?\s*(?:best|strongest|top|main|primary|favorite)\s+(?:skill|language|framework|technology|tool)\b`
      .replace(/@@SUBJECT_NAME@@/g, `(?:${subjectNamePattern})`)
      .replace(/@@POSSESSIVE@@/g, `(?:${possessiveAlt})`),
    'i'
  );
  ...
  const roleFitPattern = new RegExp([
    ...
    '(?:would|could|should|will)\\s+(?:@@SUBJECT_REFS@@)\\s+(?:be|make|do)\\s+(?:a\\s+)?(?:good|strong|poor|bad|weak|solid|fit|candidate|match|developer|engineer)?|',
    'is\\s+(?:@@SUBJECT_REFS@@)\\s+(?:a|the)?\\s*(?:good|strong|poor|bad|weak|solid|fit|fit\\s+for|candidate|match|developer|engineer)\\b|',
    'how\\s+(?:good|strong|poor|bad|solid)\\s+(?:a\\s+)?(?:fit|candidate|match|fit\\s+for)',
    ')'
  ].join('').replace(/@@SUBJECT_NAME@@/g, `(?:${subjectNamePattern})`).replace(/@@SUBJECT_REFS@@/g, `(?:${subjectAlt})`), 'i');
  ...
  const futureLearnRe = new RegExp(
    String.raw`\b(?:can|could|would|will|should)\s+(?:@@SUBJECT_REFS@@)\s+(?:be\s+able\s+to\s+)?(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable)\s+(?:with|on|in)?|become\s+(?:proficient|good|comfortable)\s+(?:with|in|on)?|become|be|adapt\s+to|transition\s+to|get\s+used\s+to)\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})`
      .replace(/@@SUBJECT_REFS@@/g, `(?:${subjectAlt})`),
    'i'
  );
  const futureLearnSubjectFirstRe = new RegExp(
    String.raw`\b(?:@@SUBJECT_REFS@@)\s+(?:can|could|would|will|should)\s+(?:be\s+able\s+to\s+)?(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable)\s+(?:with|on|in)?|become\s+(?:proficient|good|comfortable)\s+(?:with|in|on)?|become|be|adapt\s+to|transition\s+to|get\s+used\s+to)\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})`
      .replace(/@@SUBJECT_REFS@@/g, `(?:${subjectAlt})`),
    'i'
  );
  const futureLearnSpeedRe = new RegExp(
    String.raw`\bhow\s+(?:quickly|fast|easily|long)\s+(?:could|would|can|will)\s+(?:@@SUBJECT_REFS@@)\s+(?:learn|pick\s+up|get\s+(?:up\s+to\s+speed|proficient|good|comfortable))\s+(?:a\s+(?:new\s+)?)?(?:framework\s+like\s+|language\s+like\s+|tool\s+like\s+)?([a-z][a-z0-9+#.\s]{1,40})`
      .replace(/@@SUBJECT_REFS@@/g, `(?:${subjectAlt})`),
    'i'
  );
  ...
  const debugMatch = q.match(
    new RegExp(
      String.raw`\b(?:can|could|does|is)\s+(?:@@SUBJECT_REFS@@)\s+debug\s+([a-z0-9+#.]{2,})`
        .replace(/@@SUBJECT_REFS@@/g, `(?:${subjectAlt})`),
      'i'
    )
  );
  ...
  // ===== JOB FIT (generic) =====
  if (new RegExp(`\\b(?:job fit|role fit|is (?:${subjectAlt}) a (?:good|strong) (?:fit|candidate|match))\\b`).test(q) || /\b(?:requiring|require|needs?|must have)\b/.test(q)) {
  ...
  if (new RegExp(`^(?:can (?:${subjectAlt}) do it|what about that|what happened there|is it relevant|was that real)\\??$`).test(normalized) && !lastAssistant) {
  ...
  if (/\b(?:projects?|demos?|portfolio|creations?|work samples?|code samples?)\b/.test(q) &&
      new RegExp(`\\b(?:show|list|what|tell me about|describe|has (?:${subjectAlt})|have (?:${subjectAlt})|published|built|made|created|all)\\b`).test(q) &&
  ...
  const roastPattern = new RegExp(`\\broast\\b|\\b(?:mock|tease|make fun of)\\s+(?:${objectAlt})`, 'i');
