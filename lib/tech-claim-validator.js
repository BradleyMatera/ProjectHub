...
function isTechInEvidence(technology, evidenceText, knownTechnologies = null) {
  ...
  if (knownTechnologies && knownTechnologies.size > 0) {
    ...
    if (!known) {
      // Multi-word technology names like "GitHub Actions" or "AWS Lambda" may
      // not appear as a single canonical token in the known set, but are
      // legitimate when the leading word is a known technology and the full
      // phrase is present in the scoped evidence.
      const words = String(technology).trim().split(/\s+/).filter(Boolean);
      if (words.length > 1) {
        const firstCanonical = canonicalize(words[0]);
        const firstResolved = resolveAlias(firstCanonical);
        const firstKnown = knownTechnologies.has(firstCanonical) || knownTechnologies.has(firstResolved);
