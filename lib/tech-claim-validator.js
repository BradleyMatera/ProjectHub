...
// "vanilla JavaScript".
//
// The match must appear in a non-negated clause of the scoped evidence —
// "Rust is not documented" contains the phrase but asserts the opposite.
// knownTechnologies may assist alias recognition (via resolveAlias inside
// the caller's canonicalization) but must never authorize a scoped
// relationship on its own: a technology used by an unrelated project is not
// evidence that this entity uses it.
function isTechInEvidence(technology, evidenceText, knownTechnologies = null) {
  ...
  // Check each non-negated clause of the scoped evidence for the phrase.
  const sentences = String(evidenceText).split(/(?<=[.!?])\s+|\n+/).filter(s => s.trim());
  for (const sentence of sentences) {
    for (const clause of splitClauses(sentence)) {
      if (hasNegation(clause)) continue;
      if (phraseAppears(clause, technology, tokenNormalizer)) return true;
    }
  }
