...
// Uses true normalized token comparison on BOTH sides with the same
// canonicalizer, avoiding unsafe substring matches like "Go" matching "Google".
// If knownTechnologies is provided, the tech must be in that set (prevents
// a full-KB source string from making every mentioned tech appear supported).
function isTechInEvidence(technology, evidenceText, knownTechnologies = null) {
  ...
  const techResolved = resolveAlias(techCanonical);

  if (knownTechnologies && knownTechnologies.size > 0) {
    return knownTechnologies.has(techCanonical) || knownTechnologies.has(techResolved);
  }

  const evidenceTokens = extractEvidenceTokens(evidenceText);

  // Check resolved alias match
  if (evidenceTokens.has(techResolved)) return true;
  // Check direct canonical match
  if (evidenceTokens.has(techCanonical)) return true;
