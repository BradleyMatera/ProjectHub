...
// Phrases like "AWS services" / "cloud services" are generic categories,
// not specific technologies. A claim "uses AWS services such as Lambda" should
// validate the concrete examples, not the category itself.
const GENERIC_SERVICE_PROVIDERS = new Set([
  'aws', 'azure', 'gcp', 'google cloud', 'cloud', 'database', 'compute',
  'storage', 'networking', 'security', 'monitoring', 'analytics', 'ai', 'ml',
]);

function isGenericServiceCategory(cleanTech) {
  const m = String(cleanTech).match(/^(.+?)\s+services?$/i);
  if (!m) return false;
  return GENERIC_SERVICE_PROVIDERS.has(m[1].toLowerCase());
}
...
const COMMON_ENGLISH_WORDS = new Set([
  ...
  'troubleshooting', 'debugging', 'documentation', 'support', 'customer',
  'evidence', 'mixed',
...
const TECH_CLAIM_CONTEXTS = [
  ...
  /\b(?:uses?|used|utiliz\w+|built\s+with|developed\s+with|implemented\s+with|written\s+in|written\s+with|powered\s+by|runs\s+on|relies\s+on|depends\s+on)\s+([A-Za-z][A-Za-z0-9+#.\s-]{1,40}?)(?:\s+(?:for|in|on|to|and|;)|(?:,|\.(?:\s|$))|$)/gi,
  ...
  /\b(?:knows?|has\s+experience\s+(?:with|in|at)|proficient\s+in|skilled\s+in|familiar\s+with|expertise\s+in|has\s+worked\s+with|worked\s+with|has\s+used|programs?\s+in|codes?\s+in|writes?\s+in|develops?\s+in)\s+([A-Za-z][A-Za-z0-9+#.\s-]{1,40}?)(?:\s+(?:for|in|on|to|and|;)|(?:,|\.(?:\s|$))|$)/gi,
  ...
  /\b(?:built\s+with|developed\s+with|implemented\s+with|uses?|used|written\s+in|knows?|has\s+experience\s+(?:with|in|at)|proficient\s+in|skilled\s+in)\s+[A-Za-z][A-Za-z0-9+#.,\s-]{1,40}?\s+and\s+([A-Za-z][A-Za-z0-9+#.-]{1,30})(?:\s+(?:for|in|on|to|;)|(?:,|\.(?:\s|$))|$)/gi,
  ...
  /\b(?:built\s+with|developed\s+with|implemented\s+with|uses?|used|written\s+in|knows?|has\s+experience\s+(?:with|in|at)|proficient\s+in|skilled\s+in)\s+[A-Za-z][A-Za-z0-9+#.\s-]{1,30}?,\s+([A-Za-z][A-Za-z0-9+#.-]{1,30})(?:\s+(?:for|in|on|to|and|;)|(?:,|\.(?:\s|$))|$)/gi,
  // "includes/featuring X" — tech list
  /\b(?:includes?|featuring|incorporating)\s+([A-Za-z][A-Za-z0-9+#.\s-]{1,40}?)(?:\s+(?:for|in|on|to|and|;)|(?:,|\.(?:\s|$))|$)/gi,
];
...
function isTechInEvidence(technology, evidenceText, knownTechnologies = null) {
  ...
  // First check whether the exact multiword phrase appears in the scoped
  // evidence. This lets project- or runtime-specific descriptions that are
  // literally in the evidence (e.g. "deterministic query understanding") be
  // quoted without requiring them to be pre-registered as "known technologies".
  if (phraseAppears(evidenceText, technology, tokenNormalizer)) return true;

  // If the phrase itself is not in the evidence, fall back to the known
  // technologies set. A known skill/project tech is accepted even when the
  // scoped evidence string is truncated, because the tenant knowledge is the
  // authoritative source. Unknown single-word techs are rejected.
  if (knownTechnologies && knownTechnologies.size > 0) {
    const resolved = resolveAlias(techCanonical);
    if (knownTechnologies.has(techCanonical) || knownTechnologies.has(resolved)) return true;
  }
      ...
      for (const pattern of TECH_CLAIM_CONTEXTS) {
        ...
        while ((match = pattern.exec(clause)) !== null) {
          ...
          for (let g = 1; g < match.length; g++) {
            ...
            // Split on lead-in examples ("such as", "like", "including") first,
            // then on commas and "and" to get individual technologies
            const techParts = rawTech
              .split(/\s+(?:such\s+as|like|including)\s+/i)
              .flatMap(t => t.split(/,\s*|\s+and\s+/))
            ...
            for (const tech of techParts) {
              ...
              const cleanTech = tech
                .replace(/\s+(?:such\s+as|as|in|on|for|with|by|to|using|during|while|at|and|like|including|,|\.|;|$).*$/i, '')
                .replace(/[.,!?;)]+$/, '')
                .trim();
              ...
              // Skip generic service categories like "AWS services" or "cloud services"
              // that introduce concrete examples ("AWS services such as Lambda").
              if (isGenericServiceCategory(cleanTech)) continue;
