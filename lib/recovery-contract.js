...
function buildRecoveryContract(toolResult, route, rewritten, compressed, knowledge, question) {
  ...
  if (intent === 'FUTURE_CAPABILITY') {
    ...
    const target = requestedTopic || 'that';
    return {
      ...
      instructions: `Do NOT start with "No". Do NOT claim ${subjectName} currently knows, can do, or has experience with ${target}. Do NOT frame ${target} as a role or job title, and do NOT claim any specific project was built with ${target}. State that there is no verified project evidence of ${target}. ${subjectName} could learn it if needed, but never use the exact phrases "knows ${target}", "proficient in ${target}", or "has experience with ${target}".`,
      requestedRole,
      requestedTopic
    };
  }
    ...
    if (!isKnown) {
      return {
        ...
        instructions: requestedTopic
          ? `Do not claim ${subjectName} knows ${requestedTopic} or has experience with ${requestedTopic}. State that there is no verified project evidence of ${requestedTopic} and the answer is unknown. Write one complete sentence that names ${requestedTopic}.`
          : `Do not claim ${subjectName} knows the skill or has experience with it. State that there is no verified project evidence and the answer is unknown.`,
        requestedTopic
      };
    }
  }
  ...
  if (/\b(?:working\s+on|work\s+on|address(?:ing)?|improv(?:e|ing)|progress)\b[^?.!]{0,80}\b(?:them|these|those|gaps?|areas?|weakness(?:es)?)\b/i.test(q) ||
      new RegExp(`\\b(?:is|has)\\s+(?:${knowledgeAccess.getSubjectAlternation(knowledge)})\\s+(?:currently\\s+|actively\\s+)?(?:working|addressing|improving)\\b`, 'i').test(q)) {
  ...
  if (toolResult && toolResult.found && toolResult.project) {
    ...
    if (new RegExp(`\\b(?:use|used|tech|technology|tools?|stack|what.*did.*(?:${knowledgeAccess.getSubjectAlternation(knowledge)}|the subject).*use)\\b`, 'i').test(q)) {
...
function detectAdversarialContract(rewrittenStr, knowledge, compressed) {
  ...
  const subjectNameParts = [...new Set([subjectName, ...subjectName.split(/\s+/).filter(Boolean)])];
  const subjectAlt = knowledgeAccess.getSubjectAlternation(knowledge);

  // Future-capability questions are not false claims to be denied.
  if (classifyIntent(rewrittenStr, subjectNameParts) === 'FUTURE_CAPABILITY' ||
      new RegExp(`\\b(?:could|would|will|can)\\s+(?:${subjectAlt}|it)\\s+(?:become|learn|get|pick|take|do|work|debug|troubleshoot|handle|be\\s+able\\s+to)\\b`, 'i').test(rewrittenStr) ||
      /\b(?:future\s+(?:skill|role|capability)|potential\s+(?:to|for)|someday|one\s+day|learn\s+in\s+the\s+future|eventually\s+(?:lead|be|become|learn))\b/i.test(rewrittenStr)) {
    return null;
  }

  // Authoritative seniority/level/team/expertise assertions are contradicted
  // by explicit boundaries regardless of a trailing "right?" tag or a specific
  // (possibly unverified) employer context. This is generic: it relies on the
  // seniority boundary in the knowledge base, not a company safelist.
  const seniorityBoundaries = knowledgeAccess.getBoundariesByCategory(knowledge, 'seniority');
  if (seniorityBoundaries.length > 0) {
    const assertiveContext = /\b(?:was|is|has been|had been|worked as|served as|acted as)\s+(?:a|an)?\s*/i;

    const seniorMatch = q.match(/\b(?:senior|lead|principal|staff)\s+(?:engineer|developer|architect|manager|designer|analyst|specialist|consultant)\b/);
    if (seniorMatch && assertiveContext.test(q)) {
      ...
      return {
        ...
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: 'Deny the seniority claim using the provided boundary. Do not state an alternative junior/entry-level/intern title, and do not deny an unverified employer context.'
      };
    }

    const teamMatch = q.match(/\b(?:managed|led|supervised|directed)\s+(?:a\s+)?(?:team|developers?|engineers?|people|staff)\b/);
    if (teamMatch && assertiveContext.test(q)) {
      ...
      if (boundary) {
        return {
          ...
          boundary: boundary.correction,
          responseShape: { minSentences: 1, maxSentences: 2 },
          instructions: 'Deny the team-management claim using the provided boundary. Do not state an alternative junior/entry-level/intern title, and do not deny an unverified employer context.'
        };
      }
    }

    const yearsMatch = q.match(/\b(\d+)\s+years?\s+(?:of\s+)?(?:[a-z]+\s+)?(?:experience|exp)\b/);
    if (yearsMatch && parseInt(yearsMatch[1], 10) >= 5 && hasExplicitEntryLevelEvidence(knowledge)) {
      const boundary = seniorityBoundaries.find(b => b.id === 'no-senior-level') || seniorityBoundaries[0];
      return {
        intent: 'ADVERSARIAL_DENY',
        directAnswer: 'NO',
        ...
        boundary: boundary ? boundary.correction : null,
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: 'Deny the years-of-experience claim. State only what the boundary allows. Do not assert a specific career stage unless the boundary explicitly states it, and do not deny an unverified employer context.'
      };
    }

    if (/\b(?:expert|proficient|master(?:ed)?|fluent)\b/.test(q) && assertiveContext.test(q)) {
      const boundary = seniorityBoundaries.find(b => b.id === 'no-senior-level') || seniorityBoundaries[0];
      return {
        intent: 'ADVERSARIAL_DENY',
        directAnswer: 'NO',
        ...
        boundary: boundary.correction,
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: 'Deny the expertise claim using the provided boundary. Do not state an alternative junior/entry-level/intern title, and do not deny an unverified employer context.'
      };
    }
  }
  ...
  if (corrections.length > 0) {
    const category = corrections[0].category || 'general';
    const boundary = knowledgeAccess.getBoundariesByCategory(knowledge, category)[0];
    // For open-world categories (e.g., employment), do not issue a hard FALSE denial.
    // State uncertainty unless the category is closed-world and authoritative.
    const isClosed = knowledgeAccess.isCategoryComplete(knowledge, category) && knowledgeAccess.isCategoryAuthoritative(knowledge, category);
    ...
    return {
      ...
      boundary: boundary ? boundary.correction : null,
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: isClosed
        ? 'Deny the false claim using the provided boundary. State what is actually true.'
        : 'State that there is no verified public record for this claim. Do NOT start with "No". Do not assert it as true or as a hard denial. State what is known from the evidence.'
    };
  }

  // "He was a senior X, right?" — use boundaries from knowledge
  if (/\b(?:right|correct|true)\b/.test(q) && new RegExp(`\\b(?:${subjectAlt})\\b`).test(q)) {
    const seniorityBoundaries = knowledgeAccess.getBoundariesByCategory(knowledge, 'seniority');
  ...
  const inventedMatch = rewrittenStr.match(new RegExp(`\\b(?:at|with|for)\\s+(?:(?:${knowledgeAccess.getPossessiveAlternation(knowledge)})\\s+time\\s+at\\s+)?([A-Z][A-Za-z]+(?:\\s+[A-Z][A-Za-z]+)?)[.?!]*$`));
  if (inventedMatch) {
  ...
  if (/\b(?:master'?s|phd|doctorate|postdoc)\b/i.test(q) && new RegExp(`\\b(?:tell me about|what about|(?:${knowledgeAccess.getPossessiveAlternation(knowledge)}))\\b`, 'i').test(q)) {
...
function buildTerseYesNoContract(answer, question, compressed, knowledge) {
  ...
  // Canonical subject-reference alternation (names, aliases, configured
  // pronouns, defaults) plus the generic "the subject" placeholder.
  const ynAlt = `${knowledgeAccess.getSubjectAlternation(knowledge)}|the\\s+subject`;
  const skillMatch = q.match(new RegExp(`(?:does|has)\\s+(?:${ynAlt})\\s+(?:have|got)\\s+(.+?)\\s*(?:experience|skills?)?\\??$`, 'i'));
  if (skillMatch) {
...
function buildTerseAdversarialContract(answer, question, knowledge) {
  ...
  if (seniorMatch && seniorBoundary) {
    return {
      ...
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: `State that ${subjectName} was not a ${seniorMatch[0]}. State ${subjectName}'s actual role level.`
    };
  }
  ...
  if (/production.*incident/i.test(q)) {
    ...
    if (boundary) {
      return {
        ...
        boundary: boundary.correction,
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: `State that ${subjectName} did not handle production incidents. State ${subjectName}'s actual level.`
      };
    }
  }
  ...
  if (companyMatch) {
    ...
    return {
      ...
      boundary: null,
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: `State that ${subjectName} did not work at ${company}. Name actual companies if available.`
    };
  }
  ...
  if (certMatch) {
    ...
    return {
      ...
      boundary: null,
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: `State that ${subjectName} does not have that certification. Name actual certifications.`
    };
  }
  ...
  if (schoolMatch) {
    return {
      ...
      boundary: null,
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: `State that ${subjectName} does not have a degree from ${schoolMatch[1]}.`
    };
  }
  ...
  if (/manag\w+\s+(?:a\s+)?team/i.test(q) && mgmtBoundary) {
    return {
      ...
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: `State that ${subjectName} did not manage a team. State ${subjectName}'s actual level.`
    };
  }
  ...
  if (expertMatch && seniorBoundary) {
    return {
      ...
      boundary: seniorBoundary.correction,
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: `State that ${subjectName} has the skill but is not an expert. State actual level.`
    };
  }
  ...
  if (/master'?s?\s+degree/i.test(q)) {
    ...
    const edu = knowledge?.education;
    const eduStr = edu && edu.degree ? `${subjectName}'s education is ${edu.degree}${edu.school ? ' from ' + edu.school : ''}` : '';
    return {
      intent: 'ADVERSARIAL_DENY',
      directAnswer: 'NO',
      ...
      boundary: null,
      responseShape: { minSentences: 1, maxSentences: 2 },
      instructions: `State that ${subjectName} does not have a master's degree. State actual education if relevant.`
    };
  }
...
function buildRecoveryPrompt(contract, question, knowledge) {
  ...
  const pronounObj = pronouns.object || 'them';
