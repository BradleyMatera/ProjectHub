...
function buildRecoveryContract(toolResult, route, rewritten, compressed, knowledge, question) {
  ...
  if (/\b(?:what\s+concerns?|what\s+worries|what\s+risks?|what\s+reservations)\b/i.test(q)) {
    ...
    return {
      ...
      keyFacts: isEntryLevel
        ? knowledgeAccess.getBoundaries(knowledge).slice(0, 3).map(b => rawFact('boundary', b.correction, { category: b.category, id: b.id }))
        : [rawFact('seniority', 'entry-level', { basis: 'internship and training projects' })],
      boundary: isEntryLevel ? (knowledgeAccess.getBoundariesByCategory(knowledge, 'seniority')[0] || {}).correction || null : null,
  ...
  if (adversarialMatch) {
    ...
    if (years >= 5 && isEntryLevel) {
      return {
        ...
        directAnswer: 'NO',
        keyFacts: [rawFact('experience_years', years, { note: 'unsupported' }), rawFact('seniority', 'entry-level', { basis: 'internship and project experience' })],
  ...
  if (/\b(?:worth|interview|recommend|hire|good fit)\b/i.test(q)) {
    ...
    return {
      ...
      boundary: isEntryLevel ? 'entry-level candidate with internship and project experience' : null,
      responseShape: { minSentences: 2, maxSentences: 3 },
  ...
  if (toolResult && toolResult.matchedSkills !== undefined && (toolResult.gaps || toolResult.strong || toolResult.partial)) {
    ...
    return {
      ...
      keyFacts: [
        ...
        isEntryLevel ? rawFact('seniority', 'entry-level', { basis: 'internship and project-based' }) : null
      ].filter(Boolean),
      boundary: isEntryLevel ? (knowledgeAccess.getBoundariesByCategory(knowledge, 'seniority')[0] || {}).correction || null : null,
    ...
    if (yearsMatch && parseInt(yearsMatch[1], 10) >= 5) {
      ...
      if (isEntryLevel) {
        ...
        return {
          ...
          keyFacts: boundary ? [rawFact('boundary', boundary.correction)] : [rawFact('seniority', 'entry-level', { basis: 'internship and project experience' })],
          boundary: boundary ? boundary.correction : null,
          responseShape: { minSentences: 1, maxSentences: 2 },
          instructions: 'Deny the years-of-experience claim using the provided boundary. State the actual level.'
        };
      }
    }
...
function buildRecoveryPrompt(contract, question, knowledge) {
  ...
  const seniorityBoundaries = knowledgeAccess.getBoundariesByCategory(knowledge, 'seniority');
  const entryLevelNote = seniorityBoundaries.length > 0
    ? `Do NOT use inflation language (expert, extensive, proficient). ${seniorityBoundaries[0].correction}`
    : 'Do NOT use inflation language (expert, extensive, proficient).';
