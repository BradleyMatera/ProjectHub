...
function buildRecoveryContract(toolResult, route, rewritten, compressed, knowledge, question) {
  ...
  if (adversarialMatch) {
    ...
    if (years >= 5) {
      return {
        ...
        boundary: seniorityBoundary ? seniorityBoundary.correction : null,
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: 'Start with No. State that the evidence does not support that many years of experience. Do NOT assert a specific career stage such as "entry-level" or "junior".'
      };
    }
  }
  ...
  if (/\b(?:worth|interview|recommend|hire|good fit)\b/i.test(q)) {
    ...
    const topSkills = (knowledge?.skills?.languagesAndFrameworks || knowledge?.skills?.core || []).slice(0, 3);
    const keyProjects = (knowledge && knowledge.projects || []).slice(0, 2).map(p => p.name);
    return {
      intent: 'RECOMMENDATION',
      directAnswer: 'YES',
      factState: 'TRUE',
      keyFacts: [
        rawFact('core_skills', topSkills),
        rawFact('projects', keyProjects),
        hasInternship ? rawFact('experience', 'has internship experience') : null
      ].filter(Boolean),
      keyProjects,
      hasInternship,
      boundary: seniorityBoundary ? seniorityBoundary.correction : null,
      responseShape: { minSentences: 2, maxSentences: 3 },
      instructions: 'Give a grounded positive recommendation. Name specific projects and skills. Do not inflate seniority. Do not use the phrase "early-career" or "entry-level" as a job title. Do not assert a specific career stage beyond the evidence.'
    };
  }
  ...
  if (toolResult && toolResult.matchedSkills !== undefined && (toolResult.gaps || toolResult.strong || toolResult.partial)) {
    ...
    return {
      ...
      keyFacts: [
        ...
        seniorityBoundary ? rawFact('boundary', seniorityBoundary.correction) : null
      ].filter(Boolean),
      requiredEntities: strong.length > 0 ? strong.slice(0, 3).map(s => s.skill) : (partial.length > 0 ? partial.slice(0, 3).map(s => s.skill) : []),
      boundary: seniorityBoundary ? seniorityBoundary.correction : null,
      responseShape: { minSentences: 2, maxSentences: 4 },
      instructions: 'State the fit level. Name the matching skills and gaps. Do NOT assign a fabricated job title or role (e.g. "Full Stack Engineer Frontend"). Do not assert seniority or a specific career stage beyond the evidence.'
    };
  }
  ...
  if (toolResult && toolResult.skill !== undefined) {
    ...
    if (evidenceType === 'project' || evidenceType === 'work') {
      const projectDetails = (toolResult.details || []).filter(d => d.type === 'project' || d.type === 'work').slice(0, 2);
      const sources = [...new Set(projectDetails.map(d => d.source).filter(Boolean))];
      const allTech = [...new Set(projectDetails.flatMap(d => d.tech || d.skills || []))].slice(0, 5);
      return {
        intent: 'SKILL_PROJECT',
        directAnswer: 'YES_BUT_LIMITED',
        keyFacts: [
          rawFact('skill', skill, { evidence: evidenceType, sources }),
          allTech.length > 0 ? rawFact('used_with', allTech) : null
        ].filter(Boolean),
        boundary: null,
        responseShape: { minSentences: 2, maxSentences: 3 },
        instructions: `State that ${skill} appears in the evidence only in project/work context. Do NOT say he is good at it, proficient, or that it is a core skill. Name the project or work context and keep the claim limited to the evidence shown.`
      };
    }

    if (evidenceType === 'certification') {
      const certDetails = (toolResult.details || []).filter(d => d.type === 'certification').slice(0, 2);
      const sources = [...new Set(certDetails.map(d => d.source).filter(Boolean))];
      const certSkills = [...new Set(certDetails.flatMap(d => d.skills || []))].slice(0, 5);
      return {
        intent: 'SKILL_CERTIFICATION',
        directAnswer: 'YES',
        keyFacts: [
          rawFact('skill', skill, { evidence: 'certification', sources }),
          certSkills.length > 0 ? rawFact('certification_skills', certSkills) : null
        ].filter(Boolean),
        boundary: null,
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: `Confirm that ${skill} is backed by a certification. Name the certification and any related skills it covers.`
      };
    }
  ...
  if (/\b(?:right|correct|true)\b/.test(q) && /\b(?:he|she|they)\b/.test(q)) {
    ...
    if (seniorityBoundaries.length > 0 && seniorMatch) {
      ...
      return {
        ...
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: 'Deny the seniority claim using the provided boundary. Do NOT assert a specific career stage such as "entry-level" or "junior".'
      };
    }
    ...
    if (yearsMatch && parseInt(yearsMatch[1], 10) >= 5) {
      if (hasExplicitEntryLevelEvidence(knowledge)) {
        ...
        return {
          ...
          boundary: boundary ? boundary.correction : null,
          responseShape: { minSentences: 1, maxSentences: 2 },
          instructions: 'Deny the years-of-experience claim. State only what the boundary allows. Do NOT assert a specific career stage such as "entry-level" or "junior".'
        };
      }
    }
    if (seniorityBoundaries.length > 0 && /\b(?:expert|proficient|master(?:ed)?|fluent)\b/.test(q)) {
      ...
      return {
        ...
        boundary: boundary.correction,
        responseShape: { minSentences: 1, maxSentences: 2 },
        instructions: 'Deny the expertise claim using the provided boundary. Do NOT assert a specific career stage or proficiency level beyond the boundary.'
      };
    }
  }
