...
function buildRecoveryContract(toolResult, route, rewritten, compressed, knowledge, question) {
  ...
  if (intent === 'FUTURE_CAPABILITY') {
    ...
    return {
      ...
      instructions: `Do not claim he currently knows, can do, or has experience with ${target}. State that there is no verified evidence, but ${target} is something he could learn if needed.`,
      requestedRole,
      requestedTopic
    };
  }
  ...
  if (intent === 'SKILL' || intent === 'SKILL_EVIDENCE') {
    const requestedTopic = extractRequestedTopic(q, knowledge, subjectNames);
    ...
    if (!isKnown) {
      return {
        ...
        instructions: requestedTopic
          ? `Do not claim he knows ${requestedTopic}. State that there is no project evidence of ${requestedTopic} and the answer is unknown.`
          : 'Do not claim he knows the skill. State that there is no project evidence and the answer is unknown.',
        requestedTopic
      };
    }
  }
