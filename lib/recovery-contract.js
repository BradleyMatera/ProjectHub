...
function buildRecoveryContract(toolResult, route, rewritten, compressed, knowledge, question) {
  ...
  if (intent === 'FUTURE_CAPABILITY') {
    ...
    return {
      ...
      instructions: `Do NOT start with "No". Do NOT claim he currently knows, can do, or has experience with ${target}. Do NOT frame ${target} as a role or job title, and do NOT claim any specific project was built with ${target}. State that there is no verified project evidence of ${target}. He could learn it if needed, but never use the exact phrases "knows ${target}", "proficient in ${target}", or "has experience with ${target}".`,
      requestedRole,
      requestedTopic
    };
  }
