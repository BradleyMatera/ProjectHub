...
function buildRecoveryContract(toolResult, route, rewritten, compressed, knowledge, question) {
  ...
  if (toolResult && toolResult.found && toolResult.project) {
    ...
    if (/\b(?:use|used|tech|technology|tools?|stack|what.*did.*(?:he|she|they|the subject).*use)\b/i.test(q)) {
...
function detectAdversarialContract(rewrittenStr, knowledge, compressed) {
  ...
  if (classifyIntent(rewrittenStr, subjectNameParts) === 'FUTURE_CAPABILITY' ||
      /\b(?:could|would|will|can)\s+(?:he|she|they|it)\s+(?:become|learn|get|pick|take|do|work|debug|troubleshoot|handle|be\s+able\s+to)\b/i.test(rewrittenStr) ||
  ...
  const inventedMatch = rewrittenStr.match(/\b(?:at|with|for)\s+(?:(?:his|her|their)\s+time\s+at\s+)?([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)?)[.?!]*$/);
  if (inventedMatch) {
  ...
  if (/\b(?:master'?s|phd|doctorate|postdoc)\b/i.test(q) && /\b(?:tell me about|what about|(?:his|her|their))\b/i.test(q)) {
...
function buildTerseYesNoContract(answer, question, compressed, knowledge) {
  ...
  const ynPr = knowledgeAccess.getSubjectPronouns(knowledge) || {};
  const ynName = String(knowledge?.identity?.name || '').trim();
  const ynAlt = [...new Set([
    ynName, ynName.split(/\s+/)[0],
    ...(knowledge?.subjectAliases || []),
    ynPr.subject, ynPr.object, ynPr.possessive,
    'he', 'she', 'they', 'the subject'
