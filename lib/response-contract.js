...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (directAnswer === 'NO') {
    instructions.push('Start with "No" in a full sentence to deny only the specific false assertion. Do NOT state an alternative junior/entry-level/intern title or derive a career stage. Do NOT deny an unverified employer context. Then state only that the specific claim is not documented or not verified. Use a phrase such as "not a senior [role]", "not a [role]", "does not document a [role] role", "no verified evidence", or "no public evidence". Keep the denial to one or two sentences and do not add unrelated details.');
