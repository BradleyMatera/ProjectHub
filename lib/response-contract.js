...
function determineFactState(intent, subIntent, evidenceStrength, directAnswer, selectedFacts) {
  ...
  if (subIntent === 'NEGATIVE_ASSESSMENT') {
    ...
    // NEGATIVE_ASSESSMENT requires explicit authoritative gap evidence.
    // Without a direct negative boundary, the fact state is UNKNOWN.
    return 'UNKNOWN';
  }
...
function determineDirectAnswer(question, intent, evidence, knowledge) {
  ...
  // Role phrases like "junior frontend" are expanded to skill hints so the
  // evidence check is meaningful; only unambiguous matches count.
  if (intent === 'JOB_FIT') {
    let required = null;
    const roleMatch = question.match(/(?:requiring|require|needs?)\s+(.+?)(?:\?|$)/i);
    if (roleMatch) {
      required = roleMatch[1].toLowerCase();
    } else {
      const requested = extractRequestedRole(question, knowledge);
      if (requested) required = requested.toLowerCase();
    }
    if (required) {
      const ROLE_SKILL_HINTS = {
        frontend: ['javascript', 'html', 'css', 'react', 'typescript', 'vue', 'angular', 'front-end'],
        'front-end': ['javascript', 'html', 'css', 'react', 'typescript', 'vue', 'angular'],
        backend: ['node.js', 'node', 'python', 'sql', 'database', 'api', 'express', 'django', 'mongodb', 'postgresql'],
        'back-end': ['node.js', 'node', 'python', 'sql', 'database', 'api', 'express', 'django', 'mongodb', 'postgresql'],
        fullstack: ['javascript', 'node.js', 'react', 'html', 'css', 'node', 'typescript'],
        'full-stack': ['javascript', 'node.js', 'react', 'html', 'css', 'node', 'typescript'],
        devops: ['docker', 'kubernetes', 'aws', 'ci/cd', 'cicd', 'github actions', 'terraform'],
        'ci/cd': ['github actions', 'cicd', 'jenkins'],
        data: ['python', 'sql', 'database', 'pandas', 'machine learning', 'data analysis', 'tableau']
      };

      const tokens = required.split(/\s+and\s+|\s*,\s*|\s*\/\s*|\s+/)
        .map(s => s.trim())
        .filter(s => s.length >= 2 && !/\b(?:role|position|job|junior|senior|developer|engineer)\b/.test(s));
      if (tokens.length === 0) return null;

      const requiredSkillSets = tokens.map(token => {
        const hints = ROLE_SKILL_HINTS[token];
        return hints ? [token, ...hints] : [token];
      });

      const matches = [];
      const gaps = [];
      for (const skillSet of requiredSkillSets) {
        let found = false;
        ...
        if (found) matches.push(skillSet[0]);
        else gaps.push(skillSet[0]);
      }

      const hasExplicitRequirements = !!roleMatch;

      if (matches.length === 0) {
        // Underspecified role (e.g. "junior frontend role?") with no verified
        // matches is not a hard NOT_FIT — it is open-world. NOT_FIT is only
        // appropriate when the user supplied explicit missing requirements.
        return hasExplicitRequirements ? 'NOT_FIT' : 'UNKNOWN';
      }
      if (matches.length === requiredSkillSets.length) return 'FIT';
      return 'PARTIAL_FIT';
    }
  ...
  // Negative assessment — there is no verified public weakness unless a boundary says so
...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (directAnswer === 'NO') {
    ...
    instructions.push('State clearly that the answer is unknown based on verified data. Do not invent a positive or negative claim.');
  }
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    instructions.push('This is a future/potential question. Answer only with what can be reasonably inferred from current evidence and motivation. Do NOT start with "No". Do NOT frame the target as a role or job title. Do NOT claim a specific project was built using the target technology. Only describe transferable relevant evidence (e.g., JavaScript, WebGPU) and future learning potential. If there is no direct evidence, state the current status is unknown and the subject could learn it with time.');
  }
