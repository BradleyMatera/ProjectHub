...
function buildResponseContract(question, evidence, knowledge, history = []) {
  ...
  if (subIntent === 'NEGATIVE_ASSESSMENT') {
    const gaps = knowledge?.skills?.learningOrAdjacent || [];
...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    instructions.push('Do NOT start with "No". This is a future/potential question. State the current evidence, then describe learning potential. Do NOT deny current ability or claim the target is already known.');
  }
