...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    ...
    instructions.push(`Do NOT start with "No", "not", or any negative/absence statement such as "There is no verified evidence". This is a future/potential question. Start with the current documented skills from the evidence as a foundation, then describe future learning potential for ${target}. You MUST explicitly name "${target}" and you MUST include at least one of the phrases "can learn", "could learn", "future learning", "learning potential", or "yes" in the answer. Do NOT claim the subject already knows ${target}, has verified experience with ${target}, or does the target role now. Do not invent transferable work-style traits. The requested role or skill is a target, not a historical job title; do not say the subject held it.`);
    if (requestedRole) {
