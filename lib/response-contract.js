...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    instructions.push('Do NOT start with "No". This is a future/potential question. State the current evidence, then describe learning potential. Do NOT deny current ability or claim the target is already known. Cite only documented skills from the evidence as a foundation; do not invent transferable work-style traits. The requested role or skill is a target, not a historical job title; do not say the subject held it.');
  }
  ...
  if (factState) {
    if (factState === 'UNKNOWN') {
      ...
      instructions.push('FACT_STATE: Evidence contradicts the claim. Deny the specific false assertion. Do NOT state an alternative junior/entry-level/intern title or derive a career stage from the denial. Do NOT name the actual employer, job title, internship, or dates from the evidence. State only that the specific claim is not supported by the verified profile.');
