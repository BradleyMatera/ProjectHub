...
function extractRequestedRole(question, knowledge) {
  ...
  const target = lower.match(/\b(?:hiring|recruiting)\s+for\s+(?:an?\s+)?([^.!?;,]+?)(?=\s+(?:at|with|requiring|requires?)\b|[.!?;,]|$)/) ||
    ...
    lower.match(/^(?:why\s+)?is\s+([a-z0-9+#.\s-]+?)\s+(?:not\s+)?(?:an?\s+)?(?:good|strong|poor|bad|weak)?\s*fit\b/);
  if (target && !/^(?:he|she|they|it|this|that)$/i.test(target[1].trim())) {
    const role = target[1].replace(/\s+(?:role|position|job)\s*$/, '').trim();
...
function getIntentKeywords(intent) {
  const keywordMap = {
    ...
    RECRUITER: ['experience', 'project', 'skill', 'internship', 'junior', 'candidate', 'strength'],
