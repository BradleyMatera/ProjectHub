...
function determineForbiddenClaims(intent, knowledge, evidenceStrength, subIntent = '', factState = null, requestedTopic = null) {
  ...
  // Future capability must not be framed as current knowledge/experience and
  // must not echo the SKILL_EVIDENCE UNKNOWN disclaimer template.
  if ((subIntent === 'FUTURE_CAPABILITY' || intent === 'FUTURE_CAPABILITY') && requestedTopic) {
    forbidden.push(`already knows ${requestedTopic}`, `verified experience with ${requestedTopic}`, `there is no verified evidence of ${requestedTopic}; it is not documented in the profile`);
  }
...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    ...
    instructions.push(`Do NOT start with "No", "not", or any negative/absence statement. This is a future/potential question. Start by saying either "Yes, ${subjectName} can learn ${target}" or "${subjectName} could learn ${target}". Then briefly mention adjacent documented skills from the evidence and make clear ${subjectName} does not currently have verified ${target} experience, using a fresh sentence — do NOT reuse or echo the previous turn's exact disclaimer. You MUST name "${target}" and you MUST include one of the phrases "can learn", "could learn", "future learning", "learning potential", or "yes". Do NOT claim the subject already knows ${target}, has verified experience with ${target}, or does the target role now.`);
    if (requestedRole) {
  ...
  if (subIntent === 'SKILL_EVIDENCE' && requestedTopic) {
    if (factState === 'UNKNOWN') {
      if (/\b(?:well|expert|advanced|proficient|mastery|master|highly\s+skilled|very\s+experienced|deep\s+knowledge|strong\s+in|good\s+at|thoroughly|extensively)\b/i.test(question)) {
        instructions.push(`There is no verified evidence of ${requestedTopic} in the profile. The question asks about proficiency or mastery, which is not documented. Answer with a short, grounded denial that includes the words "no", "not", "documented", and "project experience". Example forms: "There is no verified evidence that ${subjectName} knows ${requestedTopic} well, and it is not documented in the profile." or "It is not documented whether ${subjectName} is advanced at ${requestedTopic}; ${subjectName} has project experience with it."`);
      } else {
        instructions.push(`There is no verified evidence of ${requestedTopic} in the profile. Answer with a short denial that includes the words "no", "not", and "no verified evidence". Example forms: "No, there is no verified evidence that ${subjectName} knows ${requestedTopic}, and it is not documented in the profile." or "There is no verified evidence of ${requestedTopic}; it is not documented."`);
      }
    } else {
  ...
  if (subIntent === 'ENTITY_FACET') {
    if (factState === 'UNKNOWN' || evidenceStrength === 'UNKNOWN') {
      instructions.push(`The requested facet "${requestedTopic}" is not established in the knowledge base for ${subject || 'this entity'}. Do NOT state a specific value. Answer with a short, qualified statement that includes the words "no verified evidence", "not documented", or "not established". Example forms: "There is no verified evidence of a ${requestedTopic} for ${subject || 'this entity'}; it is not documented." or "The ${requestedTopic} of ${subject || 'this entity'} is not established in the available record."`);
    } else {
