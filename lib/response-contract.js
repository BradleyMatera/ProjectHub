...
function determineDirectAnswer(question, intent, evidence, knowledge) {
  ...
  if (intent === 'ADVERSARIAL') {
    ...
    if (employerMatch && knowledge) {
      ...
      // Only issue a hard NO when the tenant explicitly declares the employment
      // history is both complete and authoritative. Otherwise fall through and
      // let the claim-entity check return UNKNOWN for unsupported employers.
