...
function determineDirectAnswer(question, intent, evidence, knowledge) {
  ...
  if (intent === 'SKILL') {
    ...
    if (directMatch) {
      ...
      const weakEvidence = !/(?:professional|production|employed|senior|lead|staff)\b/i.test(evLower);
...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    instructions.push('Do NOT start with "No", "not", or any negative/absence statement such as "There is no verified evidence". This is a future/potential question. Start with the current documented skills from the evidence as a foundation, then describe future learning potential for the target. You MUST include at least one of the words "future", "potential", "learn", "learning", or "could" in the answer. Do NOT claim the subject already has the skills for the target role, does the target role now, or has "relevant experience" / "gained experience" as that role. Do not invent transferable work-style traits. The requested role or skill is a target, not a historical job title; do not say the subject held it.');
    if (requestedRole) {
      ...
      instructions.push('Do NOT name any specific technology, framework, or tool unless it appears explicitly in the supplied EVIDENCE. Do not describe the target as a current or past job title.');
    }
  }
  ...
  if (claimCeiling && evidenceStrength !== 'IDENTITY' && factState !== 'UNKNOWN') {
  ...
  if (subIntent === 'QUALIFICATIONS') {
    instructions.push('The user is asking about the subject\'s skills, tech stack, or qualifications. List the documented skills, languages, frameworks, tools, and cloud services from the evidence that answer the question. Be specific: use exact names from the facts (e.g., JavaScript, TypeScript, React, Node.js, AWS). Do not add items that are not explicitly named.');
  }
