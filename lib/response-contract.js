...
function buildResponseContract(question, evidence, knowledge, history = []) {
  ...
  if (subIntent === 'NEGATIVE_ASSESSMENT') {
    ...
    for (const gap of [...honestGaps, ...learning]) {
      const text = typeof gap === 'string' ? gap : gap?.name;
      if (text && !keyFacts.includes(text)) keyFacts.push(text);
    }
    // Include any authoritative negative assessment text (interview/faq/direct) as a key fact.
    const assessment = knowledgeAccess.findAuthoritativeNegativeAssessment(knowledge, q);
    if (assessment?.answer && !keyFacts.includes(assessment.answer)) {
      keyFacts.push(assessment.answer);
    }
  }
...
function classifySubIntent(question, intent, knowledge, subjectNames = []) {
  ...
  if (intent === 'META') {
    ...
    if (/\b(?:what can(?:'t|not)\s+you\s+do|what\s+can\s+you\s+not\s+do|what limits|daily cap|rate limit|what are your limits)\b/.test(lower)) return 'META_LIMITS';
...
function selectContractFacts(rankedFacts, subIntent, subject, activeEntities) {
  ...
  if (String(subIntent || '').startsWith('META_')) {
...
function determineDirectAnswer(question, intent, evidence, knowledge) {
  ...
  if (intent === 'ADVERSARIAL') {
    ...
    // Employment claim against a closed-world authoritative company list.
    const employerMatch = qLower.match(/\b(?:worked|worked at|was|is)\s+(?:at\s+|for\s+|with\s+)?([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)?)\b/);
  ...
  if (intent === 'JOB_FIT') {
    ...
    if (requestedRole) {
      ...
      const matchedTokens = roleTokens.filter(token =>
        [...tokenToSkills.get(token)].some(name => hasEvidence(name))
      );

      if (matchedTokens.length === roleTokens.length) return 'FIT';
...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    instructions.push('Do NOT start with "No". This is a future/potential question. State the current evidence, then describe learning potential. Do NOT deny current ability or claim the target is already known. The requested role or skill is a target, not a historical job title; do not say the subject held it.');
  }
  ...
  if (factState) {
    if (factState === 'UNKNOWN') {
      ...
      instructions.push('FACT_STATE: Evidence contradicts the claim. Deny the specific false assertion. Do NOT state an alternative junior/entry-level/intern title or derive a career stage from the denial. Do NOT deny an unverified employer context. State only what the verified profile documents about the claim.');
  ...
  if (directAnswer === 'NO') {
    instructions.push('Start with "No" in a full sentence to deny only the specific false assertion. Do NOT state an alternative junior/entry-level/intern title or derive a career stage. Do NOT deny an unverified employer context. Then state only what the verified profile documents.');
    ...
    instructions.push('Do NOT start with Yes or No. State clearly that the answer is unknown or not documented based on verified data. Name the specific topic or entity. Do not invent a positive or negative claim.');
  }

  if (subIntent === 'SKILL_EVIDENCE' && requestedTopic) {
    if (factState === 'UNKNOWN') {
      instructions.push(`There is no verified evidence of ${requestedTopic} in the profile. Do not say "he knows ${requestedTopic}" or "he does not know ${requestedTopic}". State only that it is not documented or verified, and name the topic.`);
    } else {
  ...
  if (subIntent === 'META_CAPABILITIES') {
    instructions.push('List the candidate/profile topics you can answer: projects, skills, experience, education, certifications, role fit, and contact info if public. Do not claim unrelated AI abilities or self-improvement.');
  }
  if (subIntent === 'CONTACT') {
  ...
  if (subIntent === 'NEGATIVE_ASSESSMENT') {
    ...
    if (factState === 'TRUE') {
      ...
      if (asksBadAt) {
        ...
        instructions.push('If no personal weakness is explicitly verified, say that it is unknown or not established in the verified/public profile. You may name documented learning or gap areas, but label them as learning/gap areas rather than personal weaknesses. Start with "Based on the public profile" or a similar grounded phrase.');
      }
    }
  }
  if (subIntent === 'JOB_FIT' || subIntent === 'RECRUITER_RECOMMENDATION') {
    instructions.push('This is a role-fit or interview question. Ground the answer in project evidence, skills, and documented gaps. Do not invent a job title, employer, employment dates, or seniority that are not in the verified facts. The requested role is hypothetical; do not say the subject held it or was a [role] at a company.');
  }
