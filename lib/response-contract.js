...
function extractRequestedRole(question, knowledge) {
  ...
  const lower = question.toLowerCase();
  const target = lower.match(/\b(?:hiring|recruiting)\s+for\s+(?:an?\s+)?([^.!?;,]+?)(?=\s+(?:at|with|requiring|requires?)\b|[.!?;,]|$)/) ||
  ...
  if (target && !/^(?:he|she|they|it|this|that)$/i.test(target[1].trim())) {
    const role = target[1].replace(/^\s*(?:a|an|the)\b\s*/i, '').replace(/\s+(?:role|position|job)\s*$/, '').trim();
  ...
  const cleanRole = value => String(value || '')
    ...
    .replace(/\s+(?:requiring|requires?|needs?|must\s+have|with)\b.*$/i, '')
    .trim();
  ...
  const cleanTarget = value => {
    ...
    v = v.replace(/\s*(?:role|roles|position|positions|job|jobs)\s*$/i, '').trim();
