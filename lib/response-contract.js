...
const { buildRelationshipGraph, EXPERTISE_RELATIONS } = require('./relationship-graph');
 ...
 */
function buildResponseContract(question, evidence, knowledge, history = [], policy = null) {
  const normalizedEntities = require('./knowledge-entities').normalizeKnowledgeEntities(knowledge);
  const targets = policy?.alternatives || (policy?.activeEntity ? [policy.activeEntity] : []);
  const selectedEntities = normalizedEntities.filter(e => targets.some(n =>
    [e.name, ...e.aliases].some(alias => alias.toLowerCase() === String(n).toLowerCase())));

  // Generic facet follow-up branch (e.g., "What about its deployment?"). The
  // policy has already assessed requested-proposition support via the graph, so
  // the contract consumes policy factState/stance directly.
  if (policy?.isFacet) {
    const shape = policy.responseShape || getResponseShape('VERIFIED_FACT');
    const facts = (policy.supportingFacts || []).slice(0, 8);
    const target = selectedEntities[0] || { name: targets[0] };
    const targetName = target.name || targets[0];
    return {
      intent: 'FOLLOW_UP',
      subIntent: 'ENTITY_FACET',
      entityScope: true,
      tenantSubject: knowledge?.identity?.name || 'the tenant',
      assistantName: knowledge?.agent?.name || 'Scout',
      subject: targetName,
      activeEntities: targets,
      contextEntities: targets,
      requiredEntities: targets.concat(policy.requestedTopic ? [policy.requestedTopic] : []),
      mustMentionEntities: targets.concat(policy.requestedTopic ? [policy.requestedTopic] : []),
      evidenceEntities: selectedEntities.map(e => e.name),
      directAnswer: policy.directAnswer || null,
      answerStance: policy.answerStance || 'QUALIFY',
      requiredStance: policy.requiredStance || 'QUALIFY',
      evidenceStatus: policy.evidenceStatus || 'UNKNOWN',
      factState: policy.factState || 'UNKNOWN',
      evidenceStrength: facts.length ? 'DIRECT' : 'UNKNOWN',
      claimCeiling: facts.length ? `has ${policy.requestedTopic}` : `no verified evidence for ${policy.requestedTopic}`,
      keyFacts: facts,
      requiredFacts: facts, requiredRelationships: [], optionalFacts: [], rankedFacts: [],
      memberEvidence: null,
      dimension: null,
      dimensionKind: null,
      dimensionSupport: null,
      memberDimensionSupport: null,
      supportingFacts: facts,
      requestedTopic: policy.requestedTopic,
      setOperation: policy.setOperation,
      responseShape: shape,
      ...
      naturalInstructions: [
        ...(shape?.requirements || []),
        policy.boundary,
        'Answer the facet follow-up from the requested evidence only; do not introduce skills, technologies, or project experience that are not part of the facet.'
      ].filter(Boolean).join('\n')
    };
  }

  if (selectedEntities.some(e => ['product', 'service'].includes(e.type)) &&
      (policy?.setOperation === 'COMPARE' || ['product', 'service'].includes(policy?.entityType))) {
    const comparison = policy.setOperation === 'COMPARE';
    const shape = policy.responseShape;
    const facts = (policy.supportingFacts || []).slice(0, 8);
    const keyFacts = selectedEntities.map(e => JSON.stringify({ name: e.name, type: e.type, description: e.description, attributes: e.attributes }));
    return {
      intent: comparison ? 'COMPARISON' : 'FACTUAL',
      subIntent: comparison ? 'ENTITY_COMPARISON' : 'ENTITY_DETAILS',
      entityScope: true,
      tenantSubject: knowledge?.identity?.name || 'the tenant',
      assistantName: knowledge?.agent?.name || 'Scout',
      subject: comparison ? null : selectedEntities[0].name,
      activeEntities: targets,
      contextEntities: targets,
      requiredEntities: comparison ? [] : targets,
      mustMentionEntities: comparison ? [] : targets,
      evidenceEntities: selectedEntities.map(e => e.name),
      directAnswer: null,
      answerStance: policy.isFacet ? (policy.answerStance || 'QUALIFY') : null,
      requiredStance: policy.isFacet ? (policy.requiredStance || 'QUALIFY') : null,
      evidenceStatus: policy.isFacet ? (policy.evidenceStatus || 'UNKNOWN') : (policy.evidenceStatus || null),
      factState: policy.factState || (comparison ? (policy.dimensionSupport === 'FULL' ? 'TRUE' : 'UNKNOWN') : 'TRUE'),
      evidenceStrength: comparison ? policy.dimensionSupport : 'DIRECT',
      keyFacts,
      requiredFacts: [], requiredRelationships: [], optionalFacts: [], rankedFacts: [],
      memberEvidence: policy.memberEvidence,
      dimension: policy.dimension,
      dimensionKind: policy.dimensionKind,
      dimensionSupport: policy.dimensionSupport,
      memberDimensionSupport: policy.memberDimensionSupport,
      supportingFacts: facts,
      setOperation: policy.setOperation,
      responseShape: shape,
      boundary: policy.boundary,
      ...
      naturalInstructions: [
        ...(shape?.requirements || []),
        policy.boundary,
        'Collection membership establishes entity type, not manufacture, authorship, or historical work performed by the tenant.'
      ].filter(Boolean).join('\n')
    };
  }

  // ===== GAP PROPOSITION (requestedRelation: has_gap) =====
  // The response-policy classifier already determined that the user is asking
  // whether a specific entity is documented as a gap. Honor that semantic
  // contract explicitly instead of letting broad keyword classification
  // collapse it into NEGATIVE_ASSESSMENT.
  if (policy?.requestedRelation === 'has_gap' && policy?.directAnswer === 'YES') {
    const activeEntity = policy.activeEntity || policy.requestedTopic || '';
    const subjectName = (knowledge?.identity?.name || 'the candidate');
    const facts = (policy.supportingFacts || []).map(f => f.object || f.text).filter(Boolean);
    const gapResponseShape = getResponseShape('SKILL_EVIDENCE');
    const boundary = policy.boundary || 'Confirmed gap — answer affirmatively and briefly name the evidence.';
    const claimCeiling = policy.claimCeiling || 'has a documented gap for';
    return {
      intent: 'SKILL',
      subIntent: 'SKILL_EVIDENCE',
      policyMode: 'SKILL_EVIDENCE',
      tenantSubject: subjectName,
      assistantName: knowledge?.agent?.name || 'Scout',
      subject: subjectName,
      activeEntities: [activeEntity],
      contextEntities: [],
      requiredEntities: [subjectName, activeEntity],
      mustMentionEntities: [subjectName, activeEntity],
      evidenceEntities: [activeEntity],
      directAnswer: 'YES',
      answerStance: 'AFFIRM',
      requiredStance: 'AFFIRM',
      evidenceStatus: 'VERIFIED',
      factState: 'TRUE',
      evidenceStrength: 'GAP',
      claimCeiling,
      keyFacts: facts,
      requiredFacts: facts,
      optionalFacts: [],
      rankedFacts: [],
      supportingFacts: policy.supportingFacts || [],
      requestedTopic: activeEntity,
      responseShape: {
        ...gapResponseShape,
        requirements: [
          'Answer "Yes" and name the documented gap or learning area from the evidence.',
          'Do not call it a skill, experience, or verified expertise.',
          'Keep the answer brief and grounded.'
        ]
      },
      boundary,
      forbiddenClaims: policy.forbiddenClaims || [],
      forbiddenEntities: [],
      isNegationConfirmation: false,
      naturalInstructions: [
        `Answer the question with "Yes" because ${subjectName} has a documented gap or learning area: ${facts.join(', ')}.`,
        `CLAIM_CEILING: The strongest allowed claim is "${claimCeiling} ${activeEntity}". Do not exceed this ceiling.`,
        boundary
      ].filter(Boolean).join('\n')
    };
  }
  ...
  // Trust a policy-supplied active entity when the turn is elliptical (bare
  // entity or facet) and the resolver cannot see it from the question text.
  if (policy?.activeEntity && !activeEntities.some(e =>
    e.toLowerCase() === String(policy.activeEntity).toLowerCase())) {
    activeEntities.push(policy.activeEntity);
  }
  ...
  const isFutureCapability = intent === 'FUTURE_CAPABILITY' || subIntent === 'FUTURE_CAPABILITY';
  const isJobFit = intent === 'JOB_FIT' || subIntent === 'JOB_FIT';
  const isSkillOrFollowUp = ['SKILL', 'FOLLOW_UP'].includes(intent);
  const requestedTopic = isFutureCapability
    ? (extractRequestedRole(q, knowledge) || extractRequestedTopic(q, knowledge, subjectNames))
    : (isSkillOrFollowUp ? extractRequestedTopic(q, knowledge, subjectNames) : null);
  const rankedFacts = rankCandidateFacts(evidence, {
    intent,
    subIntent,
    activeEntities,
    requestedTopic: intent === 'JOB_FIT' ? extractRequestedRole(q, knowledge) : requestedTopic,
    knowledge
  });
  ...
  let isNegationConfirmation = directAnswer === 'YES' && isNegatedPremiseQuestion(q, knowledge);
  let factStateOverride = null;
  ...
  // Merge authoritative policy stance before downstream fact-state/instruction
  // generation. The policy classifier has already performed proposition-specific
  // relation support assessment, so its directAnswer/answerStance/evidenceStatus
  // must take precedence over the generic contract heuristics.
  let answerStance = null;
  let requiredStance = null;
  let evidenceStatus = null;
  if (policy) {
    if (policy.directAnswer) directAnswer = policy.directAnswer;
    if (policy.answerStance) answerStance = policy.answerStance;
    if (policy.requiredStance) requiredStance = policy.requiredStance;
    if (policy.evidenceStatus) evidenceStatus = policy.evidenceStatus;
    if (policy.factState) factStateOverride = policy.factState;
    if (policy.isNegationConfirmation) isNegationConfirmation = true;
  }
  ...
  let factState = determineFactState(intent, subIntent, evidenceStrength, directAnswer, selectedFacts, q, knowledge);
  if (factStateOverride) factState = factStateOverride;
  ...
  const forbiddenClaims = determineForbiddenClaims(intent, knowledge, evidenceStrength, subIntent, factState, requestedTopic);
  ...
  const isFutureRoleFramed = isFutureCapability &&
    /\b(?:become|be)\s+(?:a|an)\s+.*?\b(?:engineer|developer|architect|manager|role|job|position)\b/i.test(q);
  const requestedRole = (isJobFit || isFutureRoleFramed) ?
  ...
  // Build the expertise relation ceiling from the knowledge graph. This lets
  // validators distinguish documented expertise from overclaim while keeping
  // the contract portable across tenants.
  const relGraph = buildRelationshipGraph(knowledge);

  // Primary requested-facet questions ("What <X> does/did/has <subject>...?").
  // If supported values exist the answer must name at least one; if none
  // exist the answer must qualify rather than substitute an unrelated fact.
  const primaryFacet = (intent === 'META' || intent === 'HELP') ? null
    : assessPrimaryFacet({
      question: q,
      knowledge,
      graph: relGraph,
      subjectName: subject || knowledge?.identity?.name
    });
  if (primaryFacet?.matched && primaryFacet.supported) {
    const facetFacts = primaryFacet.values.map(v => `${primaryFacet.facet}: ${v}`);
    keyFacts = [...new Set([...facetFacts, ...keyFacts])].slice(0, 10);
    // Supported facets are affirmative: the answer should enumerate the
    // documented values, not hedge with open-world qualifiers.
    factState = 'TRUE';
    if (!evidenceStatus || evidenceStatus === 'UNKNOWN') evidenceStatus = 'SUPPORTED';
  }
  const allowedExpertiseRelations = (relGraph.triples || []).filter(t =>
    EXPERTISE_RELATIONS.has(t.relation) && t.subjectNorm === relGraph.subjectNorm
  ).map(t => ({ subject: t.subject, relation: t.relation, object: t.object }));

  // Answer obligation — what the answer owes the user semantically. When the
  // requested facet/relation has supported values, the answer must surface at
  // least one concrete value; claiming "no verified evidence" while supported
  // values exist is a false-unknown. Computed generically from the facet
  // assessment, the relationship graph, and knowledge collections — never
  // from benchmark vocabulary.
  let supportedValues = primaryFacet?.matched && primaryFacet?.supported
    ? [...primaryFacet.values]
    : [];
  let obligationFacet = primaryFacet?.matched ? primaryFacet.facet : null;
  let obligationEnumeration = !!(primaryFacet?.matched && primaryFacet?.supported);
  // Subject-capability evaluation class — questions asking for an overall
  // assessment of the tenant subject's capabilities (profile summaries,
  // qualification enumerations, fit/recommendation judgments). The
  // 'RECRUITER' label is a historical name for this generic semantic class;
  // the obligation depends on the capability-evaluation semantics, not on
  // any recruiter-domain vocabulary. Remaining debt: renaming the intent
  // itself is a broader refactor tracked separately.
  const SUBJECT_EVALUATION_INTENTS = new Set(['QUALIFICATIONS', 'PROFILE', 'RECRUITER']);
  const SUBJECT_EVALUATION_SUBINTENTS = new Set(['QUALIFICATIONS', 'PROFILE_SUMMARY', 'RECRUITER', 'STRENGTH_EVIDENCE', 'RECRUITER_RECOMMENDATION']);
  if (!supportedValues.length && (SUBJECT_EVALUATION_INTENTS.has(intent) || SUBJECT_EVALUATION_SUBINTENTS.has(subIntent))) {
    // Factual capability relations (not seniority-flavored expertise) — the
    // documented values a qualifications/profile answer may surface.
    const CAPABILITY_RELATIONS = new Set(['has_skill', 'uses_tech', 'has_cert', 'has_degree', 'worked_at', 'interned_at']);
    supportedValues = [...new Set((relGraph.triples || [])
      .filter(t => CAPABILITY_RELATIONS.has(t.relation) && t.subjectNorm === relGraph.subjectNorm)
      .map(t => t.object).filter(Boolean))];
    obligationFacet = obligationFacet || 'qualifications';
    obligationEnumeration = obligationEnumeration || (intent === 'QUALIFICATIONS' || subIntent === 'QUALIFICATIONS');
  }
  if (!supportedValues.length && /project/i.test(String(intent || '') + ' ' + String(subIntent || ''))) {
    supportedValues = (knowledge?.projects || [])
      .map(p => String(p?.name || p?.title || '').trim()).filter(Boolean);
    obligationFacet = obligationFacet || 'projects';
    obligationEnumeration = supportedValues.length > 0;
  }
  const answerObligation = {
    kind: obligationEnumeration ? 'enumeration' : (primaryFacet?.matched || requestedTopic ? 'value' : 'none'),
    relation: primaryFacet?.relations?.[0] || null,
    facet: obligationFacet || requestedTopic || null,
    expectedValueKind: obligationFacet || requestedTopic || null,
    enumeration: obligationEnumeration,
    supportedValuesAvailable: supportedValues.length > 0,
    supportedValues,
    minimumConcreteValues: supportedValues.length ? 1 : 0,
    unknownAllowed: !(primaryFacet?.matched && primaryFacet.supported) && supportedValues.length === 0
  };
  ...
  let naturalInstructionsText = Array.isArray(naturalInstructions)
    ? naturalInstructions.join('\n')
    : String(naturalInstructions || '');
  if (primaryFacet?.matched) {
    if (primaryFacet.supported) {
      naturalInstructionsText =
        `The question asks for ${primaryFacet.facet}. Answer using the documented values: ${primaryFacet.values.join(', ')}. Name at least one of them; do not substitute an unrelated fact.\n` +
        naturalInstructionsText;
    } else {
      naturalInstructionsText =
        `The question asks for ${primaryFacet.facet}. No documented ${primaryFacet.facet} exists for ${subject || 'the subject'}. State clearly that this is not documented or verified — do not substitute an unrelated fact (employment, projects, skills) as if it answered the question.\n` +
        naturalInstructionsText;
    }
  }
  ...
  return {
    ...
    allowedExpertiseRelations,
    responseShape,
    forbiddenClaims,
    policyMode,
    naturalInstructions: naturalInstructionsText,
    primaryFacet,
    answerObligation,
  };
}
 ...
 */
function isNegatedPremiseQuestion(question, knowledge) {
  ...
  const subj = knowledgeAccess.getSubjectAlternation(knowledge);
  if (new RegExp(`\\b(?:was|did|is|has)\\s+(?:${subj})\\b`).test(q) &&
...
function extractSubjectNames(question, knowledge) {
  ...
  if (knowledge?.identity?.preferredName) add(knowledge.identity.preferredName);
  if (Array.isArray(knowledge?.identity?.aliases)) knowledge.identity.aliases.forEach(add);
  if (Array.isArray(knowledge?.subjectAliases)) knowledge.subjectAliases.forEach(add);
  return [...names];
}
...
function extractRequestedTopic(question, knowledge, excludedNames = []) {
  ...
  const stopWords = new Set([
    'the','and','what','about','does','do','did','he','she','they','his','her','him','know','use','used','can','could','would','should','will','is','was','are','were','have','has','had','with','for','this','that','you','your','me','i','we','it','be','being','been','to','of','in','on','at','a','an','as','if','or','so','no','not','yes','how','when','where','why','who','which','there','their','them','then','than','also','only','just','like','get','got','learn','job','role','work','company','project','projects','skill','skills','debug','build','built','write','code','create','created','develop','developed','implement','handle','troubleshoot','list','listed','show','shows','shown','name','named','make','made','tech','stack','technology','technologies','framework','frameworks','computer','computers','database','databases','backend','frontend',
  ...
  // Configured subject pronouns are subject references, never requested topics.
  const topicPronouns = knowledgeAccess.getSubjectPronouns(knowledge);
  for (const form of [topicPronouns.subject, topicPronouns.object, topicPronouns.possessive]) {
    if (form) stopWords.add(form);
  }
  ...
  // Examples: "Can they actually code?", "Do they vibe code?", "Can they code under pressure?",
  // "Do they understand programming or just generate it?". These ask about coding capability or
  ...
  // Drop tokens that are just the subject (e.g., resolved anaphora "Morgan").
...
function classifySubIntent(question, intent, knowledge, subjectNames = []) {
  const lower = question.toLowerCase();
  // Non-technical / outside-tech experience must route to EXPERIENCE regardless
  // of the top-level intent that matched earlier.
  const nonTechExperiencePattern = /\b(?:non[-\s]?technical|nontechnical|not\s+technical|not\s+in\s+(?:tech|technology)|outside\s+(?:of\s+)?(?:tech|technology|software|development|coding|engineering)|before\s+(?:moving|getting)\s+(?:into|to)\s+(?:tech|technology|software|development|coding|engineering)|career\s+before\s+(?:tech|technology|software|development|coding|engineering)|work\s+(?:outside|before|other\s+than|besides|apart\s+from)\s+(?:tech|technology|software|development|coding|engineering))\b/i;
  const experienceWithNonTech = /\b(?:experience|background|career|work|job|role|employment)\b[^.!?]{0,80}\b(?:outside|non[-\s]?technical|not\s+in|before|prior\s+to|other\s+than|besides|apart\s+from)\b[^.!?]{0,40}\b(?:tech|technology|software|development|coding|engineering)\b/i;
  const nonTechFollowedByExperience = /\b(?:outside\s+(?:of\s+)?(?:tech|technology|software|development|coding|engineering)|before\s+(?:tech|technology|software|development|coding|engineering))\b[^.!?]{0,60}\b(?:experience|background|career|work|job|role|employment)\b/i;
  if (nonTechExperiencePattern.test(lower) || experienceWithNonTech.test(lower) || nonTechFollowedByExperience.test(lower)) {
    return 'EXPERIENCE';
  }
  ...
  if (intent === 'JOB_FIT') return 'JOB_FIT';
  ...
  // Collection questions ("what about his projects?", "list the projects") ask for
  // an enumeration of project entities, not evidence about a topic named "projects".
  if (!requestedTopic && /\bprojects?\b/.test(lower) && ['FOLLOW_UP', 'GENERAL', 'SKILL', 'PROJECT'].includes(intent)) return 'PROJECT_DETAILS';
  ...
  const genericSkillPattern = /\b(?:tech\s+stack|technology\s+stack|stack|frameworks?|databases?|computers?|technolog(?:y|ies)|good\s+at|skills?|coding|programming)\b/;
  ...
  if (/\b(?:bad\s+at|weak\s+at|what\s+is\s+[^.!?]{1,40}\s+bad\s+at|negative\s+trait|worst\s+at|weakest\s+at)\b/.test(lower)) return 'NEGATIVE_ASSESSMENT';
  ...
  if (/\b(?:real-world|real world|work experience|professional experience|production experience)\b/.test(lower) ||
      new RegExp(`\\b(?:companies?|employers?|where\\s+(?:has|did|does)\\s+(?:${knowledgeAccess.getSubjectAlternation(knowledge)})\\s+(?:work|worked|been\\s+employed)|work\\s+history|employment\\s+history)\\b`).test(lower) ||
...
function rankCandidateFacts(evidence, context) {
  ...
  const relationshipTerms = {
    ...
    QUALIFICATIONS: ['skill', 'skills', 'tech', 'technology', 'experience', 'certification', 'degree', 'project', 'framework', 'frameworks'],
    ...
    RECRUITER_RECOMMENDATION: ['experience', 'project', 'skill', 'gap', 'boundary', 'evidence'],
    ...
    META_IDENTITY: ['scout', 'assistant', 'name', 'runtime',
      String(context.knowledge?.identity?.name || context.knowledge?.agent?.productName || '')
        .toLowerCase().split(/\s+/)[0]].filter(Boolean),
...
function isRuntimeFact(fact, knowledge) {
  const lower = String(fact?.text || '').toLowerCase();
  const productTokens = [knowledge?.agent?.productName, knowledge?.agent?.product,
    ...(knowledge?.projects || []).slice(0, 3).map(p => p.name)]
    .filter(Boolean).map(n => String(n).toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  const alt = ['scout', 'runtime', 'scope', 'capabilities', 'model', 'provider', 'cloudflare',
    'rate limit', 'daily cap', 'privacy', 'session', ...productTokens].join('|');
  return new RegExp(`\\b(?:${alt})\\b`).test(lower);
}
...
function selectContractFacts(rankedFacts, subIntent, subject, activeEntities, knowledge) {
  ...
  if (['META_IDENTITY', 'META_INFRASTRUCTURE', 'META_LIMITS', 'META_PRIVACY'].includes(String(subIntent || ''))) {
    const runtimeFacts = rankedFacts.filter(fact => isRuntimeFact(fact, knowledge));
  ...
  if (['SKILL_EVIDENCE', 'JOB_FIT', 'RECRUITER', 'RECRUITER_RECOMMENDATION', 'STRENGTH_EVIDENCE', 'PROFILE_SUMMARY'].includes(subIntent)) {
...
function determineRequiredEntities(question, intent, activeEntities, knowledge, context = {}) {
  ...
  // For negative assessment, documented gaps/learning areas must be named
  // (not presented as the user's personal weakness, but explicitly listed).
  if (context.subIntent === 'NEGATIVE_ASSESSMENT' && knowledge) {
    const gapSources = [
      ...(knowledge.summary?.honestGaps || []),
      ...(knowledge.summary?.learningGaps || [])
    ];
    for (const [group, values] of Object.entries(knowledge.skills || {})) {
      if (Array.isArray(values) && /learning|gap|adjacent/i.test(group)) {
        gapSources.push(...values);
      }
    }
    const gapSentences = gapSources
      .filter(Boolean)
      .map(g => String(g).split(/[.!?]/).map(s => s.trim()).filter(Boolean)[0])
      .filter(Boolean);
    for (const sentence of gapSentences.slice(0, 2)) {
      if (sentence.length > 3 && !required.includes(sentence)) required.push(sentence);
    }
  }
...
function determineClaimCeiling(evidenceStrength) {
  const map = {
    ...
    MIXED: 'has documented experience with',
  };
...
function extractRequestedRole(question, knowledge) {
  ...
  const lower = question.toLowerCase();
  const REQUIREMENT_SUFFIX = /\s+(?:requiring|requires?|needs?|needing|must\s+have|looking\s+for|seeking|with\s+experience\s+(?:with|in)|experience\s+(?:with|in)|proficiency\s+(?:with|in)|familiar(?:ity)?\s+with|skilled\s+(?:with|in)|knowledge\s+of|expertise\s+(?:in|with))\b.*$/i;
  const target = lower.match(/\b(?:hiring|recruiting)\s+for\s+(?:an?\s+)?([^.!?;,]+?)(?=\s+(?:at|with|requiring|requires?|needs?|needing|must\s+have|looking\s+for|seeking|experience\s+(?:with|in))\b|[.!?;,]|$)/) ||
    lower.match(/^(?:what about|how about|and)\s+(?:an?\s+)?([a-z0-9+#.\s-]+?)\s+(?:role|position|job)\b/) ||
    lower.match(/^(?:why\s+)?is(?:n't|nt|\s+not)?\s+([a-z0-9+#.\s-]+?)\s+(?:not\s+)?(?:an?\s+)?(?:good|strong|poor|bad|weak)?\s*fit\b/) ||
    // "looking for a data scientist requiring Python" — the title has no
    // "role"/"position"/"job" marker, so capture it here and stop at the
    // next requirement marker to keep title words out of the criteria.
    lower.match(/\b(?:looking\s+for|seeking)\s+(?:an?\s+)?([a-z0-9+#.\s-]+?)(?=\s+(?:requiring|requires?|needs?|needing|must\s+have|with\s+experience\s+(?:with|in)|experience\s+(?:with|in)|proficiency\s+(?:with|in)|familiar(?:ity)?\s+with|skilled\s+(?:with|in)|knowledge\s+of|expertise\s+(?:in|with))\b|[.!?;,]|$)/);
  if (target && !new RegExp(`^(?:${knowledgeAccess.getSubjectAlternation(knowledge, { includeNames: false })}|it|this|that)$`, 'i').test(target[1].trim())) {
    const role = target[1].replace(/^\s*(?:a|an|the)\b\s*/i, '').replace(/^(?:role|position|job)\s+as\s+/i, '').replace(REQUIREMENT_SUFFIX, '').replace(/\s+(?:role|position|job)\s*$/, '').trim();
    const names = extractSubjectNames(question, knowledge);
    if (!names.some(name => name.toLowerCase() === role)) return role;
  }
  const cleanRole = value => String(value || '')
    .replace(/^\s*(?:a|an|the|for|as|in|at|with)\s+/i, '')
    .replace(/\s*(?:role|position|job)\b.*$/i, '')
    .replace(REQUIREMENT_SUFFIX, '')
    .replace(/\s+with\b.*$/i, '')
    .trim();
  // Try the "<title> role" form first: in "looking for a frontend developer
  // role requiring AWS" the "role <tail>" alternative below would otherwise
  // capture the requirement marker tail ("requiring aws") as the role.
  const m2 = lower.match(/\b(?:for\s+(?:a\s+|an\s+)?|as\s+(?:a\s+|an\s+)?)([a-z\s]+?)\s+(?:role|position|job)\b/);
  if (m2) {
    const r = cleanRole(m2[1]);
    if (r) return r;
  }
  const m = lower.match(/\b(?:fit\s+(?:for|as)\s+(?:a\s+|an\s+)?|(?:fit|role|position|job)\s+(?:as\s+a\s+|a\s+)?)((?!(?:requiring|requires?|needs?|needing|must\s+have|looking\s+for|seeking|experience\s+(?:with|in)|proficiency\s+(?:with|in)|familiar(?:ity)?\s+with|skilled\s+(?:with|in)|knowledge\s+of|expertise\s+(?:in|with))\b)[a-z\s]+?)(?:\?|\s+(?:at|for|with|and)\b|$)/);
  if (m) {
    const r = cleanRole(m[1]);
    if (r) return r;
  }
  // Future capability target role/skill. Leading function words and
  // infinitive/light verbs are stripped iteratively so "learn to use X"
  // yields "X". A candidate left with no content word — a manner phrase like
  // "on the job" describing HOW learning happens — is not a target role.
  const MANNER_WORDS = new Set(['a', 'an', 'the', 'to', 'for', 'as', 'in', 'on', 'at',
    'with', 'by', 'through', 'via', 'use', 'using', 'do', 'doing', 'job', 'jobs',
    'work', 'working', 'role', 'roles', 'position', 'positions', 'fly', 'go',
    'own', 'side', 'way', 'task', 'tasks']);
  const cleanTarget = value => {
    let v = String(value || '').trim();
    for (let i = 0; i < 5; i++) {
      const next = v.replace(/^(?:to|a|an|the|for|as|in|on|at|with|by|through|via|use|using|do|doing)\s+/i, '');
      if (next === v) break;
      v = next;
    }
    v = v.replace(REQUIREMENT_SUFFIX, '')
      .replace(/\s*(?:role|roles|position|positions|job|jobs)\s*$/i, '').trim();
    if (!v.split(/\s+/).some(w => !MANNER_WORDS.has(w))) return null;
    return v;
  };
  const m3 = lower.match(/\b(?:become|be)\s+(?:a\s+)?([a-z\s]+?)(?:\?|\.\s*$|$)/);
  if (m3) {
    const r = cleanTarget(m3[1]);
    if (r) return r;
  }
  const m4 = lower.match(/\b(?:learn|pick\s+up|get\s+good\s+at)\s+([a-z0-9+#.\s]+?)(?:\?|\.\s*$|$)/);
  if (m4) {
    const r = cleanTarget(m4[1]);
    if (r) return r;
  }
  return null;
}
...
function getIntentKeywords(intent) {
  const keywordMap = {
    ...
    JOB_FIT: ['experience', 'project', 'skill', 'gap', 'boundary', 'evidence'],
    RECRUITER: ['experience', 'project', 'skill', 'gap', 'boundary', 'evidence'],
...
function determineDirectAnswer(question, intent, evidence, knowledge) {
  ...
  if (intent === 'ADVERSARIAL') {
    ...
    if (new RegExp(`\\bwas\\s+(?:${knowledgeAccess.getSubjectAlternation(knowledge)})\\b`).test(question) && /\bnot\b/.test(question)) {
      return 'YES'; // Confirming the negation
    }
    // Standard adversarial: "He was X, right?" — check if X is in evidence
    const claimEntity = new RegExp(`\\b(?:${knowledgeAccess.getSubjectAlternation(knowledge)}${_subjectNameAlt})\\b.*?\\b(?:was|is|has|have|did|worked|attended|managed|handled|built)\\b\\s+(?:a\\s+|an\\s+)?(.+)`, 'i').exec(question);
    if (claimEntity) {
  ...
  if (intent === 'SKILL') {
    ...
    if (directMatch) {
      ...
      // Explicit negative evidence can produce NO only when it references the specific topic.
      const negativePattern = new RegExp(`\\b(?:no\\s+${escapedTech}|not\\s+(?:familiar|skilled|experienced|proficient)(?:\\s+(?:with|in|using))?\\s+${escapedTech}|does\\s+not\\s+know\\s+${escapedTech})\\b`, 'i');
  ...
  if (intent === 'JOB_FIT') {
    ...
    if (requestedRole) {
      ...
      if (roleTokens.length === 0) return 'UNKNOWN';
...
function getResponseShape(intent, subIntent) {
  ...
  const shapes = {
    ...
    JOB_FIT: {
      ...
      maxSentences: 3,
      requirements: [
        'State whether the subject fits the role',
 ...
 */
function determineForbiddenClaims(intent, knowledge, evidenceStrength, subIntent = '', factState = null, requestedTopic = null) {
  const forbidden = [];
  ...
  // Negative assessment without authoritative support must not assert specific
  // personal-weakness language. Documented learning/gap areas may still be named.
  if (subIntent === 'NEGATIVE_ASSESSMENT' && factState !== 'TRUE' && factState !== 'PARTIAL') {
    forbidden.push(
      'struggles with', 'struggle with', 'struggle', 'struggling',
      'bad at', 'poor at', 'weak at', 'terrible at', 'difficulty', 'difficult',
      'his weakness is', 'her weakness is', 'their weakness is', 'the subject\'s weakness is',
      'his weaknesses are', 'her weaknesses are', 'their weaknesses are', 'the subject\'s weaknesses are'
    );
    const poss = knowledgeAccess.getSubjectPronouns(knowledge)?.possessive;
    if (poss && !/^(?:his|her|their)$/.test(poss)) {
      forbidden.push(`${poss} weakness is`, `${poss} weaknesses are`);
    }
  }

  // Future capability must not be framed as current knowledge/experience.
  // These are CLAIM-level bans (do not assert present ability), never
  // exact-sentence bans — the model phrases the bounded answer itself.
  if ((subIntent === 'FUTURE_CAPABILITY' || intent === 'FUTURE_CAPABILITY') && requestedTopic) {
    forbidden.push(`already knows ${requestedTopic}`, `verified experience with ${requestedTopic}`);
  }
...
function buildNaturalInstructions(intent, directAnswer, keyFacts, activeEntities, responseShape, question,
  ...
  const instructions = [];
  const subjectName = subject || 'the candidate';
  ...
  if (subIntent === 'FUTURE_CAPABILITY') {
    ...
    instructions.push(`This is a future/potential question, not a current-ability question. Answer in a future frame: convey that ${subjectName} could plausibly learn or grow into "${target}", briefly name adjacent documented skills from the evidence, and clearly distinguish that from verified present ability — ${subjectName} does not currently have verified ${target} experience. You MUST name "${target}". Do NOT start with a flat denial of the possibility, do NOT claim the subject already knows ${target}, has verified experience with ${target}, or does the target role now. Phrase the answer naturally; do not reuse or echo a previous turn's wording.`);
    if (requestedRole) {
  ...
  if (claimCeiling && evidenceStrength !== 'IDENTITY' && factState !== 'UNKNOWN' && !genericQualifications) {
    instructions.push(`CLAIM_CEILING: The strongest allowed claim is "${claimCeiling} <topic>". Do not exceed this ceiling and never quote this phrase to the user.`);
  }
  if (requestedRole) {
    instructions.push(`REQUESTED_ROLE: "${requestedRole}" is a hypothetical target, not a historical role. Do not claim ${subjectName} held it.`);
  }
  ...
  if (directAnswer === 'NO') {
    ...
    if (isNegativeCapability) {
      instructions.push('The user is asserting that the subject cannot use basic technology. Start with "No" and briefly name the documented technical skills from the evidence that show the claim is false. Do not hedge with "no evidence" or "not explicitly stated".');
    } else {
      instructions.push(`Start with "No" in a full sentence to deny the specific false claim. Use the verified profile or ${subjectName} as the subject of the sentence; do NOT use "we", "me", "I", or "my". Say the verified profile "does not document" the claim, or say there is "no verified evidence" / "no public evidence". Example forms: "No, the verified profile does not document [subject] as a [role] at [employer]." or "No, [subject] is not documented as a [role]." Do not assert ${subjectName} never worked at the employer and do not mention certifications, internships, or production ownership. Keep the denial to one or two sentences.`);
    }
    ...
    instructions.push(`State that ${subjectName} fits the role. Name the specific matching skills and evidence.`);
  } else if (directAnswer === 'PARTIAL_FIT') {
    instructions.push(`State that ${subjectName} partially fits. Name matching skills AND note the gaps.`);
  } else if (directAnswer === 'NOT_FIT') {
    instructions.push(`State that ${subjectName} does not fit the role. Name the missing requirements.`);
  } else if (directAnswer === 'UNKNOWN') {
    instructions.push('Do NOT begin with the standalone word "No" or "Yes" and do not affirm or deny the claim. State clearly — in your own words — that the requested fact is not established by the verified/public record, and name the specific topic or entity. Do not turn absence of evidence into a closed-world impossibility claim.');
  }
  ...
  if (subIntent === 'SKILL_EVIDENCE' && requestedTopic) {
    if (factState === 'UNKNOWN') {
      if (/\b(?:well|expert|advanced|proficient|mastery|master|highly\s+skilled|very\s+experienced|deep\s+knowledge|strong\s+in|good\s+at|thoroughly|extensively)\b/i.test(question)) {
        instructions.push(`State in your own words that current proficiency or mastery in "${requestedTopic}" is not established by the verified record — the question asks about degree of skill, which is not documented. You may mention adjacent documented project experience. Do not assert the subject cannot use ${requestedTopic}, and do not claim documented mastery either.`);
      } else {
        instructions.push(`State in your own words that current ${requestedTopic} experience is not established by the verified record, naming "${requestedTopic}" explicitly. Do not turn absence of evidence into a claim that the subject cannot learn or use it, and do not affirm current ability either.`);
      }
    } else {
  ...
  if (subIntent === 'ENTITY_FACET') {
    if (factState === 'UNKNOWN' || evidenceStrength === 'UNKNOWN') {
      instructions.push(`The requested facet "${requestedTopic}" is not established in the knowledge base for ${subject || 'this entity'}. Do NOT state a specific value. State in your own words that this facet is not documented or established for ${subject || 'this entity'}, naming the facet.`);
    } else {
      instructions.push(`The requested facet "${requestedTopic}" for ${subject || 'this entity'} is supported by the evidence. State the verified value directly and cite the source.`);
    }
  }
  if (subIntent === 'RATIONALE') {
  ...
  if (subIntent === 'META_CAPABILITIES') {
    instructions.push('List only the candidate/profile topics you can answer: projects, skills, experience, education, certifications, role fit, and public contact info. Do not describe the runtime stack, AI model, provider, hosting, or infrastructure. Do not mention the configured provider, model, hosting platform, or generative details unless the user explicitly asks for infrastructure.');
  }
  if (subIntent === 'CONTACT') {
    instructions.push(`List the public contact methods from the facts, such as email, social profiles, portfolio, and public phone. Do not invent contact methods or provide private/home contact information.`);
  }
  ...
  if (subIntent === 'NEGATIVE_ASSESSMENT') {
    ...
    if (factState === 'TRUE') {
      ...
      if (asksBadAt) {
        instructions.push(`The question asks what ${subjectName} is "bad at". Do not claim ${subjectName} is bad at any specific skill. State that the answer is unknown or not established, then name the documented learning/gap areas from the included facts as the only items that are documented.`);
      } else if (asksCurrentProgress) {
        instructions.push(`This asks about current progress on previously discussed learning/gap areas. Name the documented learning/gap items explicitly in your answer. Do not infer active work from transferable skills. State current progress only if the facts explicitly establish it with present-tense active language; otherwise say that whether ${subjectName} is currently working on those specific items is unknown or not verified in the public profile. Prefer phrasing like: "Whether ${subjectName} is currently working on [learning area] is not verified in the public profile."`);
      } else {
        instructions.push('Open by stating, in your own words, that the answer is unknown or not established in the public/verified profile; do not invent a personal weakness. After that opening, you MUST name at least one documented learning or gap area from the provided facts, and label it as a documented learning/gap area rather than a personal weakness.');
        instructions.push(`Use neutral, fact-based wording. Do NOT write "struggle", "struggles", "struggling", "struggled", "difficulty", "difficult", "weakness", "weak", "bad at", "poor at", "weak at", "terrible at", or any similar negative-trait phrase when describing the documented gap/learning area. Cite the actual documented learning/gap area from the provided facts, not a canned example.`);
      }
    }
  }
  if (subIntent === 'JOB_FIT' || subIntent === 'RECRUITER' || subIntent === 'RECRUITER_RECOMMENDATION') {
    instructions.push('This is an assessment or role-fit question. Assess the subject against the requested criteria using verified subject evidence and documented boundaries. State matching evidence and limitations without treating the requested criteria as historical facts. Do not invent a job title, employer, employment dates, or seniority that are not in the verified facts. The requested role or criteria are hypothetical; do not say the subject held them. Do not attribute the requested domain to any specific project; compare the subject\'s documented skills to the role without saying a project "uses" or "involves" that domain.');
  }
  ...
  if (keyFacts.length > 0) {
    ...
    if (subIntent === 'NEGATIVE_ASSESSMENT' && factState === 'UNKNOWN') {
      instructions.push(`The following documented learning or gap areas MUST be named after a clear unknown/grounded opening. Name at least one. Do NOT present them as the answer or as a biggest/main/personal weakness: ${truncate(facts, 260)}`);
    } else {
