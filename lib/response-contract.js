...
function extractRequestedTopic(question, knowledge, excludedNames = []) {
  ...
  // 2. Generic extraction for unknown technologies and named entities.
...
function extractRequestedRole(question, knowledge) {
  ...
  if (m) return m[1].replace(/\b(role|position|job)\s*$/i, '').trim() || m[1].trim();
