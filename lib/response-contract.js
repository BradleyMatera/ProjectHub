...
function determineDirectAnswer(question, intent, evidence, knowledge) {
  ...
  if (intent === 'JOB_FIT') {
    ...
    if (required) {
      const requiredSkills = required.split(/\s+and\s+|\s*,\s*|\s*\/\s*|\s+/).map(s => s.trim()).filter(s => s.length >= 2 && !/\b(?:role|position|job|junior|senior|frontend|backend|fullstack|full-stack|developer|engineer)\b/.test(s));
