...
const { buildRelationshipGraph, resolveEntity, canonicalEntityNorm } = require('./relationship-graph');
...
function buildSemanticPlan({ question, resolvedQuestion, history, knowledge, sessionState, policy, resolution } = {}) {
  ...
  // is an entity-level topic shift. Entity equivalence is CANONICAL graph
  // identity, not literal string equality — referencing the active entity by
  // a declared alias or alternate casing/punctuation continues the same
  // target; only a genuinely different canonical entity shifts.
  const canon = n => (graph ? canonicalEntityNorm(graph, n) : _sameName(n, '') ? '' : String(n || '').trim().toLowerCase());
  const sameCanonicalEntity = (a, b) => {
    const ca = canon(a);
    const cb = canon(b);
    return Boolean(ca && cb && ca === cb);
  };
  const namedNewEntity = explicitEntities.find(e =>
    !priorActiveEntity || !sameCanonicalEntity(e.name, priorActiveEntity.name));
  const namedSameEntity = explicitEntities.find(e =>
    priorActiveEntity && sameCanonicalEntity(e.name, priorActiveEntity.name));
  const namesActiveEntity = Boolean(namedSameEntity);
  ...
  const activeEntity = namedNewEntity
    ...
    : namedSameEntity
      // The current turn explicitly named the same canonical entity — it is
      // still this turn's target (no entity shift, but no null target either).
      ? { name: priorActiveEntity.name, type: priorActiveEntity.type || namedSameEntity.type || 'unknown', source: 'current-turn' }
      : (continuationType !== 'none' && !topicShift ? priorActiveEntity : null);
  const activeEntitySource = activeEntity
    ? ((namedNewEntity || namedSameEntity) ? 'current-turn' : 'history')
    : null;
