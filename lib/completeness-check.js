...
// Canonical pronoun alternations — the same subject-reference model used by
// every other semantic module (knowledge-access is the single source).
// Configured per-request via configureSubjectPronouns(knowledge).
let _subjectPronounAlt = 'he|she|they';
let _possessivePronounAlt = 'his|her|their';
let _objectPronounAlt = 'him|her|them';
let _subjectRefTokens = new Set();
let _subjectNameTokens = new Set();
...
function configureSubjectNames(names = []) {
  ...
  _subjectNameTokens = new Set((names || []).filter(Boolean)
    .flatMap(n => String(n).toLowerCase().split(/\s+/)));
}

// Extend the configured subject reference model with the tenant's pronouns.
// All three configured forms count as subject references in any position —
// users and models freely misapply forms ("zir weakness is").
function configureSubjectPronouns(knowledge) {
  const pr = getSubjectPronouns(knowledge);
  const esc = s => String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  _subjectPronounAlt = [...new Set(['he', 'she', 'they', pr.subject, pr.object, pr.possessive])]
    .filter(t => t && t.length >= 2).map(esc).join('|');
  _possessivePronounAlt = [...new Set(['his', 'her', 'their', pr.possessive])]
    .filter(t => t && t.length >= 2).map(esc).join('|');
  _objectPronounAlt = [...new Set(['him', 'her', 'them', pr.object, pr.possessive])]
    .filter(t => t && t.length >= 2).map(esc).join('|');
  _subjectRefTokens = new Set([pr.subject, pr.object, pr.possessive]
    .map(t => String(t || '').toLowerCase()).filter(t => t.length >= 2));
}
...
function classifyIntent(question, names) {
  ...
  const subj = `(?:${_subjectPronounAlt}${buildSubjectPattern(names)})`;
  ...
  if (/\b(?:social\s+security|ssn|password|credit\s+card|bank\s+account|home\s+address|phone\s+number|date\s+of\s+birth|passport)\b/.test(q) ||
      /\b(?:give\s+me\s+every|all\s+(?:details|the\s+details|personal)|every\s+(?:personal\s+)?detail|exhaustive|personal\s+details?)\b/.test(q) && new RegExp(`\\b(?:detail|personal|about\\s+(?:${_objectPronounAlt}|the\\s+candidate))\\b`).test(q)) {
    return 'REFUSAL';
  }
  ...
  if (new RegExp(`\\b(?:could\\s+${subj}\\s+(?:become|learn|get|pick|take|be\\s+able\\s+to)|would\\s+${subj}\\s+(?:be\\s+able\\s+to|be|become|learn)|will\\s+${subj}\\s+(?:be|become|learn)|can\\s+${subj}\\s+(?:learn|pick\\s+up|get\\s+good|become)|${subj}\\s+(?:can|could|will|would)\\s+(?:learn|pick\\s+up|get\\s+good|become)|future\\s+(?:skill|role|capability)|potential\\s+(?:to|for)|someday|one\\s+day|learn\\s+in\\s+the\\s+future|eventually\\s+(?:lead|be|become|learn))\\b`).test(q)) {
  ...
  if (/\b(?:pretend|suppose|imagine|assume)\b/.test(q) &&
      new RegExp(`\\b(?:${_subjectPronounAlt}|it${_subjectNamePattern})\\b`).test(q) &&
  ...
  if (/\b(?:right|correct|true|isn'?t it|don'?t you think)\b/.test(q) &&
      new RegExp(`\\b(?:${_subjectPronounAlt}|it${_subjectNamePattern}|evidence|no\\s+evidence)\\b`).test(q)) {
    return 'ADVERSARIAL';
  }

  // Negation question — "He was not X, was he?"
  if (new RegExp(`\\b(?:was|did|is|has|have)\\s+(?:${_subjectPronounAlt})\\b`).test(q) &&
  ...
  if (new RegExp(`\\b(?:what\\s+qualifications?|(?:${_possessivePronounAlt})\\s+qualifications?|qualifications?\\s+(?:does|has|do)\\b|what\\s+(?:is|are)\\s+(?:${_possessivePronounAlt}|the)\\s+(?:tech\\s+stack|technology\\s+stack|stack)|(?:${_possessivePronounAlt}|the)\\s+(?:tech\\s+stack|technology\\s+stack|stack)|what\\s+skills?\\s*(?:does|has|is)\\b)\\b`).test(q)) {
    return 'QUALIFICATIONS';
  }

  // Negative hiring / fit questions — "Why shouldn't I hire?", "What could go wrong hiring?",
  // "What concerns should a recruiter know?"
  if (/\b(?:why\s+shouldn'?t|why\s+not|shouldn'?t\s+(?:hire|employ)|not\s+(?:hire|employ)|reason\s+not\s+to\s+(?:hire|employ)|what.*(?:downside|risk|concern|drawback).*(?:hire|recruit|candidate|know)|concerns?.*(?:recruiter|hir|candidate)|risks?.*(?:hiring|recruit|candidate))\b/i.test(q)) {
    return 'NEGATIVE_ASSESSMENT';
  }

  // Roast / playful criticism — use documented gaps, not insults.
  if (/\broast\b/i.test(q)) {
    return 'NEGATIVE_ASSESSMENT';
  }

  // Examples of employment — "give me an example of his jobs" is an employer
  // listing request, not a fit assessment.
  if (new RegExp(`\\b(?:examples?\\s+of|list|name)\\b.{0,20}\\b(?:jobs?|roles?|positions?|employers?|companies|places\\s+(?:${_subjectPronounAlt})\\s+(?:worked|worked\\s+at))\\b`, 'i').test(q)) {
    return 'EXPERIENCE';
  }
  ...
  // CONTACT — public contact info (email, linkedin, github, portfolio, links, public phone)
  if (/(?:\bcontact\b|email|linkedin|github|portfolio|links?|urls?|reach|get.*touch|send.*message|\bphone\b)/.test(q) &&
      !/home address|ssn|password|credit card|bank|private/.test(q)) {
    return 'CONTACT';
  }

  // Explanation requests disguised as yes/no questions — "can he explain X" is
  // asking for an explanation, not a capability yes/no. Route to GENERAL so the
  // answer must carry content rather than a bare "yes".
  if (/^(?:can|could|would)\b.*\b(?:explain|describe|walk\s+(?:me|us)\s+through|tell\s+(?:me|us)\s+about)\b/i.test(q)) {
    return 'GENERAL';
  }
  ...
  if (/^(?:does|do|is|was|has|have|can|could|would|will|are|were|did|should)\b/.test(q)) {
    ...
    if (/\b(?:know|use|used|familiar|experience with|skilled|done with|debug|build|write|create|code|handle|troubleshoot|implement|develop|program|work with|good\s+at|best\s+at|strong\s+at)\b/.test(q)) {
      return 'SKILL';
    }
    return 'YES_NO';
  }
  ...
  // The "<topic> experience" alternative routes "AWS experience" / "military
  // experience" / "his experience" to EXPERIENCE instead of a generic PROFILE.
  if (new RegExp(`\\b(?:companies?|employers?|where\\s+(?:has|did|does)\\s+(?:${_subjectPronounAlt})\\s+(?:work|worked|been\\s+employed)|work\\s+history|employment\\s+history|army\\s+service|military\\s+service|military\\s+.*\\bservice\\b)\\b|[\\w.+#-]+\\s+experience\\b`).test(q)) {
    return 'EXPERIENCE';
  }
  ...
  // Non-technical / outside-tech experience: generic variants must route to
  // EXPERIENCE, not be mis-routed to PROJECT or PROFILE.
  ...
  if (nonTechExperiencePattern.test(q) || experienceWithNonTech.test(q) || nonTechFollowedByExperience.test(q)) {
    return 'EXPERIENCE';
  }
...
function evaluateCompleteness(answer, question, evidence, responseContract) {
  ...
  // Qualifications answers must name concrete technologies from the evidence —
  // a bare category list ("languages, frameworks, cloud services") is not an answer.
  if (responseContract?.subIntent === 'QUALIFICATIONS') {
    const GENERIC_CAPS = new Set(['the', 'a', 'an', 'q', 'project', 'tech', 'technologies', 'technology', 'using', 'with', 'built', 'source', 'blog', 'faq', 'answer', 'summary', 'profile', 'skills', 'languages', 'frameworks', 'tools', 'workflows']);
    const evTech = new Set();
    for (const m of String(evidence || '').matchAll(/\b[A-Z][A-Za-z0-9+#.]{2,24}\b/g)) {
      const t = m[0].toLowerCase();
      if (t.length >= 3 && !GENERIC_CAPS.has(t) && !_subjectNameTokens.has(t) && !_subjectRefTokens.has(t)) evTech.add(t);
    }
    const answerTokens = new Set(text.toLowerCase().match(/[a-z0-9+#.]{2,24}/g) || []);
    const named = [...evTech].filter(t => answerTokens.has(t));
    const hasDotted = /\b[a-z0-9+#]+(?:\.js|\.net)\b|c\+\+|c#/i.test(text);
    if (evTech.size >= 2 && named.length === 0 && !hasDotted) {
      return { complete: false, reason: 'QUALIFICATIONS_NO_CONCRETE_TECH', intent, evidenceUtilization: 0 };
    }
  }

  // Answer-obligation checks — generic, driven by the response contract's
  // computed obligation (facet, enumeration, supported values). Supported
  // values come from the knowledge graph / collections, matched against the
  // answer by normalized containment with no-space and word variants.
  const obligation = responseContract?.answerObligation;
  if (obligation && Array.isArray(obligation.supportedValues) && obligation.supportedValues.length) {
    const answerLower = text.toLowerCase();
    const answerNoSpace = answerLower.replace(/[^a-z0-9+#]/g, '');
    const surfaced = obligation.supportedValues.filter(value => {
    ...
    // FALSE_UNKNOWN_WITH_SUPPORT: the answer asserts "no verified / not
    // documented / unknown" for the requested proposition while supported
    // values exist and none were surfaced. Partial hedges that still name a
    // supported value are allowed.
    if (obligation.unknownAllowed === false) {
      const claimsUnknown = /\b(?:no verified|not documented|no documented|no evidence|no record|not (?:in|on) (?:the |his |her |their |zir )?(?:record|profile|knowledge|documentation)|isn'?t documented|no information|unknown\b|not available|not (?:publicly )?(?:listed|provided|specified|established))/i.test(text);
      if (claimsUnknown && surfaced.length === 0) {
        return { complete: false, reason: 'FALSE_UNKNOWN_WITH_SUPPORT', intent, evidenceUtilization: 0 };
      }
    }

    // ENUMERATION_MISSING_VALUES: the user asked for members/values of a
    // facet and supported values exist, but the answer surfaced none.
    if (obligation.enumeration && surfaced.length < (obligation.minimumConcreteValues || 1)) {
      return { complete: false, reason: 'ENUMERATION_MISSING_VALUES', intent, evidenceUtilization: 0 };
    }
  }
...
function getFactText(fact) {
  ...
  if (responseContract?.subIntent === 'SKILL_EVIDENCE') {
    ...
    const hasCandidateLink = new RegExp(`\\b(?:${_subjectPronounAlt}|${_possessivePronounAlt}|candidate|person|subject)\\b`, 'i').test(text);
  ...
  if (evidence && Array.isArray(evidence)) {
    for (const ev of evidence) {
      ...
      for (const kw of keywords) {
        if (kw.length >= 5 && !_subjectRefTokens.has(kw) && !/^(?:the|this|that|with|from|have|been|which|what|where|when|they|them|their|there|then|than|also|would|could|should|about|after|before|between|during|while|these|those|each|every|some|many|much|more|most|such|very|into|onto|upon|within|without|because|since|however|therefore|moreover|additionally|furthermore|nevertheless|nonetheless)\b/.test(kw)) {
  ...
  switch (intent) {
    ...
    case 'SKILL':
      ...
      if (!new RegExp(`\\b(?:${_subjectPronounAlt}${_subjectNamePattern}|${_possessivePronounAlt}|candidate|${_objectPronounAlt})\\b`, 'i').test(text)) {
    ...
    case 'FOLLOW_UP':
      ...
      if (!isProjectFollowUp &&
          !new RegExp(`\\b(?:${_subjectPronounAlt}${_subjectNamePattern}|${_possessivePronounAlt}|candidate|${_objectPronounAlt})\\b`, 'i').test(text)) {
        return { complete: false, reason: 'SKILL_NO_CANDIDATE_LINK', intent, evidenceUtilization };
      }
      return { complete: true, reason: 'OK', intent, evidenceUtilization };
...
module.exports = { classifyIntent, evaluateCompleteness, configureSubjectNames, configureSubjectPronouns };
...
function isNegatedPremiseQuestionInline(question) {
  ...
  if (new RegExp(`\\b(?:was|did|is|has)\\s+(?:${_subjectPronounAlt})\\b`).test(q) &&
      /\b(?:not|no|never)\b/.test(q)) {
    return true;
  }
  return false;
