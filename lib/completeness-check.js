...
function classifyIntent(question, names) {
  ...
  if (/\b(?:your\s+name|who\s+are\s+you|what\s+are\s+you|what\s+is\s+your\s+name|what\s+can\s+you\s+(?:do|help|answer)|what\s+can\s+i\s+ask\s+you|what\s+do\s+you\s+know|what\s+are\s+you\s+for|are\s+you\s+(?:an?\s+)?(?:ai|assistant)|which\s+assistant\s+are\s+you|what\s+model|what\s+provider|what\s+llm|what\s+ai|which\s+model|which\s+provider|what\s+powers\s+you|what\s+is\s+your\s+stack|what\s+mcp|what\s+connections|are\s+you\s+online|how\s+is\s+this\s+hosted|what\s+systems|what\s+is\s+this\s+chatbot|is\s+my\s+chat\s+private|is\s+my\s+conversation\s+private|what\s+data\s+do\s+you\s+use|is\s+this\s+hosted\s+on|where\s+is\s+data\s+sent|do\s+you\s+store|privacy|what\s+can(?:'t|not)\s+you\s+do|what\s+can\s+you\s+not\s+do|what\s+limits|daily\s+cap|rate\s+limit|what\s+are\s+your\s+limits)\b/.test(q)) {
    return 'META';
  }

  // FUTURE_CAPABILITY — questions about potential/learning, not current skill evidence
  if (new RegExp(`\\b(?:could\\s+${subj}\\s+(?:become|learn|get|pick|take|do|work|debug|troubleshoot|handle)|would\\s+${subj}\\s+(?:be\\s+able\\s+to|be|learn|work|debug|troubleshoot|handle)|will\\s+${subj}\\s+(?:be|learn|work|debug|troubleshoot|handle)|can\\s+${subj}\\s+(?:learn|pick\\s+up|get\\s+good|do|become|work|debug|troubleshoot|handle)|future\\s+(?:skill|role|capability)|potential\\s+(?:to|for)|someday|one\\s+day|learn\\s+in\\s+the\\s+future|eventually\\s+(?:lead|be|become|learn))\\b`).test(q)) {
  ...
  // Profile — "tell me about the candidate" but NOT "tell me about ProjectHub" or companies/degrees
