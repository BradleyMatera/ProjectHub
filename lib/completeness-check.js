...
function classifyIntent(question, names) {
  ...
  if (/\b(?:companies?|employers?|where\s+(?:has|did|does)\s+(?:he|she|they)\s+(?:work|worked|been\s+employed)|work\s+history|employment\s+history|army\s+service|military\s+service|military\s+.*\bservice\b)\b/.test(q)) {
    return 'EXPERIENCE';
  }
