...
function classifyIntent(question) {
  ...
  if (/\b(?:your\s+name|who\s+are\s+you|what\s+are\s+you|what\s+can\s+you\s+do|are\s+you\s+(?:an?\s+)?(?:ai|assistant)|which\s+assistant\s+are\s+you)\b/.test(q)) {
    return 'META';
  }

  // FUTURE_CAPABILITY — questions about potential/learning, not current skill evidence
  if (/\b(?:could\s+(?:he|they|she)\s+(?:become|learn|get|pick|take|do|work|debug|troubleshoot|handle)|would\s+(?:he|they|she)\s+(?:be\s+able\s+to|be|learn|work|debug|troubleshoot|handle)|will\s+(?:he|they|she)\s+(?:be|learn|work|debug|troubleshoot|handle)|can\s+(?:he|they|she)\s+(?:learn|pick\s+up|get\s+good|do|become|work|debug|troubleshoot|handle)|future\s+(?:skill|role|capability)|potential\s+(?:to|for)|someday|one\s+day|learn\s+in\s+the\s+future|eventually\s+(?:lead|be|become|learn))\b/.test(q)) {
