...
function evaluateCompleteness(answer, question, evidence, responseContract) {
  ...
  if (responseContract && responseContract.requiredEntities && responseContract.requiredEntities.length > 0) {
    ...
    for (const entity of responseContract.requiredEntities) {
      ...
      // Check multiple variants: exact, no-space, space-separated
      const variants = [
        eLower,
        eNoSpace,
        eLower.replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase(),
      ];
      const found = variants.some(v => answerLower.includes(v) || answerNoSpace.includes(v.replace(/[^a-z0-9]/g, '')));
  ...
  if (responseContract && responseContract.keyFacts && responseContract.keyFacts.length > 0) {
    ...
    for (const fact of responseContract.keyFacts) {
      ...
      // A fact is "covered" if at least 30% of its significant words appear in the answer
      ...
      if (matched / significantWords.length >= 0.3) {
        coveredFacts++;
      }
    }
    // If NO key facts are covered and the answer is short, it's likely generic
    if (coveredFacts === 0 && words.length < 25 && responseContract.keyFacts.length > 0) {
