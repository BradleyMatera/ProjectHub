...
function classifyIntent(question, names) {
  ...
  if (/\b(?:examples?\s+of|list|name)\b.{0,20}\b(?:jobs?|roles?|positions?|employers?|companies|places\s+(?:he|she|they)\s+(?:worked|worked\s+at))\b/i.test(q)) {
    return 'EXPERIENCE';
  }
  ...
  if (/\b(?:companies?|employers?|where\s+(?:has|did|does)\s+(?:he|she|they)\s+(?:work|worked|been\s+employed)|work\s+history|employment\s+history|army\s+service|military\s+service|military\s+.*\bservice\b)\b|[\w.+#-]+\s+experience\b/.test(q)) {
    return 'EXPERIENCE';
  }
