...
function classifyIntent(question, names) {
  ...
  // Qualifications — before JOB_FIT so "what qualifications" is not reduced to a role-fit lookup
  if (/\b(?:what\s+qualifications?|his\s+qualifications?|qualifications?\s+(?:does|has|do)\b)\b/.test(q)) {
