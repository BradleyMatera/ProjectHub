...
function classifyIntent(question) {
  ...
  if (/\b(?:could\s+(?:he|they|she)\s+(?:become|learn|get|pick|take|do)|would\s+(?:he|they|she)\s+(?:be\s+able\s+to|be|learn)|will\s+(?:he|they|she)\s+(?:be|learn)|can\s+(?:he|they|she)\s+(?:learn|pick\s+up|get\s+good|do)|future\s+(?:skill|role|capability)|potential\s+(?:to|for)|someday|one\s+day|learn\s+in\s+the\s+future|eventually\s+(?:lead|be|become|learn))\b/.test(q)) {
