...
function evaluateCompleteness(answer, question, evidence, responseContract) {
  ...
  // For GREETING and CONVERSATIONAL modes, skip evidence completeness checks.
  // These modes don't require evidence overlap or required entity coverage.
  ...
  if (policyMode === 'GREETING' || policyMode === 'CONVERSATIONAL') {
  ...
  // the question with NO/NOT_FIT polarity. A short denial like "Bradley has not
  ...
  switch (intent) {
    ...
    case 'PROJECT':
      // Denials about fabricated entities (e.g., "Bradley has not worked at Microsoft.")
