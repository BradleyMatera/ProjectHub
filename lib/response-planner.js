...
const PRIMARY_FACET_VERBS = [
  { verbs: /^(?:publish|published|publishin|written|wrote|write|author|authored|blog|blogged)$/, collectionRe: /blog|publication|article|post|writing/i, noun: 'publications' },
...
function assessPrimaryFacet({ question, knowledge, graph, subjectName } = {}) {
  ...
  // Shape A: "what/which <NP> <aux> <subj> ..." (aux may be elided by the NP)
  const mA = q.match(/\b(?:what|which)\s+([a-z0-9+#.'\s-]{2,60}?)\s+(?:did|does|do|has|have|had|is|are|was|were|get|got|earn|receive|use|used|hold|holds?)\s+([a-z0-9+#.'\s-]{2,40}?)(?:\b[\s?.,!]|$)/);
  if (mA) {
    if (isSubjectRef(mA[2])) facetPhrase = mA[1].trim();
    else {
      const resolved = resolveEntity(graph, mA[2]);
      if (resolved && resolved !== graph.subjectNorm) { entitySubject = resolved; facetPhrase = mA[1].trim(); }
    }
  }
  ...
  // Shape C: "what <aux> <subj> <verb>" — "what has he published", "what did he win"
  if (!facetPhrase && !verb) {
    const mC = q.match(/\b(?:what|which)\s+(?:did|does|do|has|have|had)\s+([a-z0-9+#.'\s-]{2,40}?)\s+([a-z]{3,})\b/);
    if (mC && isSubjectRef(mC[1])) verb = mC[2];
  }
  ...
  if (verb) {
    ...
    const isCategoryFacet = [head, _singular(head)].some(h =>
    ...
    if (!relations.size) {
      // No known relation and no collection will match below → not a facet we own.
  ...
  // Entity-attribute facets (e.g., a product's warranty/price attributes).
  if (!values.length && entitySubject && head) {
    const item = _collectionItems(knowledge, 'products')
      .concat(_collectionItems(knowledge, 'projects'))
      .find(it => normalizeEntity(_collectionItemName(it)) === entitySubject);
