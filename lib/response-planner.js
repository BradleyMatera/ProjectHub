...
function assessPrimaryFacet({ question, knowledge, graph, subjectName } = {}) {
  ...
  const subjectNameParts = new Set(
    String(subjectDisplay || '').toLowerCase().split(/[^a-z0-9+#.]+/).filter(w => w.length >= 3)
  );
  ...
  if (mA) {
    const trySubject = (subj, verb) => {
      ...
      if (resolved && resolved !== graph.subjectNorm) {
  ...
  if (!facetPhrase && !verb) {
    ...
    if (mC) {
      if (isSubjectRef(mC[1])) verb = mC[2];
        ...
        if (resolved && resolved !== graph.subjectNorm) { entitySubject = resolved; verb = mC[2]; }
      }
    }
  }
