...
function assessPrimaryFacet({ question, knowledge, graph, subjectName } = {}) {
  ...
  const subjectAliases = new Set([graph.subjectNorm, ...(graph.subjectAliases || [])].filter(Boolean));
  const subjectDisplay = subjectName || graph.subjectName || knowledge.identity?.name || '';
  // Name parts come from every known subject name — the caller's subjectName
  // is a framing subject (e.g. a company pulled from ranked facts) and may
  // not be the tenant subject at all.
  const subjectNameParts = new Set(
    [subjectDisplay, graph.subjectName, knowledge.identity?.name]
      .filter(Boolean).join(' ').toLowerCase()
      .split(/[^a-z0-9+#.]+/).filter(w => w.length >= 3)
  );
  // Canonical subject-reference set — "What skills does ze have?" must resolve
  // to the tenant subject, not an unknown entity. Includes configured pronoun
  // forms, default pronouns, and subject names/aliases.
  ...
  const isSubjectRef = token => {
    ...
    return subjectAliases.has(n) || subjectAliases.has(t) ||
      subjectNameParts.has(t) ||
  ...
  if (mA) {
    const trySubject = (subj, verb) => {
      if (isSubjectRef(subj)) { facetPhrase = mA[1].trim(); requestVerb = verb; return true; }
  ...
  if (!facetPhrase) {
    ...
    if (mB && isSubjectRef(mB[1])) verb = mB[2];
  }
  ...
  if (!facetPhrase && !verb) {
    ...
    if (mC) {
      if (isSubjectRef(mC[1])) verb = mC[2];
      else {
