...
function allSkillNames(knowledge) {
  return Object.values(knowledge?.skills || {})
    .flatMap(value => Array.isArray(value) ? value.map(skillItemName) : [])
    .filter(Boolean);
}
...
function assessEvidenceStrength(entities, knowledge, graph) {
  ...
  const strength = {};
  const skillValues = allSkillNames(knowledge);
 ...
 *            memberSupport:Object|null, support:'SUPPORTED'|'UNKNOWN'}}
 */
function assessComparisonSupport(question, members, graph) {
  ...
  const rel = q.match(/\b(?:used?|uses|using|built with|written in|runs? on|deployed (?:on|at|to)|made (?:of|with|from)|powered by)\s+([a-z0-9+#.\- ]{2,40}?)(?=[?!.,\s]|$)/i);
  if (rel && rel[1]) {
    const target = rel[1].trim();
    const memberSupport = {};
    for (const m of memberList) {
      memberSupport[m] = ['uses_tech', 'deployed_at', 'has_property', 'made_of']
        .some(r => graph && checkRelationship(graph, m, r, target).supported) ? 'SUPPORTED' : 'UNKNOWN';
    }
    return {
      dimension: target, dimensionKind: 'relation', memberSupport,
      support: Object.values(memberSupport).some(v => v === 'SUPPORTED') ? 'SUPPORTED' : 'UNKNOWN'
    };
  }
  ...
  const evaluative = EVALUATIVE_WORDS.test(q) || (stem && !DIMENSION_STEMS[stem]);
  if (!stem || evaluative) {
  ...
  const dimKey = DIMENSION_STEMS[stem] || stem;
  const dimNorm = normalizeEntity(dimKey);
  const stemNorm = normalizeEntity(stem);
  const memberSupport = {};
  for (const m of memberList) {
    const triples = graph ? getEntityRelationships(graph, m) : [];
    const supported = triples.some(t =>
      ...
      (t.relation === 'comparative_advantage' &&
        (String(t.object || '').toLowerCase().includes(stem) || String(t.object || '').toLowerCase().includes(dimKey)))
    );
    memberSupport[m] = supported ? 'SUPPORTED' : 'UNKNOWN';
  }
  return {
...
module.exports = { planResponse, formatPlanForPrompt, assessEvidenceStrength, classifyIntent, assessComparisonSupport };
