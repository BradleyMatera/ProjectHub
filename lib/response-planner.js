 ...
 *
 * No Bradley-specific logic. No hardcoded entity names.
 */
...
function determineDirectAnswer(question, intent, relationships, knowledge, entities) {
  ...
  // Adversarial — check if the claim is supported
  if (intent === 'ADVERSARIAL') {
    // Extract the claim from the question
    // "He was a senior AWS engineer, right?" → check if "senior AWS engineer" is supported
    if (/\b(?:senior|lead|principal|staff|architect)\b/.test(q)) {
      return 'no'; // Entry-level, not senior
    }
    if (/\b\d+\s+years?\b/.test(q)) {
      return 'no'; // No years claims in knowledge
    }
    if (/\b(?:mit|stanford|harvard|google|netflix|microsoft|amazon)\b/.test(q) &&
        !/\b(?:aws|amazon web services)\b/.test(q)) {
      // Check if this entity is in the knowledge
      const entityNorm = q.match(/\b(mit|stanford|harvard|google|netflix|microsoft)\b/)?.[1];
      if (entityNorm) {
        const graph = knowledge ? buildRelationshipGraph(knowledge) : null;
        if (graph) {
          const eNorm = entityNorm.replace(/[^a-z0-9]/g, '');
          const found = Array.from(graph.entityIndex.keys()).some(k => k.includes(eNorm));
          if (!found) return 'no';
        }
      }
    }
    if (/\b(?:expert|specialist|certified)\b/.test(q)) {
      // Check if expertise is claimed
      return 'no'; // Entry-level, not expert
    }
    if (/\b(?:team lead|managed a team|management)\b/.test(q)) {
      return 'no'; // No management experience
    }
    if (/\b(?:production|live incidents|on-call)\b/.test(q)) {
      return 'no'; // Intern, not production
    }
