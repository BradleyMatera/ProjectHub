...
function allSkillNames(knowledge) {
  ...
  const groups = Array.isArray(skills) ? skills : Object.values(skills || {});
  return groups
    .flatMap(value => Array.isArray(value) ? value.map(skillItemName) : [skillItemName(value)])
    .filter(Boolean);
}
...
function determineDirectAnswer(question, intent, relationships, knowledge, entities) {
  ...
  if (intent === 'SKILL') {
    ...
    if (skillMatch) {
      ...
      const graph = knowledge ? buildRelationshipGraph(knowledge) : null;
      if (graph) {
        const a = assessEntityEvidence(graph, skill);
        if (a.status === 'VERIFIED') return 'yes';
        if (a.status === 'GAP' || a.status === 'ADJACENT') return 'unknown';
...
function assessComparisonSupport(question, members, graph) {
  ...
  if (graph) {
    for (const t of (graph.triples || [])) {
      if (t.relation === 'has_property' && t.meta?.property) {
  ...
  for (const m of memberList) {
    ...
    if (propTriple) supportingFacts.push({ member: m, relation: 'has_property', property: dimKey, object: propTriple.object });
    else if (advTriple) supportingFacts.push({ member: m, relation: 'comparative_advantage', object: advTriple.object });
  }
  return {
