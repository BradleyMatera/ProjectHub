...
const { buildRelationshipGraph, checkRelationship, getEntityRelationships, assessEntityEvidence, resolveEntity, canonicalEntityNorm } = require('./relationship-graph');
...
const { COMMON_ENGLISH_WORDS } = require('./tech-claim-validator');
...
function assessRelationSupport({ graph, subject, target, requestedRelation } = {}) {
  ...
  // Canonical identity: exact entities/declared aliases win over declared
  // subject aliases, which win over weak tenant name parts. Do NOT
  // reimplement precedence here — canonicalEntityNorm is the single rule.
  ...
  const subjectOut = triples.filter(t => canonical(t.subject) === subjectNorm);
  const outgoing = subjectOut.filter(t => canonical(t.object) === targetNorm);
  const direct = outgoing.filter(t => directRelations.has(t.relation));
  if (direct.length) return result('SUPPORTED', direct);
  // Gap descriptions are natural-language phrases (e.g. "Most ZebraLang-style problems").
  // Allow a target like "ZebraLang" to match a gap that contains it.
  if (requestedRelation === 'has_gap') {
    const minFuzzy = 4;
    const fuzzy = subjectOut.filter(t =>
      t.relation === 'has_gap' &&
      targetNorm.length >= minFuzzy &&
      t.objectNorm &&
      (t.objectNorm.includes(targetNorm) || targetNorm.includes(t.objectNorm)) &&
      !t.objectNorm.startsWith('no') &&
      !t.objectNorm.startsWith('not'));
    if (fuzzy.length) return result('SUPPORTED', fuzzy);
  }
...
// ---------------------------------------------------------------------------
//
// Generic English noun → graph relation. This is language semantics, not
// tenant truth: "certifications" asks about the has_cert relation in any
// domain. Head nouns are matched singular/plural by stripping a trailing s.
const PRIMARY_FACET_NOUNS = {
  has_cert: ['certification', 'certificate', 'cert', 'credential'],
  has_degree: ['degree', 'diploma'],
  has_skill: ['skill', 'technology', 'language', 'framework', 'tool', 'service',
    'database', 'library', 'protocol', 'api', 'runtime', 'platform'],
  uses_tech: ['technology', 'stack', 'framework', 'tool', 'service', 'platform',
    'database', 'library', 'protocol', 'api', 'runtime'],
  worked_at: ['employer', 'company', 'organization', 'organisation'],
  interned_at: ['internship'],
  employed_as: ['role', 'position', 'title'],
  attended: ['school', 'university', 'college'],
  deployed_at: ['deployment', 'host'],
  has_award: ['award', 'honor', 'honour', 'recognition', 'prize']
};

// Past-verb facets: "has he published anything" asks about authored output.
// Verbs map to knowledge collection-name patterns rather than truth values.
const PRIMARY_FACET_VERBS = [
  { verbs: /^(?:publish|published|publishing|written|wrote|write|author|authored|blog|blogged)$/, collectionRe: /blog|publication|article|post|writing/i, noun: 'publications' },
  { verbs: /^(?:win|won|receive|received|get|got|earn|earned)$/, collectionRe: /award|honor|honour|recognition|prize/i, relations: ['has_award'], noun: 'awards' },
  { verbs: /^(?:earn|earned|hold|held)$/, collectionRe: /cert|degree|diploma|credential/i, relations: ['has_cert', 'has_degree'], noun: 'credentials' }
];
...
const _FACET_QUALIFIER_STOP = new Set(['a', 'an', 'the', 'any', 'some', 'of', 'for', 'in',
  'kind', 'kinds', 'type', 'types', 'sort', 'sorts', 'specific']);

function _facetRelationsForNoun(head) {
  const h = _singular(head);
  const out = [];
  for (const [rel, nouns] of Object.entries(PRIMARY_FACET_NOUNS)) {
    if (nouns.includes(h) || nouns.includes(head)) out.push(rel);
  }
  return out;
}

function _collectionItems(knowledge, key) {
  const v = knowledge?.[key];
  if (Array.isArray(v)) return v;
  if (v && typeof v === 'object' && Array.isArray(v.records)) return v.records;
  return [];
}

function _collectionItemName(item) {
  if (typeof item === 'string') return item;
  if (!item || typeof item !== 'object') return '';
  return String(item.name || item.title || item.label || item.skill || '').trim();
}

/**
 * Detect a primary question that asks for the members/values of one facet of
 * the tenant subject (or a named entity): "What awards did he get?", "What
 * AWS services has he used?", "Has he published anything?".
 *
 * Resolution is data-driven: the facet head noun maps to generic graph
 * relations and/or a knowledge collection whose key matches the noun. No
 * domain vocabulary or tenant truth is hardcoded.
 *
 * Returns { matched, facet, surfaces, supported, values[], subject } —
 * matched=false when the question is not a primary facet question.
 */
function assessPrimaryFacet({ question, knowledge, graph, subjectName } = {}) {
  const none = { matched: false };
  const q = String(question || '').toLowerCase().trim();
  if (!q || !knowledge || !graph) return none;

  const subjectDisplay = subjectName || graph.subjectName || knowledge.identity?.name || '';
  // Weak name parts come ONLY from the tenant's declared subject names — the
  // caller's subjectName is a framing subject (e.g. a company pulled from
  // ranked facts) and must not turn a selected project/company/product into
  // a tenant alias.
  const subjectNameParts = new Set(
    [graph.subjectName, knowledge.identity?.name]
      .filter(Boolean).join(' ').toLowerCase()
      .split(/[^a-z0-9+#.]+/).filter(w => w.length >= 3)
  );
  // Pronoun-only subject references — these can never be entities. Name
  // tokens are deliberately excluded: names go through canonical entity
  // resolution so a real entity wins over a weak name-part collision.
  const subjectPronouns = knowledgeAccess.getSubjectPronouns(knowledge);
  const pronounRefs = new Set([
    ...
    'the subject', 'the candidate', 'this person'
  ].filter(t => t && t.length >= 2));
  // Weak subject fallback — only reached after canonical entity resolution
  // fails to claim the token.
  const isSubjectRef = token => {
    const t = String(token || '').trim().toLowerCase();
    if (pronounRefs.has(t)) return true;
    const n = normalizeEntity(t);
    return subjectNameParts.has(t) ||
      (graph.subjectNorm && n && graph.subjectNorm.endsWith(n));
  };

  let facetPhrase = null;
  let verb = null;
  let entitySubject = null;

  // Shape A: "what/which <NP> <aux> <subj> <verb?>" (aux may be elided by
  // the NP). The trailing verb is captured because the request predicate
  // changes facet semantics: "what services does X offer" asks about
  // offerings, not the uses_tech relation.
  const mA = q.match(/\b(?:what|which)\s+([a-z0-9+#.'\s-]{2,60}?)\s+(?:did|does|do|has|have|had|is|are|was|were|get|got|earn|receive|use|used|hold|holds?)\s+([a-z0-9+#.'\s-]{2,40}?)(?:\s+([a-z][a-z']{2,}))?(?:\b[\s?.,!]|$)/);
  let requestVerb = null;
  if (mA) {
    const trySubject = (subj, verb) => {
      const token = String(subj || '').trim().toLowerCase();
      // Pronouns are always subject references and never entities.
      if (pronounRefs.has(token)) { facetPhrase = mA[1].trim(); requestVerb = verb; return true; }
      // Canonical graph resolution decides next: exact entities and declared
      // entity aliases outrank subject name parts, so "Avery" the project
      // wins over "Avery" the tenant's first name.
      const resolved = resolveEntity(graph, subj);
      // A token that resolves to the tenant subject itself (via fuzzy/alias
      // resolution, e.g. "bradley" → "bradleymatera") is a subject reference,
      // not a named entity — otherwise facet values get filtered against the
      // wrong subject and the facet is falsely reported as unsupported.
      if (resolved === graph.subjectNorm) {
        facetPhrase = mA[1].trim(); requestVerb = verb; return true;
      }
      if (resolved && resolved !== normalizeEntity(token)) {
        entitySubject = resolved; facetPhrase = mA[1].trim(); requestVerb = verb; return true;
      }
      if (resolved && resolved === normalizeEntity(token) &&
          graph.entityIndex.has(resolved) && (graph.entityIndex.get(resolved) || []).length > 0) {
        entitySubject = resolved; facetPhrase = mA[1].trim(); requestVerb = verb; return true;
      }
      // Weak fallback: bare subject name parts only apply when no real entity
      // claimed the name.
      if (isSubjectRef(token)) { facetPhrase = mA[1].trim(); requestVerb = verb; return true; }
      if (resolved) {
        entitySubject = resolved; facetPhrase = mA[1].trim(); requestVerb = verb; return true;
      }
      return false;
    };
    // The lazy subject group may split a multi-word name — "morgan" + verb
    // "vale". When the captured "verb" is actually a subject name part, or
    // the short subject resolves to nothing, retry with the verb group
    // folded back into the subject and read the real verb from the text
    // that follows it.
    const folded = mA[3] ? `${mA[2]} ${mA[3]}` : null;
    const idx = folded ? q.indexOf(folded) : -1;
    const restVerb = idx >= 0
      ? (q.slice(idx + folded.length).match(/^\s*([a-z][a-z']{2,})/) || [null, null])[1]
      : null;
    if (mA[3] && subjectNameParts.has(mA[3].toLowerCase())) {
      if (!trySubject(folded, restVerb ? restVerb.toLowerCase() : null)) {
        trySubject(mA[2], mA[3].toLowerCase());
      }
    } else if (!trySubject(mA[2], mA[3] ? mA[3].toLowerCase() : null) && mA[3]) {
      trySubject(folded, restVerb ? restVerb.toLowerCase() : null);
    }
  }
  // Shape B: "<aux> <subj> <verb> any/anything/ever" — "has he published anything"
  if (!facetPhrase) {
    const mB = q.match(/\b(?:has|have|did|does)\s+([a-z0-9+#.'\s-]{2,40}?)\s+([a-z]{3,})\s+(?:any|anything|something|ever)\b/);
    if (mB) {
      const token = mB[1].trim().toLowerCase();
      if (pronounRefs.has(token)) verb = mB[2];
      else {
        const resolved = resolveEntity(graph, mB[1]);
        if (resolved === graph.subjectNorm) verb = mB[2];
        else if (resolved && resolved !== normalizeEntity(token)) { entitySubject = resolved; verb = mB[2]; }
        else if (graph.entityIndex.has(resolved) && (graph.entityIndex.get(resolved) || []).length > 0) { entitySubject = resolved; verb = mB[2]; }
        else if (isSubjectRef(token)) verb = mB[2];
        else if (resolved) { entitySubject = resolved; verb = mB[2]; }
      }
    }
  }
  // Shape C: "what <aux> <subj> <verb>" — "what has he published", "what did
  // he win", "what is avery publishing". The subject may also be a named
  // entity ("what did Northstar publish").
  if (!facetPhrase && !verb) {
    const mC = q.match(/\b(?:what|which)\s+(?:did|does|do|has|have|had|is|are|was|were)\s+([a-z0-9+#.'\s-]{2,40}?)\s+(publish(?:ed|ing)?|written|wrote|write|authored?|blog(?:ged|ged)?|wins?|won|received?|gets?|got|earned?|held|holds?)\b/);
    if (mC) {
      const token = mC[1].trim().toLowerCase();
      if (pronounRefs.has(token)) verb = mC[2];
      else {
        const resolved = resolveEntity(graph, mC[1]);
        if (resolved === graph.subjectNorm) verb = mC[2];
        else if (resolved && resolved !== normalizeEntity(token)) { entitySubject = resolved; verb = mC[2]; }
        else if (graph.entityIndex.has(resolved) && (graph.entityIndex.get(resolved) || []).length > 0) { entitySubject = resolved; verb = mC[2]; }
        else if (isSubjectRef(token)) verb = mC[2];
        else if (resolved) { entitySubject = resolved; verb = mC[2]; }
      }
    }
  }

  if (!facetPhrase && !verb) return none;

  const subjNorm = entitySubject || graph.subjectNorm;
  const subjectLabel = entitySubject
    ? (graph.triples.find(t => t.subjectNorm === entitySubject)?.subject || entitySubject)
    : subjectDisplay;
  ...
  if (verb) {
    const spec = PRIMARY_FACET_VERBS.find(v => v.verbs.test(verb));
    if (!spec) return none;
    facetLabel = spec.noun;
    surfaces.add(spec.noun.replace(/s$/, ''));
    surfaces.add(spec.noun);
    surfaces.add(verb);
    if (spec.relations) spec.relations.forEach(r => relations.add(r));
    if (spec.collectionRe) collectionRes.push(spec.collectionRe);
  } else {
    // Head noun = last content word; earlier words are qualifiers ("aws services").
    const words = facetPhrase.split(/[^a-z0-9+#.]+/).filter(w => w && !_FACET_QUALIFIER_STOP.has(w));
    if (!words.length) return none;
    head = words[words.length - 1];
    qualifiers = words.slice(0, -1);
    facetLabel = head;
    surfaces.add(head);
    surfaces.add(_singular(head));
    for (const rel of _facetRelationsForNoun(head)) {
      relations.add(rel);
      for (const n of PRIMARY_FACET_NOUNS[rel]) { surfaces.add(n); surfaces.add(n + 's'); }
    }
    // Predicate semantics: an offering verb ("what services does X offer")
    // asks what the subject provides — not which technologies they use. The
    // noun 'service' is ambiguous between offering and cloud-technology; the
    // verb disambiguates. Offering questions fall back to a services/offerings
    // collection match (or unsupported) rather than enumerating skills.
    let offeringAsked = false;
    if (requestVerb && /^(?:offer|offers|offered|provide|provides|provided|sell|sells|sold|deliver|delivers|delivered|supply|supplies|supplied)$/.test(requestVerb)) {
      relations.delete('has_skill');
      relations.delete('uses_tech');
      offeringAsked = true;
    }
    // Type-category facets ("databases", "frameworks") denote a subtype of
    // skill/technology. Without a qualifier ("AWS services") they can only be
    // enumerated when the graph types the values — otherwise every skill
    // would falsely qualify, so defer to normal retrieval instead.
    const isCategoryFacet = !offeringAsked && [head, _singular(head)].some(h =>
      (PRIMARY_FACET_NOUNS.has_skill || []).includes(h) ||
      (PRIMARY_FACET_NOUNS.uses_tech || []).includes(h));
    if (isCategoryFacet && !qualifiers.length) {
      const typedMatch = (graph.triples || []).some(t =>
        t.relation === 'is_type' &&
        new RegExp(`\\b${_singular(head).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i').test(String(t.object || '')));
      if (!typedMatch) return none;
    }
    // Data-driven collection match: a knowledge collection whose key matches
    // the facet noun (singular/plural/camelCase) backs the facet directly.
    collectionRes.push(new RegExp(`^${head.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$|^${_singular(head).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i'));
    if (!relations.size) {
      // No known relation and no collection will match below → not a facet we
      // own — unless a named entity is the subject: entity attributes
      // (price, warranty, duration...) can still answer it via the fallback
      // below even when no top-level collection exists.
      const anyCollection = Object.keys(knowledge).some(k2 => {
        const kh = humanizeIdentifier(k2).toLowerCase();
        return kh === head || kh === _singular(head) || _singular(kh) === _singular(head) || collectionRes[0].test(k2) || collectionRes[0].test(kh);
      });
      if (!anyCollection && !entitySubject) return none;
    }
  }
  ...
  if (relations.size) {
    for (const t of graph.triples || []) {
      if (!relations.has(t.relation)) continue;
      if (t.subjectNorm !== subjNorm && normalizeEntity(t.subject) !== subjNorm) continue;
      const v = t.object || t.meta?.property;
      if (v) values.push(String(v));
    }
  }
  for (const key of Object.keys(knowledge)) {
    const kh = humanizeIdentifier(key).toLowerCase();
    const hit = collectionRes.some(re => re.test(key) || re.test(kh)) ||
      (head && (kh === head || _singular(kh) === _singular(head)));
    if (!hit) continue;
    for (const item of _collectionItems(knowledge, key)) {
      const n = _collectionItemName(item);
      if (n) values.push(n);
    }
  }
  // Entity-attribute facets (e.g., a product's warranty, a service's
  // duration). Resolve against every normalized entity collection —
  // products, services, projects, codePens — not a hardcoded pair.
  if (!values.length && entitySubject && head) {
    const { normalizeKnowledgeEntities } = require('./knowledge-entities');
    const item = normalizeKnowledgeEntities(knowledge)
      .find(it => normalizeEntity(it.name) === entitySubject);
    const attrs = item?.attributes || {};
    for (const [k2, v2] of Object.entries(attrs)) {
      if (_singular(k2.toLowerCase()) === _singular(head)) values.push(String(v2));
    }
    // Scalar top-level fields are attributes too ("price", "duration" on the
    // record itself rather than under attributes/properties).
    if (item) {
      for (const [k2, v2] of Object.entries(item)) {
        if (['raw', 'attributes', 'provenance', 'aliases'].includes(k2)) continue;
  ...
  // Qualifier filter ("AWS services" → values tied to the AWS entity).
  if (qualifiers.length && values.length) {
    const aliasTokens = new Set();
    for (const qual of qualifiers) {
      aliasTokens.add(qual.toLowerCase());
      const qe = resolveEntity(graph, qual);
      if (qe) {
        for (const t of graph.triples || []) {
          if (t.subjectNorm === qe || t.objectNorm === qe) {
            for (const name of [t.subject, t.object]) {
              for (const w of String(name || '').toLowerCase().split(/[^a-z0-9+#.]+/)) {
                if (w.length >= 3 && w !== head && w !== _singular(head)) aliasTokens.add(w);
              }
            }
          }
        }
        // Also alias/name parts of the resolved entity itself
        for (const t of graph.triples || []) {
          if (t.relation === 'has_alias' && t.subjectNorm === qe) aliasTokens.add(String(t.object).toLowerCase());
        }
        for (const w of qe.split(/\s+/)) if (w.length >= 3 && w !== head && w !== _singular(head)) aliasTokens.add(w);
      }
      // The qualifier may be a short form of a longer entity name ("aws" for
      // "Amazon Web Services (AWS)"). Add tokens from every entity name that
      // contains the qualifier so "amazon dynamodb" matches the aws family.
      const qualRe = new RegExp(`\\b${qual.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
      // Skip artifact names (projects, demos, tutorials): their descriptive
      // tokens ("analysis" in "AWS Cost-Analysis Model") are not part of the
      // qualifier's entity family.
      const artifactTypeRe = /project|codepen|demo|tutorial|app\b|site|widget|game|page|workflow|model|lab|assignment|capstone/i;
      const artifactNames = new Set((graph.triples || [])
        .filter(t => t.relation === 'is_type' && artifactTypeRe.test(String(t.object || '')))
        .map(t => t.subjectNorm));
      for (const t of graph.triples || []) {
        for (const name of [t.subject, t.object]) {
          const ln = String(name || '').toLowerCase();
          if (!qualRe.test(ln)) continue;
          if (artifactNames.has(normalizeEntity(name))) continue;
          for (const w of ln.split(/[^a-z0-9+#.]+/)) {
            if (w.length >= 3 && w !== head && w !== _singular(head)) aliasTokens.add(w);
          }
        }
      }
    }
    // Drop generic-English alias tokens (e.g. "web" from "Amazon Web
    // Services") — they would match unrelated values.
    const aliasPr = knowledgeAccess.getSubjectPronouns(knowledge) || {};
    const aliasPrForms = new Set([aliasPr.subject, aliasPr.object, aliasPr.possessive]
      .map(v => String(v || '').toLowerCase()).filter(Boolean));
    for (const tok of [...aliasTokens]) {
      if (COMMON_ENGLISH_WORDS.has(tok) || aliasPrForms.has(tok)) aliasTokens.delete(tok);
    }
    values = values.filter(v => {
      const lv = v.toLowerCase();
      return [...aliasTokens].some(tok => {
        const esc = tok.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        return new RegExp(`\\b${esc}\\b`).test(lv);
      });
    });
    // The bare qualifier itself is the family name — it is not a
    // responsive value for "which <qualifier> services". Keep only concrete
    // members so a vague answer cannot satisfy the facet.
    const qualNorms = new Set(qualifiers.map(q2 => normalizeEntity(q2)));
    values = values.filter(v => !qualNorms.has(normalizeEntity(v)));
  }

  values = [...new Set(values)].filter(v => v.length >= 2).slice(0, 10);
  return {
    matched: true,
    facet: facetLabel,
...
function determineDirectAnswer(question, intent, relationships, knowledge, entities) {
    ...
    if (graph) {
      ...
      if (entityMatch) {
        for (const entity of entityMatch) {
          ...
          if (!found && !knowledgeAccess.getSubjectReferenceSet(knowledge).has(eNorm) &&
              !/\b(the)\b/i.test(entity)) {
  ...
  // Skill question — "Do they know X?" / "Have they used X?"
  ...
  if (intent === 'SKILL') {
    ...
    if (skillMatch) {
      ...
      if (graph) {
        ...
        const subject = namedSubject && !knowledgeAccess.getSubjectReferenceSet(knowledge).has(String(namedSubject[1]).toLowerCase()) && !/^(?:the subject)$/.test(namedSubject[1])
...
function assessComparisonSupport(question, members, graph) {
  ...
  if (graph) {
    const memberNorms = new Set(memberList.map(m => canonicalEntityNorm(graph, m)));
    for (const t of (graph.triples || [])) {
...
module.exports = { planResponse, formatPlanForPrompt, assessEvidenceStrength, classifyIntent, assessComparisonSupport, assessRelationSupport, assessFacetSupport, assessPrimaryFacet };
