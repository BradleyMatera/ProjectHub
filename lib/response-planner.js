...
function assessPrimaryFacet({ question, knowledge, graph, subjectName } = {}) {
  ...
  // Configured tenant pronouns are subject references too — "What skills does
  // ze have?" must resolve to the tenant subject, not an unknown entity.
  const subjectPronouns = knowledgeAccess.getSubjectPronouns
    ? knowledgeAccess.getSubjectPronouns(knowledge) : null;
  const pronounRefs = new Set(['he', 'she', 'they', 'him', 'her', 'them',
    'the subject', 'the candidate', 'this person']);
  for (const form of [subjectPronouns?.subject, subjectPronouns?.object, subjectPronouns?.possessive]) {
    const v = String(form || '').trim().toLowerCase();
    if (v) pronounRefs.add(v);
  }
  ...
  if (qualifiers.length && values.length) {
    ...
    for (const tok of [...aliasTokens]) {
      if (COMMON_ENGLISH_WORDS.has(tok)) aliasTokens.delete(tok);
    }
