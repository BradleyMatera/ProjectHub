...
function buildRagChunks(knowledge) {
  ...
  (directAnswers || []).forEach((da, i) => {
    if (da?.answer) {
      const patterns = Array.isArray(da.questionPatterns) ? da.questionPatterns.join('; ') : da.id || `direct-${i}`;
      add('direct-answer', `Q: ${patterns} A: ${da.answer}`);
    }
  });
  return chunks;
}
