...
function buildRagChunks(knowledge, { deploymentFacts = [] } = {}) {
  ...
  // Identity chunk only when a real subject exists — empty-knowledge
  // configurations (General Scout) must not emit a filler "the candidate"
  // sentence as if a tenant subject were present.
  if (identity?.name) {
    const title = identity?.title || 'candidate';
    const article = /^[aeiou]/i.test(String(title).trim()) ? 'an' : 'a';
    add('identity', `${identity.name} is ${article} ${title} based in ${identity?.location || 'their location'}.`);
    ...
    if (contactMethods.length) add('contact', `Public contact methods for ${identity.name}: ${contactMethods.join(', ')}.`);
  }
  ...
  (experience || []).forEach(e => add('experience', `${e.role}${e.company ? ` at ${e.company}` : ''}${e.dates ? ` (${e.dates})` : ''}: ${e.summary || ''}`, { name: e.company || e.role }));
  (projects || []).forEach(p => {
    const linkText = [p.url, p.repo, p.platform, p.deployedAt, p.deployment].filter(Boolean).join(', ');
    ...
    const attrText = attrs
      ...
      : '';
    add('project', `Project ${p.name}: ${p.description || ''}${p.tech?.length ? ` Tech: ${p.tech.join(', ')}.` : ''}${attrText ? ` Properties: ${attrText}.` : ''}${linkText ? ` Links: ${linkText}.` : ''}`, { name: p.name });
  });
  ...
  (sourceMaterial || []).forEach((m, i) => {
    if (m?.content) {
      const title = m.title ? String(m.title).trim() : '';
      const text = title ? `${title}: ${m.content}` : String(m.content);
      add('source', text, { name: title || `source-${i}` });
    }
  });
  (blogCatalog?.records || []).forEach((post, i) => {
    if (post?.title || post?.brief) {
      const title = (post.title || 'Post').trim();
      const text = `${title}: ${post.brief || ''}${post.url ? ` URL: ${post.url}` : ''}`;
      add('blog', text, { name: title });
    }
  });
  (directAnswers || []).forEach((da) => {
    if (da?.answer) {
      add('direct-answer', String(da.answer), { name: da.id || 'direct' });
    }
  });

  // Core runtime/system facts are retrievable RAG data, not hard-coded
  // answer branches. The file is tenant- AND deployment-neutral: provider,
  // model, hosting, and allocation claims arrive via `deploymentFacts`,
  // generated from actual runtime configuration — an Ollama deployment
  // never inherits a Cloudflare claim. Package-scoped scope claims belong
  // in the package's own knowledge.systemFacts section.
  for (const fact of (scoutRuntimeKnowledge?.facts || [])) {
    if (!fact?.text) continue;
    add(fact.tag || 'scout-runtime', String(fact.text), {
      name: fact.id || 'runtime',
      runtimeFact: true,
  ...
  for (const fact of deploymentFacts) {
    if (!fact?.text) continue;
    add(fact.tag || 'scout-deployment', String(fact.text), {
      name: fact.id || 'scout-deployment',
      runtimeFact: true,
      deploymentFact: true,
      runtimeFactId: fact.id || null
    });
  ...
  // Tenant-declared system/scope facts — what THIS application covers.
  // Packages claim their own scope here; the Core injects none.
  for (const fact of (knowledge?.systemFacts || [])) {
    if (!fact?.text) continue;
    add(fact.tag || 'system-fact', String(fact.text), {
      name: fact.id || 'system-fact',
      runtimeFact: true,
      runtimeFactId: fact.id || null
    });
  }
  ...
  for (const entity of normalizeKnowledgeEntities(knowledge).filter(item => item.type !== 'project')) {
    if (entity.type === 'codepen' && !entity.description
      && Object.keys(entity.attributes).length === 0 && !entity.raw.tech?.length
      && !(entity.raw.platform || entity.raw.deployedAt || entity.raw.deployment)) continue;
    ...
    const links = [entity.raw.url || entity.raw.link, entity.raw.repo,
        entity.raw.platform, entity.raw.deployedAt, entity.raw.deployment]
