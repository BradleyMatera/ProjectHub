...
const CURRENT_EMPLOYMENT_RE = /\b(?:is\s+(?:currently\s+)?(?:a\s+)?(?:\w+\s+)?(?:at|with)\s+|works\s+(?:currently\s+)?(?:at|with|for)\s+|is\s+now\s+(?:a\s+)?\w+\s+at|currently\s+works\s+(?:at|for|with))\b/i;
