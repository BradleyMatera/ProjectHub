...
const { getKnownTechnologies, getSubjectPronouns } = require('./knowledge-access');
...
function validateProjectTechnologyRelationships(text, contract, knowledge, evidenceText = '') {
  ...
  function buildCandidateSubjectRe(k) {
    ...
    const configuredSubj = getSubjectPronouns(k)?.subject;
    if (configuredSubj) names.add(String(configuredSubj).toLowerCase());
  ...
  const configuredSubj = String(getSubjectPronouns(knowledge)?.subject || '')
    .trim().toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const clauseHeadAlt = [...new Set(['it', 'he', 'she', 'they', configuredSubj, 'there', 'the', 'a', 'an',
  ...
  for (const project of knowledge.projects || []) {
    ...
    for (const sentence of sentences) {
      ...
      for (const clause of clauses) {
        ...
        const objects = technologyRelationObjects(names, clause, { normalizeToken: techNormalizer });
        for (const tech of globalTech) {
          ...
          if (evidenceSupportsTechnologyRelation(names, tech, [{ ...project, kind: 'project' }], { normalizeToken: techNormalizer })) continue;
