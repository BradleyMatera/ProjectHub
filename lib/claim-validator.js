...
function validateProjectTechnologyRelationships(text, contract, knowledge, evidenceText = '') {
  ...
  for (const project of knowledge.projects || []) {
    ...
    for (const sentence of sentences) {
      ...
      if (!relationVerb.test(sentence) || negativeRelation.test(sentence)) continue;
      // Skip sentences where a personal subject used a technology in the project.
      // If the sentence starts with a personal subject and the project name appears
      // after the relation verb, this is a personal-skill claim (e.g. "He has used
      // React to build ProjectHub"), not an unsupported project tech-stack claim.
      const matchedName = names.find(name => lower.includes(normalizeText(name)));
      if (candidateSubjectRe.test(sentence) && matchedName) {
        const verbMatch = sentence.match(relationVerb);
        const projectMatch = sentence.match(new RegExp(matchedName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'));
        if (verbMatch && projectMatch && projectMatch.index > verbMatch.index + verbMatch[0].length) continue;
      }
      for (const tech of globalTech) {
        if (!tech || tech.length < 2) continue;
        if (!phraseAppearsInSentence(sentence, tech)) continue;
...
function validateClaims(answer, question, contract, evidenceText, knowledge) {
  ...
  if (requestedTopic && !skillIsVerified) {
  ...
  if (requestedTopic && !knownSkill && !skillIsVerified) {
