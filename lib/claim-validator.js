...
function validateProjectTechnologyRelationships(text, contract, knowledge, evidenceText = '') {
  ...
  function buildCandidateSubjectRe(k) {
    ...
    if (k?.identity?.name) names.add(k.identity.name);
    if (k?.identity?.preferredName) names.add(k.identity.preferredName);
  ...
  for (const project of knowledge.projects || []) {
    ...
    for (const sentence of sentences) {
      ...
      const inProjectRe = matchedName ? new RegExp(`\\bin\\s+${matchedName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i') : null;
      if (candidateSubjectRe.test(sentence) && inProjectRe && inProjectRe.test(sentence)) continue;
