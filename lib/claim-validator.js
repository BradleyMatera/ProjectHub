...
const CURRENT_EMPLOYMENT_RE = /\b(?:is\s+(?:currently\s+)?(?:a\s+)?(?:\w+\s+)?(?:at|with)\s+|is\s+(?:currently|now)\s+an?\s+\w+(?:\s+\w+){1,3}\s+at\s+|is\s+(?:currently\s+)?(?:working|employed)\s+(?:at|for|by)\s+|works\s+(?:currently\s+)?(?:at|with|for)\s+|is\s+now\s+(?:a\s+)?\w+\s+at|currently\s+works\s+(?:at|for|with))\b/i;
