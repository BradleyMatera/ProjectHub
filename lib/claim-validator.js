...
function validateProjectTechnologyRelationships(text, contract, knowledge) {
  const invalid = [];
  ...
  for (const project of knowledge.projects || []) {
    ...
    for (const sentence of sentences) {
      ...
      for (const tech of globalTech) {
        if (!tech || tech.length < 2 || !lower.includes(tech)) continue;
...
function validateClaims(answer, question, contract, evidenceText, knowledge) {
  ...
  if (NEGATIVE_TRAIT_RE.test(text)) {
    // Only allowed when the question explicitly asks for a negative assessment,
    // the contract factState is not UNKNOWN, and the evidence supports the trait.
    ...
    for (const m of matches) {
      ...
      if (!isNegativeQuestion || factState === 'UNKNOWN' || !isEntityInEvidence(trait, evidenceText)) {
