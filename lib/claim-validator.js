...
function validateProjectTechnologyRelationships(text, contract, knowledge, evidenceText = '') {
  ...
  for (const project of knowledge.projects || []) {
    ...
    for (const sentence of sentences) {
      ...
      if (negativeRelation.test(sentenceForTech)) continue;
      ...
      for (const clause of clauses) {
        ...
        for (const tech of globalTech) {
          ...
          if (!phraseAppearsInSentence(clause, tech)) continue;
          if (projectTech.has(tech)) continue;
          // Allow the tech if the project's own name or description explicitly
          // contains it (e.g. "AWS Serverless Metadata Extraction Workflow"
          // naturally contains "AWS").
          const projectCorpus = `${project.name || ''} ${project.description || ''}`;
          if (projectCorpus.trim() && phraseAppears(projectCorpus, tech, techNormalizer)) continue;
