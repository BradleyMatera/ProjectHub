...
// tenant-specific strings. Tenant-specific names come from the knowledge object.

const { getKnownTechnologies, getSubjectPronouns, getSubjectReferenceSet } = require('./knowledge-access');
const scoutIdentity = require('./scout-identity');
const { evidenceSupportsTechnologyRelation, phraseAppears, technologyRelationObjects } = require('./evidence-relations');
...
// "is currently working/employed at|for|by" and "is currently/now a <multiword
// title> at" are employment phrasing. "with" is only kept on the original
// short form — "working with TypeScript" or "a language with generics" are
// usage, not employment. Multi-word titles require an explicit temporal cue
// so "is a TypeScript developer at his job" stays a role claim, not a
// current-employment one.
const CURRENT_EMPLOYMENT_RE = /\b(?:is\s+(?:currently\s+)?(?:a\s+)?(?:\w+\s+)?at\s+|is\s+(?:currently|now)\s+an?\s+\w+(?:\s+\w+){1,3}\s+at\s+|is\s+(?:currently|now)\s+with\s+|is\s+(?:currently\s+)?(?:working|employed)\s+(?:at|for|by)\s+|works\s+(?:currently\s+)?(?:at|for)\s+|is\s+now\s+(?:a\s+)?\w+\s+at|currently\s+works\s+(?:at|for))\b/i;
...
const CEILING_DISALLOWED = {
  ...
  'has documented experience with': /\b(?:expert|mastery|master(?:ed|y)?|deep\s+knowledge|seasoned|veteran|professional\s+engineer)\b/i,
};
...
function validateProjectTechnologyRelationships(text, contract, knowledge, evidenceText = '', graph = null) {
  const invalid = [];
  if (!knowledge || !text) return invalid;

  const requestedTopic = normalizeText(contract?.requestedTopic || '');
  const graphTech = (graph || buildRelationshipGraph(knowledge)).triples.filter(triple =>
    ['uses_tech', 'uses_platform'].includes(triple.relation) && !triple.meta?.inferred);
  const globalTech = new Set([...getKnownTechnologies(knowledge), ...graphTech.map(triple => triple.object)].map(normalizeText).filter(Boolean));
  ...
  function buildCandidateSubjectRe(k) {
    const names = getSubjectReferenceSet(k);
  ...
  // Split a sentence into clause-like segments at clear clause boundaries
  // (;, :, or a conjunction introducing a new subject/determiner/copula).
  // This stops a relation verb in one clause (e.g. "uses a two-branch release
  // model, and it has a source for AI coding agents") from causing unrelated
  // technology words in later clauses to be flagged.
  // The conjunction lookahead must include the tenant's configured subject
  // pronoun: "…, and ze has used Vue" is a new clause whatever the pronoun set.
  const prForms = (() => { const pr = getSubjectPronouns(knowledge) || {};
    return [pr.subject, pr.object, pr.possessive]
      .map(v => String(v || '').trim().toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).filter(Boolean); })();
  const clauseHeadAlt = [...new Set(['it', 'he', 'she', 'they', ...prForms, 'there', 'the', 'a', 'an',
    'this', 'that', 'which', 'who', 'what', 'was', 'were', 'is', 'are', 'has', 'had',
    'have', 'does', 'did', 'do', 'will', 'would', 'could', 'should'].filter(Boolean))].join('|');
  const CLAUSE_BOUNDARY = new RegExp(`\\s*[:;]\\s+|(?:,?\\s*(?:and|but|or|nor|yet|so)\\s+(?=(?:${clauseHeadAlt})\\b))`, 'i');
  function splitIntoClauses(sentence) {
    const parts = sentence.split(CLAUSE_BOUNDARY).filter(Boolean).map(s => s.trim()).filter(s => s.length >= 6);
    return parts.length ? parts : [sentence.trim()];
  }

  for (const project of knowledge.projects || []) {
    const names = [project.name, ...(project.aliases || [])].filter(Boolean);
    const projectTech = new Set((project.tech || []).map(techNormalizer).filter(Boolean));
    for (const triple of graphTech) {
      if (names.some(name => normalizeText(name) === normalizeText(triple.subject))) {
        projectTech.add(techNormalizer(triple.object));
      }
    }
    for (const sentence of sentences) {
      // URLs are availability evidence, not technology claims. Strip them so
      // a hosted project URL does not register the host as a project technology.
      ...
      const clauses = splitIntoClauses(sentenceForTech);
      for (const clause of clauses) {
        if (!relationVerb.test(clause)) continue;
        const objects = technologyRelationObjects(names, clause, { normalizeToken: techNormalizer, subjectRefTokens: prForms });
        for (const tech of globalTech) {
          if (!tech || tech.length < 2) continue;
          if (!objects.some(object => phraseAppearsInSentence(object, tech))) continue;
          if (projectTech.has(techNormalizer(tech))) continue;
          // Allow project descriptions only when an affirmative proposition
          // links the project to the technology; a name or comparison mention
          // alone does not establish actual use.
          if (evidenceSupportsTechnologyRelation(names, tech, [{ ...project, kind: 'project' }], { normalizeToken: techNormalizer, subjectRefTokens: prForms })) continue;
          if (evidenceText && evidenceSupportsTechnologyRelation(names, tech, evidenceText, { normalizeToken: techNormalizer, subjectRefTokens: prForms })) continue;
 ...
 */
function validateClaims(answer, question, contract, evidenceText, knowledge, evidence = null, graph = null) {
  ...
  if (roleMatch) {
    ...
    const isHistoricalClaim = /\b(?:worked\s+as|was\s+a|has\s+experience\s+as|served\s+as|employed\s+as|acted\s+as)\b/i.test(text);
    ...
    // Requested role must not be treated as held. Only flag a role claim when
    // the matched title actually matches the requested role and the clause is
    // not a conditional/comparison mention.
    if (requestedRole && isKnownRole(requestedRole, knowledge) === false) {
      const reqLower = String(requestedRole).toLowerCase();
      const claimLower = claimedRole.toLowerCase();
      const matchesRequestedRole = claimLower.includes(reqLower) || reqLower.includes(claimLower);
      const matchIndex = roleMatch.index ?? text.indexOf(roleMatch[0]);
      const ctx = text.slice(Math.max(0, matchIndex - 200), Math.min(text.length, matchIndex + roleMatch[0].length + 200));
      const isConditional = /\b(?:whether|if|could|would|not\s+(?:documented|verified|established|clear)|no\s+(?:verified|public)\s+(?:evidence|record)|unknown|unclear)\b/i.test(ctx);
      if (matchesRequestedRole && /\b(?:was|is|has\s+been|worked\s+as)\b/i.test(ctx) && !isConditional) {
  ...
  invalid.push(...validateProjectTechnologyRelationships(text, contract, knowledge,
    Array.isArray(evidence) && evidence.length ? evidence : evidenceText, graph));
  ...
  if (requestedTopic && !knownSkill && !skillIsVerified && !topicIsNegated) {
    ...
    // Any claim "Morgan knows X" is unsupported unless X appears in the evidence.
