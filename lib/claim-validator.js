...
const CURRENT_EMPLOYMENT_RE = /\b(?:is\s+(?:currently\s+)?(?:an?\s+)?(?:\w+\s+){0,3}(?:at|with)\s+|is\s+(?:currently\s+)?(?:working|employed)(?:\s+as\s+(?:an?\s+)?[\w\s]{0,40}?)?\s+(?:at|for|with|by)\s+|works\s+(?:currently\s+)?(?:at|with|for)\s+|is\s+now\s+(?:an?\s+)?\w+(?:\s+\w+){0,3}\s+at\b|currently\s+works\s+(?:at|for|with))\b/i;
