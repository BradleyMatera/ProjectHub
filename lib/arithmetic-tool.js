...
function findArithmeticSubtasks(text) {
  ...
  while ((m = binRe.exec(src)) !== null) {
    ...
    if (sym === '-' && /^\d/.test(m[1].trim()) && /^\d/.test(m[3].trim())) {
      ...
      // A bare NNNN-NNNN span is a year/serial range even with empty context —
      // only an explicit cue word ("what is", "calculate") makes it arithmetic.
      if (!/\s/.test(between) && /^\d{4}$/.test(m[1].trim()) && /^\d{4}$/.test(m[3].trim()) &&
          !/[a-z]/i.test(src.slice(0, m.index) + src.slice(m.index + m[0].length))) continue;
    }
    // A digit/digit/digit chain with no spaces is a date ("1/2/2024"), not a
    // division — a bare N/N component of a three-part slash sequence is not
    // an arithmetic subtask.
    if (sym === '/' && /^\d/.test(m[1].trim()) && /^\d/.test(m[3].trim())) {
      const opStart = m.index + m[1].length;
      const between = src.slice(opStart, m.index + m[0].length - m[3].length);
      if (!/\s/.test(between) &&
          (/^\/\s*\d/.test(src.slice(m.index + m[0].length)) ||
           /\d\s*\/$/.test(src.slice(0, m.index)))) continue;
    }
  ...
  while ((m = wordRe.exec(src)) !== null) {
    ...
    // A "what"/"now" continuation only counts when it asks for the held
    // quantity — "What is the role?" must not turn "closes 1" into a fact.
    const nextText = nextClause.trim();
    const heldRe = heldNoun
      ? new RegExp('\\b' + heldNoun.replace(/s$/, '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + 's?\\b', 'i')
      : null;
    const cueInNext = nextText.length > 0 && nextText.length < 80 && (
      /^\s*how\s+(?:many|much)\b/i.test(nextText) ||
      (/^\s*(?:what|now)\b/i.test(nextText) &&
        ((heldRe && heldRe.test(nextText)) ||
          /\b(?:left|remaining|remains?|total|altogether|in all|result|answer)\b/i.test(nextText)))
    );
...
function asksForComputedResult(text) {
  const t = String(text || '');
  if (/\b(?:what|how many|how much|equals?|tell me|calculate|compute|solve|remains?|remaining|left|total|altogether|work out|figure out)\b/i.test(t)) return true;
  // Cue-less explicit arithmetic — "19 minus 6?", "2+2", "15% of 200" — still
  // asks for a computed result; without this the control path attaches no
  // computed facts and nothing enforces the verified result.
  return /\d\s*[-+*/×x÷]\s*\d|\b\d+\s+(?:plus|minus|times|divided\s+by|mod(?:ulo)?)\s+\d+|\d+\s*%\s+of\s+\d+/i.test(t);
}
