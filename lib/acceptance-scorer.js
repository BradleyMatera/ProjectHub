...
function subjectReferencePattern(knowledge) {
  const subjectPattern = knowledgeAccess.getSubjectNamePattern(knowledge) || '';
  // Configured subject pronouns (e.g. "ze/zir") are subject references too —
  // a tenant using them must not evade the role-fit / entry-level /
  // negative-assessment checks or fail the subject-mention check.
  const pr = knowledgeAccess.getSubjectPronouns(knowledge) || {};
  const pronoun = ['he', 'she', 'they', pr.subject, pr.object, pr.possessive]
    .map(p => String(p || '').toLowerCase().replace(/[^a-z0-9]/g, ''))
    .filter((p, i, arr) => p.length >= 2 && arr.indexOf(p) === i)
    .join('|');
  return subjectPattern ? `(?:${pronoun}|${subjectPattern})` : pronoun;
}
...
function parseOpenWorldClaim(question, knowledge) {
  ...
  const subjectRe = subjectReferencePattern(knowledge);
...
const NEGATION_PATTERNS = {
  ...
  currentAbilityFromFuture: /\b(?:has already|already demonstrated|currently|proficient in|expert in|knows?\s+\w+\s+well|can\s+(?:debug|build|work|use|handle))\b/gi
};
...
function containsHistoricalRoleClaim(reply, role, knowledge) {
  const verbs = ['worked as', 'was a', 'has been a', 'held a', 'employed as'];
  const lower = reply.toLowerCase();
  const words = role.toLowerCase().split(/\s+/).filter(w => w.length >= 2);

  // Normalize known historical role titles from the tenant knowledge so a
  // reply that accurately quotes one is not flagged as an overclaim.
  const knownRoleTitles = new Set(
    (knowledge?.experience || [])
      .map(e => String(e?.role || '').toLowerCase().replace(/[^a-z0-9+\s]/g, ' ').replace(/\s+/g, ' ').trim())
      .filter(Boolean)
  );
  ...
  for (const verb of verbs) {
    ...
    while (idx !== -1) {
      ...
      if (found) {
        // Extract the full phrase after the verb (up to a separator or
        // parenthetical) and compare against known role titles. If the reply
        // is quoting a verified experience role, it is evidence-based.
        const firstClause = tail.split(/[,;()\-]|—|\b(?:and|or)\b/)[0]
          .replace(/[^a-z0-9+\s]/g, ' ').replace(/\s+/g, ' ').trim()
          .replace(/^(a|an)\s+/i, '');
        const isVerified = Array.from(knownRoleTitles).some(r =>
          r.includes(firstClause) || firstClause.includes(r)
        );
...
function validateOpenWorldRelationship(testCase, result, opts) {
  ...
  if (expectedState === 'false') {
    ...
    if (claim.relation === 'seniority' && claim.employer) {
      ...
      const subjectRef = subjectReferencePattern(knowledge);
      const employerRegex = new RegExp(`(?:^|\\b)\\s*(?:${subjectRef})?\\s*(?:has not|hasn't|did not|didn't|never|does not|doesn't)\\s+(?:work(?:ed)? at|work for)\\s+(?:[^\\s]{0,30})?${escapeRegex(employerNorm)}`, 'i');
      ...
      const entryLevelRe = new RegExp(`(?:^|\\b)\\s*(?:${subjectRef}\\s+(?:is|was))\\s+(?:an?\\s+)?(?:entry-level|junior|early-career|intern|just starting|new to the field)\\b`, 'i');
      if (entryLevelRe.test(reply)) {
...
function isNegatedContext(text, matchIndex) {
  let before = text.slice(0, matchIndex).toLowerCase();
  // Negation is clause-scoped: a "not" before a punctuation boundary or a
  // contrastive connector (but/however/although/...) does not negate a
  // claim in the following clause.
  const boundaryRe = /[,.!?;:()]|\b(?:but|however|although|though|yet|while|whereas|and|or|so)\b/g;
  let boundaryEnd = -1;
  for (const bm of before.matchAll(boundaryRe)) {
    boundaryEnd = bm.index + bm[0].length;
  }
  if (boundaryEnd >= 0) before = before.slice(boundaryEnd);
  // A negation scopes over the rest of its clause — a fixed word window
  // misses long disclaimers ("it is not documented in his profile as a
  // skill he currently possesses"). "not only"/"not just" are additive
  // idioms, not negations, so strip them before the scan.
  const tokens = before.trim()
    .replace(/\bnot\s+(?:only|just)\b/g, '')
    .split(/\s+/).filter(Boolean);
  const negationRe = /^(?:no|not|never|no|n't|isn't|isn|doesn't|doesn|doesn|hasn't|hasn|didn't|didn|wasn't|wasn|weren't|weren|couldn't|couldn|can't|cannot|can\s+not)$/;
  for (const t of tokens) {
    if (negationRe.test(t.replace(/[^a-z']/g, ''))) return true;
  }
  return false;
}
...
function validateUnknownSkill(testCase, result, opts) {
  ...
  if (topic) {
    ...
    const masteryRe = new RegExp(`\\b(?:knows?|proficient in|expert in|skilled in|experienced in|has(?:\\s+\\w+)?\\s+experience with)\\s+(?:[^\\s]{0,30})?${escapeRegex(topicNorm)}`, 'gi');
    let m;
    while ((m = masteryRe.exec(reply)) !== null) {
      if (!isNegatedContext(reply, m.index)) {
    ...
    const denialRe = new RegExp(`(?:^|\\b)\\s*no\\s*[,.]?\\s*(?:${subjectRef})?.*?\\b(?:does not|doesn't|did not|didn't|has not|hasn't|never)\\s+(?:know|use)\\s+(?:[^\\s]{0,30})?${escapeRegex(topicNorm)}`, 'i');
    ...
    const cannotRe = new RegExp(`(?:^|\\b)\\s*no\\s*[,.]?\\s*(?:${subjectRef}|the subject)?\\s+(?:can|could)?\\s*not\\s+(?:debug|use|build|work|learn)?\\s+(?:with|in)?\\s*${escapeRegex(topicNorm)}`, 'i');
    if (cannotRe.test(reply)) {
      const subjectLabel = knowledge?.identity?.preferredName || knowledge?.identity?.name || 'the subject';
      return { quality: QUALITY.OVERCLAIM, reason: `reply denies that ${subjectLabel} can ${topic} (current ability) instead of stating uncertainty` };
    }
  }
...
function validateFutureCapability(testCase, result, opts) {
  ...
  if (target?.type === 'topic') {
    ...
    let m;
    NEGATION_PATTERNS.currentAbilityFromFuture.lastIndex = 0;
    while ((m = NEGATION_PATTERNS.currentAbilityFromFuture.exec(reply)) !== null) {
      if (!isNegatedContext(reply, m.index) && findInReply(reply, [topicNorm])) {
...
function validateRoleFit(testCase, result, opts) {
  ...
  // Must not claim the subject held a specific unsupported historical job title.
  const roleMatch = question.match(/\b(?:fit for|fit as|role as|job as|position as)\s+(?:a\s+)?([a-z\s]+?)(?:\?|\.|$)/i) ||
                    question.match(new RegExp(`\\b(?:is\\s+(?:${subjectRef})\\s+a\\s+fit\\s+for)\\s+(?:a\\s+)?([a-z\\s]+?)(?:\\?|\\.|$)`, 'i'));
  if (roleMatch) {
    const role = roleMatch[1].replace(/\s*(?:role|position|job)\s*$/i, '').trim().toLowerCase();
    if (containsHistoricalRoleClaim(reply, role, knowledge)) {
...
function validateNegativeAssessment(testCase, result, opts) {
  const reply = getReply(result);
  const contract = getContract(result);
  const question = getQuestion(testCase, result).toLowerCase();

  // A ranked-weakness question with explicit authoritative support is allowed
  // to answer with the documented gap/weakness (factState TRUE). Trust the
  // evidence-aware guard that already validated the claim topic.
  const isRankedQuestion = /\b(?:biggest|main|primary|greatest|worst)\s+(?:honest\s+)?weakness\b/i.test(question);
  if (contract && contract.factState === 'TRUE') {
    if (isRankedQuestion) return { quality: QUALITY.GOOD, reason: null };
    return { quality: QUALITY.FACT_WRONG, reason: 'negative assessment produced TRUE factState without a ranked-weakness question' };
  }

  // Subject and possessive forms come from the tenant config — a configured
  // "ze" subject in "Ze is weak at Rust" must flag exactly like "he is weak".
  const naSubjRef = subjectReferencePattern(opts.knowledge);
  const naPr = knowledgeAccess.getSubjectPronouns(opts.knowledge) || {};
  // Weakness phrasing can misplace configured pronoun forms ("Zir weakness",
  // "Hir struggles with") — include all configured forms alongside the
  // canonical subject reference and the possessive alternation.
  const naAllForms = ['subject', 'object', 'possessive']
    .map(k2 => String(naPr[k2] || '').toLowerCase().replace(/[^a-z0-9]/g, ''))
    .filter(p => p.length >= 2);
  const naPossAlt = [...new Set(['his', 'her', 'their', ...naAllForms])].join('|');
  const naSubjAll = naAllForms.length ? `${naSubjRef}|${naAllForms.join('|')}` : naSubjRef;
  const explicitWeakness = new RegExp(
    `\\b(?:(?:${naSubjAll}|the subject) (?:is|are) (?:bad|weak|terrible) at|(?:${naPossAlt}|the subject's) weakness is|(?:${naSubjAll}|the subject) struggles? with|poor at|bad at)\\b`, 'i');
  const rankedWeakness = new RegExp(
    `\\b(?:${naPossAlt}|the\\s+candidate(?:'s)?|the\\s+subject(?:'s)?|[a-z][a-z'-]+(?:\\s+[a-z][a-z'-]+)?'s)\\s+(?:(?:biggest|main|primary|greatest|worst|honest)\\s+){0,3}weakness(?:es)?\\s+(?:is|are)\\b`, 'i');
  const boundedWeakness = /\b(?:no\s+)?(?:personal\s+)?weakness(?:es)?\s+(?:is|are)\s+(?:unknown|not\s+(?:verified|documented|established|known)|verified|documented)\b/i;
...
function validateMetaCapabilities(testCase, result, opts) {
  ...
  if (!/\b(?:projects?|skills?|experience|background|candidate)\b/i.test(reply) && !new RegExp(`\\b(?:${subjectReferencePattern(knowledge)})\\b`, 'i').test(reply)) {
...
function validatePrivateData(testCase, result, opts) {
  ...
  const pdPossAlt = knowledgeAccess.getPossessiveAlternation(opts.knowledge);
  if (new RegExp(`\\b(?:address is|home is|(?:${pdPossAlt}) address|(?:${pdPossAlt}) home|phone number is)\\b`, 'i').test(reply)) {
