...
function extractTechList(text, afterIndex) {
  const tail = text.slice(afterIndex);
  // Stop at sentence boundaries, strong separators, or prepositions that end a tech list
  const end = tail.search(/[.!?;]|\n|\b(?:but|however|although|while|whereas|with|for|from|by|to|at|using)\b/);
  const phrase = (end === -1 ? tail : tail.slice(0, end)).trim();
  if (!phrase) return [];
  const parts = phrase.split(/\s*,\s*|\s+and\s+|\s+\+\s+/);
  const out = [];
  for (const part of parts) {
    const token = normalizeToken(part).replace(/^(the|a|an|with|using|in)\s+/, '');
    // Reject implausibly long narrative tokens and tokens that embed generic stop words
    if (token.length >= 2 && token.length <= 25 &&
        !/(?:the|and|with|for|from|by|to|at|using|project|experience|building|applications|familiar|used|has)\b/.test(token)) {
      out.push(token);
    }
  }
  return out;
...
function projectTechRelationshipClaims(reply, knowledge) {
  ...
  const relationVerbs = [
    'built with', 'built using', 'built in', 'was built with', 'was built using', 'was built in',
    'uses', 'use', 'used', 'utilizing', 'utilized', 'written in', 'implemented in',
    'is a', 'was a', 'project uses', 'project used', 'stack is', 'tech is'
  ];
    ...
    for (const name of names) {
      ...
      while (true) {
        ...
        for (const verb of relationVerbs) {
          const vidx = window.indexOf(verb);
          if (vidx === -1) continue;
          const after = vidx + verb.length;
          const techs = extractTechList(window, after);
          const project = knowledge.projects.find(pr => pr.name === p.name);
          const projectTechs = ((project)?.tech || []).map(t => normalizeToken(t));
          // Build a blocklist from the project's own name and aliases so description
          // phrases that embed the project name are not treated as technology claims.
...
function validatePluralReferent(testCase, result, opts) {
  ...
  if (/\b(?:them|these|those)\b/i.test(question)) {
    ...
    const hasResolution = learningNorms.some(l => normalizeText(reply).includes(l)) ||
                          boundaryItems.some(b => normalizeText(reply).includes(normalizeText(b)));

    if (!hasResolution && !/\b(?:no public evidence|not documented|unknown|not listed|public evidence does not|does not indicate)\b/i.test(reply)) {
