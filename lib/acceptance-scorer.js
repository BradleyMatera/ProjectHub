...
function extractTechList(segment) {
  const relationPrefix = /^(?:(?:it)\s+)?(?:implemented\s+using|implemented\s+with|developed\s+using|developed\s+with|built\s+using|built\s+with|powered\s+by|written\s+in|utilizing|utilized|using|uses|used|use)\b\s*/i;
  const narrativeStop = /\b(?:for|to|which|that|when|while|because|where|who|whose|its)\b.*$/i;
  const reject = /^(?:and|with|for|the|a|an|as|well|also|it|its|their|his|her|this|that|which|project|app|application|platform|service|demo|site|system|development|frontend|backend|front-end|back-end|frontenddevelopment|backenddevelopment)$/i;
  ...
  return String(segment || '')
    ...
    .map(t => t.replace(/^(?:the|a|an|with|using)\s+/i, '').replace(/\s+(?:code|project|application|app)$/i, '').trim())
    .filter(t => t.length >= 2 && t.length <= 80 && !reject.test(t))
    .map(t => normalizeToken(t))
    .filter(Boolean);
}

function getAllProjectNameTokens(knowledge) {
  const list = [];
  if (!Array.isArray(knowledge?.projects)) return list;
  for (const p of knowledge.projects) {
    list.push({ name: p.name, aliases: p.aliases || [] });
  }
  return list;
}

function projectTechRelationshipClaims(reply, knowledge) {
  const bad = [];
  if (!knowledge?.projects || !reply) return bad;

  // Match the longest relation first, on word boundaries. This prevents
  // "use" from matching inside "used" and leaving junk tokens like "dit".
  const relationVerbs = [
    'implemented using', 'implemented with',
    'developed using', 'developed with',
    'built using', 'built with',
    'powered by', 'written in',
    'utilizing', 'utilized', 'using', 'uses', 'used', 'use'
  ].sort((a, b) => b.length - a.length);

  const projects = getAllProjectNameTokens(knowledge);
  ...
  for (const p of projects) {
    ...
    for (const name of names) {
      ...
      let idx = 0;
      while (true) {
        const found = lowerReply.indexOf(lowerName, idx);
        if (found === -1) break;
        idx = found + 1;
        // window around the project mention
        const start = Math.max(0, found - 140);
        const end = Math.min(lowerReply.length, found + lowerName.length + 140);
        const window = lowerReply.slice(start, end);

        let matched = null;
        for (const verb of relationVerbs) {
          const escapedVerb = verb.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+');
          const verbMatch = new RegExp(`\\b${escapedVerb}\\b`, 'i').exec(window);
          ...
          const project = knowledge.projects.find(pr => pr.name === p.name);
          const projectNorm = normalizeToken(project?.name || p.name);
          const aliases = project?.aliases || p.aliases || [];

          matched = extractTechList(after).filter(candidate => {
            if (!candidate) return false;
            if (projectNorm.includes(candidate) || candidate.includes(projectNorm)) return false;
            if (aliases.some(a => {
              const aliasNorm = normalizeToken(a);
              return aliasNorm && (aliasNorm.includes(candidate) || candidate.includes(aliasNorm));
            })) return false;
            return true;
          });
          break;
        ...
        const project = knowledge.projects.find(pr => pr.name === p.name);
        const projectTechs = ((project)?.tech || []).map(t => normalizeToken(t));
        const projectNameTokens = names
          .flatMap(n => [n, ...((project?.aliases || []))])
          .map(n => normalizeToken(n))
          .filter(Boolean);
        for (const t of matched) {
          if (projectNameTokens.some(pn => pn.length >= 4 && (t.includes(pn) || pn.includes(t)))) continue;
          const isKnown = projectTechs.some(pt => pt === t || (pt.length >= 4 && (pt.includes(t) || t.includes(pt))));
          if (!isKnown) {
            bad.push({ project: p.name, claimedTech: t, projectTechs });
          }
        }
      }
    }
  ...
  // Deduplicate by (project, claimedTech)
  const seen = new Set();
  return bad.filter(b => {
    const key = `${b.project}|${b.claimedTech}`;
