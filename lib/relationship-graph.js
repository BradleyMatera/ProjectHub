...
function buildRelationshipGraph(knowledge) {
  ...
  if (k.skills && typeof k.skills === 'object') {
    const skillGroups = Array.isArray(k.skills) ? [['listed', k.skills]] : Object.entries(k.skills);
    ...
    for (let i = 0; i < k.projects.length; i++) {
      ...
      if (attrs) {
        for (const [attrKey, attrVal] of Object.entries(attrs)) {
          if (attrVal !== null && attrVal !== undefined && String(attrVal).trim()) {
            add(pname, 'has_property', String(attrVal), `projects[${i}].attributes.${attrKey}`, { property: attrKey });
          }
        }
      }
    }
  ...
  if (Array.isArray(k.codePens)) {
    for (let i = 0; i < k.codePens.length; i++) {
      const cp = k.codePens[i];
      const cpName = cp.title || cp.name || '';
      if (!cpName) continue;
      ...
      // codepen aliases
      if (Array.isArray(cp.aliases)) {
        for (const alias of cp.aliases) {
          if (alias && typeof alias === 'string') {
            add(cpName, 'has_alias', alias, `codePens[${i}].aliases`);
          }
        }
      }
      ...
      const cpAttrs = (cp.attributes && typeof cp.attributes === 'object' && !Array.isArray(cp.attributes)) ? cp.attributes
        : (cp.properties && typeof cp.properties === 'object' && !Array.isArray(cp.properties)) ? cp.properties
        : null;
      if (cpAttrs) {
        for (const [attrKey, attrVal] of Object.entries(cpAttrs)) {
          if (attrVal !== null && attrVal !== undefined && String(attrVal).trim()) {
            add(cpName, 'has_property', String(attrVal), `codePens[${i}].attributes.${attrKey}`, { property: attrKey });
          }
        }
      }
    }
