...
function resolveEntity(graph, name) {
  ...
  // from the subject's triples.
  const declared = graph.declaredSubjectAliases || graph.subjectAliases;
  if (declared && declared.has(norm)) return graph.subjectNorm;

  // Exact entity matches and declared entity aliases outrank a bare subject
  // name part: a project literally named "Avery" must resolve to itself even
  // when the tenant subject is "Avery Stone".
