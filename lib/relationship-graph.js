...
function buildRelationshipGraph(knowledge) {
  ...
  if (Array.isArray(k.projects)) {
    for (const entity of entities.filter(item => item.type === 'project')) {
      ...
      // project deployed_at (from URL — URL is explicit deployment evidence)
      if (p.url) {
  ...
  // --- CodePens → deployed_at CodePen, is_type, built_by ---
  // CodePens are a separate collection from projects. They are published on
  // the CodePen platform. This ensures the graph can distinguish between
  // a GitHub Pages project and a CodePen, preventing wrong-platform attribution.
  if (Array.isArray(k.codePens)) {
    for (const entity of entities.filter(item => item.type === 'codepen')) {
      ...
      // codepen deployed_at CodePen (from URL — explicit deployment evidence)
      const cpUrl = cp.url || cp.link || '';
      if (cpUrl && cpUrl.includes('codepen.io')) {
        add(cpName, 'deployed_at', 'CodePen', `codePens[${i}].url`, { url: cpUrl });
      }
...
function assessEntityEvidence(graph, name) {
  ...
  // real word in the entity's display name ("LeetCode" inside "Most LeetCode-
