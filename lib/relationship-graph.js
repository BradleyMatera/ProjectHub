...
function assessEntityEvidence(graph, name) {
  ...
  const result = { status: 'UNKNOWN', mentioned: false, canonical, positive: [], gap: [], adjacent: [] };
  if (!graph || !name) return result;
  const norm = normalizeEntity(name);
  if (!norm) return result;

  // Canonical/alias resolution first, then the looser partial index so both
  // "AWS internship" (alias) and "data structures" (substring of a gap name)
  // reach their real triples.
  const resolved = resolveEntity(graph, name);
  const seen = new Set();
  const triples = [
    ...(graph.entityIndex.get(resolved) || []),
    ...getEntityRelationships(graph, name),
    ...(resolved !== norm ? getEntityRelationships(graph, resolved) : [])
  ].filter(t => {
    const key = `${t.subjectNorm}|${t.relation}|${t.objectNorm}|${t.source}`;
    if (seen.has(key)) return false;
    seen.add(key);
  ...
  for (const t of triples) {
    if (CLAIMING_RELATIONS.has(t.relation)) result.positive.push(t);
    else if (GAP_RELATIONS.has(t.relation)) result.gap.push(t);
    else if (ADJACENT_RELATIONS.has(t.relation)) result.adjacent.push(t);
  }
  result.status = result.positive.length ? 'VERIFIED'
    : result.gap.length ? 'GAP'
    : result.adjacent.length ? 'ADJACENT'
    : 'UNKNOWN';
  return result;
}
