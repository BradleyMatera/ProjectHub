...
const EXPERTISE_RELATIONS = new Set([
  'has_expertise', 'has_extensive_experience', 'specializes_in', 'proficient_in', 'adept_at'
]);
...
function buildRelationshipGraph(knowledge) {
  const k = knowledge || {};
  const triples = [];
  const entities = normalizeKnowledgeEntities(k);
  const subjectName = k.identity?.name || k.agent?.subjectName || 'the subject';
  const subjectNorm = normalizeEntity(subjectName);
  // Subject references: declared subjectAliases plus the subject's own name
  // parts ("bradley", "matera") so a bare first/last name resolves as the
  // tenant subject regardless of which name variant a caller passes in.
  // Declared aliases are strong references; bare name parts are weak — an
  // exact entity or declared entity alias named "Avery" must not be shadowed
  // by a subject named "Avery Stone".
  const declaredSubjectAliases = new Set([
    subjectNorm,
    normalizeEntity(k.identity?.preferredName || ''),
    ...(k.subjectAliases || []).map(a => normalizeEntity(a)),
  ].filter(Boolean));
  // Normalize each whitespace part with normalizeEntity so punctuation joins
  // ("O'Neil" → "oneil") match how lookups normalize the same name.
  const subjectNameParts = new Set(
    String(subjectName).split(/\s+/).map(p => normalizeEntity(p)).filter(w => w.length >= 3)
  );
  const subjectAliases = new Set([...declaredSubjectAliases, ...subjectNameParts]);
  ...
  const addAliases = entity => {
    for (const alias of entity.aliases) add(entity.name, 'has_alias', alias, entity.provenance.aliases);
  };
  const addAttributes = (entity, legacySource = false) => {
    for (const [key, value] of Object.entries(entity.attributes)) {
      if (value !== null && value !== undefined && String(value).trim()) {
        const source = legacySource ? `${entity.sourcePath}.attributes.${key}` : entity.provenance.attributes[key];
        add(entity.name, 'has_property', String(value), source, { property: key });
      }
    }
  };

  // --- Person → has_skill (from skills section) ---
  // Accepts both the grouped map { group: [skills] } and a flat array
  // [skills] — tenants should not lose has_skill edges over a schema detail.
  for (const skill of normalizeKnowledgeSkills(k)) {
    if (!skill.name) continue;
    const source = Array.isArray(k.skills?.[skill.group]) ? `skills.${skill.group}` : skill.sourcePath;
    add(subjectName, 'has_skill', skill.name, source, { group: skill.group });
  }

  // --- Projects → uses_tech, is_type, built_by, deployed_at ---
  if (Array.isArray(k.projects)) {
    for (const entity of entities.filter(item => item.type === 'project')) {
      const i = entity.sourceIndex;
      const p = entity.raw;
      const pname = entity.name;
      if (!pname) continue;
      ...
      // project deployed_at comes from explicit tenant metadata, not URL inference.
      // The tenant may declare platform, deployedAt, or deployment as the semantic host.
      const deployField = ['platform', 'deployedAt', 'deployment'].find(f => p[f]);
      if (deployField) {
        add(pname, 'deployed_at', p[deployField], `projects[${i}].${deployField}`, { url: p.url });
      }

      // NOTE: founder_of, company_behind, and published_on triples are NOT
      // inferred from built_by or URL evidence. These relations require
      // explicit source fields in the knowledge base.
      // Only add triples when the source data explicitly provides them.

      // founder_of: only from explicit projects[i].founder field
      if (p.founder) {
        add(p.founder, 'founder_of', pname, `projects[${i}].founder`);
      }

      // company_behind: only from explicit projects[i].company field
      if (p.company) {
        add(p.company, 'company_behind', pname, `projects[${i}].company`);
      }

      // published_on: only from explicit projects[i].published_on field
      if (p.published_on) {
        add(pname, 'published_on', p.published_on, `projects[${i}].published_on`);
      }

      // comparative_advantage: only from explicit projects[i].comparative_advantage field
      if (p.comparative_advantage) {
        add(pname, 'comparative_advantage', p.comparative_advantage, `projects[${i}].comparative_advantage`);
      }

      // project has_property type (if type field exists)
      if (p.type) add(pname, 'has_property', p.type, `projects[${i}].type`, { property: 'type' });

      // Generic entity attributes → has_property (meta.property = the
      // attribute key). "price", "weight", "warranty" — whatever the tenant
      // declares. This is how comparison dimensions find real support without
      // any domain vocabulary.
      addAttributes(entity, true);
    }
  }

  // --- CodePens → deployed_at (from explicit platform metadata), is_type, built_by ---
  // CodePens are a separate collection from projects. The graph still records
  // the legacy 'CodePen' type, but the deployment platform is taken from the
  // tenant's explicit metadata, not from the URL.
  if (Array.isArray(k.codePens)) {
    for (const entity of entities.filter(item => item.type === 'codepen')) {
      const i = entity.sourceIndex;
      const cp = entity.raw;
      const cpName = entity.name;
      if (!cpName) continue;

      // codepen is_type
      add(cpName, 'is_type', 'CodePen', `codePens[${i}]`);

      // codepen built_by subject
      add(cpName, 'built_by', subjectName, `codePens[${i}]`, { inferred: true });

      // codepen deployed_at comes from explicit tenant metadata only.
      const cpDeployField = ['platform', 'deployedAt', 'deployment'].find(f => cp[f]);
      if (cpDeployField) {
        add(cpName, 'deployed_at', cp[cpDeployField], `codePens[${i}].${cpDeployField}`, { url: cp.url || cp.link });
      }
      ...
      // codepen uses_tech
      if (Array.isArray(cp.tech)) {
        for (const tech of cp.tech) {
          if (tech && typeof tech === 'string') {
            add(cpName, 'uses_tech', tech, `codePens[${i}].tech`);
          }
        }
      }

      // Generic entity attributes → has_property (same as projects)
      addAttributes(entity, true);
    }
  }

  for (const entity of entities.filter(item => item.type === 'product' || item.type === 'service')) {
    const { name, type, raw, sourcePath, provenance, description } = entity;
    add(name, 'is_type', type, sourcePath);
    if (raw.category) add(name, 'is_type', raw.category, `${sourcePath}.category`);
    if (raw.type) add(name, 'has_property', raw.type, `${sourcePath}.type`, { property: 'type' });
    if (description) add(name, 'has_property', description, provenance.description, { property: 'description' });
    // Deployment metadata parity with projects/CodePens — same supported
    // field names, same truthful provenance.
    const entityDeployField = ['platform', 'deployedAt', 'deployment'].find(f => raw[f]);
    if (entityDeployField) {
      add(name, 'deployed_at', raw[entityDeployField], `${sourcePath}.${entityDeployField}`, { url: raw.url || raw.link });
    }
    addAliases(entity);
  ...
  // Tenants may document gaps as prose; the gap name becomes a known entity
  // so model mentions of it are not flagged as fabricated.
  if (Array.isArray(k.summary?.honestGaps)) {
    for (const gap of k.summary.honestGaps) {
      if (typeof gap === 'string' && gap.trim()) {
        // Extract the gap name (before any period or explanation, and strip
        // parenthetical abbreviations so they become aliases of the base name)
        const rawName = gap.split('.')[0].trim();
        const gapName = rawName.replace(/\s*\([^)]*\)\s*/g, ' ').trim();
        if (gapName) {
          add(subjectName, 'has_gap', gapName, 'summary.honestGaps');
          // Extract parenthetical abbreviations and short leading noun phrases
          // as aliases so abbreviated or rephrased gap names resolve to the same
          // gap without requiring tenant-specific vocabulary.
          const parens = rawName.match(/\(([a-z0-9+#.\s-]{2,20})\)/i);
          if (parens) {
            add(gapName, 'has_alias', parens[1].trim(), 'summary.honestGaps');
          }
          const leadingNoun = gapName.match(/^([a-z0-9+#.\s-]{2,40})\s+(?:is|are|was|were|means?|refers?|–|-|:)/i);
          if (leadingNoun && leadingNoun[1].trim().toLowerCase() !== gapName.toLowerCase()) {
            add(gapName, 'has_alias', leadingNoun[1].trim(), 'summary.honestGaps');
          }
        }
      }
    }
  ...
  // --- Identity-derived entities (title, location, pitch) ---
  // These are real attributes of the subject and should not be flagged as
  // fabricated when the model restates them.
  const identity = k.identity || {};
  if (identity.title) {
    add(subjectName, 'is_type', identity.title, 'identity.title');
    for (const word of identity.title.split(/[\s,;:&()\-]+/).filter(p => p.length >= 2)) {
      add(subjectName, 'has_property', word, 'identity.title');
    }
  }
  if (identity.location) {
    add(subjectName, 'has_property', identity.location, 'identity.location');
    for (const word of identity.location.split(/[,\s]+/).filter(p => p.length >= 2)) {
      add(subjectName, 'has_property', word, 'identity.location');
    }
  }
  if (identity.shortPitch) {
    const pitchWords = identity.shortPitch.match(/\b[A-Z][A-Za-z0-9+#.-]{1,}\b/g) || [];
    for (const word of pitchWords) {
      if (word.length >= 2) add(subjectName, 'has_property', word, 'identity.shortPitch');
    }
  }

  // --- Subject-level property sections (goals, availability, etc.) ---
  // Free-text tenant declarations about the subject become has_property
  // triples so claims like "is open to relocation" can be grounded. Both the
  // full value and its content words are indexed — the same treatment
  // identity.title and identity.location already get.
  // Portable shapes: a section may be an object of string values, a single
  // string, or an array of strings. Each entry is kept as its own coherent
  // source unit (with its own polarity) rather than flattened together.
  for (const [sectionName, section] of [['goals', k.goals], ['availability', k.availability], ['preferences', k.preferences]]) {
    if (section == null) continue;
    const entries = typeof section === 'string'
      ? [[sectionName, section]]
    ...
    for (const [key, value] of entries) {
      ...
      const meta = { property: key, value: value.trim() };
      add(subjectName, 'has_property', value.trim(), `${sectionName}.${key}`, meta);
      for (const word of value.trim().split(/[^A-Za-z0-9+#.]+/).filter(w => w.length >= 4)) {
        add(subjectName, 'has_property', word, `${sectionName}.${key}`, meta);
      }
    }
  }

  for (const [record, path] of [[identity, 'identity'], ...(k.experience || []).map((entry, i) => [entry, `experience[${i}]`])]) {
    for (const relation of EXPERTISE_RELATIONS) {
      const values = Array.isArray(record?.[relation]) ? record[relation] : [record?.[relation]];
      values.forEach((value, i) => {
        if (typeof value === 'string' && value.trim()) {
          add(subjectName, relation, value.trim(), `${path}.${relation}${Array.isArray(record[relation]) ? `[${i}]` : ''}`);
        }
      });
    }
  }

  // --- Portable authored relationships ---
  // Tenants may supply explicit triples outside the inferred project/skill
  // schema (e.g. Atlas uses_tech React.js, Atlas uses_platform Vercel).
  if (Array.isArray(k.relationships)) {
    for (const [i, rel] of k.relationships.entries()) {
      if (!rel || !rel.subject || !rel.object || !rel.relation) continue;
      add(rel.subject, rel.relation, rel.object, `relationships[${i}]`, rel.meta || {});
    }
  }
...
function resolveEntity(graph, name) {
  ...
  // Exact entity matches and declared entity aliases take precedence over
  // subject references — a project literally named "Avery" must resolve to
  // itself even when the tenant subject is "Avery Stone" and "Avery" is a
  // declared preferred-name alias.
  if (graph.aliasToCanonical && graph.aliasToCanonical.has(norm)) {
    return normalizeEntity(graph.aliasToCanonical.get(norm));
  }

  // Check direct entity index — only entries backed by real triples; bare
  // name-part aliases are indexed with empty lists and must not count.
  if (graph.entityIndex.has(norm) && graph.entityIndex.get(norm).length > 0) return norm;

  // A declared subject alias resolves to the tenant subject itself — never
  // to a bare alias key, which would detach downstream subject-Norm filters
  // from the subject's triples. Applies only when no real entity claimed
  // the name first.
  const declared = graph.declaredSubjectAliases || graph.subjectAliases;
  if (declared && declared.has(norm)) return graph.subjectNorm;

  // A bare first/last name part refers to the tenant subject — but only when
  // no real entity claimed the name first.
  if (graph.subjectNameParts && graph.subjectNameParts.has(norm)) return graph.subjectNorm;
  if (!graph.subjectNameParts && graph.subjectAliases && graph.subjectAliases.has(norm)) return graph.subjectNorm;
  ...
  for (const key of graph.entityIndex.keys()) {
    ...
    // Exact substring match — require a minimum length ratio so a short
    // incidental token cannot resolve into an unrelated entity ("son" ⊂
    // "masoncountywa" is not an entity reference).
    ...
    // Words need length >= 4: a 3-char token like "son" inside a longer
    // concatenated key ("masoncountywa") is a substring accident, not a word.
    const normWords = norm.split(/(?=[A-Z])/).filter(w => w.length >= 4);
    const keyWords = key.split(/(?=[A-Z])/).filter(w => w.length >= 4);
...
/**
 * Canonical identity normalization for cross-reference comparisons.
 *
 * THE single identity-precedence rule for the whole pipeline:
 *   A. exact canonical entity / declared entity alias (aliasToCanonical,
 *      entityIndex backed by real triples)
 *   B. canonical full tenant subject / declared strong tenant alias
 *   C. weak tenant first/last-name part — only when no real entity claimed it
 *   D. the safe normalized value itself
 *
 * Unlike resolveEntity this performs NO description or fuzzy matching —
 * callers comparing stored names need deterministic exact identity so that
 * unrelated names can never be merged. The extractor's 'subject' sentinel
 * maps to subjectNorm when a graph subject exists.
 */
function canonicalEntityNorm(graph, value) {
  const norm = normalizeEntity(value);
  if (!norm) return norm;
  if (!graph) return norm;
  if (norm === 'subject' && graph.subjectNorm) return graph.subjectNorm;
  if (graph.aliasToCanonical && graph.aliasToCanonical.has(norm)) {
    return normalizeEntity(graph.aliasToCanonical.get(norm));
  }
  if (graph.entityIndex && graph.entityIndex.has(norm) && graph.entityIndex.get(norm).length > 0) {
    return norm;
  }
  const declared = graph.declaredSubjectAliases || graph.subjectAliases;
  if (declared && declared.has(norm)) return graph.subjectNorm;
...
function checkRelationship(graph, subject, relation, object) {
  ...
  const RELATION_SYNONYMS = {
    ...
    attended: ['has_degree'],
    // "has experience with X" is entailed by stronger documented experience
    // relations (one-directional — a bare has_experience triple does NOT
    // satisfy an expertise claim).
    has_experience: ['has_extensive_experience', 'has_expertise', 'specializes_in', 'has_skill'],
  };
 ...
 */
function entityNormMatches(graph, norm, candidateNorm) {
  if (norm === candidateNorm) return true;
  if (graph.aliasToCanonical?.has(norm) &&
      normalizeEntity(graph.aliasToCanonical.get(norm)) === candidateNorm) {
    return true;
  }
  if (!norm || !candidateNorm) return false;
  const shorterLen = Math.min(norm.length, candidateNorm.length);
  const longerStr = norm.length >= candidateNorm.length ? norm : candidateNorm;
  const shorterStr = norm.length < candidateNorm.length ? norm : candidateNorm;
  if (shorterLen === 0) return false;
  if (longerStr.startsWith(shorterStr) && shorterLen / longerStr.length >= 0.6) return true;
  if ((candidateNorm.includes(norm) || norm.includes(candidateNorm)) && shorterLen / longerStr.length >= 0.8) return true;
  return false;
}
...
function getEntityTypes(entityName, graph) {
  ...
  return graph.triples
    .filter(t => t.relation === 'is_type' && entityNormMatches(graph, norm, t.subjectNorm))
    .map(t => t.object)
    .filter(Boolean);
}
...
function getEntityDescription(entityName, graph) {
  ...
  const entities = graph.entities || normalizeKnowledgeEntities(k);
  const exact = entities.find(entity => normalizeEntity(entity.name) === norm
    || entity.aliases.some(alias => normalizeEntity(alias) === norm));
  if (exact) return exact.description;
  // Check projects
  for (const entity of entities) {
    const pNorm = normalizeEntity(entity.name);
...
// Discourse membership and knowledge status are independent axes: a target the
// user introduced in conversation can be VERIFIED, GAP, ADJACENT, or UNKNOWN
// in the tenant knowledge base — introduction itself proves nothing either way.
//
// Relation semantics (domain-neutral classes, not entity names):
//   CLAIMING   — a real-world fact is asserted involving the target
//                (has_skill, worked_at, interned_at, employed_as, has_cert,
//                 has_degree, attended, built_by, founder_of, company_behind,
//                 published_on, comparative_advantage)
//   GAP        — has_gap: documented gap/learning area; evidence OF a gap,
//                never positive expertise
//   ADJACENT   — uses_tech / uses_platform: verified usage inside owned work;
//                supports an assessment, not a direct-experience claim
//   structural — is_type, has_alias, has_property, deployed_at, built_during:
//                establish mention/existence only, no support
const CLAIMING_RELATIONS = new Set([
  'has_skill', 'worked_at', 'interned_at', 'employed_as', 'has_cert',
  'has_degree', 'attended', 'built_by', 'founder_of', 'company_behind',
  'published_on', 'comparative_advantage'
]);
const GAP_RELATIONS = new Set(['has_gap']);
const ADJACENT_RELATIONS = new Set(['uses_tech', 'uses_platform']);

/**
 * Assess the knowledge status of a single target against the relationship
 * graph. Canonical resolution (aliases, punctuation-insensitive normalization)
 * happens through resolveEntity so "front-end"/"frontend"/alias forms do not
 * produce different statuses.
 *
 * @param {Object} graph from buildRelationshipGraph()
 * @param {string} name target as written (discourse alternative, entity, ...)
 * @returns {{status:'VERIFIED'|'GAP'|'ADJACENT'|'UNKNOWN', mentioned:boolean,
 *            canonical:string, positive:Array, gap:Array, adjacent:Array}}
 */
function assessEntityEvidence(graph, name) {
  const canonical = String(name || '');
  const result = {
    status: 'UNKNOWN', mentioned: false, canonical,
    positive: [], gap: [], adjacent: [],
    resolution: { method: 'none', confidence: 'none' }
  };
  if (!graph || !name) return result;
  const norm = normalizeEntity(name);
  if (!norm) return result;

  // Resolution confidence: exact normalized hits and canonical aliases are
  // high-confidence identity; a leading-substring canonical form ("junior
  // frontend developer" → ".../ Freelance Contributor") is still identity-
  // preserving; anything fuzzier (mid-string containment, description or
  // word-overlap resolution) is low confidence and must not promote a claim
  // to VERIFIED on its own.
  const resolved = resolveEntity(graph, name);
  const aliasResolved = graph.aliasToCanonical && graph.aliasToCanonical.has(norm);
  const resolvedConf = resolved === norm ? 'high'
    : aliasResolved ? 'high'
    : ((resolved.startsWith(norm) || norm.startsWith(resolved)) && norm.length >= 6) ? 'high'
    : 'low';
  result.resolution = {
    method: resolved === norm ? 'exact' : aliasResolved ? 'alias' : (resolvedConf === 'high' ? 'canonical-prefix' : 'fuzzy'),
    confidence: resolved !== norm || graph.entityIndex.has(norm) ? resolvedConf : 'none'
  };

  // Collect triples with their own confidence: triples indexed under the
  // resolved canonical inherit the resolution confidence; loose partial-index
  // hits are always low-confidence context.
  const seen = new Set();
  const triples = [];
  const push = (t, conf) => {
    const key = `${t.subjectNorm}|${t.relation}|${t.objectNorm}|${t.source}|${conf}`;
    if (seen.has(key)) return;
    seen.add(key);
    triples.push({ ...t, _conf: conf });
  };
  for (const t of (graph.entityIndex.get(resolved) || [])) push(t, result.resolution.confidence === 'none' ? 'low' : resolvedConf);
  if (resolved !== norm) {
    for (const t of (graph.entityIndex.get(norm) || [])) push(t, 'high');
  }
  // Partial-index containment is only identity-confidence when the query is a
  // real word in the entity's display name ("ZebraLang" inside "Most ZebraLang-
  // style problems") or a leading substring; an embedded fragment ("kubernet"
  // inside "kubernetesdeployment") is weak context in EITHER direction.
  const wordRe = norm.length >= 4 ? new RegExp(`(^|[^a-z0-9])${norm}([^a-z0-9]|$)`, 'i') : null;
  for (const [key, tList] of graph.entityIndex.entries()) {
    if (key === norm) continue;
    if (norm.length < 4 || !(key.includes(norm) || norm.includes(key))) continue;
    for (const t of tList) {
      const display = `${t.subject || ''} ${t.object || ''}`.toLowerCase();
      const conf = key.startsWith(norm) || norm.startsWith(key) || (wordRe && wordRe.test(display)) ? 'high' : 'low';
      push(t, conf);
    }
  }

  result.mentioned = triples.length > 0;
  result.canonical = resolved;
  for (const e of triples) {
    if (CLAIMING_RELATIONS.has(e.relation)) result.positive.push(e);
    else if (GAP_RELATIONS.has(e.relation)) result.gap.push(e);
    else if (ADJACENT_RELATIONS.has(e.relation)) result.adjacent.push(e);
  }
  const confidentPositive = result.positive.filter(t => t._conf === 'high');
  const confidentGap = result.gap.filter(t => t._conf === 'high');
  // A low-confidence fuzzy match cannot create a strong factual status in
  // either direction: no VERIFIED from a fuzzy claiming edge, and no
  // definitive GAP from a fuzzy gap collision — weak evidence stays ADJACENT.
  result.status = confidentPositive.length ? 'VERIFIED'
    : confidentGap.length ? 'GAP'
    : (result.positive.length || result.gap.length || result.adjacent.length) ? 'ADJACENT'
    : 'UNKNOWN';
  return result;
}
