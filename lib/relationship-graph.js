...
function resolveEntity(graph, name) {
  ...
  // project's description, treat it as a reference to that project.
  // e.g., "webGPU learning demo" matches the Triangle Shader Lab description
  ...
  if (graph.knowledge && Array.isArray(graph.knowledge.projects)) {
    ...
    const nameNorm = norm;
    for (const proj of graph.knowledge.projects) {
      ...
      if (nameNorm.length >= 10 && descNorm.includes(nameNorm)) {
        return normalizeEntity(proj.name);
      }
      // Also check word-level overlap: if all significant words from the
      // name appear in the description, it's likely a reference to that project
