...
function buildRelationshipGraph(knowledge) {
  ...
  if (Array.isArray(k.projects)) {
    for (const entity of entities.filter(item => item.type === 'project')) {
      ...
      const deploymentPlatform = p.platform || p.deployedAt || p.deployment;
      if (deploymentPlatform) {
        add(pname, 'deployed_at', deploymentPlatform, `projects[${i}].platform`, { url: p.url });
      }
  ...
  if (Array.isArray(k.codePens)) {
    for (const entity of entities.filter(item => item.type === 'codepen')) {
      ...
      const cpPlatform = cp.platform || cp.deployedAt || cp.deployment;
      if (cpPlatform) {
        add(cpName, 'deployed_at', cpPlatform, `codePens[${i}].platform`, { url: cp.url || cp.link });
      }
  ...
  for (const [sectionName, section] of [['goals', k.goals], ['availability', k.availability], ['preferences', k.preferences]]) {
    ...
    for (const [key, value] of Object.entries(section)) {
      ...
      add(subjectName, 'has_property', value.trim(), `${sectionName}.${key}`, { property: key });
      for (const word of value.trim().split(/[^A-Za-z0-9+#.]+/).filter(w => w.length >= 4)) {
        add(subjectName, 'has_property', word, `${sectionName}.${key}`, { property: key });
      }
    }
  }
