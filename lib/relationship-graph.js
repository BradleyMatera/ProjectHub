...
function assessEntityEvidence(graph, name) {
  ...
  for (const [key, tList] of graph.entityIndex.entries()) {
    if (key !== norm && key !== resolved && norm.length >= 4 && (key.includes(norm) || norm.includes(key))) {
      for (const t of tList) push(t, 'low');
    }
  }
  ...
  // A low-confidence fuzzy match onto a claiming edge is adjacent evidence,
  // not verification — "data structures" must not verify via a similarly
  // named certification.
  result.status = confidentPositive.length ? 'VERIFIED'
    : result.gap.length ? 'GAP'
    : (result.positive.length || result.adjacent.length) ? 'ADJACENT'
    : 'UNKNOWN';
  return result;
}
