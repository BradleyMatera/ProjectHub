...
function extractEntitiesFromText(text, knowledge, options = {}) {
  ...
  while ((em = employerContextPattern.exec(text)) !== null) {
    ...
    const termLower = term.toLowerCase();
    if (knownNames.has(termLower)) continue;
    if (/\b(?:he|she|it|they|we|I|you|me)\b/i.test(term)) continue;
    if (!found.some(e => e.name.toLowerCase() === termLower)) {
...
function extractRoleTargets(text) {
  if (!text) return [];
  const patterns = [
  ...
  for (const pattern of patterns) {
    for (const match of text.matchAll(pattern)) {
      ...
      for (const part of phrase.split(/\s*(?:,|\band\b|\bor\b|\bversus\b)\s*/i)) {
        ...
        if (/\b(?:i|me|my|we|our|you|your|he|him|his|she|her|they|their|it|its|this|that|these|those|is|are|was|were|has|have|can|could|would|should|not|fit|role|job|position|roles|jobs|positions)\b/i.test(name)) continue;
...
const NON_ALTERNATIVE_START = /^(?:how|what|who|whose|whom|where|when|why|which|is|are|was|were|do|does|did|can|could|would|should|will|shall|may|might|am|it|its|he|she|they|we|i|you|yes|no|ok|okay|sure|thanks|thank|please|tell|show|give|let)\b/i;
...
function extractContinuation(text) {
  ...
  if (!name) return null;
  if (REFERENTIAL_NAME.test(name) || NON_ALTERNATIVE_START.test(name) || FACET_NAME.test(name)) {
...
function extractCorrection(text) {
  ...
  if (!name || REFERENTIAL_NAME.test(name) || NON_ALTERNATIVE_START.test(name)) return null;
  return { name, notName };
}
...
function buildConversationState(history, knowledge, semanticState) {
  ...
  for (const turn of history || []) {
    ...
    for (const target of extractRoleTargets(text)) {
