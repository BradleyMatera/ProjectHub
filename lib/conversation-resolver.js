...
function selectPluralDiscourseObject(question, objects) {
  ...
  const byType = {
    ...
    fit: ['role-set', 'strength', 'skill-set'],
    role: ['role-set'],
    roles: ['role-set'],
    strongest: ['role-set', 'strength', 'skill-set'],
    relate: ['experience-set', 'role-set', 'project-set'],
  };
  let preferred = null;
  for (const [verb, types] of Object.entries(byType)) {
    if (lower.includes(verb)) { preferred = types; break; }
  }
  const explicitRoles = objects.find(o => o.type === 'role-set' && o.source === 'user' && o.claimContext);
  if (preferred?.[0] === 'role-set') return explicitRoles || null;
...
function resolveReferent(question, convState, knowledge) {
  ...
  if (rewritten.split(/\s+/).length <= 12) {
    ...
    if (/\bwhich\s+of\s+(?:them|these|those)\b/i.test(rewritten) && comparisonEntities?.length >= 2) {
  ...
  if ((/\b(it|that|there|this|the other one|the other project)\b/i.test(q) && !active && !subjectFirst) ||
      /\bwhich\s+of\s+(?:them|these|those)\b/i.test(q)) {
