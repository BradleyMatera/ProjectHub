...
function extractEntitiesFromText(text, knowledge, options = {}) {
  ...
  function addByNameOrDistinctiveWord(name, type) {
    ...
    const genericSuffixes = new Set(['inc', 'llc', 'ltd', 'group', 'corp', 'corporation', 'company', 'co', 'and', 'the', 'of']);
    const parts = lowerName.split(/\s+/).filter(w => w.length >= 3 && !genericSuffixes.has(w));
    for (const part of parts) {
      const p = lower.indexOf(part);
      if (p >= 0) {
        found.push({ name, type, position: p });
        return;
      }
    }
  }
...
function buildConversationState(history, knowledge) {
  ...
  if (projectCompanyMentions.length > 0) {
    // Sort by recency, then source (user mentions take priority over assistant),
    // then position, so an assistant's later skill/capability digression does not
    // steal the active referent from a user-explicit project.
    const sourceWeight = e => e.source === 'user' ? 2 : 1;
    const sorted = [...projectCompanyMentions].sort((a, b) =>
      b.turnIndex - a.turnIndex || sourceWeight(b) - sourceWeight(a) || (b.position || 0) - (a.position || 0)
    );
...
function resolveReferent(question, convState, knowledge) {
  ...
  if (!q) return { resolved: false, entity: null, rewrittenQuery: q, referentType: null };
  ...
  if (resolved) {
    return { resolved: true, entity, rewrittenQuery: rewritten, referentType };
  }
  ...
  if (/\b(it|that|there|this|the other one|the other project)\b/i.test(q) && !active && !subjectFirst) {
    return { resolved: false, entity: null, rewrittenQuery: q, referentType: 'unresolved' };
  }
