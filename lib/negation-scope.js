...
const NEGATION_WORDS = /\b(?:not|never|neither|nor|without|doesn't|doesn?t|don't|don?t|isn't|isn?t|wasn't|wasn?t|won't|won?t|can't|can?t|cannot|couldn't|couldn?t|shouldn't|shouldn?t|wouldn't|wouldn?t|hasn't|hasn?t|haven't|haven?t|hadn't|hadn?t|aren't|aren?t|weren't|weren?t|did not|does not|do not|is not|was not|no evidence|no proof|no indication|no verified record|no documented record|no known record|no verified evidence|no documented evidence|no such|not a (?:known|verified|documented)|not a single|never any)\b/i;
