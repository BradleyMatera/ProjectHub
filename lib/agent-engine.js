...
const { executeAgentTool, TOOL_DEFINITIONS } = require('./agent-tools');
const { buildReasoningPacket, buildSynthesisPacket } = require('./context-packet');
...
const STEP_TIMEOUT_MS = Math.max(2000, Math.min(parseInt(process.env.SCOUT_STEP_TIMEOUT_MS || '6000', 10), 12000));
const TOTAL_BUDGET_MS = Math.max(4000, Math.min(parseInt(process.env.SCOUT_TOTAL_BUDGET_MS || '15000', 10), 25000));
...
async function runAgentLoop({ question, conversationState, evidence, knowledge, sessionId, model, rules }) {
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    emit('reasoning_decision', { step, decision: reasonResult.decision, latencyMs: reasonResult.latencyMs });
    ...
    if (reasonResult.decision.action === 'answer') {
      ...
      const validation = validateAnswer(reasonResult.decision.answer, sourceText, question);
      emit('validation', { verdict: validation.verdict, reasons: validation.reasons });
      if (validation.valid) {
        emit('agent_complete', { step, totalLatencyMs: Date.now() - startedAt, ollamaLatencyMs: totalOllamaLatencyMs });
        return {
      ...
      emit('direct_answer_rejected', { reasons: validation.reasons });
    ...
    if (seenToolCalls.has(callKey)) {
      ...
      if (toolResults.length > 0) {
        ...
        if (synthResult.ok) {
          const sourceText = buildSourceText(evidence, toolResults);
          const validation = validateAnswer(synthResult.answer, sourceText, question);
          emit('validation', { verdict: validation.verdict, reasons: validation.reasons });
          if (validation.valid) {
            emit('agent_complete', { step, totalLatencyMs: Date.now() - startedAt, ollamaLatencyMs: totalOllamaLatencyMs });
            return {
    ...
    emit('synthesis_rejected', { step, reasons: validation.reasons });
...
async function probeAgent(model) {
  ...
  ], { timeoutMs: 5000, numPredict: 16, format: 'json', temperature: 0 });
