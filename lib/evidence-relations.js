...
function technologyRelationObjects(subjectNames, text, options = {}) {
  ...
  for (const rawClause of clauses) {
    ...
    if (!cues.length) {
      ...
      if (subjM && !tailIsEntityExtension(subjM[1])) subjectSeen = true;
      continue;
    }
    for (const cue of cues) {
      ...
      const implicit = (scoped && implicitSubjectRe.test(prefix)) ||
        (subjectSeen && !prefix.trim());
      if (!subjectMatch && !implicit) continue;
      if (subjectMatch && tailIsEntityExtension(subjectMatch[1])) continue;
      if (subjectMatch) subjectSeen = true;
