...
function technologyRelationObjects(subjectNames, text, options = {}) {
  ...
  // Longest names first: a declared multi-word name ("Atlas Dashboard") must
  // match before its first-word prefix ("Atlas") can.
  ...
  // The catch-all tail may hold an appositive or modifier ("Atlas, a CLI
  // tool,") but not a bare word sequence: "Avery Chen uses React" must not
  // resolve the subject "Avery" — "Chen" extends the name into a different
  // entity rather than modifying it. Input may be lowercased by callers, so
  // case cannot distinguish an entity extension; a tail with no punctuation
  // and no function word is treated as one.
  const TAIL_FUNCTION_WORDS = new Set(['a', 'an', 'the', 'of', 'for', 'to', 'in', 'on',
    'at', 'and', 'or', 'by', 'with', 'that', 'which', 'who', 'from', 'as', 'its',
    'my', 'his', 'her', 'their', 'our', 'your', 'this', 'these', 'those', 'new',
    'latest', 'current', 'own', 'built', 'made', ...(options.subjectRefTokens || [])]);
  const tailIsEntityExtension = tail => {
    const t = String(tail || '').trim();
    if (!t) return false;
    if (/[,;:'"()–—-]/.test(t)) return false;
    return !t.split(/\s+/).some(w => TAIL_FUNCTION_WORDS.has(w.toLowerCase()));
  };
  const implicitSubjectRe = /^(?:(?:it|(?:the|this)\s+(?:project|application|app|workflow))\s+)?(?:(?:is|was|has|been|currently|also|now)\s+)*$/i;
  // Rendered metadata labels — "Tech: React, Node" — are field syntax, not
  // clause boundaries. The label's value list belongs to the subject already
  // established in the same block ("Project Atlas: ... Tech: React").
  const LABEL_FIELD_RE = /^\s*(?:tech(?:nolog(?:y|ies))?(?:\s*stack)?|stack|skills?(?:\s*set)?|tools|frameworks|languages|key\s*technologies|built\s*(?:with|using))\s*:\s*(.+)$/i;
  // Same labels, non-anchored — "Atlas: Maps Tech: React" carries the label
  // field inside the subject's tail rather than at a clause start.
  const LABEL_FIELD_INLINE_RE = new RegExp(LABEL_FIELD_RE.source.replace(/^\^\\s\*/, ''), 'i');
  // A colon does NOT split clauses: "Atlas: uses React" is one proposition
  // whose subjectRe already tolerates the label colon. Splitting there
  // stranded the subject from its relation cue.
  ...
  // Track WHICH subject is current — a label field ("Tech: Vue") attaches to
  // the subject of the clause that precedes it, so a new entity clause clears
  // the context instead of letting its label ground an earlier subject.
  const FUNCTION_HEADS = new Set(['it', 'the', 'this', 'that', 'there', 'a', 'an',
    'he', 'she', 'they', 'what', 'which', 'who', 'its', 'no', 'not', 'all',
    'each', 'every', 'some', 'none', 'his', 'her', 'their',
    ...(options.subjectRefTokens || [])]);
  const otherEntityHead = text2 => {
    const head = String(text2 || '').trim()
      .match(/^([a-z][\w+#.-]*(?:\s+[a-z][\w+#.-]*){0,3})\s+(?:is|are|was|were|has|have|had|uses?|used|runs?|built)\b/i);
    if (!head) return false;
    const headWord = head[1].trim().split(/\s+/)[0].toLowerCase();
    if (FUNCTION_HEADS.has(headWord)) return false;
    return !names.some(n =>
      phraseAppears(head[1].trim(), n, normalizeToken) ||
      phraseAppears(n, head[1].trim(), normalizeToken));
  };
  let currentSubject = null;
  for (const rawClause of clauses) {
    const clause = rawClause.trim().replace(/^(?:yes|no|correct),\s*/i, '').replace(/^[*\-\s]+/, '');
    const labelField = clause.match(LABEL_FIELD_RE);
    if (labelField) {
      if (currentSubject) {
        for (const item of labelField[1].split(/,|\s+and\s+|\s*\/\s*/)) {
          const obj = item.trim();
          if (obj && !NON_ACTUAL_TECHNOLOGY.test(obj)) objects.push(obj);
        }
      }
      continue;
    }
    ...
    if (!cues.length) {
      // A clause that is only a subject name ("Atlas:" / "Project Atlas")
      // establishes the subject for following label fields and cue-only
      // clauses in the same block.
      const subjM = clause.match(subjectRe) ||
        (clause.includes(':') ? clause.slice(clause.lastIndexOf(':') + 1).trim().match(subjectRe) : null);
      if (subjM) {
        const subjTail = subjM[1] || '';
        // A tail after a subject-label colon ("Project Atlas: Maps") is
        // label content, not a name extension; the unpunctuated guard still
        // covers "Avery Chen".
        const tailLabel = subjTail.match(LABEL_FIELD_INLINE_RE);
        if (subjM[0].includes(':') || tailLabel || !tailIsEntityExtension(subjTail)) {
          currentSubject = subjM[0];
          // A label field embedded in the same clause ("Atlas: Maps Tech:
          // React") belongs to this subject too.
          if (tailLabel) {
            for (const item of tailLabel[1].split(/,|\s+and\s+|\s*\/\s*/)) {
              const obj = item.trim();
              if (obj && !NON_ACTUAL_TECHNOLOGY.test(obj)) objects.push(obj);
            }
          }
      ...
      } else if (otherEntityHead(clause)) {
        currentSubject = null;
      }
      continue;
    }
    for (const cue of cues) {
      ...
      // A leading non-subject label ("Note:", "Summary:") does not split the
      // clause anymore, so retry the subject match after a label colon —
      // "Note: Atlas uses React" still resolves Atlas as the subject.
      const subjectMatch = prefix.match(subjectRe) ||
        (prefix.includes(':') ? prefix.slice(prefix.lastIndexOf(':') + 1).trim().match(subjectRe) : null);
      const implicit = (scoped && implicitSubjectRe.test(prefix)) ||
        (currentSubject && !prefix.trim());
      if (subjectMatch && tailIsEntityExtension(subjectMatch[1])) {
        currentSubject = null;
        continue;
      }
      if (!subjectMatch && !implicit) {
        if (otherEntityHead(prefix)) currentSubject = null;
        continue;
      }
      if (subjectMatch) currentSubject = subjectMatch[0];
...
function isAuthoritativeBlock(source) {
  ...
  return ['project', 'direct-answer', 'faq'].includes(tag);
}
...
function evidenceSupportsTechnologyRelation(subjectNames, technology, evidenceInput, options = {}) {
  ...
  for (const block of blocks) {
    ...
    const objects = technologyRelationObjects(subjectNames, blockText, { ...block, normalizeToken, subjectRefTokens: options.subjectRefTokens });
