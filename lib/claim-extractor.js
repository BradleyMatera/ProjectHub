...
const RELATION_PATTERNS = [
  ...
  { re: /\b(?:he|she|they|bradley|brad|scout)\s+(?:built|developed|created|made|wrote|designed)\s+(?:the\s+|a\s+|an\s+)?([A-Z][A-Za-z0-9+#.\s-]{2,40})/i, relation: 'built_by', type: 'FACT' },
  ...
  { re: /\b(?:he|she|they|bradley|brad)\s+(?:has|holds?)\s+(AWS\s+(?:SAA|SAP|DVA|SOA|DBS|ANS|MLS|SCS|CLF))\b/i, relation: 'has_cert', type: 'FACT' },
    ...
    for (const pattern of RELATION_PATTERNS) {
      ...
      if (subject || object) {
        ...
        if ((pattern.relation === 'has_experience' || pattern.relation === 'worked_at' ||
            ...
            pattern.relation === 'has_extensive_experience' || pattern.relation === 'specializes_in' ||
            pattern.relation === 'adept_at') && !pattern.overclaim) {
