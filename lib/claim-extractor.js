...
function ensureSubjectAltFromGraph(graph) {
  ...
  // Configured subject pronouns (e.g. "ze/zir" or { subject: 'ze' }) are
  // subject references too — "ze is open to relocation" must extract like
  // "he is open to relocation" or the claim bypasses grounding entirely.
  const pr = graph.knowledge?.identity?.pronouns;
  const pronounSubject = typeof pr === 'string' ? pr.split('/')[0] : pr?.subject;
  for (const p of String(pronounSubject || '').toLowerCase().split(/[^a-z]+/)) {
    if (p.length >= 2 && !/^(?:he|she|they)$/.test(p)) subjectSet.add(p);
  }
  ...
  for (let sIdx = 0; sIdx < sentences.length; sIdx++) {
    ...
    for (const pattern of RELATION_PATTERNS) {
      ...
      if (pattern.relation === 'uses_tech') {
        if (match[1] && match[2]) {
          ...
          if (!cleanSubj || startsLowercase || /^(?:is|was|are|were|has|have|includes?|that|which|where\s+he|where\s+she|where\s+they|he|she|they)\b/i.test(cleanSubj)) {
            ...
            if (!cleanSubj || (startsLowercase && cleanSubj === cleanEntityName(match[1]))) {
              ...
              if (/^(?:he|she|they|his|her|their|the subject|the candidate)\b/i.test(String(match[1] || '').trim()) ||
                  /^\s*(?:he|she|they)\b/i.test(matchClause.slice(0, match.index))) {
                cleanSubj = 'subject';
              } else {
        ...
        if (/^(?:he|she|they)$/i.test(s)) {
          subject = 'subject';
        } else if (/^[A-Z]/.test(s) && !/^(?:The|A|An|This|That|His|Her|Their|Its)$/i.test(s)) {
          subject = s;
        } else {
          continue;
        }
        ...
        if (subject && /^(?:he|she|they)$/i.test(subject)) subject = 'subject';
