...
// Derive subjectAlt from graph when module-level config hasn't been set
function ensureSubjectAltFromGraph(graph) {
  if (subjectAlt || !graph || !graph.triples) return;
  ...
  if (subjectSet.size > 0) {
    const escaped = [...subjectSet].map(n => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    subjectAlt = '|' + escaped.join('|');
...
function buildRelationPatterns() {
  ...
  { re: /\b(?:he|she|they)\s+(?:is|are|was|were)\s+(?:a|an)\s+([a-z][a-z0-9+#.'\s-]{1,40}?)(?=\s*(?:[.;!?,]|\s+(?:and|but|or|who|which|that|because|since|while|when|if|as|so)\b|$))/i, relation: 'is_type', type: 'FACT' },
