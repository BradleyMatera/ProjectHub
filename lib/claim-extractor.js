...
const RELATION_PATTERNS = [
  ...
  { re: /\b([A-Z][A-Za-z0-9+#.\s-]{2,40}?)\s+(?:uses?|used|using|built with|built using|developed with|implemented with|leveraged|includes?|incorporates?)\s+([A-Z][A-Za-z0-9+#.\s-]{1,30})/i, relation: 'uses_tech', type: 'FACT' },
  { re: /\b(?:his|her|their)\s+([A-Z][A-Za-z0-9+#.\s-]{2,40}?)\s+(?:uses?|used|using|built with|built using|developed with|includes?)\s+([A-Z][A-Za-z0-9+#.\s-]{1,30})/i, relation: 'uses_tech', type: 'FACT' },
  ...
  { re: /\b(?:he|she|they|bradley|brad)\s+(?:has|holds?|earned|received)\s+(?:a\s+)?([A-Z][A-Za-z0-9+#.\s-]{5,50}\s+cert\w*)/i, relation: 'has_cert', type: 'FACT' },
...
function extractClaims(answer, graph, question = '') {
  ...
  for (const sentence of sentences) {
    ...
    for (const pattern of RELATION_PATTERNS) {
      ...
      if (subject || object) {
        claims.push({
          subject: subject ? cleanEntityName(subject) : subject,
          relation: pattern.relation,
          object: object ? cleanEntityName(object) : object,
