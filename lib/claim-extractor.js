...
const RELATION_PATTERNS = [
  ...
  { re: /\b(?:he|she|they|bradley|brad|scout)\s+(?:had|has)\s+(?:a|an)?\s*(?:[A-Za-z0-9\s-]{1,35}\s+)?(?:internship|intern|trainee|employee|engineer|developer)\s+(?:at|for|with)\s+([A-Z][A-Za-z0-9+#.&-]+(?:\s+[A-Z][A-Za-z0-9+#.&-]+)*)/i, relation: 'interned_at', type: 'FACT' },
  { re: /\b(?:he|she|they|bradley|brad|scout)\s+(?:worked|was|employed|spent time)\s+(?:at|for)\s+([A-Z][A-Za-z0-9+#.&-]+(?:\s+[A-Z][A-Za-z0-9+#.&-]+)*)/i, relation: 'worked_at', type: 'FACT' },
  // "was employed by X" / "was hired by X" — reversed word order
  { re: /\b(?:he|she|they|bradley|brad|scout)\s+(?:was|were|is)\s+(?:employed|hired|contracted)\s+(?:by|at|for)\s+([A-Z][A-Za-z0-9+#.&-]+(?:\s+[A-Z][A-Za-z0-9+#.&-]+)*)/i, relation: 'worked_at', type: 'FACT' },
  { re: /\b(?:he|she|they|bradley|brad|scout)\s+(?:interned|was an intern|was a trainee)\s+(?:at|for)\s+([A-Z][A-Za-z0-9+#.&-]+(?:\s+[A-Z][A-Za-z0-9+#.&-]+)*)/i, relation: 'interned_at', type: 'FACT' },
  ...
  // "X embeds/includes/features Y" — cross-project attribution check
  // e.g. "Scout embeds the Pokedex UI" → check if supported
  // Subject must be a proper noun (entity name), not "tech stack" etc.
  // Exclude common phrases that use "includes" naturally
  { re: /\b([A-Z][A-Za-z0-9+#.\s-]{2,40}?)\s+(?:embeds|features|integrates|incorporates)\s+(?:the\s+)?([A-Z][A-Za-z0-9+#.\s-]{2,40})/i, relation: 'includes', type: 'FACT' },
  // Standalone "embeds/features/integrates the [Project]" — subject may be distant
  // Note: "includes" and "contains" excluded to avoid false positives with "tech stack includes"
  { re: /\b(?:embeds|features|integrates|incorporates)\s+(?:the\s+)?([A-Z][A-Za-z0-9+#.\s-]{2,40})/i, relation: 'includes_object_only', type: 'FACT' },
  ...
  { re: /\b(?:has|have|with)\s+([A-Z][A-Za-z0-9+#.\s-]{1,30}?)\s+experience\b/i, relation: 'has_experience', type: 'FACT' },
