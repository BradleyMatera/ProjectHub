...
const { EXPERTISE_RELATIONS, canonicalEntityNorm } = require('./relationship-graph');
...
// Default is empty — subject names should be provided via SCOUT_SUBJECT_NAMES env var
// or derived from the knowledge base at runtime.
const SUBJECT_NAMES = (process.env.SCOUT_SUBJECT_NAMES || '')
  .split(',')
...
let subjectAlt = SUBJECT_NAMES.length > 0
  ? '|' + SUBJECT_NAMES.map(n => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')
  : '';

// Possessive alternation for subject references — defaults plus the configured
// possessive form, merged in ensureSubjectAltFromGraph.
let possAlt = 'his|her|their';

// Lowercase configured subject-reference tokens (all three pronoun cases),
// refreshed by ensureSubjectAltFromGraph. isRefToken treats configured forms
// exactly like the built-in he|she|they|his|her|their set.
let _subjectRefForms = new Set();
function isRefToken(w) {
  const v = String(w || '').toLowerCase();
  return /^(?:he|she|they|him|them|his|her|their|its)$/.test(v) || _subjectRefForms.has(v);
}

// Merge graph-derived subject references into subjectAlt. Runs even when
// subjectAlt was already configured (server-gemini calls configureEntityNames
// before extraction) so configured subject pronouns always reach the relation
// patterns.
function ensureSubjectAltFromGraph(graph) {
  if (!graph) return;
  const subjectSet = new Set();
  if (graph.triples) {
    for (const t of graph.triples) {
      if (t.relation === 'worked_at' || t.relation === 'built_by' || t.relation === 'has_skill' || t.relation === 'has_experience' || t.relation === 'has_degree') {
        const name = t.relation === 'built_by' ? t.object : t.subject;
        if (name) {
          const parts = String(name).toLowerCase().split(/\s+/).filter(p => p.length >= 3);
          for (const p of parts) subjectSet.add(p);
        }
      }
    }
  }
  // All configured subject pronoun forms (e.g. ze/zir/hir) are subject
  // references too — "zir internship at X" must extract like "his internship
  // at X" or the claim bypasses grounding entirely. Single normalization:
  // knowledge-access getSubjectPronouns.
  const pr = getSubjectPronouns(graph.knowledge);
  for (const p of [pr.subject, pr.object, pr.possessive]) {
    if (p && p.length >= 2 && !/^(?:he|she|they|him|her|them|his|their)$/.test(p)) subjectSet.add(p);
  }
  // The same configured possessive joins the possessive alternation so
  // "zir time at X" is extracted like "his time at X".
  const possSet = new Set(['his', 'her', 'their']);
  if (pr.possessive && pr.possessive.length >= 2) possSet.add(pr.possessive);
  _subjectRefForms = new Set([pr.subject, pr.object, pr.possessive]
    .map(v => String(v || '').toLowerCase()).filter(v => v.length >= 2));
  const nextPossAlt = [...possSet].map(n => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
  if (nextPossAlt !== possAlt) possAlt = nextPossAlt;
  if (subjectSet.size > 0) {
    const existing = new Set(subjectAlt ? subjectAlt.replace(/^\|/, '').split('|') : []);
    const merged = new Set(existing);
    for (const n of subjectSet) merged.add(n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    const next = '|' + [...merged].join('|');
    if (next !== subjectAlt) {
      subjectAlt = next;
      // Also update _subjectNameParts for isSubjectEntity checks
      for (const n of subjectSet) _subjectNameParts.add(n);
      // Rebuild relation patterns so regexes pick up the new subjectAlt
      RELATION_PATTERNS = buildRelationPatterns();
    }
  } else if (possAlt !== 'his|her|their') {
    RELATION_PATTERNS = buildRelationPatterns();
  }
}

// Dynamic sets for entity filtering — populated from knowledge/identity at runtime
let _assistantNames = new Set();
let _subjectNameParts = new Set(SUBJECT_NAMES);

// Check if a normalized entity is the assistant (e.g., "scout")
function isAssistantEntity(norm) {
  return _assistantNames.has(norm);
}

// Check if a normalized entity is the subject (candidate)
function isSubjectEntity(norm) {
  return _subjectNameParts.has(norm);
}

// Check if an entity name is a known project (dynamic, from graph)
function isProjectEntity(name) {
  if (!name) return false;
  const norm = normalizeEntity(name);
  // Projects are typically multi-word proper nouns or known project names
  // This is a permissive check — the graph entityIndex will do the real filtering
  return _projectNames.has(norm);
}
let _projectNames = new Set();

// Configure subject/assistant/project names from knowledge at runtime
function configureEntityNames(opts = {}) {
  if (Array.isArray(opts.subjectNames)) {
    _subjectNameParts = new Set(opts.subjectNames.map(n => n.toLowerCase()));
    // Rebuild subjectAlt and relation patterns
    const escaped = opts.subjectNames.filter(Boolean).map(n => n.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    subjectAlt = escaped.length > 0 ? '|' + escaped.join('|') : '';
    RELATION_PATTERNS = buildRelationPatterns();
  }
  if (Array.isArray(opts.assistantNames)) {
    _assistantNames = new Set(opts.assistantNames.map(n => n.toLowerCase()));
  }
  if (Array.isArray(opts.projectNames)) {
    _projectNames = new Set(opts.projectNames.map(n => normalizeEntity(n)));
  }
}

function isDegreeCredential(s) {
  if (!s) return false;
  const degreeTerms = /\b(?:degree|bachelor|master|doctorate|ph\.?d|b\.?s\.?|b\.?a\.?|m\.?s\.?|m\.?a\.?|mba|associate|diploma|certification)\b/i;
  return degreeTerms.test(s);
}

function isFalseCertCapture(s) {
  if (!s) return false;
  return /\b(?:experience|internship|project|work|role|job)\b/i.test(s);
}
...
// Patterns that use subjectAlt are rebuilt when subject names are configured.
function buildRelationPatterns() {
  // Certification body must end in a cert word and must not cross
  // intervening narrative words or a preceding degree phrase.
  const certDisallowed = 'from|at|in|of|and|or|degree|bachelor|master|doctorate|ph\\.?d|university|college|school|diploma|experience|internship|project|work|role|job';
  const certBody = '([A-Z][A-Za-z0-9+#.-]*(?:[-/\\s]+(?!\\b(?:' + certDisallowed + ')\\b)[A-Za-z0-9+#.-]+)*?)';
  const certTerminal = '\\s+(?:certifications?|certificates?|cert)\\b';
  return [
  ...
  // Expertise patterns require explicit relation evidence and a compatible claim ceiling.
  { re: /\b(?:has|have|with)\s+(?:extensive|deep|strong|broad)\s+experience\s+(?:in|with|at)\s+([A-Z][A-Za-z0-9+#.\s-]{1,100})/i, relation: 'has_extensive_experience', type: 'FACT' },
  { re: /\b(?:expertise|expert)\s+(?:in|with|at)\s+([A-Z][A-Za-z0-9+#.\s-]{1,100})/i, relation: 'has_expertise', type: 'FACT' },
  { re: /\b(?:is|was)\s+(?:an?\s+)?([A-Z][A-Za-z0-9+#.\s-]{1,100}?)\s+expert\b/i, relation: 'has_expertise', type: 'FACT' },
  { re: /\b(?:specializ\w+)\s+(?:in|with|at)\s+([A-Z][A-Za-z0-9+#.\s-]{1,100})/i, relation: 'specializes_in', type: 'FACT' },
  { re: /\b(?:proficient|proficiency)\s+(?:in|with)\s+([A-Z][A-Za-z0-9+#.\s-]{1,100})/i, relation: 'proficient_in', type: 'FACT' },
  { re: /\b(?:adept)\s+(?:in|with|at)\s+([A-Z][A-Za-z0-9+#.\s-]{1,100})/i, relation: 'adept_at', type: 'FACT' },
  ...
  { re: new RegExp('\\b(?:' + possAlt + ')\\s+([A-Z][A-Za-z0-9+#.\'\\s-]{2,40}?)\\s+(?:[Uu]ses?|[Uu]sed|[Tt]utiliz\\w+|built with|developed with)\\s+([A-Z][A-Za-z0-9+#.\\s-]{1,30})'), relation: 'uses_tech', type: 'FACT' },
  ...
  { re: new RegExp('\\b([A-Z][A-Za-z0-9+#.\\s-]{2,40}?)\\s+(?:was|is)\\s+(?:built|developed|created|made|written|designed)\\b[^.]*?(?:during|while|at)\\s+(?:(?:' + possAlt + ')\\s+)?(?:internship\\s+at\\s+|time\\s+at\\s+|work\\s+at\\s+)?([A-Z][A-Za-z0-9+#.\\s&-]{2,40})', 'i'), relation: 'built_during', type: 'FACT' },
  ...
  { re: new RegExp('\\b(?:' + possAlt + ')\\s+(?:time|internship|work|experience)\\s+at\\s+([A-Z][A-Za-z0-9+#.&-]+(?:\\s+[A-Z][A-Za-z0-9+#.&-]+)*)', 'i'), relation: 'worked_at', type: 'FACT' },

  // Education patterns — exclude "experience" to avoid matching "has experience in ... at ..."
  { re: new RegExp('\\b(?:he|she|they' + subjectAlt + ')\\s+(?:has|holds?|earned|received|got)\\s+(?:a\\s+)?(?!experience\\b)([A-Z][A-Za-z0-9+#.\\s-]{2,40}?)\\s+(?:from|at)\\s+([A-Z][A-Za-z0-9+#.\\s-]{2,40})', 'i'), relation: 'has_degree', type: 'FACT' },
  ...
  // Certification patterns — the capture must end in a cert word and must not
  // cross intervening narrative words like "experience" or "internship", nor
  // swallow a preceding degree phrase that happens to end with "certification".
  // "Certified" should be part of the body, not the terminal cert word.
  { re: new RegExp('\\b(?:he|she|they' + subjectAlt + ')\\s+(?:has|holds?|earned|received|completed)\\s+(?:(?:a|the)\\s+)?' + certBody + certTerminal, 'i'), relation: 'has_cert', type: 'FACT' },
  { re: new RegExp('\\b(?:he|she|they' + subjectAlt + ')\\s+(?:is|was)\\s+(?:a\\s+)?' + certBody + certTerminal, 'i'), relation: 'has_cert', type: 'FACT' },
  ...
  // Second conjunct: "... and holds AWS certifications"
  { re: new RegExp('(?:and|,)\\s+(?:he|she|they' + subjectAlt + ')?\\s*(?:has|holds?|earned|received|completed)\\s+(?:(?:a|the)\\s+)?' + certBody + certTerminal, 'i'), relation: 'has_cert', type: 'FACT' },

  // "X is a Y" (type assertion) — only match short, specific type nouns
  // Capture hyphenated and capitalized type words (e.g. "Early-career Software Engineer")
  // but stop before prepositions/articles that indicate the type has ended.
  { re: /\b([A-Z][A-Za-z0-9+#.\s-]{2,40}?)\s+(?:is|was)\s+(?:a|an)\s+([A-Za-z][A-Za-z0-9+#.]*(?:[-'][A-Za-z0-9]+)*(?:\s+[A-Za-z][A-Za-z0-9+#.]*(?:[-'][A-Za-z0-9]+)*){0,4})\b/i, relation: 'is_type', type: 'FACT' },

  // Availability/capability propositions — "X is open to Y", "X is able to Y".
  // These are NOT is_type/occupation claims (no article), but they are still
  // factual propositions: the predicate must be grounded in the knowledge
  // graph or retrieved evidence like any other property claim.
  { re: /\b([A-Z][A-Za-z0-9+#.\s-]{2,40}?)\s+(?:is|are|was|were)\s+(?:open\s+to|available\s+for|willing\s+to|ready\s+for|able\s+to|capable\s+of|prepared\s+for)\s+([A-Za-z][A-Za-z0-9+#.,\s-]{1,60}?)(?=[.;!?]|\s+(?:and|but|or|which|who|that|because|since|while|when|if|as|so)\b|,|$)/i, relation: 'has_property', type: 'FACT' },
  { re: new RegExp('\\b(?:he|she|they' + subjectAlt + ')\\s+(?:is|are|was|were)\\s+(?:open\\s+to|available\\s+for|willing\\s+to|ready\\s+for|able\\s+to|capable\\s+of|prepared\\s+for)\\s+([a-z][a-z0-9+#.,\\s-]{1,60}?)(?=[.;!?]|\\s+(?:and|but|or|which|who|that|because|since|while|when|if|as|so)\\b|,|$)', 'i'), relation: 'has_property', type: 'FACT' },

  // Pronoun type assertion — "he is a devoted father". Subjective or
  // personal attribute claims about the subject still need proposition-level
  // support; a broad value like "family" does not entail a specific relation.
  { re: new RegExp("\\b(?:he|she|they" + subjectAlt + ")\\s+(?:is|are|was|were)\\s+(?:a|an)\\s+([a-z][a-z0-9+#.'\\s-]{1,40}?)(?=\\s*(?:[.;!?,]|\\s+(?:and|but|or|who|which|that|because|since|while|when|if|as|so)\\b|$))", 'i'), relation: 'is_type', type: 'FACT' },

  // Gerund identity assertion about the subject — "evidence for being a
  // dedicated father", "known for being a mentor". Same proposition-level
  // support requirement as the "is a" form; without it the model can dodge
  // the is/was patterns by rephrasing as "being a X".
  { re: /\b(?:for|of|as)\s+being\s+(?:a|an)\s+([a-z][a-z0-9+#.'\s-]{1,40}?)(?=\s*(?:[.;!?,]|\s+(?:and|but|or|who|which|that|because|since|while|when|if|as|so|in|at|on|for|with|by|to|of|from)\b|$))/i, relation: 'is_type', type: 'FACT' },

  // Possession/relation assertion — "she has an older sister", "he has a
  // daughter". Abstract summary nouns ("a strong background") are filtered in
  // the dispatch; concrete objects must be grounded in one coherent source.
  { re: /\b((?:he|she|they)|[A-Za-z][A-Za-z0-9+#.-]*(?:\s+[A-Za-z][A-Za-z0-9+#.-]*){0,3})\s+has\s+(?:a|an)\s+([a-z][a-z0-9+#.'-]*(?:\s+[a-z][a-z0-9+#.'-]*){0,3})(?=\s*(?:[.;!?,]|\s+(?:and|but|or|who|which|that|because|since|while|when|if|as|so|in|at|on|for|with|by|to|of|from|named|called)\b|$))/i, relation: 'has_relation', type: 'FACT' },

  // Named-subject experience claim with a lowercase object — "Beacon has
  // production experience with distributed systems". Capitalized objects are
  // already covered; this form lets assistant-subject claims reach the
  // ownership check instead of escaping extraction entirely.
  { re: /\b([A-Za-z][A-Za-z0-9+#.-]*(?:\s+[A-Za-z][A-Za-z0-9+#.-]*){0,3})\s+(?:has|have)\s+(?:[a-z][a-z-]*\s+){0,3}experience\s+(?:in|with|at|of|using)\s+([a-z][a-z0-9+#.-]*(?:\s+[a-z][a-z0-9+#.-]*){0,3})(?=\s*(?:[.;!?,]|\s+(?:and|but|or|who|which|that|because|since|while|when|if|as|so)\b|$))/i, relation: 'has_experience', type: 'FACT' },
  ...
  // Generic "X on Y" platform attribution; Y is taken verbatim from the claim
  // and then checked against the tenant's explicit deployed_at/published_on triples.
  { re: /\b([A-Z][A-Za-z0-9+#.\s-]{2,40}?)\s+on\s+([A-Z][A-Za-z0-9+#.\s-]{2,40})/i, relation: 'deployed_on', type: 'FACT' },
  ...
  { re: new RegExp('\\b(?:' + possAlt + ')\\s+experience\\s+(?:in|with|at)\\s+([A-Z][A-Za-z0-9+#.-]{1,30})', 'i'), relation: 'has_experience', type: 'FACT' },
...
let RELATION_PATTERNS = buildRelationPatterns();
...
function extractClaims(answer, graph, question = '', history = []) {
  ...
  // Ensure subjectAlt is populated from graph if not configured at runtime
  ensureSubjectAltFromGraph(graph);

  // A tenant name-part word counts as a subject reference only when no real
  // entity canonically claims it — a project named "Avery" under tenant
  // "Avery Stone" keeps claim subject "Avery" instead of collapsing to the
  // 'subject' sentinel and borrowing/stealing subject relationships.
  const isSubjectRefWord = w => {
    if (!_subjectNameParts.has(w)) return false;
    if (!graph) return true;
    const wNorm = normalizeEntity(w);
    if (canonicalEntityNorm(graph, w) === graph.subjectNorm) return true;
    if (graph.aliasToCanonical && graph.aliasToCanonical.has(wNorm)) return false;
    if (graph.entityIndex && graph.entityIndex.get(wNorm)?.length > 0) return false;
    return true;
  };

  // Full-token subject-reference test honoring canonical precedence: real
  // pronouns and configured pronouns always refer to the subject; a NAME hit
  // on the subjectAlt alternation only refers to the subject when no real
  // entity claims the name (project "Avery" vs tenant "Avery Stone").
  const isSubjectRef = s => {
    const low = String(s || '').trim().toLowerCase();
    if (!low) return false;
    if (/^(?:he|she|they|it|him|her|them)$/.test(low)) return true;
    if (_subjectRefForms && _subjectRefForms.has(low)) return true;
    if (subjectAlt && new RegExp('^(?:' + subjectAlt.slice(1) + ')$', 'i').test(String(s).trim())) {
      return isSubjectRefWord(low);
    }
    return false;
  };
  ...
  const questionReferents = new Set();
  for (const entity of graph.entities || []) {
    if ([entity.name, ...entity.aliases].some(name => {
      const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      return new RegExp(`(?:^|[^a-z0-9])${escaped}(?=$|[^a-z0-9])`, 'i').test(question);
    })) questionReferents.add(entity.name);
  }
  const ambiguousReferent = questionReferents.size > 1;
  if (questionReferents.size) primaryEntity = ambiguousReferent ? null : [...questionReferents][0];
  ...
  if (question) {
    ...
    if (contextMatch && contextMatch[1]) {
      ...
      if (!isAssistantEntity(candidateNorm) && !isSubjectEntity(candidateNorm)) {
  ...
  if (!primaryEntity && !ambiguousReferent && Array.isArray(history) && history.length > 0 && graph && graph.entityIndex) {
    for (let i = history.length - 1; i >= 0; i--) {
      ...
      for (const cap of capMatches) {
        ...
        if (graph.entityIndex.has(norm) && !isAssistantEntity(norm) && !isSubjectEntity(norm)) {
          primaryEntity = cap;
          break;
        }
      }
  ...
  for (let sIdx = 0; sIdx < sentences.length; sIdx++) {
    ...
    // Resolve coreference: replace leading "It", "This", "That" (with an
    // optional generic noun like "application" or "project") with primary entity
    let resolvedSentence = sentence;
    if (primaryEntity) {
      resolvedSentence = sentence.replace(
        /^(It|This|That|The project|The system)(?:\s+(?:application|project|system|app|platform|tool|website|widget|chatbot|agent))?\s+(is|was|uses|used|has|integrates|includes|involves|built|developed|created|answers|embeds|features|incorporates|contains)\b/i,
        `${primaryEntity} $2`
      );
    }
    ...
    const uniqueEntities = [...new Set(compoundEntities)].filter(e =>
      !/^(Both|They|This|That|These|Those|JavaScript|Node|Express|React|HTML|CSS|AWS|GitHub|University)$/i.test(e) ||
      isProjectEntity(e)
    );
    ...
    for (const pattern of RELATION_PATTERNS) {
      ...
      if ((pattern.overclaim || EXPERTISE_RELATIONS.has(pattern.relation)) && clauseHasNegation[matchClauseIdx]) continue;
      ...
      if (pattern.relation === 'uses_tech') {
        if (match[1] && match[2]) {
          ...
          const refAltDyn = [...new Set(['he', 'she', 'they', ..._subjectRefForms])]
            .map(v => v.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
          if (!cleanSubj || startsLowercase || new RegExp('^(?:is|was|are|were|has|have|includes?|that|which|where\\s+(?:' + refAltDyn + ')|' + refAltDyn + ')\\b', 'i').test(cleanSubj)) {
            ...
            if (leadingNounMatches) {
              const cleanLeading = leadingNounMatches
                ...
                .filter(n => n && !isRefToken(n) && !/^(The|A|An|Where|This|That|These|Those)$/i.test(n));
            ...
            if (!cleanSubj || (startsLowercase && cleanSubj === cleanEntityName(match[1]))) {
              // A pronoun subject ("He has used X") refers to the tenant
              // subject, never to the question's active entity.
              if (new RegExp('^(?:he|she|they|' + possAlt + '|the subject|the candidate' + subjectAlt + ')\\b', 'i').test(String(match[1] || '').trim()) ||
                  new RegExp('^\\s*(?:he|she|they' + subjectAlt + ')\\b', 'i').test(matchClause.slice(0, match.index))) {
                cleanSubj = 'subject';
              } else {
                cleanSubj = (primaryEntity ? cleanEntityName(primaryEntity) : null) || null;
          ...
          // Clean object: strip leading/trailing punctuation and trailing prepositional clauses like "as part of a serverless metadata workflow"
          let cleanObj = match[2].trim().replace(/^[,;:\s]+/, '').replace(/\s+(?:as|in|on|for|with|by|to|using|during|while|at|and|,)\b.*$/i, '').replace(/[.,!?;)]+$/, '').trim();

          subject = cleanSubj;
          object = cleanObj;

          // Check if the matched object is part of a short list (e.g. "Node.js, and Express").
          // Only look a short distance after the match and stop at the first preposition
          // to avoid dragging in unrelated capitalized phrases from later in the sentence.
          const rawRemainder = resolvedSentence.slice(match.index + match[0].length, match.index + match[0].length + 50);
          const listRemainder = rawRemainder.split(/[,;]?\s*(?:in|on|at|for|with|by|to|from|of|as|during|while|because|since)\b/i)[0];
          const extraEntities = listRemainder.match(/\b(?:and|,)\s+(?:an?\s+|the\s+|advanced\s+|AI\s+|assistant\s+|named\s+)*([A-Z][A-Za-z0-9+#.-]+(?:\s+[A-Z][A-Za-z0-9+#.-]+)*)\b/g) || [];
          for (const item of extraEntities) {
            const cleanItem = item.replace(/^\s*(?:and|,)+\s*(?:(?:an?|the|advanced|AI|assistant|named)\s+)*/i, '').replace(/[.,!?;)]+$/, '').trim();
        ...
        if (match[1]) {
          ...
          // The subject using a tech should be has_experience, not uses_tech
          // "Node.js uses JavaScript" is a language relationship, not a project tech
          if (primaryEntity) {
            const peNorm = normalizeEntity(primaryEntity);
            // Skip if primary entity is the subject or a known technology
            if (isSubjectEntity(peNorm) || peNorm === 'subject') {
              continue;
            }
        ...
        if (match[1] && match[2]) {
          ...
          // One-group pattern: "he/<subject> built X" → subject=X (the thing), object='subject'
          subject = match[1].trim();
          object = 'subject';
        }
        ...
        if (match[1] && match[2]) {
          ...
          if (!isDegreeCredential(credential)) continue;
          subject = 'subject';
          object = match[2].trim(); // school is the key entity
        }
      } else if (pattern.relation === 'has_cert') {
        if (match[1]) {
          let rawCert = match[1].trim();
          if (isFalseCertCapture(rawCert)) {
            // The regex may have crossed conjunctions or narrative words.
            // Try to recover the rightmost certificate mention.
            const segments = rawCert.split(/\s*(?:,|;|\band\b)\s*/i)
              .map(s => s.trim())
              .filter(Boolean);
            const valid = [...segments].reverse()
              .find(s => /\bcert\w*$/i.test(s) && !isFalseCertCapture(s));
            if (valid) rawCert = valid;
            else { subject = null; object = null; continue; }
          }
          subject = 'subject';
          object = rawCert;
        ...
        if (match[1] && match[2]) {
          subject = match[1].trim();
          // Conjoined/possessive lead-ins ("Avery has a daughter and is a
          // devoted father") capture a fragment — reduce it to the trailing
          // capitalized entity run so the claim still names the subject.
          const tail = subject.replace(/\s+(?:and|but|or)\s*$/i, '')
            .match(/\b([A-Z][A-Za-z0-9+#.-]*(?:\s+[A-Z][A-Za-z0-9+#.-]*)*)\s*$/);
          if (tail) subject = tail[1];
          // The broad named-subject pattern can capture a configured pronoun
          // (e.g. "hir") as the subject — canonicalize subject references so
          // grounding treats the claim as about the tenant, not an entity.
          const subjLow = subject.toLowerCase();
          const pron = getSubjectPronouns(graph?.knowledge);
          const subjRefs = new Set(['he', 'she', 'they', 'him', 'her', 'them',
            pron?.subject, pron?.object, pron?.possessive]
            .map(v => String(v || '').toLowerCase()).filter(Boolean));
          if (subjRefs.has(subjLow)) subject = 'subject';
          object = match[2].trim();
          // Reject type assertions that continue into relative clauses or coordinating conjunctions
          // (e.g. "Morgan is a skilled developer who knows React" should not become a type claim).
          if (/\b(?:who|which|that|and|or|but)\b/i.test(object)) continue;
        } else if (match[1]) {
          // One-group pronoun form: "he is a <type>" — about the subject.
          subject = 'subject';
          object = match[1].trim();
          if (/\b(?:who|which|that|and|or|but)\b/i.test(object)) continue;
        }
      } else if (pattern.relation === 'has_relation') {
        const s = match[1] ? match[1].trim() : '';
        const o = match[2] ? match[2].trim() : '';
        if (!o) continue;
        const oWords = o.toLowerCase().split(/\s+/).filter(Boolean);
        // Abstract/summary nouns ("a strong background", "a lot of
        // experience") are descriptions, not verifiable relations. The head
        // noun must be concrete for the claim to be checkable.
        const ABSTRACT_REL_NOUNS = new Set(['background', 'experience', 'foundation',
          'record', 'history', 'understanding', 'familiarity', 'knowledge', 'expertise',
          'ability', 'skill', 'skills', 'lot', 'number', 'couple', 'range', 'variety',
          'set', 'list', 'collection', 'portfolio', 'mix', 'combination', 'total',
          'amount', 'passion', 'reputation', 'track', 'command', 'grasp', 'handle',
          'sense', 'knack', 'gift', 'talent', 'strength', 'weakness', 'question',
          'problem', 'issue', 'concern', 'doubt', 'chance', 'plan', 'idea', 'way',
          'series', 'group', 'team', 'network', 'base', 'level', 'degree of',
          'trajectory', 'path', 'journey', 'growth', 'potential', 'learning',
          'development', 'progress', 'arc', 'curve', 'trend', 'pattern', 'style',
          'approach', 'mindset', 'attitude', 'ethic', 'area', 'aspect', 'fit',
          'culture', 'capacity', 'aptitude', 'drive', 'motivation', 'focus']);
        const GENERIC_ADJ = /^(?:strong|proven|good|great|deep|solid|broad|wide|long|new|old|big|small|few|several|many|real|true|nice|clear)$/i;
        const head = oWords[oWords.length - 1];
        if (!head || ABSTRACT_REL_NOUNS.has(head) || GENERIC_ADJ.test(head) || /^\d/.test(o)) continue;
        // Scalar-attribute heads ("a verified price", "a rated capacity") name
        // a measurable property of the subject, not an entity standing in a
        // relation to it — they are validated as properties elsewhere.
        const ATTRIBUTE_HEAD_NOUNS = new Set(['price', 'cost', 'value', 'size',
          'weight', 'height', 'length', 'width', 'depth', 'speed', 'rating',
          'version', 'name', 'title', 'capacity', 'storage', 'warranty',
          'deadline', 'budget', 'salary', 'age', 'color', 'colour', 'duration',
          'latency', 'throughput', 'resolution', 'frequency', 'bandwidth',
          'temperature', 'score', 'rank', 'rate', 'fee', 'tag']);
        if (ATTRIBUTE_HEAD_NOUNS.has(head.toLowerCase())) continue;
        // "has a <noun> of/at/for <value>" is a property-with-value statement
        // ("has a verified price of $10") — the captured noun is an attribute
        // name, not a relational object. Named/called/location continuations
        // ("a brother named Sam", "a house in Austin") keep the relation.
        const afterMatch = answer.slice((match.index || 0) + match[0].length);
        if (/^\s*(?:of|at|for|is|was|are|were|:|=|worth|priced|costing|valued|totaling|totalling)\s+\S/.test(afterMatch)) continue;
        if (isSubjectRef(s)) {
          subject = 'subject';
        } else if (/^[A-Z]/.test(s) && !isRefToken(s) && !/^(?:The|A|An|This|That)$/i.test(s)) {
          subject = s;
        } else {
          continue;
        }
        if (subject !== 'subject' && _subjectNameParts.size > 0) {
          const subjWords = new Set(subject.toLowerCase().split(/[^a-z0-9+#.]+/).filter(Boolean));
          if ([...subjWords].some(isSubjectRefWord)) subject = 'subject';
        }
        object = o;
      } else if (pattern.relation === 'has_property') {
        if (match[1] && match[2]) {
          // Two-group pattern: named entity subject.
          subject = match[1].trim();
          object = match[2].trim();
        } else if (match[1]) {
          // One-group pronoun/subject pattern — the claim is about the subject.
          subject = 'subject';
          object = match[1].trim();
        }
        if (subject && isSubjectRef(subject)) subject = 'subject';
        // A captured subject phrase containing the tenant subject's name
        // ("Avery Chen knows JavaScript and is able to ...") is still a claim
        // about the subject — resolve it so property evidence can match.
        if (subject && subject !== 'subject' && _subjectNameParts.size > 0) {
          const subjWords = new Set(subject.toLowerCase().split(/[^a-z0-9+#.]+/).filter(Boolean));
          if ([...subjWords].some(isSubjectRefWord)) subject = 'subject';
        }
        // The assistant's own capabilities are not subject properties — Scout's
        // scope is governed by the contract, not by tenant evidence.
        if (subject && subject !== 'subject' && isAssistantEntity(normalizeEntity(subject))) {
          subject = null; object = null; continue;
        }
        if (subject && /^(?:it|this|that|there)$/i.test(subject)) {
          subject = null; object = null; continue;
        }
        // Coordinated property predicates — "open to relocation and willing
        // to travel" — are separate propositions about the same subject. The
        // main pattern only captures the first; extract each remaining
        // predicate so the validator checks them independently instead of
        // letting an unsupported claim piggyback on a supported one.
        if (subject && object) {
          const clauseText = clauses[matchClauseIdx] || sentence;
          const after = clauseText.slice((match.index || 0) + match[0].length);
          const coordRe = /(?:^|[\s,])(?:and\s+|or\s+)?(?:open\s+to|available\s+for|willing\s+to|ready\s+for|able\s+to|capable\s+of|prepared\s+for)\s+([a-z][a-z0-9+#.,\s-]{0,60}?)(?=[.;!?]|\s+(?:and|but|or|which|who|that|because|since|while|when|if|as|so)\b|,|$)/gi;
          let pm;
          while ((pm = coordRe.exec(after)) !== null) {
            const extraObj = pm[1].replace(/\s+(?:and|but|or)\b.*$/i, '').trim();
            if (extraObj && normalizeEntity(extraObj) !== normalizeEntity(object)) {
              claims.push({
                subject,
        ...
        // Trim the predicate at clause boundaries.
        if (object) {
          object = object.replace(/\s+(?:and|but|or|which|who|that|because|since|while|when|if|as|so)\b.*$/i, '').replace(/[.,!?;)]+$/, '').trim();
          if (!object) { subject = null; continue; }
        }
        ...
        // "X was published on Y" / "X is deployed on Y" → subject=X (project), object=Y (platform)
        if (match[1] && match[2]) {
          subject = match[1].trim();
          object = match[2].trim();
        }
        ...
        if (match[1] && match[2]) {
          // Named-subject form: "<Name> has <modifier> experience with <object>".
          // Keep the named subject so the validator's ownership check can
          // distinguish assistant claims from tenant-subject claims.
          const s = match[1].trim();
          if (isSubjectRef(s)) {
            subject = 'subject';
          } else if (/^[A-Z]/.test(s)) {
            subject = s;
            if (_subjectNameParts.size > 0) {
              const subjWords = new Set(subject.toLowerCase().split(/[^a-z0-9+#.]+/).filter(Boolean));
              if ([...subjWords].some(isSubjectRefWord)) subject = 'subject';
            }
          } else {
      ...
      if (subject || object) {
        ...
        const sentenceStarters = new Set(['so', 'okay', 'now', 'then', 'but',
          ...
          'source', 'code', 'source code', 'repository', 'repo',
          'documentation', 'docs', 'website', 'site', 'page', 'url', 'link',
          'project', 'the project', 'this project', 'that project']);
        if (cleanedSubject && sentenceStarters.has(cleanedSubject.toLowerCase())) continue;
        ...
        if ((pattern.relation === 'has_experience' || pattern.relation === 'worked_at' ||
            ...
            (!EXPERTISE_RELATIONS.has(pattern.relation) || pattern.relation === 'proficient_in')) {
        ...
        if (cleanedObject) {
          ...
          // An implicit 'subject' claim duplicating an explicit named-entity
          // claim in the same sentence ("Avery has experience with Go" → both
          // avery:Go and subject:Go) would let a colliding subject name steal
          // a real entity's relationships — keep the named claim only.
          if (cleanedSubject === 'subject' && claims.some(c =>
            c.raw === sentence.trim() && c.relation === pattern.relation &&
            c.subject !== 'subject' &&
            normalizeEntity(c.object || '') === normalizeEntity(cleanedObj))) continue;
          claims.push({
...
function cleanEntityName(name) {
  ...
  // Strip leading pronouns/possessives and stray punctuation
  cleaned = cleaned.replace(/^[,;:\s]+/, '');
  cleaned = cleaned.replace(new RegExp('^(?:' + possAlt + '|The|A|An|And)\\s+', 'i'), '');
  ...
  const stopWords = /\s+(?:as|for|in|at|to|from|with|by|on|of|is|was|are|were|and|or|but|including|such|like|part|which|that|while|during|because|since|has|have|had|based|located|residing|specializing|working|focusing)(?:\s+|$)/i;
