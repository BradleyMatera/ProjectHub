...
function commitDiscourseTurn(sessionId, question, policy, knowledge) {
  ...
  const q = String(question || '').trim();
  const correction = extractCorrection(q, knowledge);
  const removal = !correction && extractRemoval(q);
  const continuation = !correction && !removal && extractContinuation(q, knowledge);
  ...
  if (correction) {
    ...
    // A full substantive turn owns the discourse: same relation extends the
    // set; a different relation with its own entities replaces the frame; a
    // different entity-less relation closes the stale frame so it cannot
    // hijack later continuations. Turn-count expiry is only a fallback —
    // semantic topic change wins immediately.
    const names = policyEntityNames();
    if (names.length && frame && policy.mode === frame.intent) {
      appendAlternatives(frame, names, 'explicit');
    } else if (names.length) {
      frame = seedFrame(policy.mode, names, policy.subjectEntity);
    } else if (frame && policy.mode !== frame.intent) {
      frame = null;
    } else if (frame) {
      frame.updatedAtTurn = turn;
    }
