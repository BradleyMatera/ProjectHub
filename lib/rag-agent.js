...
function buildRagPrimaryPacket({ question, ragEvidence, enrichment, guardrails, conversationState, maxTokens, responseContract }) {
  ...
  const systemLines = isMeta ? [
    ...
    (responseContract?.subIntent === 'EXPERIENCE' && /\b(?:non[-\s]?technical|nontechnical|not\s+technical|not\s+in\s+(?:tech|technology)|outside\s+(?:of\s+)?(?:tech|technology|software|development|coding|engineering)|before\s+(?:moving|getting)\s+(?:into|to)\s+(?:tech|technology|software|development|coding|engineering)|career\s+before\s+(?:tech|technology|software|development|coding|engineering)|work\s+(?:outside|before|other\s+than|besides|apart\s+from)\s+(?:tech|technology|software|development|coding|engineering))\b/i.test(question)
      ? 'The visitor is asking about non-technical or outside-of-tech experience. Answer using only the non-technical roles from the EVIDENCE. Do not list software, IT, cloud, web development, coding, or technical support roles. The EVIDENCE contains both technical and non-technical roles; select only the non-technical ones.'
      : ''),
...
async function runRagPrimaryAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  let ragText = buildRagEvidenceText(evidence, RAG_EVIDENCE_MAX_CHARS, RAG_EVIDENCE_MAX_ITEMS, evidenceIntent, knowledge, question);
  ...
  if (responseContract?.intent === 'ADVERSARIAL' && responseContract?.factState === 'FALSE' && responseContract?.forbiddenClaims?.length) {
    ...
    if (filteredEvidence.length && filteredEvidence.length < evidence.length) {
      ragText = buildRagEvidenceText(filteredEvidence, RAG_EVIDENCE_MAX_CHARS, RAG_EVIDENCE_MAX_ITEMS, evidenceIntent, knowledge, question);
  ...
  if (responseContract?.subIntent === 'PROFILE_SUMMARY') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 3, evidenceIntent, knowledge, question);
  ...
  if (responseContract?.subIntent === 'STRENGTH_EVIDENCE') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 4, 'SKILL', knowledge, question);
  ...
  if (responseContract?.subIntent === 'RECRUITER_RECOMMENDATION') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 5, 'RECRUITER', knowledge, question);
  ...
  if (responseContract?.subIntent === 'EXPERIENCE') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 6, 'EXPERIENCE', knowledge, question);
  ...
  if (responseContract?.subIntent === 'PROJECT_DETAILS') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 5, 'PROJECT', knowledge, question);
