...
async function runRagPrimaryAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  // For contact questions, drop the contact chunk so the model does not echo
  // the exact "Public contact methods for..." sentence in the answer.
  if (responseContract?.subIntent === 'CONTACT') {
    const nonContactEvidence = evidence.filter(item => {
      const tag = item.kind || item.tag || '';
      return tag !== 'contact';
    });
    const safeEvidence = nonContactEvidence.length ? nonContactEvidence : evidence;
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 3, evidenceIntent);
    emit('rag_evidence_filtered', { before: evidence.length, after: safeEvidence.length, chars: ragText.length, reason: 'contact' });
  }
