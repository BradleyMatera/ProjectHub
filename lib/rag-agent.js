...
function buildRagEvidenceText(evidence, maxChars = RAG_EVIDENCE_MAX_CHARS, maxItems = RAG_EVIDENCE_MAX_ITEMS) {
  ...
  const priorityTags = new Set(['identity', 'pitch', 'summary', 'what-he-does', 'looking-for', 'target-roles']);
  const sorted = [...evidence]
    .filter(e => e && (e.description || e.text))
    .map(e => {
      const boost = tagBoost[e.kind] || tagBoost[e.tag] || 1.0;
      return { ...e, boostedScore: (e.evidenceScore || 0) * boost, isPriority: priorityTags.has(e.kind) || priorityTags.has(e.tag) };
    })
    .sort((a, b) => {
...
function buildRagPrimaryPacket({ question, ragEvidence, enrichment, guardrails, conversationState, maxTokens }) {
  ...
  const systemLines = [
    ...
    'Speak about Bradley in third person (he/his); never use I/my/me. Answer in natural, complete sentences — not lists or fragments. For explanatory questions, write 2-4 concise sentences. For yes/no questions, start with Yes/No, then add one sentence of evidence.',
    'EXAMPLE:\nEVIDENCE: FACT 1 [identity] Bradley Matera is an Early-career Software Engineer based in Davis, Illinois. FACT 2 [pitch] He is an early-career software engineer with a background in web development, AWS support engineering training, troubleshooting, documentation, and AI-assisted development.\nQ: Tell me about Bradley.\nA: {"answer": "Bradley Matera is an early-career software engineer based in Davis, Illinois. His background is in web development, AWS support engineering training, and troubleshooting."}',
...
async function runRagPrimaryAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  const isControlTurn = CONTROL_MODES.has(policyContract?.mode) && !forceSubstantive;
  if (isControlTurn) {
