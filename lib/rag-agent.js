...
function buildRagEvidenceText(evidence, maxChars = RAG_EVIDENCE_MAX_CHARS, maxItems = RAG_EVIDENCE_MAX_ITEMS, intent = null) {
  ...
  const intentBoosts = {
    ...
    NEGATIVE_ASSESSMENT: { boundaries: 1.5, 'direct-answer': 1.4, story: 1.2, pitch: 1.2, 'scout-runtime': 0.6 },
...
async function runRagPrimaryAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  const ragText = buildRagEvidenceText(evidence, RAG_EVIDENCE_MAX_CHARS, RAG_EVIDENCE_MAX_ITEMS, evidenceIntent);
