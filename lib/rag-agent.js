...
function backfillEvidence(evidence, intent, knowledge) {
  ...
  if (intent === 'EXPERIENCE') {
    (knowledge.experience || []).forEach((e, i) => {
      if (!e) return;
...
function buildRagEvidenceText(evidence, maxChars = RAG_EVIDENCE_MAX_CHARS, maxItems = RAG_EVIDENCE_MAX_ITEMS, intent = null, knowledge = null) {
  const filledEvidence = backfillEvidence(evidence, intent, knowledge);
  const baseBoost = {
    identity: 1.2,
    pitch: 1.2,
    summary: 1.2,
...
async function runRagPrimaryAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  let ragText = buildRagEvidenceText(evidence, RAG_EVIDENCE_MAX_CHARS, RAG_EVIDENCE_MAX_ITEMS, evidenceIntent, knowledge);
  ...
  if (responseContract?.intent === 'ADVERSARIAL' && responseContract?.factState === 'FALSE' && responseContract?.forbiddenClaims?.length) {
    ...
    if (filteredEvidence.length && filteredEvidence.length < evidence.length) {
      ragText = buildRagEvidenceText(filteredEvidence, RAG_EVIDENCE_MAX_CHARS, RAG_EVIDENCE_MAX_ITEMS, evidenceIntent, knowledge);
  ...
  if (responseContract?.subIntent === 'PROFILE_SUMMARY') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 3, evidenceIntent, knowledge);
  ...
  if (responseContract?.subIntent === 'STRENGTH_EVIDENCE') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 4, 'SKILL', knowledge);
  ...
  if (responseContract?.subIntent === 'RECRUITER_RECOMMENDATION') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 5, 'RECRUITER', knowledge);
  ...
  if (responseContract?.subIntent === 'EXPERIENCE') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 6, 'EXPERIENCE', knowledge);
  ...
  if (responseContract?.subIntent === 'PROJECT_DETAILS') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 5, 'PROJECT', knowledge);
