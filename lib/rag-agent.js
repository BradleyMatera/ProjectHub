...
function buildRagEvidenceText(evidence, maxChars = RAG_EVIDENCE_MAX_CHARS, maxItems = RAG_EVIDENCE_MAX_ITEMS, intent = null) {
  ...
  const intentAllowedTags = {
    PROFILE: new Set([
      ...
      'relocation', 'education', 'experience', 'certification', 'direct-answer', 'boundaries'
    ])
  }[intent];
...
function buildGuardrails(responseContract, policyContract, question, knowledge) {
  ...
  if (responseContract?.subIntent === 'PROFILE_SUMMARY') {
    rails.push('This is a profile summary. Describe the subject using only identity, summary, pitch, and education facts. Do NOT list specific programming languages, frameworks, tools, or technologies unless they appear explicitly in the EVIDENCE. Do not generalize from the subject\'s title or location.');
  }
