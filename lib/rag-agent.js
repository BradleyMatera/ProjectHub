...
function buildGuardrails(responseContract, policyContract, question, knowledge) {
  ...
  if (responseContract?.subIntent === 'PROFILE_SUMMARY') {
    rails.push('This is a profile summary. Write EXACTLY one sentence that states the subject\'s name, exact title, and location from the identity fact. Do not add a second sentence. Do not replace the title with generic words like "person" or "candidate". Do NOT mention any project, technology, programming language, framework, tool, cloud service, skill, or professional focus area by name (e.g., Python, AWS, Docker, JavaScript, HTML, CSS, React, Git, GitHub, Cloudflare, Go, Rust, automated testing, cloud-based). Draw only from identity, pitch, and summary facts.');
  }
...
function buildRagPrimaryPacket({ question, ragEvidence, enrichment, guardrails, conversationState, maxTokens, responseContract }) {
  ...
  const systemLines = isMeta ? [
    ...
    (responseContract?.subIntent === 'PROFILE_SUMMARY'
      ? 'Use only the supplied evidence. This is a profile summary. Write EXACTLY one sentence that states the subject\'s name, title, and location. Do NOT name any project, technology, programming language, framework, tool, cloud service, skill, or professional focus area. Do not add a second sentence. Never invent employers, experience, dates, roles, or seniority.'
