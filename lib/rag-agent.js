...
function backfillEvidence(evidence, intent, knowledge) {
  ...
  const existingTags = new Set(evidence.map(e => e.kind || e.tag || ''));

  if (intent === 'EXPERIENCE' && !existingTags.has('experience')) {
  ...
  if (intent === 'NEGATIVE_ASSESSMENT' && !existingTags.has('gaps')) {
    const gapSources = [];
