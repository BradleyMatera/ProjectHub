...
function buildRagEvidenceText(evidence, maxChars = RAG_EVIDENCE_MAX_CHARS, maxItems = RAG_EVIDENCE_MAX_ITEMS, intent = null) {
  const baseBoost = {
    identity: 1.2,
    pitch: 1.2,
    summary: 1.2,
  ...
  const intentBoosts = {
    ...
    NEGATIVE_ASSESSMENT: { boundaries: 1.5, 'direct-answer': 1.5, faq: 1.5, story: 1.2, pitch: 1.2, 'scout-runtime': 0.6 },
...
async function runRagPrimaryAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  let ragText = buildRagEvidenceText(evidence, RAG_EVIDENCE_MAX_CHARS, RAG_EVIDENCE_MAX_ITEMS, evidenceIntent);
  ...
  if (responseContract?.intent === 'ADVERSARIAL' && responseContract?.factState === 'FALSE' && responseContract?.forbiddenClaims?.length) {
    ...
    if (filteredEvidence.length && filteredEvidence.length < evidence.length) {
      ragText = buildRagEvidenceText(filteredEvidence, RAG_EVIDENCE_MAX_CHARS, RAG_EVIDENCE_MAX_ITEMS, evidenceIntent);
  ...
  if (responseContract?.subIntent === 'PROFILE_SUMMARY') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 3, evidenceIntent);
  ...
  if (responseContract?.subIntent === 'STRENGTH_EVIDENCE') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 4, 'SKILL');
  ...
  if (responseContract?.subIntent === 'RECRUITER_RECOMMENDATION') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 5, 'RECRUITER');
  ...
  if (responseContract?.subIntent === 'EXPERIENCE') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 6, 'EXPERIENCE');
  ...
  if (responseContract?.subIntent === 'PROJECT_DETAILS') {
    ...
    ragText = buildRagEvidenceText(safeEvidence, RAG_EVIDENCE_MAX_CHARS, 5, 'PROJECT');
