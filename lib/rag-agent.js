...
async function runRagPrimaryAgent({ question, conversationState, evidence, knowledge, sessionId, model, policyContract, deadlineAt, abortSignal }) {
  ...
  // For recruiter recommendations, keep evidence scoped to skills, projects,
  // experience, and summary so the model does not drift into source/blog claims
  // that fail validation.
  if (responseContract?.subIntent === 'RECRUITER_RECOMMENDATION') {
    const recEvidence = evidence.filter(item => {
      const tag = item.kind || item.tag || '';
      return tag.startsWith('skills-') || ['project', 'experience', 'summary', 'pitch', 'what-he-does', 'target-roles'].includes(tag);
    });
