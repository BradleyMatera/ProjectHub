...
function buildGuardrails(responseContract, policyContract, question, knowledge) {
  ...
  if (responseContract?.subIntent === 'PROFILE_SUMMARY') {
    rails.push('This is a profile summary. Use the subject\'s exact title and location from the identity fact. Do not replace the title with generic words like "person" or "candidate". Draw only from identity, pitch, summary, what-he-does, and target-role facts. You may mention skills and technologies that appear explicitly in those facts, but do not list any other tools, frameworks, or technologies.');
  }
