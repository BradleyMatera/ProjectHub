  ...
  for (const claim of claims) {
    ...
    if (claim.subject && claim.object && claim.relation) {
      ...
      if (claim.relation === 'has_experience') {
        // Generic category phrases are valid summary statements — skip relationship check.
        // "experience in front-end technologies" doesn't claim a specific false fact.
        const genericCategoryRe = /^(?:front[- ]?end|back[- ]?end|full[- ]?stack|cloud|software|web|mobile|devops|infrastructure|development|programming|engineering|technology|technologies|tech|coding|scripting|automation|testing|networking|database|api|system|systems)\b/i;
        if (genericCategoryRe.test(claim.object)) {
          details.push({ claim, verdict: 'skipped', message: 'Generic category phrase — not a specific false claim' });
          continue;
        }
