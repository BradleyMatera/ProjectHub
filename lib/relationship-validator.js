...
const { checkRelationship, getEntityTypes, isTypedEntity, getEntityDescription, collectGraphTypeWords } = require('./relationship-graph');
const { normalizeEntity } = require('./canonical-entities');
const { evidenceSupportsTechnologyRelation } = require('./evidence-relations');
 ...
 */
function validateRelationships(answer, graph, question = '', history = [], evidenceText = '', evidence = null) {
    ...
    if (claim.subject && claim.object && claim.relation) {
      ...
      if (claim.relation === 'is_type') {
        ...
        if (assertedWords.length === 0) {
          ...
          // However, if the asserted type is ONLY adjectives + generic nouns
          // (like "unique feature" or "interesting project"), it's too vague
          // to be a mischaracterization — accept it.
          // Derive specific nouns from the graph's is_type triples dynamically.
          const graphTypeWords = collectGraphTypeWords(graph);
          const hasSpecificNoun = assertedType.split(/\s+/).some(w => graphTypeWords.has(w));
          if (!hasSpecificNoun) {
            // No specific noun — too vague to be wrong
  ...
  if (Array.isArray(history) && history.length > 0 && graph && graph.entityIndex) {
    ...
    const questionHasEntity = questionCaps.some(cap => {
      ...
      const norm = normalizeEntity(cap);
      if (isSubjectOrAssistantEntity(norm)) return false;
      return graph.entityIndex.has(norm) ||
