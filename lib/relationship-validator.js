...
function validateRelationships(answer, graph, question = '', history = [], evidenceText = '') {
  ...
  if (Array.isArray(history) && history.length > 0 && graph && graph.entityIndex) {
    ...
    // Skip if the question is about learning, skills, or general experience —
    // these questions naturally span multiple projects and contexts
    const asksAboutLearning = /\b(?:what.*learn|what.*learned|how.*learn|skills?|experience|best\s+at|strongest|weakness|gaps?|grow|improve)\b/i.test(question);
