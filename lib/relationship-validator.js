    ...
    if (claim.subject && claim.object && claim.relation) {
      ...
      if (claim.relation === 'is_type') {
        ...
        if (assertedWords.length === 0) {
          // All words are generic — accept (too vague to be wrong)
        ...
        for (const cap of answerCaps) {
          ...
          if (norm !== normalizeEntity(primaryEntity) && norm !== 'he' && norm !== 'she' && norm !== 'they' && norm !== 'this' && norm !== 'that' && norm !== 'the') {
            ...
            if (entityInfo && (Array.isArray(entityInfo) ? entityInfo.length > 0 : entityInfo)) {
              // Only check uses_tech when the answer entity is a TECHNOLOGY (not a project).
              // Mentioning a project in the answer is not a uses_tech claim about the
              // primary entity — it's just referencing a different project (handled by
              // context drift). This prevents false "React uses_tech ProjectHub" claims.
              const eTriples = Array.isArray(entityInfo) ? entityInfo : [entityInfo];
              const isProject = eTriples.some(t => t.relation === 'built_by' || t.relation === 'is_type');
              if (!isProject) {
