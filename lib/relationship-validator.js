    ...
    if (claim.subject && claim.object && claim.relation) {
      ...
      if (claim.relation === 'is_type') {
        ...
        if (assertedWords.length === 0) {
          ...
          const specificNouns = ['management', 'recruiter', 'database', 'assistant',
            'chatbot', 'dashboard', 'calculator', 'game', 'store', 'shop',
            'marketplace', 'forum', 'blog', 'cms', 'crm', 'erp', 'search',
            'browser', 'editor', 'viewer', 'player', 'renderer', 'compiler',
            'debugger', 'profiler', 'monitor', 'logger', 'tracker'];
          const hasSpecificNoun = assertedType.split(/\s+/).some(w => specificNouns.includes(w));
          if (!hasSpecificNoun) {
      ...
      if (claim.relation === 'includes') {
        // "X embeds/includes Y" — check if X and Y are different projects
        // If both are known projects but no relationship exists, it's context drift
        const knowledge = graph.knowledge;
        const subjNorm = (subj || '').toLowerCase().replace(/[^a-z0-9]/g, '');
        const subjProj = knowledge && knowledge.projects && knowledge.projects.find(p => {
          const pNorm = (p.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
          return pNorm.includes(subjNorm.slice(0, 6)) || subjNorm.slice(0, 6).includes(pNorm);
        });
        const objNorm = normalizeEntity(claim.object || '');
        const objProj = knowledge && knowledge.projects && knowledge.projects.find(p => {
          const pNorm = (p.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
          return pNorm.includes(objNorm.slice(0, 6)) || objNorm.slice(0, 6).includes(pNorm);
        });
        if (subjProj && objProj && subjProj.name !== objProj.name) {
          // Both are different known projects — claiming one includes the other is context drift
          unsupportedClaims.push({
            subject: subj,
            relation: 'context_drift',
            object: obj,
            raw: claim.raw,
            reason: `context drift: ${subjProj.name} does not include ${objProj.name}`
          });
          details.push({
            claim,
            verdict: 'unsupported_relationship',
            message: `context drift: ${subjProj.name} does not include ${objProj.name}`
          });
        } else {
      ...
      if (claim.relation === 'includes_object_only') {
        // "embeds/includes the [Project]" — subject is implicit (pronoun)
        // Check if the object is a known project. If so, this may be context drift
        // if the primary entity (from question context) is a different project.
        const knowledge = graph.knowledge;
        const objNorm = normalizeEntity(claim.object || '');
        const objProj = knowledge && knowledge.projects && knowledge.projects.find(p => {
          const pNorm = (p.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
        ...
        if (objProj) {
          // The object is a known project. Flag as potential context drift.
          unsupportedClaims.push({
            subject: 'unknown',
            relation: 'context_drift',
            object: obj,
            raw: claim.raw,
            reason: `context drift: answer embeds/includes ${objProj.name}, which is a separate project`
          });
          details.push({
            claim,
            verdict: 'unsupported_relationship',
            message: `context drift: answer embeds/includes ${objProj.name}, which is a separate project`
          });
        }
        continue;
      }
