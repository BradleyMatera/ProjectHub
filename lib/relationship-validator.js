...
const { checkRelationship, normalizeEntity } = require('./relationship-graph');
const { normalizeEntity: normalizeCanonical } = require('./canonical-entities');
 ...
 */
function validateRelationships(answer, graph, question = '') {
  const claims = extractClaims(answer, graph, question);
  ...
  for (const claim of claims) {
    ...
    if (claim.subject && claim.object && claim.relation) {
      ...
      // has_experience in X → check worked_at, interned_at, has_skill, has_role
      let checkRelation = claim.relation;
 ...
 */
function detectFabricatedEntities(answer, graph) {
  const text = String(answer || '');
  const fabricated = [];

  // Detect negation context — if an entity is mentioned in a negation
  // ("did not work at Microsoft", "never worked at Google", "no evidence of X"),
  // it should NOT be flagged as fabricated. The entity is being refuted, not asserted.
  const negationContexts = [];
  const negRe = /\b(?:no(?:t|ne|,)?|never|didn'?t|did not|doesn'?t|does not|isn'?t|is not|wasn'?t|was not|no evidence|not (?:a )?(?:known|verified|documented))\b[^.!?]{0,60}/gi;
  let negMatch;
  while ((negMatch = negRe.exec(text)) !== null) {
    negationContexts.push(negMatch[0].toLowerCase());
  }
  ...
  const commonWords = new Set([
    ...
    'You', 'I', 'He', 'She', 'It', 'They', 'We', 'Vue', 'VueJS',
  ...
  for (const term of techTerms) {
    const norm = normalizeCanonical(term);
    ...
    if (!found) {
      ...
      // Tech terms: contain uppercase + lowercase, or are all-caps acronyms
      const looksLikeTech = /^[A-Z][a-z]+/.test(term) || /^[A-Z]{2,}$/.test(term);
      if (looksLikeTech) {
        // Check if this entity is mentioned in a negation context
        // ("did not work at Microsoft", "never worked at Google")
        // If so, it's being refuted, not asserted — don't flag as fabricated.
        const termLower = term.toLowerCase();
        const inNegation = negationContexts.some(ctx => ctx.includes(termLower));
        if (!inNegation) {
