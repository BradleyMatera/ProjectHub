 ...
 */
function validateRelationships(answer, graph, question = '', history = [], evidenceText = '') {
  ...
  for (const claim of claims) {
    ...
    if (claim.subject && claim.object && claim.relation) {
      ...
      if (!result.supported && checkRelation === 'uses_tech') {
        ...
        if (!result.supported && evidenceText) {
          const subjectNames = getSubjectNames(subj, graph);
          const techNormalizer = (token) => resolveTechAlias(canonicalizeTech(token));
          if (evidenceSupportsTechnologyRelation(subjectNames, obj, evidenceText, { normalizeToken: techNormalizer })) {
