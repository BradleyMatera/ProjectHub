...
function buildInsights(snapshot) {
  ...
  if (snapshot.free) {
    insights.push({ severity: 0, tag: 'free-status', text: `Total actual cost this month: $0.000000 — shadow cost if paid: $${monthUsd}. All usage inside free tiers.` });
  } else {
    insights.push({ severity: 2, tag: 'free-status', text: `A free-tier limit has been exceeded this period. Shadow cost if paid: $${monthUsd}. Review headroom below.` });
  }
  ...
  for (const h of snapshot.headroom || []) {
    if (h.pct >= 100) {
      ...
      const series = dailySeries(snapshot.days, h.source, h.metric.includes('Bytes') ? 'bytes' : 'seconds');
  ...
  // 3. Trend: last 7 days vs prior 7 days shadow cost
  ...
  if (costSeries.length >= 8) {
    ...
    if (prior7 > 0) {
      ...
      insights.push({ severity: Math.abs(deltaPct) > 100 ? 1 : 0, tag: 'trend', text: `Shadow cost trend: ${dir} ${Math.abs(deltaPct)}% week-over-week ($${(last7 / 1e6).toFixed(6)} vs $${(prior7 / 1e6).toFixed(6)}).` });
    }
  }
