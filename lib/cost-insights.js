...
function buildInsights(snapshot) {
  ...
  const unpricedSources = snapshot.shadowCost?.unpricedSources || [];
  const complete = snapshot.shadowCost?.monthComplete !== false;
  const paidText = complete
    ? `if these weren't free, you'd have paid $${monthUsd}`
    : `if these weren't free, the known-priced portion would be $${monthUsd}; total is incomplete because Cloudflare pricing is unverified`;
  if (snapshot.free === true) {
    insights.push({ severity: 0, tag: 'free-status', text: `Total actual cost this month: $0.000000 — ${paidText}. All usage inside free tiers.` });
  } else if (snapshot.free === false) {
    insights.push({ severity: 2, tag: 'free-status', text: `A free-tier limit has been exceeded this period. ${paidText}. Review headroom below.` });
  } else {
    insights.push({ severity: 1, tag: 'free-status', text: `No known free-tier limit is exceeded, but complete free-tier status is unverified because Cloudflare neuron usage for the current model cannot be calculated from the published rates. ${paidText}.` });
  }
