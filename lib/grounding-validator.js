...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null, policyMode = null, responseContract = null) {
  ...
  // Future-capability questions with UNKNOWN fact state must not be answered
  // with a positive assertion of current ability (e.g., "Yes, he has experience
  // with Rust and can debug it"). The contract already forbids that claim.
  if (responseContract?.subIntent === 'FUTURE_CAPABILITY' && responseContract?.factState === 'UNKNOWN') {
    if (/^\s*yes\b/i.test(text) && /\b(?:has (?:experience|worked|used|built) with|can (?:debug|use|build|work|handle))\b/i.test(text)) {
      return { valid: false, reasons: ['future_capability_yes_overclaim'], verdict: 'overclaim', cleaned: text };
    }
  }
