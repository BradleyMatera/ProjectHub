...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null) {
  ...
  // Common English words / generic descriptors that are NOT entities to ground.
  // These appear in model outputs describing projects (e.g. "Static Gen 1 Pokedex" from
  // the Interactive Pokedex description) but are not fabricated entity claims.
  const GENERIC_DESCRIPTORS = new Set([
    'static', 'gen', 'interactive', 'generation', 'calculator', 'testing', 'entry',
    'entries', 'devops', 'terraform', 'bash', 'kubernetes', 'udemy', 'coursera',
    'linkedin', 'pluralsight', 'profile', 'summary', 'overview', 'background',
  ]);
  ...
  const isOpenEndedQuestion = /\b(quick version|summary|brief|tell me about|honest thing|best at|strongest|weakness|weaknesses|gap|gaps|why (?:should|would) i|interview(?:ing)?|mit|harvard|stanford|degree from|actually do|bet on|role|worth|concerns?|explain it|okay now|what should|who is|who'?s|what'?s (?:his|the)|what are his|what do you|should i|would you|does he|could he|favorite|coolest|most (?:complex|impressive|relevant)|how (?:does|would)|fit (?:a|the)|aligns?)\b/i.test(question);
  if (groundedMatches.size < 2 && !isRefutation && !isShortYesNo && !isInventedEntityRefutation && !isOpenEndedQuestion) { reasons.push('insufficient_content_overlap'); }
