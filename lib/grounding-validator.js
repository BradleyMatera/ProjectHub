...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null) {
  ...
  if (knowledge) {
    try {
      ...
      const builtForCompanyPattern = /\b(?:built|developed|created|designed|implemented|wrote|deployed|maintained|contributed)\s+(?:a\s+|an\s+|the\s+)?(?:project|application|software|system|tool|feature|product)?\s*(?:called\s+|named\s+|for\s+)?[A-Z][A-Za-z0-9\s]*?\s+for\s+([A-Z][A-Za-z0-9&.\s-]{2,40}?)(?:\.|$|,)/g;
      let builtForMatch;
