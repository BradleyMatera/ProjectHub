...
const { validateRelationships, detectExpandedOverclaim, detectFabricatedEntities, wordInUnit } = require('./relationship-validator');
const { buildRelationshipGraph, canonicalEntityNorm } = require('./relationship-graph');
...
const { getKnownTechnologies, findAuthoritativeNegativeAssessment, getSubjectPronouns } = require('./knowledge-access');
...
const { resultSurfaceForms, asksForComputedResult } = require('./arithmetic-tool');

const _FACET_GENERIC_TOKENS = new Set(['web', 'app', 'apps', 'system', 'systems', 'project',
  'projects', 'platform', 'platforms', 'service', 'services', 'the', 'inc', 'llc', 'labs',
  'corp', 'certified', 'certification', 'associate', 'practitioner']);

// A facet value counts as stated when the answer contains the full value,
// its parenthetical alias, or a non-generic name part (>=4 chars).
function _facetValueMentioned(text, value) {
  const t = text.toLowerCase();
  const v = String(value || '').toLowerCase();
  if (v && t.includes(v)) return true;
  const paren = v.match(/\(([^)]+)\)/);
  if (paren && paren[1].trim().length >= 2 && t.includes(paren[1].trim())) return true;
  return v.split(/[^a-z0-9+#.]+/)
    .some(tok => tok.length >= 4 && !_FACET_GENERIC_TOKENS.has(tok) && t.includes(tok));
}
...
function supportsProjectProvenance(target, claim, projects, evidenceInput) {
  ...
  // A block's metadata applies only when it names the target exactly — a
  // prefix match lets "Alpha" supply provenance for "Alpha Beta".
  const targetNames = new Set(
    [target.name, ...(target.aliases || [])].filter(Boolean).map(n => n.toLowerCase())
  );
  for (const block of evidenceBlocks) {
    const names = [block.name, block.sourceEntity, ...(block.aliases || [])].filter(Boolean);
    if (names.some(name => targetNames.has(name.toLowerCase()))) {
...
function detectCrossProjectProvenance(text, knowledge, evidenceText = '', evidence = null) {
  ...
  for (const sentence of splitSentences(text)) {
    ...
    for (const other of projects) {
      ...
      const descPr = getSubjectPronouns(knowledge) || {};
      const descPrForms = new Set([descPr.subject, descPr.object, descPr.possessive]
      ...
      if (/\b(?:uses?|using|built with|powered by|tech(?:nology| stack)?)\b/i.test(sentence)) {
        for (const tech of other.tech || []) {
          ...
          if (!targetCorpus.includes(techLower)) {
            const names = [target.name, ...(target.aliases || [])].filter(Boolean);
            const relationEvidence = Array.isArray(evidence) && evidence.length ? evidence : evidenceText;
            if (!relationEvidence || !evidenceSupportsTechnologyRelation(names, tech, relationEvidence, { normalizeToken: techNormalizer, subjectRefTokens: [...descPrForms] })) {
...
// Stance patterns are built per tenant so configured pronouns ("ze/zir")
// parse with the same polarity as the defaults. Cached per pronoun set.
const stancePatternCache = new Map();
function leadingStancePatterns(knowledge) {
  const subj = String(getSubjectPronouns(knowledge)?.subject || '').trim().toLowerCase();
  const esc = s => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const SING = ['he', 'she', subj].filter(Boolean).map(esc).filter((v, i, a) => a.indexOf(v) === i).join('|');
  const ALL = ['he', 'she', 'they', subj].filter(Boolean).map(esc).filter((v, i, a) => a.indexOf(v) === i).join('|');
  const key = SING + '||' + ALL;
  let cached = stancePatternCache.get(key);
  if (cached) return cached;
  cached = {
    affirm: new RegExp(`^(?:yes|yeah|yep|yup|indeed|certainly|absolutely|it\\s+is|(?:${ALL})\\s+(?:is|are)|(?:${ALL})\\s+(?:does|do|has|have|can)|there\\s+is|there\\s+are)\\b`, 'i'),
    deny: new RegExp(`^(?:no|nope|nah|not|(?:${ALL})\\s+isn['’]?t|(?:${SING})['’]?s\\s+not|(?:${ALL})['’]?re\\s+not|(?:${ALL})\\s+aren['’]?t|(?:${ALL})\\s+(?:is|are)\\s+not\\b|(?:${ALL})\\s+(?:doesn['’]?t|does\\s+not|don['’]?t|do\\s+not|hasn['’]?t|has\\s+not|haven['’]?t|have\\s+not|cannot|can['’]?t)|there\\s+is\\s+no|there\\s+is\\s+not|there\\s+isn['’]?t|there\\s+are\\s+no|it\\s+is\\s+not|it\\s+isn['’]?t)\\b`, 'i'),
    qualify: new RegExp(`^(?:the\\s+(?:available\\s+)?evidence|it\\s+is\\s+not|there\\s+is\\s+(?:insufficient|no\\s+verified|not\\s+enough|no)\\s+(?:evidence|information|data|record)|no\\s+verified\\s+evidence\\s+establishes|there\\s+is\\s+no\\s+direct\\s+evidence\\s+that|there\\s+isn['’]?t\\s+enough\\s+evidence\\s+to|the\\s+profile\\s+(?:does\\s+not|doesn['’]?t)\\s+(?:document|verify|establish)|the\\s+public\\s+profile\\s+(?:does\\s+not|doesn['’]?t)|the\\s+(?:available\\s+)?record\\s+(?:does\\s+not|doesn['’]?t)\\s+(?:document|verify|establish)|it\\s+is\\s+not\\s+(?:documented|verified|known|clear)|i\\s+(?:don['’]?t|do\\s+not)\\s+(?:have|know)|i\\s+cannot\\s+confirm|it\\s+is\\s+unclear|it\\s+is\\s+unknown|the\\s+answer\\s+is\\s+unknown|there\\s+is\\s+(?:insufficient|inadequate)|the\\s+(?:available\\s+)?evidence\\s+is\\s+(?:insufficient|inconclusive|unclear)|i\\s+don['’]?t\\s+have\\s+(?:verified|sufficient|enough)\\s+(?:evidence|information|data|records)|the\\s+profile\\s+documents\\s+(?:a\\s+gap|.*?rather\\s+than\\s+.*?)|the\\s+profile\\s+lists\\s+.*?as\\s+a\\s+(?:gap|learning\\s+area|documented\\s+weakness)|no\\s+[a-z]+\\s+(?:is|are)\\s+(?:currently\\s+)?(?:documented|verified|listed|recorded|established|mentioned)|no\\s+documented\\s+[a-z]+|i\\s+(?:don['’]?t|do\\s+not)\\s+(?:see|find)\\s+(?:any|a)|(?:[a-z0-9+#.'’-]+\\s+){0,4}(?:cannot|can['’]?t|can\\s+not|could\\s+not|couldn['’]?t)\\s+(?:confirm|verify|determine|establish)\\b|(?:${ALL})\\s+(?:does|do|has|have)(?:n['’]?t|\\s+not)\\s+(?:have|work|use|hold|demonstrate|show|possess|document|been|worked|used|held|demonstrated|shown|documented)|(?:${ALL})\\s+(?:has|have)\\s+no\\b|(?:${ALL})\\s+(?:is|are)\\s+not\\s+(?:documented|verified|listed|recorded|known))\\b`, 'i')
  };
  stancePatternCache.set(key, cached);
  return cached;
}
...
// Epistemic uses of the modal ("Morgan cannot confirm whether Rust is
// documented") are bounded qualification, not a capability denial — the
// lookahead excludes confirm/verify/determine/establish/tell phrasing.
const LEADING_NAMED_DENY_RE = /^(?!(?:the|it|there|this|that|a|an|i|we|you|my|his|her|their|its)\b)[A-Z][A-Za-z0-9+#.'’-]*(?:\s+[A-Za-z0-9+#.'’]+){0,3}?\s+(?:definitely|absolutely|certainly|simply|just|really|clearly|currently)?\s*(?:cannot|can['’]?t|could\s+not|is\s+unable\s+to|will\s+never|would\s+never)\s+(?!(?:confirm|verify|determine|establish|tell|say|prove|guarantee)\b)/i;
function parseLeadingStance(text, knowledge = null) {
  ...
  const P = leadingStancePatterns(knowledge);
  if (P.qualify.test(t)) return 'QUALIFY';
  if (P.deny.test(t) || LEADING_NAMED_DENY_RE.test(t)) return 'DENY';
  if (P.affirm.test(t)) return 'AFFIRM';
  return 'UNKNOWN';
}
...
function checkStance(text, question, responseContract, knowledge = null) {
  if (!responseContract) return { valid: true };
  const expected = expectedStance(responseContract);
  if (!expected) return { valid: true };
  const actual = parseLeadingStance(text, knowledge);
  ...
  if (expected === 'AFFIRM' && (actual === 'DENY' || actual === 'QUALIFY')) {
    return { valid: false, reason: 'stance_mismatch:expected_affirm', detail: `Expected affirmative leading stance; got ${actual.toLowerCase()}` };
  }
  if (expected === 'DENY' && (actual === 'AFFIRM' || actual === 'QUALIFY')) {
    return { valid: false, reason: 'stance_mismatch:expected_deny', detail: `Expected negative leading stance; got ${actual.toLowerCase()}` };
  }
  return { valid: true };
}
...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null, policyMode = null, responseContract = null, evidence = null) {
  ...
  let sourceJoined = cleanText(source, 16000);
  // Structured evidence objects carry the same retrieved text as `source` but
  // may include fields not flattened into the string; fold them in so overlap
  // grounding sees one coherent evidence view.
  if (Array.isArray(evidence)) {
    for (const e of evidence) {
      const t = typeof e === 'string' ? e : (e && (e.text || e.content || e.chunk || e.fact));
      if (t && sourceJoined.length < 32000 && !sourceJoined.includes(t)) sourceJoined += `\n${t}`;
    }
  }
  const sourceText = sourceJoined.toLowerCase();
  const reasons = [];
  ...
  if (responseContract?.subIntent === 'FUTURE_CAPABILITY' && responseContract?.factState === 'UNKNOWN') {
    ...
    const currentRoleRe = /\b(?:has\s+(?:already\s+)?(?:worked|experience)\s+(?:as|in\s+(?:a|the)\s+(?:role|position|job|capacity))|has\s+(?:already\s+)?(?:used|built)\s+(?:.*?)\s+(?:as|in\s+(?:a|the)\s+(?:role|position|job|capacity))|already\s+(?:knows|uses|can|demonstrated)|currently\s+(?:knows|uses|can)|has\s+experience\s+(?:as|with\s+(?:a|the))\s+(?:role|position|job)|can\s+already\b|can\s+(?:perform|do|handle|take\s+on|step\s+into|fill)\s+(?:the|this|that)\s+(?:role|position|job|target)|is\s+(?:already|now)\s+(?:a|an)\s+(?:senior|lead|principal|staff|frontend|backend|full-stack))\b/ig;
    ...
    // A flat binary / no-information opening is the wrong frame only when the
    // answer then collapses into a current-state lookup. An opening like
    // "No verified record, but he could learn it on the job" opens negatively
    // yet still answers the future-potential question — the future framing,
    // not the leading token, is what the contract requires.
    if ((flatBinaryStart || noInformationStart) && !futureLanguage) {
  ...
  // Canonical subject-reference alternations — configured pronoun forms plus
  // defaults, merged with the derived name alternation. Single source of truth:
  // knowledge-access. All configured forms count as subject references in any
  // position because users and models freely misapply forms.
  const _cfgPron = getSubjectPronouns(knowledge);
  const _escAlt = t => String(t).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const _localPronounAlt = [...new Set(['he', 'she', 'they', _cfgPron.subject, _cfgPron.object, _cfgPron.possessive])]
    .filter(t => t && t.length >= 2).map(_escAlt).join('|');
  const _localPossAlt = [...new Set(['his', 'her', 'their', _cfgPron.possessive])]
    .filter(t => t && t.length >= 2).map(_escAlt).join('|');
  const _localObjAlt = [...new Set(['him', 'her', 'them', _cfgPron.object, _cfgPron.possessive])]
    .filter(t => t && t.length >= 2).map(_escAlt).join('|');
  const _localSubjectRefAlt = `${_localPronounAlt}${_localSubjectNameAlt}`;
  ...
  if (CONTROL_MODES.has(policyMode)) {
    ...
    // A control turn that carries computed facts must still state them: the
    // control branch's relaxed checks cannot let a conversational-classified
    // arithmetic question ship without the verified result.
    const controlFacts = responseContract?.computedFacts || [];
    if (controlFacts.length && asksForComputedResult(question)) {
      const textLower = text.toLowerCase();
      for (const f of controlFacts) {
        const stated = resultSurfaceForms(f.result).some(form => {
          const esc = form.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          return new RegExp(`(?<![\\w])${esc}(?![\\w])`, 'i').test(textLower);
        });
        if (!stated) {
          return { valid: false, reasons: [`missing_computed_result:${f.display}`], verdict: 'unsupported', cleaned: text };
        }
      }
    }
  ...
  // When the contract already resolves fit affirmatively (directAnswer FIT),
  // a plain "fits the <role>" affirmation is the requested answer — the
  // overclaim check only guards fit claims the contract did not affirm.
  const contractAffirmsFit = responseContract?.directAnswer === 'FIT';
  if (requestedRole && requestedRole.length >= 2 && !contractAffirmsFit) {
  ...
  // The professional-inflation possessive must include every configured
  // possessive ("zir") plus his/her/their — "developed zir projects
  // professionally" inflates exactly like the default forms.
  const possPronoun = String(getSubjectPronouns(knowledge)?.possessive || '')
    .toLowerCase().replace(/[^a-z0-9]/g, '');
  const professionalInflationRe = new RegExp(
    `\\b(?:built|did|worked|developed)\\s+(?:(?:${[...new Set(['his', 'her', 'their', 'the', possPronoun].filter(p => p.length >= 2))].join('|')})\\s+)?projects?\\s+professionally\\b`, 'i');
  for (const pattern of UPGRADE_PATTERNS) {
    const upgradeRe = pattern.flag === 'professional_inflation' ? professionalInflationRe : pattern.re;
    let patternFound = false;
    for (const sent of sentences) {
      const clauses = splitClauses(sent);
      for (const clause of clauses) {
        if (upgradeRe.test(clause) && !hasNegation(clause)) {
          patternFound = true;
          break;
        }
      }
  ...
  // Numbers produced by the deterministic arithmetic/reasoning tool are
  // grounded tool output — they are injected into the evidence packet and do
  // not need to appear in tenant source text.
  const computedNums = new Set((responseContract?.computedFacts || [])
    .map(f => (f && f.result != null) ? String(f.result) : null)
    .filter(Boolean));
  // Signed results from the arithmetic tool (e.g. "the result is -1") must
  // match as "-1", not the bare magnitude — a negative computed fact is
  // grounded only when the sign is part of the extracted token. A sign glued
  // to a preceding word/digit ("3-4", "v2") is a hyphen, not a minus.
  const numberMatches = [...text.matchAll(/(?<![\w.])[-+]?\d[\d.,]*\b/g)];
  for (const match of numberMatches) {
    const num = match[0];
    if (computedNums.has(num) || computedNums.has(String(parseFloat(num)))) continue;
  ...
  // 3b. Computed-result completeness — when the question asks for a
  // calculation or quantity outcome and the arithmetic tool produced facts,
  // the answer must state each computed result (digits or number words). An
  // answer that redirects to tenant evidence instead of answering the
  // arithmetic subtask fails the requested task even if nothing it says is
  // factually false.
  const contractFacts = responseContract?.computedFacts || [];
  if (contractFacts.length && asksForComputedResult(question)) {
    const textLower = text.toLowerCase();
    for (const f of contractFacts) {
      const stated = resultSurfaceForms(f.result).some(form => {
        const esc = form.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        // Character-boundary (not \b): a leading '-' in negative results is a
        // non-word char, so \b would never match "is -2". Non-word lookarounds
        // still prevent "12" from matching the form "2".
        return new RegExp(`(?<![\\w])${esc}(?![\\w])`, 'i').test(textLower);
      });
      if (!stated) reasons.push(`missing_computed_result:${f.display}`);
    }
  }

  // 3c. Requested-facet completeness — when the question asks for the values
  // of one facet (awards, certifications, AWS services, warranty...) and the
  // contract found supported values, the answer must state at least one. When
  // the facet is unsupported, the answer must address the facet or state it
  // is undocumented — substituting an unrelated true fact fails the task.
  const pFacet = responseContract?.primaryFacet;
  if (pFacet?.matched) {
    if (pFacet.supported && Array.isArray(pFacet.values) && pFacet.values.length) {
      if (!pFacet.values.some(v => _facetValueMentioned(text, v))) {
        reasons.push(`missing_facet_value:${pFacet.facet}`);
      }
    } else if (!pFacet.supported) {
      const surfaces = (pFacet.surfaces && pFacet.surfaces.length ? pFacet.surfaces : [pFacet.facet])
        .filter(s => s && s.length >= 3);
      const mentionsFacet = surfaces.some(s => {
        const esc = String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        return new RegExp(`\\b${esc}s?\\b`, 'i').test(text);
      });
      const statesUnknown = /\b(?:not documented|no documented|no verified|not verified|unknown|no evidence|not established|isn'?t (?:documented|listed|verified|established)|doesn'?t (?:appear|show|list))\b/i.test(text);
      if (!mentionsFacet && !statesUnknown) reasons.push(`facet_not_addressed:${pFacet.facet}`);
    }
  }
  ...
  const groundedMatches = new Set(contentWords.filter(w => sourceText.includes(w) || wordInUnit(w, sourceText)));
  ...
  const isInventedEntityRefutation = new RegExp(`^(?:no[,.]?\\s+)?(?:${_localSubjectRefAlt})\\s+(?:did not|never|has not|hasn't|doesn't|does not|attended|worked)\\b`, 'i').test(text) ||
  ...
  // A documented-absence answer for an unsupported facet ("No awards are
  // documented") and a computed-result answer ("2 + 2 equals 4") cannot share
  // content words with tenant evidence — their own checks carry the load.
  const isUnansweredFacet = !!(pFacet?.matched && !pFacet.supported);
  const isComputedTurn = !!(contractFacts.length && asksForComputedResult(question));
  // Defer the overlap hard-fail until after relationship validation (step 8):
  // words inside a claim that was validated against a coherent source unit are
  // grounded by that check and are added to groundedMatches there.
  const overlapUnderThreshold = groundedMatches.size < 2 && !isRefutation && !isShortYesNo && !isInventedEntityRefutation && !isOpenEndedQuestion && !(isPureYesNo && isYesNoQuestion) && !isGreeting && !isRefusal && !isUnansweredFacet && !isComputedTurn;
  if (overlapUnderThreshold && !knowledge) { reasons.push('insufficient_content_overlap'); }
  ...
  // Claim-ceiling / evidence-strength labels are contract metadata sent to the
  // model as generation constraints — they must never surface as user prose.
  const LEAKED_CONTRACT_PHRASES = /\b(?:has mixed evidence for|mixed evidence only|claim ceiling|evidence (?:level|strength)|documented gap for|professional production ownership)\b/i;
  if (LEAKED_CONTRACT_PHRASES.test(text)) {
    reasons.push('leaked_internal_language');
  }
  ...
  for (const pattern of firstPersonClaimPatterns) {
    if (pattern.test(text)) {
      ...
      const hasThirdPerson = new RegExp(`\\b(${_localSubjectRefAlt}|${_localPossAlt}|${_localObjAlt}|the candidate|the subject)\\b`, 'i').test(text);
      if (!hasThirdPerson) {
  ...
  const assistantSubjectConflationPatterns = [
    ...
    // Host-product vs assistant conflation: the product the assistant is
    // embedded in comes from tenant knowledge, not a hardcoded name.
    ...(() => {
      const productNames = [...new Set([
        knowledge?.agent?.productName,
        knowledge?.agent?.product,
        ...(knowledge?.projects || []).map(p => p.name)
      ].filter(Boolean))];
      if (!productNames.length) return [];
      const productAlt = productNames.map(n => String(n).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
      return [
        new RegExp(`\\b(?:${productAlt})\\s+(?:and|vs\\.?|versus)\\s+${_assistantNamePattern}\\b`, 'i'),
        new RegExp(`\\b${_assistantNamePattern}\\s+(?:and|vs\\.?|versus)\\s+(?:${productAlt})\\b`, 'i')
      ];
    })(),
  ...
  const questionAboutSubject = new RegExp(`\\b(?:${_localObjAlt}|${_localPossAlt}|${_localSubjectRefAlt})\\b`, 'i').test(question || '');
  ...
  if (knowledge) {
    try {
      ...
      const falseNegationPatterns = [
        ...
        new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:has|have)\\s+no\\s+(?:experience|background)\\s+(?:with|in|building|developing|creating)\\b`, 'i'),
        new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:has|have)\\s+no\\s+(?:experience|background)\\b`, 'i'),
        // "his projects do not involve building anything" — denies all building
        new RegExp(`\\b(?:${_localPossAlt})\\s+projects\\s+(?:do\\s+not|don't)\\s+(?:involve|include)\\s+building\\b`, 'i'),
        // "he does not build" / "he didn't build anything"
        new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:does|do|did)\\s+not\\s+build\\s+(?:anything|any\\s+(?:projects|software|applications))\\b`, 'i'),
        ...
        new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:has|have)\\s+not\\s+done\\s+any\\s+professional\\s+(?:projects|work|experience)\\b`, 'i'),
      ];
      ...
      const lacksPattern = new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+lacks\\s+experience\\s+(?:with|in)\\s+([A-Z][A-Za-z0-9\\s&.-]{2,50})`, 'gi');
      ...
      const techDenialPattern = new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:has|have)\\s+not\\s+(?:worked\\s+(?:in|with|on)|used)\\s+([A-Za-z][A-Za-z0-9]+(?:\\s+[A-Za-z0-9]+){0,3})`, 'gi');
      ...
      const techDenialPattern2 = new RegExp(`\\b(?:${_localPossAlt})\\s+(?:projects(?:\\s+and\\s+(?:skills|experience|work))?|skills(?:\\s+and\\s+(?:experience|work))?|experience|work)\\s+(?:do|does)\\s+not\\s+include\\s+(?:(?:real|actual|formal|professional|production|any|much)\\s+)?([A-Za-z][A-Za-z0-9]+(?:\\s+[A-Za-z0-9]+){0,3})`, 'gi');
      const techDenialPattern3 = new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:do|does)\\s+not\\s+have\\s+(?:(?:real|actual|formal|professional|production|any|much)\\s+)?([A-Za-z][A-Za-z0-9]+(?:\\s+[A-Za-z0-9]+){0,3})\\s+experience\\b`, 'gi');
      // "has no experience with [tech]" / "has no direct experience with [tech]"
      const techDenialPattern4 = new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:has|have)\\s+(?:no|not)\\s+(?:(?:direct|real|actual|formal|professional|production|any|much)\\s+)*experience\\s+(?:with|in|using)\\s+([A-Za-z][A-Za-z0-9]+)(?=\\s+(?:or|and|\\.|,|$))`, 'gi');
  ...
  if (knowledge) {
    try {
      ...
      const relValidation = validateRelationships(text, relGraph, question, history, sourceText, evidence, { contract: responseContract });
      ...
      // Words inside a relationship claim already validated against a coherent
      // source unit are grounded by that check — count them toward content
      // overlap before applying the overlap hard-fail.
      if (overlapUnderThreshold) {
        for (const d of relValidation.details || []) {
          if (d.verdict !== 'supported' || !d.claim) continue;
          const claimWords = String(d.claim.object || '').toLowerCase().match(/[a-z][a-z0-9+#.-]{4,}/g) || [];
          for (const w of claimWords) {
            const cw = w.replace(/[.]+$/, '');
            if (sourceText.includes(cw) || wordInUnit(cw, sourceText)) groundedMatches.add(cw);
          }
        }
        // The tenant subject's name is grounded in the knowledge base itself;
        // a retrieved chunk may not repeat it. Count verified subject words.
        const subjectNames = [knowledge.identity?.name, knowledge.identity?.preferredName,
          ...(knowledge.subjectAliases || [])].filter(Boolean);
        for (const name of subjectNames) {
          for (const w of String(name).toLowerCase().match(/[a-z][a-z0-9+#.-]{4,}/g) || []) {
            if (text.toLowerCase().includes(w)) groundedMatches.add(w);
          }
        }
      ...
      const employmentPattern = new RegExp(`\\b(?:${_localSubjectRefAlt})\\s+(?:was|worked|interned|employed|served|joined|spent\\s+time)\\s+(?:(?:at|with|for)\\s+|as\\s+(?:an?\\s+)?(?:[A-Za-z]+\\s+)*?(?:intern|engineer|developer|support|cloud|specialist|analyst|architect)\\s+(?:at|with|for)\\s+|a\\s+(?:[A-Za-z]+\\s+){1,5}(?:at|with|for)\\s+)([A-Z][A-Za-z0-9&.\\s-]{2,50})`, 'gi');
      let empMatch;
      while ((empMatch = employmentPattern.exec(text)) !== null) {
        const companyName = empMatch[1].trim().replace(new RegExp(`\\s+(?:focusing|where|which|that|to|in|on|for|as|and|with|${_localPossAlt}|the)\\s.*$`, 'i'), '');
      ...
      const atCompanyPattern = new RegExp(`\\bAt\\s+([A-Z][A-Za-z0-9&.\\s-]{2,40}?),\\s+(?:${_localSubjectRefAlt})\\s+(?:built|created|developed|worked|designed|implemented|led|managed|wrote|deployed|maintained|contributed)`, 'gi');
      let atCompMatch;
      ...
      const companyAsPartPattern = new RegExp(`\\b([A-Z][A-Za-z0-9&.\\s-]{2,40}?)\\s+(?:was|is|as)\\s+part\\s+of\\s+(?:${_localPossAlt})\\s+(?:\\w+\\s+)?(?:internship|work|experience|career|job|role)`, 'g');
      ...
      if (relGraph?.subjectName || knowledge?.identity?.name || knowledge?.summary?.whoIAm) {
        ...
        // Propositional/inanimate head nouns ("a crucial fact", "the key
        // question") are never person occupations — "X is a fact for Y" is not
        // an employment claim.
        const NON_ROLE_HEADS = new Set(['fact', 'facts', 'thing', 'things',
          'point', 'question', 'answer', 'detail', 'part', 'reason', 'way',
          'step', 'piece', 'bit', 'item', 'example', 'case', 'instance',
          'matter', 'topic', 'subject', 'idea', 'concept', 'notion', 'aspect',
          'feature', 'element', 'factor', 'consideration', 'requirement',
          'prerequisite', 'start', 'starting', 'place',
          'area', 'strength', 'skill', 'ability', 'trait', 'quality',
          'attribute', 'trajectory', 'record', 'history', 'background',
          'asset', 'path', 'foundation', 'basis', 'growth', 'potential',
          'development', 'progress', 'talent', 'capacity', 'aptitude',
          'experience', 'knowledge', 'understanding', 'familiarity',
          'reputation', 'track', 'sense', 'level', 'degree', 'sign',
          'evidence', 'indicator', 'fit', 'match', 'learning', 'focus',
          'drive', 'motivation', 'mindset', 'attitude', 'ethic', 'style',
          'approach', 'pattern', 'trend', 'journey', 'arc']);
        for (const { sentence, role, company, index } of occupationClaims) {
          ...
          if (NON_ROLE_HEADS.has((roleWords[roleWords.length - 1] || '').toLowerCase())) continue;
          ...
          // A real employer phrase names an organization. Pronoun/determiner-
          // headed continuations ("for your decision", "at the company") are
          // not employer claims — "is a crucial fact for your decision" is not
          // an occupation statement.
          if (/^(?:your|yours|his|her|their|theirs|my|mine|its|our|ours|the|this|that|these|those|a|an|any|some|whether|how|if)\b/i.test(claimedCompany)) continue;
          ...
          // describes Northstar Desk's category, not employment. The exemption
          // only applies when the sentence is not about the tenant subject:
          // "Morgan is an electrical service" must still be validated as an
          // occupation claim.
          const sentTrim = sentence.trim();
          const subjectHeadRefs = [relGraph?.subjectName,
            ...(relGraph?.subjectAliases ? [...relGraph.subjectAliases] : []),
            ...(relGraph?.subjectName ? relGraph.subjectName.toLowerCase().split(/[^a-z0-9+#.]+/) : [])];
          const cfgPron = getSubjectPronouns(knowledge) || {};
          const headPronouns = new Set(['he', 'she', 'they',
            String(cfgPron.subject || '').toLowerCase(),
            String(cfgPron.object || '').toLowerCase(),
            String(cfgPron.possessive || '').toLowerCase()].filter(Boolean));
          const headIsSubject = [...headPronouns].some(p =>
              new RegExp('^' + p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'i').test(sentTrim)) ||
            subjectHeadRefs.some(ref => ref &&
              new RegExp('^' + String(ref).replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'i').test(sentTrim));
          const claimedRoleLower = role.toLowerCase();
          const entityTypeClaim = !headIsSubject && (relGraph?.triples || []).some(t =>
            t.relation === 'is_type' &&
            t.subjectNorm && canonicalEntityNorm(relGraph, t.subjectNorm) !== relGraph?.subjectNorm &&
            (() => {
          ...
          if (sentenceStart) {
            ...
            if (startNorm && canonicalEntityNorm(relGraph, startNorm) !== relGraph?.subjectNorm &&
                relGraph?.entityIndex?.get(startNorm)?.length > 0) {
              continue;
            }
          }
        ...
        // Pronoun-headed "X is a <noun>" claims are subject claims too —
        // "Ze is a support ticketing platform" must be validated against
        // documented occupations exactly like "Morgan is a ...".
        const cfgPr2 = getSubjectPronouns(knowledge) || {};
        const bareSubjAlt = [...new Set([subjectFirst,
          cfgPr2.subject, cfgPr2.object, cfgPr2.possessive]
          .map(v => String(v || '').trim().toLowerCase())
          .filter(v => v.length >= 2)
          .map(v => v.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))].join('|');
        const subjectPattern = new RegExp(
          `\\b(?:${bareSubjAlt})` +
          `(?:\\s+${secondName})?\\s+(?:is|was|are|were)\\s+(?:an?\\s+)?(\\w+(?:\\s+\\w+){0,4})`,
          'i'
        );
        ...
        if (isAMatch) {
          ...
          if (!isNegated2) {
            ...
            // When the head noun is an assessment noun ("candidate", "fit"),
            // the phrase is an evaluation, not an occupation claim — the
            // remaining factual modifiers (e.g. "junior" in "a strong junior
            // candidate") only need to appear in the evidence.
            const headNoun = (headWords[headWords.length - 1] || '').toLowerCase();
            const factualMods = headWords.filter(w => !genericAssessments.has(w));
            if (genericAssessments.has(headNoun) && factualMods.length > 0 &&
                factualMods.every(w => sourceJoined.toLowerCase().split(/[^a-z0-9+#.]+/).includes(w.toLowerCase()))) {
              // grounded assessment — fall through to skip
            } else if (!onlyAssessment) {
          ...
          if (tpMatch) {
            ...
            if (Array.isArray(evidence)) {
              const projNames = new Set(
                [proj.name, ...(proj.aliases || [])].filter(Boolean).map(n => n.toLowerCase())
              );
              for (const block of evidence) {
                const names = [block.name, block.sourceEntity, ...(block.aliases || [])].filter(Boolean);
                const namesMatch = names.some(name => projNames.has(name.toLowerCase()));
  ...
  const claimIssues = validateClaims(text, question, responseContract, sourceText, knowledge, evidence, graph);
  ...
  const stance = checkStance(text, question, responseContract, knowledge);
  ...
  const hardFail = reasons.some(r =>
    ...
    r.startsWith('missing_computed_result:') ||
    r.startsWith('missing_facet_value:') ||
    r.startsWith('facet_not_addressed:') ||
  ...
  for (const r of reasons) {
    if (r.startsWith('missing_computed_result:')) {
      rejectionDetails.push({ reason: 'MISSING_COMPUTED_RESULT', detail: `Question asked for a calculation; the answer must state the computed result: ${r.split(':').slice(1).join(':')}` });
    }
  }
  for (const r of reasons) {
    if (r.startsWith('missing_facet_value:')) {
      rejectionDetails.push({ reason: 'MISSING_FACET_VALUE', detail: `Question asked for ${r.split(':').slice(1).join(':')}; the answer must name at least one documented value` });
    }
    if (r.startsWith('facet_not_addressed:')) {
      rejectionDetails.push({ reason: 'FACET_NOT_ADDRESSED', detail: `Question asked for ${r.split(':').slice(1).join(':')}; the answer substituted an unrelated fact instead of addressing the facet` });
    }
  }
