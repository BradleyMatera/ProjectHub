...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null) {
  ...
  if (knowledge) {
    try {
      ...
      // Example: "built a project called ProjectHub for Netflix"
      const builtForCompanyPattern = /\b(?:built|developed|created|designed|implemented|wrote|deployed|maintained|contributed)\s+(?:a\s+|an\s+|the\s+)?(?:project|application|software|system|tool|feature|product)?\s*(?:called\s+|named\s+|for\s+)?[A-Z][A-Za-z0-9\s]*?\s+for\s+([A-Z][A-Za-z0-9&.\s-]{2,40}?)(?:\.|$|,)/gi;
      let builtForMatch;
      ...
      const companyAsPartPattern = /\b([A-Z][A-Za-z0-9&.\s-]{2,40}?)\s+(?:was|is)\s+part\s+of\s+(?:his|her|their)\s+(?:internship|work|experience|career|job|role)/gi;
      let capMatch;
