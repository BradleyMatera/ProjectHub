...
function validateAnswer(answer, source, question = '', knowledge = null) {
  ...
  const isInventedEntityRefutation = /^(?:no[,.]?\s+)?(?:he|she|they)\s+(?:did not|never|has not|hasn't|doesn't|does not)\b/i.test(text) ||
  ...
  if (groundedMatches.size < 2 && !isRefutation && !isShortYesNo && !isInventedEntityRefutation) { reasons.push('insufficient_content_overlap'); }
  ...
  if (knowledge) {
    try {
      ...
      const relValidation = validateRelationships(text, graph, question);
      ...
      const fabricated = detectFabricatedEntities(text, graph);
      for (const entity of fabricated) {
        reasons.push(`fabricated_entity:${entity}`);
      }
    } catch (err) {
      // Relationship validation is best-effort — don't fail the answer
      // if the graph build fails for some reason.
    }
  }
