...
const { validateTechClaims } = require('./tech-claim-validator');
...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null, policyMode = null) {
  ...
  const techClaimResult = validateTechClaims(text, source);
  ...
  const assistantSubjectConflationPatterns = [
    ...
    /\b(?:scout|the\s+assistant)\s+has\s+(?:built|developed|created|made|written|designed)\b/i,
    /\b(?:scout|the\s+assistant)\s+(?:is|was)\s+(?:an?\s+)?(?:embeddable|AI|recruiter)\b/i,
  ...
  if (knowledge) {
    try {
      ...
      const techDenialPattern3 = /\b(?:he|she|they|bradley|brad)\s+(?:do|does)\s+not\s+have\s+(?:(?:real|actual|formal|professional|production|any|much)\s+)?([A-Za-z][A-Za-z0-9]+(?:\s+[A-Za-z0-9]+){0,3})\s+experience\b/gi;
      // "has no experience with [tech]" / "has no direct experience with [tech]"
      const techDenialPattern4 = /\b(?:he|she|they|bradley|brad)\s+(?:has|have)\s+(?:no|not)\s+(?:(?:direct|real|actual|formal|professional|production|any|much)\s+)*experience\s+(?:with|in|using)\s+([A-Za-z][A-Za-z0-9]+)(?=\s+(?:or|and|\.|,|$))/gi;
  ...
  if (knowledge) {
    try {
      ...
      // "Bradley Matera is an American professional stock car racing driver"
      // when the KB says he's a junior software engineer.
      if (knowledge?.identity?.name || knowledge?.summary?.whoIAm) {
        const subjectName = knowledge?.identity?.name || 'Bradley Matera';
        ...
        const isAPattern = new RegExp(
          ...
          '(?:\\s+Matera)?\\s+(?:is|was)\\s+(?:an?\\s+)?(\\w+(?:\\s+\\w+){0,4})',
          'i'
        );
