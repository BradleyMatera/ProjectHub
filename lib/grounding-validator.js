  ...
  for (const sentence of splitSentences(text)) {
    ...
    for (const other of projects) {
      ...
      for (const marker of relationshipMarkers) {
        const otherNames = [other.name, ...(other.aliases || [])].filter(Boolean);
        const otherMentioned = otherNames.some(name => sentenceLower.includes(String(name).toLowerCase()));
        if (sentenceLower.includes(marker) && otherMentioned && otherCorpus.includes(marker) && !targetCorpus.includes(marker)) {
    ...
    try {
      ...
      if (knowledge?.identity?.name || knowledge?.summary?.whoIAm) {
        ...
        if (forbiddenMatch) {
          ...
          // Do not flag a forbidden occupation term when it appears in a
          // sentence that names a known employer — the model is likely
          // recounting actual prior experience rather than hallucinating a
          // current identity (e.g. "Construction worker at Ascend Roofing").
          const sentenceNorm = sentence.toLowerCase().replace(/[^a-z0-9]/g, '');
          const hasKnownCompany = knownCompanies && [...knownCompanies].some(known =>
            known && known.length >= 3 && sentenceNorm.includes(known)
          );
          if (!isNegated && !hasKnownCompany) {
