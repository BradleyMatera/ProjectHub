...
function documentedOccupation(role, company, knowledge, graph) {
  const roleMatches = documented => typeof documented === 'string' && phraseAppears(documented, role);
      ...
      if (knowledge?.identity?.name || knowledge?.summary?.whoIAm) {
        ...
        for (const sentence of sentences.flatMap(splitClauses)) {
          for (let i = 0; i < FORBIDDEN_OCCUPATION_TERMS.length; i += chunkSize) {
            ...
            const pattern = new RegExp('\\b(?:' + escaped.join('|') + '|neurosurgeon)\\b', 'gi');
            for (const match of sentence.matchAll(pattern)) occupationClaims.push({ sentence, role: match[0], index: match.index });
          }
