...
function detectCrossProjectProvenance(text, knowledge, evidenceText = '') {
      ...
      if (/\b(?:uses?|using|built with|powered by|tech(?:nology| stack)?)\b/i.test(sentence)) {
        for (const tech of other.tech || []) {
          ...
          if (!targetCorpus.includes(techLower)) {
            ...
            if (!evidenceText || !evidenceSupportsTechnologyRelation(names, tech, evidenceText, { normalizeToken: techNormalizer })) {
...
const LEADING_AFFIRM_RE = /^(?:yes|yeah|yep|yup|indeed|certainly|absolutely|it\s+is|he\s+is|he\s+does|he\s+has|he\s+can|there\s+is|there\s+are)\b/i;
const LEADING_DENY_RE = /^(?:no|nope|nah|not|he\s+isn['’]?t|he['’]?s\s+not|he\s+doesn['’]?t|he\s+does\s+not|he\s+hasn['’]?t|he\s+has\s+not|he\s+cannot|he\s+can['’]?t|there\s+is\s+no|there\s+is\s+not|there\s+isn['’]?t|there\s+are\s+no|it\s+is\s+not|it\s+isn['’]?t)\b/i;
const LEADING_QUALIFY_RE = /^(?:the\s+(?:available\s+)?evidence|it\s+is\s+not|there\s+is\s+(?:insufficient|no\s+verified|not\s+enough|no)\s+(?:evidence|information|data|record)|i\s+(?:don['’]?t|do\s+not)\s+(?:have|know)|the\s+profile\s+(?:does\s+not|doesn['’]?t)\s+document|the\s+public\s+profile\s+(?:does\s+not|doesn['’]?t)|it\s+is\s+not\s+(?:documented|verified|known|clear)|i\s+cannot\s+confirm|it\s+is\s+unclear|it\s+is\s+unknown|the\s+answer\s+is\s+unknown|there\s+is\s+(?:insufficient|inadequate)|the\s+(?:available\s+)?evidence\s+is\s+(?:insufficient|inconclusive|unclear)|i\s+don['’]?t\s+have\s+(?:verified|sufficient|enough)\s+(?:evidence|information|data|records)|the\s+profile\s+documents\s+(?:a\s+gap|.*?rather\s+than\s+.*?)|the\s+profile\s+lists\s+.*?as\s+a\s+(?:gap|learning\s+area|documented\s+weakness))\b/i;
...
function parseLeadingStance(text) {
  ...
  if (LEADING_DENY_RE.test(t)) return 'DENY';
...
function expectedStance(responseContract) {
  ...
  if (rs === 'AFFIRM' || as === 'AFFIRM' || da === 'YES' || da === 'FIT' || fs === 'TRUE') return 'AFFIRM';
  if (rs === 'DENY' || as === 'DENY' || da === 'NO' || da === 'NOT_FIT' || fs === 'FALSE') return 'DENY';
  if (rs === 'QUALIFY' || as === 'QUALIFY' || da === 'UNKNOWN' || fs === 'UNKNOWN') return 'QUALIFY';
  if (da === 'MIXED' || da === 'PARTIAL_FIT' || fs === 'PARTIAL') return 'QUALIFY'; // mixed may start either, safest to allow qualified
  return null;
}
...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null, policyMode = null, responseContract = null) {
  ...
  const SCAFFOLD_PATTERN = /\b(?:FACT|EVIDENCE)\s*\d+\b|\b(?:as mentioned in|based on|according to|provided by|retrieved from)\s+(?:FACT|EVIDENCE)\b|\bretrieved evidence\b|\bretrieved context\b|\bcontext block\b|\bevidence block\b|\bsource block\b/i;
  ...
  if (responseContract?.subIntent === 'FUTURE_CAPABILITY' && responseContract?.factState === 'UNKNOWN') {
    ...
    // he currently can perform the role. Mentioning skills as a foundation is OK.
    const currentRoleClaim = /\b(?:has\s+(?:already\s+)?(?:worked|experience)\s+(?:as|in\s+(?:a|the)\s+(?:role|position|job|capacity))|has\s+(?:already\s+)?(?:used|built)\s+(?:.*?)\s+(?:as|in\s+(?:a|the)\s+(?:role|position|job|capacity))|already\s+(?:knows|uses|can|demonstrated)|currently\s+(?:knows|uses|can)|has\s+experience\s+(?:as|with\s+(?:a|the))\s+(?:role|position|job)|can\s+(?:debug|use|build|work\s+with|handle)|is\s+(?:already|now)\s+(?:a|an)\s+(?:senior|lead|principal|staff|frontend|backend|full-stack))\b/i.test(text);
  ...
  for (const token of entityClaims) {
    ...
    // repeated inside a denial like "No, he doesn't know COBOL") are being
  ...
  if (knowledge) {
    try {
      const relGraph = graph || buildRelationshipGraph(knowledge);
      const relValidation = validateRelationships(text, relGraph, question, history, sourceText);
      ...
      for (const issue of detectCrossProjectProvenance(text, knowledge, sourceText)) {
  ...
  const claimIssues = validateClaims(text, question, responseContract, sourceText, knowledge);
