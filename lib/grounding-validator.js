...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null) {
  ...
  const LEAKED_PROMPT_PATTERNS = [
    ...
    /\[\s*[A-Z][A-Za-z]+\s*\]/, // [ProjectName] bracket notation
  ];
  ...
  const assistantSubjectConflationPatterns = [
    ...
    // "Scout is a developer/engineer" — Scout is an AI assistant, not a developer
    /\bscout\s+is\s+(?:a|an)\s+(?:web\s+)?(?:developer|engineer|programmer|coder|designer|architect)\b/i,
    // "Scout has experience" — Scout is an AI, it doesn't have personal experience
    /\bscout\s+has\s+(?:experience|skills|knowledge|background)\b/i,
    /\bprojecthub\s+(?:and|vs\.?|versus)\s+scout\b/i,
    /\bscout\s+(?:and|vs\.?|versus)\s+projecthub\b/i,
    // "I am not [candidate name]" — Scout should never claim to be or not be the candidate
    /\bi\s+am\s+not\s+(?:bradley|brad|the\s+candidate|the\s+subject)\b/i,
    // "I am [candidate name]" — Scout should never claim to be the candidate
    /\bi\s+am\s+(?:bradley|brad|the\s+candidate|the\s+subject)\b/i
  ];
  for (const pat of assistantSubjectConflationPatterns) {
    // "I am/I am not Bradley" is always persona confusion, regardless of negation
    ...
    } else if (pat.test(text) && !hasNegation(text)) {
      reasons.push('persona_confusion:assistant_subject_conflation');
      break;
    }
  }

  // 7c0. Scout denying its own existence — "Scout is a hypothetical character",
  // "Scout is fictional", "Scout is not a real AI" — persona confusion
  const selfDenialPatterns = [
    /\b(?:scout|i)\s+(?:is|am)\s+(?:a\s+)?(?:hypothetical|fictional|not\s+a\s+real|just\s+a\s+character|not\s+real)\b/i,
    /\b(?:scout|i)\s+(?:do(?:es)?\s+not|don'?t|doesn'?t)\s+have\s+(?:any\s+)?(?:specific\s+)?(?:technical|real)\s+(?:background|knowledge|experience)\b/i,
  ];
  ...
  if (knowledge) {
    try {
      ...
      const techDenialPattern = /\b(?:he|she|they|bradley|brad)\s+(?:has|have)\s+not\s+(?:worked\s+(?:in|with|on)|used)\s+([A-Za-z][A-Za-z0-9]+(?:\s+[A-Za-z0-9]+){0,3})/gi;
      const techTriples = relGraph.triples.filter(t =>
        t.relation === 'uses_tech' || t.relation === 'has_skill'
      );
      ...
      while ((techDenialMatch = techDenialPattern.exec(text)) !== null) {
        const deniedTech = techDenialMatch[1].trim();
        const deniedNorm = deniedTech.toLowerCase().replace(/[^a-z0-9]/g, '');
        // Extract significant words from the denied tech (skip common words)
        const skipWords = new Set(['the', 'a', 'an', 'or', 'and', 'in', 'with', 'on', 'environment', 'services', 'service', 'technologies', 'technology']);
        const deniedWords = deniedTech.toLowerCase().split(/\s+/).filter(w => w.length >= 2 && !skipWords.has(w));
        // Check if this technology is in the graph — match if any known tech
        // starts with the denied tech, or the denied tech starts with any known tech,
        // or the denied tech contains a known tech as a substring,
        // or any significant word from the denied tech matches the start of a known tech
        const isKnownTech = knownTechs.some(tech =>
          tech === deniedNorm ||
        ...
        if (isKnownTech) {
          reasons.push(`false_negation:denies_tech_experience:${deniedTech}`);
          break;
        }
      }
  ...
  if (knowledge) {
    try {
      ...
      const employmentPattern = /\b(?:Bradley(?:\s+Matera)?|He|She|They)\s+(?:was|worked|interned|employed|served|joined|spent\s+time)\s+(?:(?:at|with|for)\s+|as\s+(?:an?\s+)?(?:[A-Za-z]+\s+)*?(?:intern|engineer|developer|support|cloud|specialist|analyst|architect)\s+(?:at|with|for)\s+|a\s+(?:[A-Za-z]+\s+){1,5}(?:at|with|for)\s+)([A-Z][A-Za-z0-9&.\s-]{2,50})/g;
      let empMatch;
      ...
      const atCompanyPattern = /\bAt\s+([A-Z][A-Za-z0-9&.\s-]{2,40}?),\s+(?:he|she|they|Bradley(?:\s+Matera)?)\s+(?:built|created|developed|worked|designed|implemented|led|managed|wrote|deployed|maintained|contributed)/g;
      let atCompMatch;
      ...
      const builtForCompanyPattern = /\b(?:built|developed|created|designed|implemented|wrote|deployed|maintained|contributed)\b[^.]*?\bfor\s+(?:the\s+|a\s+|an\s+)?([A-Z][A-Za-z0-9&.\s-]{2,40}?)(?:\s+(?:app|platform|company|API|service|project)|\.|$|,)/g;
      let builtForMatch;
