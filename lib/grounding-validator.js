...
function documentedOccupation(role, company, knowledge, graph) {
  const roleMatches = documented => typeof documented === 'string' &&
    (phraseAppears(documented, role) || phraseAppears(role, documented));
  ...
  return !company && (roleMatches(knowledge?.identity?.title) || roleMatches(knowledge?.summary?.whoIAm));
}
        ...
        if (isAMatch) {
          ...
          if (!isNegated2) {
            const genericAssessments = new Set([
              'good', 'great', 'strong', 'weak', 'poor', 'solid', 'top', 'excellent', 'bad', 'terrible',
            ...
            if (!onlyAssessment) {
              // Accept the claim if any contiguous subphrase matches a
              // documented occupation, so generic modifiers (junior/senior) and
              // minor trailing words do not cause false fabricated-occupation
              // flags. This uses structural knowledge, not a profession list.
              let found = false;
              const subphraseCount = headWords.length;
              for (let len = Math.min(2, subphraseCount); len <= subphraseCount; len++) {
                for (let start = 0; start <= subphraseCount - len; start++) {
                  const sub = headWords.slice(start, start + len).join(' ');
