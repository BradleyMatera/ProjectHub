...
const SAFE_CAPITALIZED = new Set([
  ...
  'Matera', 'ProjectHub', 'Voice', 'Ops', 'Platform', 'Lambda', 'DynamoDB',
  'S3', 'Amplify', 'CloudFront', 'Full', 'Sail', 'University', 'Army',
  'Veteran', 'Davis', 'Illinois'
]);
...
function validateAnswer(answer, source, question = '') {
  ...
  // 1. Overclaim — but skip if the overclaim word appears in a negation context
  // (e.g., "No, he was not a senior engineer" should not trigger overclaim)
  const overclaimMatch = text.match(OVERCLAIM_RE);
  if (overclaimMatch) {
    // Check if the overclaim word is within a negation context
    const matchIdx = text.search(OVERCLAIM_RE);
    const before = text.slice(Math.max(0, matchIdx - 30), matchIdx);
    const after = text.slice(matchIdx, matchIdx + 60);
    const context = (before + after).toLowerCase();
    const hasNegation = /\b(not|never|no|wasn't|was not|isn't|is not|didn't|did not|doesn't|does not|rather than|instead of)\b/i.test(context);
    if (!hasNegation) {
  ...
  // 4. Entity grounding — word-boundary match (not substring) to prevent
  // "Mater" matching "Materialized" or similar false positives.
  const capitalized = text.match(/\b[A-Z][A-Za-z0-9+#.-]{1,}\b/g) || [];
  for (const token of capitalized) {
    if (SAFE_CAPITALIZED.has(token)) continue;
    const tokenLower = token.toLowerCase();
    const tokenRegex = new RegExp(`\\b${tokenLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
    if (!tokenRegex.test(sourceText)) {
