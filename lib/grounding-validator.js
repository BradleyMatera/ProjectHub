...
function validateAnswer(answer, source, question = '', knowledge = null, history = [], graph = null, policyMode = null, responseContract = null) {
  ...
  if (responseContract?.subIntent === 'FUTURE_CAPABILITY' && responseContract?.factState === 'UNKNOWN') {
    ...
    const currentAbilityClaim = /\b(?:has\s+(?:already\s+)?(?:experience|worked|used|built)|already\s+demonstrated|currently\s+(?:knows|uses|can)|has\s+used|has\s+experience\s+with|can\s+(?:debug|use|build|work\s+with|handle))\b/i.test(text);
