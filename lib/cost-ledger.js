...
// (local LLM tokens, egress bytes, compute seconds, and disk writes)
...
function emptyAgg() {
  return { calls: 0, tokensIn: 0, tokensOut: 0, bytes: 0, seconds: 0, shadowMicroUsd: 0, unpriced: false, unpricedSources: [], estimated: false };
}

// Integer micro-USD pricing. Rounds up (ceil) so shadow price never undercounts.
// Returns null when a source has no shadowRates, preserving "unknown" rather
// than silently treating it as a known zero cost.
function priceEventMicroUsd(source, event, registry) {
  const def = registry.sources[source];
  if (!def || !def.shadowRates) return null;
...
class CostLedger {
  ...
  _loadState() {
    try {
      ...
      const allowed = new Set(Object.keys(this.registry.sources));
      const trimBucket = bucket => {
        for (const source of Object.keys(bucket || {})) {
          if (!allowed.has(source)) delete bucket[source];
        }
      };
      trimBucket(this.state.allTime);
      for (const window of [this.state.hours, this.state.days, this.state.months]) {
        for (const bucket of Object.values(window || {})) trimBucket(bucket);
      }
      this.state.recentEvents = (this.state.recentEvents || []).filter(event => allowed.has(event.source));
    } catch { /* first run */ }
  }

  // record({source, kind, tokensIn, tokensOut, bytes, seconds, minutes, estimated, meta})
  record(event) {
    const ts = this.now();
    const source = String(event.source || 'unknown');
    const micro = priceEventMicroUsd(source, event, this.registry);
    const unpriced = micro == null;
    ...
    for (const bucket of buckets) {
      ...
      if (unpriced) {
        agg.unpriced = true;
        if (!agg.unpricedSources.includes(source)) agg.unpricedSources.push(source);
      } else {
    ...
    this.state.recentEvents.push({
      ...
      bytes: event.bytes || 0,
      shadowMicroUsd: unpriced ? null : micro, unpriced: unpriced, estimated: Boolean(event.estimated),
  ...
  totalsFor(windowObj) {
    ...
    for (const source of Object.keys(windowObj || {})) {
      ...
      out.bytes += a.bytes; out.seconds += a.seconds;
      out.shadowMicroUsd += Number.isFinite(a.shadowMicroUsd) ? a.shadowMicroUsd : 0;
      if (a.unpriced) {
        out.unpriced = true;
        for (const s of (a.unpricedSources || [])) {
          if (!out.unpricedSources.includes(s)) out.unpricedSources.push(s);
        }
      }
  ...
  headroom() {
    ...
    for (const [source, def] of Object.entries(this.registry.sources)) {
      ...
      if (limits.requestsPerDay) {
        result.push({ source, metric: 'requestsPerDay', used: dayAgg.calls, limit: limits.requestsPerDay, pct: Math.round((dayAgg.calls / limits.requestsPerDay) * 100), complete: true });
      }
      if (limits.egressBytesPerMonth) {
        const factor = this.registry.egressOverheadFactor || 1;
        const adj = Math.ceil(monthAgg.bytes * factor);
        result.push({ source, metric: 'egressBytesPerMonth', used: adj, limit: limits.egressBytesPerMonth, pct: Math.round((adj / limits.egressBytesPerMonth) * 100), complete: true });
      }
      if (limits.vmHoursPerMonth) {
        const hours = monthAgg.seconds / 3600;
        result.push({ source, metric: 'vmHoursPerMonth', used: Math.round(hours * 100) / 100, limit: limits.vmHoursPerMonth, pct: Math.round((hours / limits.vmHoursPerMonth) * 100), complete: true });
      }
      if (limits.requestsPerHour) {
        const hourAgg = (this.state.hours[hourKey(ts)] || {})[source] || emptyAgg();
        result.push({ source, metric: 'requestsPerHour', used: hourAgg.calls, limit: limits.requestsPerHour, pct: Math.round((hourAgg.calls / limits.requestsPerHour) * 100), complete: true });
      }
      if (limits.bandwidthBytesPerMonth) {
        result.push({ source, metric: 'bandwidthBytesPerMonth', used: monthAgg.bytes, limit: limits.bandwidthBytesPerMonth, pct: Math.round((monthAgg.bytes / limits.bandwidthBytesPerMonth) * 100), complete: true, estimated: true });
      }
      if (limits.minutesPerMonth) {
        const minutes = monthAgg.seconds / 60;
        result.push({ source, metric: 'minutesPerMonth', used: Math.round(minutes * 10) / 10, limit: limits.minutesPerMonth, pct: Math.round((minutes / limits.minutesPerMonth) * 100), complete: true });
      }
      // Cloudflare daily neuron allocation is an applicable free limit, but token→neuron
      // pricing for the production -fast model is not published, so day usage cannot be
      // converted into a reliable `used` value. Record the limit as unmeasurable/complete:false.
      if (limits.neuronsPerDay) {
        const hasDayUsage = dayAgg.calls > 0 || dayAgg.tokensIn > 0 || dayAgg.tokensOut > 0;
        if (hasDayUsage) {
          result.push({ source, metric: 'neuronsPerDay', used: null, limit: limits.neuronsPerDay, pct: null, complete: false });
        }
      }
    }
    return result;
  ...
  snapshot() {
    ...
    const dayTotals = this.totalsFor(dayAgg);
    const headroom = this.headroom();
    const anyExceeded = headroom.some(h => h.complete !== false && h.pct >= 100);
    const anyIncomplete = headroom.some(h => h.complete === false);
    const free = anyExceeded ? false : (anyIncomplete ? null : true);
    const freeStatusComplete = !anyIncomplete;
    const monthComplete = !monthTotals.unpriced;
    return {
      generatedAt: ts,
      free,
      freeStatusComplete,
      shadowCost: {
        ...
        monthComplete,
        dayMicroUsd: dayTotals.shadowMicroUsd,
        dayUsd: (dayTotals.shadowMicroUsd / 1e6).toFixed(6),
        dayComplete: !dayTotals.unpriced,
        actualUsd: free === true && monthComplete ? '0.000000' : null,
        unpricedSources: monthTotals.unpricedSources || []
      },
      headroom,
