...
function shouldUseDeterministicAgent(question) {
  const q = String(question || '').toLowerCase();
  if (q.length < 12) return false;
  if (/compare|versus|\bvs\b|difference/.test(q)) return true;
  if (/job description|role requirements|requirements for|recruiter brief|evidence|prove it|interview questions|screening questions/.test(q)) return true;
  // Short conversational fit questions have stronger dedicated answers. Use
  // evidence matching only when the user supplies concrete requirements.
  return /fit for|candidate|hire/.test(q)
    && /requiring|requires|javascript|typescript|react|node|sql|lambda|dynamodb|s3|docker|ci\/cd|linux/.test(q);
}
...
module.exports = { buildDeterministicAgentResult, joinNatural, parseLocalStyleResponse, projectNamesFromQuestion, shouldUseDeterministicAgent };
