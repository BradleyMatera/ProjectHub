...
VERIFIED/GAP/ADJACENT/UNKNOWN per target via `assessEntityEvidence()`
(`lib/relationship-graph.js`, relation-class aware — `has_skill`/`worked_at`/
`employed_as` etc. claim, `has_gap` documents a gap, `uses_tech` is adjacent
work-linked evidence, structural relations are mention-only). Set questions
carry per-member `memberEvidence` states; resolved plural ranking keeps the
frame relation plus a generic `setOperation: 'COMPARE'`. `PROFILE_SUMMARY`
prose is model-generated by default; `DIRECT_KB` survives only behind the
explicit `SCOUT_DIRECT_KB_ENABLED=true` opt-in. Test floor now 1054/1054
(`test/discourse-frames.test.js` 35 cases). Controlled A/B at `4f5ee97`
baseline vs `6d2871d`: 98/132 vs 92/132 — inside provider-variance band;
targeted 3x replication on the 12 flipped conversations showed one consistent
flip (False owner claim, identical contract both sides → 55-word harness
threshold variance) and one consistent feature win (`Which of those is the
strongest fit?` 0/3 → 3/3). Awaiting Bradley review before any `develop`
merge.
