...
if (isRemote) {
  ...
  // Copy the reset script to the VM, stop the service, reset files, and restart.
  const remoteScriptPath = '/tmp/reset-stats.js';
  const remoteCommand = `sudo systemctl stop recruiter-chat-api; sudo cp ${REMOTE_DIR}/stats.json ${REMOTE_DIR}/stats.json.backup.$(date +%Y%m%d_%H%M%S) || true; sudo cp ${REMOTE_DIR}/learned.json ${REMOTE_DIR}/learned.json.backup.$(date +%Y%m%d_%H%M%S) || true; sudo cp /tmp/reset-stats.js ${REMOTE_DIR}/reset-stats.js && cd ${REMOTE_DIR} && sudo node reset-stats.js && sudo systemctl start recruiter-chat-api`;
  execSync(
  ...
  console.log('Remote reset complete and service restarted.');
} else {
